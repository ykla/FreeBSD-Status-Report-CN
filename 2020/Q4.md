...
Linux 兼容层改进工作已经进行两年，得到了 FreeBSD 基金会的赞助，涵盖了几个不同的项目赞助，以及社区的贡献。该项目的目标是提升 FreeBSD 执行未修改 Linux 二进制文件的能力。目前的进展可在 [Linux 应用状态 Wiki 页面](https://wiki.freebsd.org/LinuxApps) 上追踪。工作现已从命令行应用转向桌面应用。
