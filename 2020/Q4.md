...
- 原文：[FreeBSD Quarterly Status Report 4th Quarter 2020](https://www.freebsd.org/status/report-2020-10-2020-12.html)
...
我们帮助规划和组织的一项活动，促进了供应商/开发者的互动，就是每年举办的湾区供应商峰会。我们不打算让疫情阻止我们举办这一宝贵的年度活动，所以我们将其转为线上！根据我们从供应商社区收到的反馈，了解他们希望我们如何组织此活动以便为他们带来益处，我们决定在 11 月举办三个半天的线上活动。一个意外的结果是，来自世界各地的更多商业用户参加了此次活动。由于供应商/开发者峰会通常是邀请制的，我们决定向全球的 FreeBSD 贡献者开放观看直播的机会。由于此次活动的成功和热烈反响，我们计划在 6 月或 7 月再举办一次。

#### 筹款工作

我们想借此机会感谢去年所有捐款帮助我们筹集资金的个人和公司。截至撰写本文时，我们筹集了 1,235,926 美元，最终的筹款总额将在 1 月中旬确定。为我们提供慷慨捐款的公司包括 Arm、NetApp、Netflix、Juniper Networks、Beckhoff、VMware、Stormshield、Tarsnap 和 Google。我们还要感谢 Koum 家族基金会授予我们的大额赞助，以及 Nginx 员工的慷慨捐款。
...
你将在本报告中了解我们在第四季度如何使用你的捐款，也可通过本报告的各单独条目了解更多信息。
...
Tiger 在代码覆盖指导的内核模糊测试工具 Syzkaller 上进行改进，添加了新的系统调用定义，使 Syzkaller 能够扩展其测试的代码。已经有多个 FreeBSD 内核 bug 修复是通过这项工作实现的。Tiger 还为 ELF 工具链的二进制实用程序集贡献了许多改进，并开发了工具来运行其他工具套件对 ELF 工具链的测试。

Zac 对 pkg 软件包管理工具进行了改进，调查并提交了 FreePBX 在 FreeBSD 上的支持补丁，并研究了编译器支持以解决堆栈冲突漏洞。
...
员工们回应了已报告的安全漏洞和发布勘误，准备了补丁，并参与了安全建议过程。我们还致力于前瞻性的安全漏洞缓解工作。Syzkaller 也提供了许多内核问题报告，这些报告促使基金会赞助了 bug 修复。我们还解决了多个与 FreeBSD/arm64 相关的问题，推动其朝着 Tier-1 架构的发展方向迈进。
...
基金会为改进 FreeBSD 基础设施提供硬件和支持。上个季度，我们继续支持位于全球各地的 FreeBSD 硬件。我们协调了 NYI 芝加哥新设施与 clusteradm 之间的工作，开始为我们计划购买的一些新 FreeBSD 硬件准备该设施。NYI 慷慨地为项目免费提供这一支持。我们还与 NYI Bridgewater 站点的新所有者进行了联系，该站点是现有 FreeBSD 基础设施的主要所在地。
...
- 继续推广 FreeBSD Office Hours 系列。一小时的会议视频可以在项目的 [YouTube 频道](https://www.youtube.com/channel/UCxLxR_oW-NAmChIcSkAyZGQ) 查找。有关更多信息，请参见本报告中的 Office Hours 部分。
...
Netflix 在我们最新的 [贡献者案例研究](https://freebsdfoundation.org/blog/freebsd-case-study-netflix/) 中更新了他们使用 FreeBSD 的原因和方式。
...
- 将软件包构建机器升级到 2020 年 10 月中旬的 head 版本。
- 将集群中的开发机器（freefall、`ref*` 和 `universe*`）升级到 2020 年 10 月中旬的 head 版本。
- 在新泽西站点安装了八台新的 x86 服务器：五台应用服务器、两台软件包构建器和一台镜像服务器。

  - 新的镜像服务器已投入生产（pkg0.nyi.freebsd.org）。
  - 两台软件包构建器已投入生产。
  - 两台应用服务器已投入生产，分别作为 Git 源和 cgit Web 前端。
- 在新泽西站点安装了两台新的 aarch64 服务器，目前两台服务器正在构建 aarch64 软件包。
- 修复了 powerpc64 软件包的镜像同步问题。
...
- 为 Web 服务进行硬件更新，以及 NYI 中的备份版本控制系统。
...
传统上，合并到季度 Ports 分支（这些分支是 HEAD 树的保守版本）需要获得 Ports 安全团队（ports-secteam@）或 portmgr@ 的批准。已经有一些经过测试的提交获得了一揽子批准，包括修正拼写错误和将网页浏览器升级到最新版本。从去年 12 月起，所有 Ports 提交者都可以自行合并，这减轻了 ports-secteam@ 的负担。

由于专利问题在不同法域中有不同的影响，专利限制已从许可框架中移除。

上个季度，多个 Port 的默认版本进行了更新：
...
Git 工作组于 2020 年 12 月基本完成了 doc 和 src（基本系统）树从 Subversion 迁移到 Git 的工作。我们目前正在处理一些小的遗留问题，并准备进行 Ports 的迁移。
...
LLDB 项目基于 LLVM 和 Clang 提供的库，提供了一个现代化的调试器。它使用 Clang 的 AST 和表达式解析器、LLVM JIT、LLVM 反汇编器等，使得调试体验“开箱即用”。它的速度极快，且比 GDB（GNU 调试器）许可更宽松。

LLDB 是 macOS 上 Xcode 中的默认调试器，支持桌面和 iOS 设备及模拟器上的 C、Objective-C 和 C++ 调试。

FreeBSD 将 LLDB 包含在基本系统中。目前，它与 GNU GDB 调试器相比存在一些限制，尚未提供完整的替代方案。它曾依赖于 LLDB 中的一个过时插件模型，这造成了不断增长的技术债务。该项目旨在使 LLDB 更接近于 GDB 的完全替代品，因此为 FreeBSD 提供了一个现代化的调试器，供软件开发人员使用。
...
FreeBSD/LLDB 开发人员和高级用户在这个坚如磐石的操作系统上的整体体验，已达到其他环境中的相同水平。此外，专注于 FreeBSD 的工作还带来了通用改进，增强了 LLDB 对 Linux 和 NetBSD 的支持。
...
Alexander Sideropoulos <[Alexander.Sideropoulos@netapp.com](mailto:Alexander.Sideropoulos@netapp.com)>

Allan Jude <[allan@klarasystems.com](mailto:allan@klarasystems.com)>
...
为了提高 NFS 的安全性，互联网草案《走向默认远程过程调用加密》指定使用 TLS 1.3 来加密用于 NFS 的 Sun RPC 连接上的所有数据流量。
...
该项目的编码工作现已完成。所有需要的 NFS 和内核 RPC 代码变更已提交至 head/current 内核，并将包含在 FreeBSD13 中。守护进程现在可以通过一个 Port 来构建，该 Port 依赖于包含 ktls 支持补丁的 OpenSSL 3 的 security/openssl-devel Port。该守护进程的 Port 名为 sysutils/nfs-over-tls，应该很快提交到 Ports 框架中。与此同时，可以按照 [https://people.freebsd.org/~rmacklem/nfs-over-tls-setup.txt](https://people.freebsd.org/~rmacklem/nfs-over-tls-setup.txt) 中的说明轻松获取该 Port。

为了支持像笔记本电脑这样的客户端，执行 TLS 握手的守护进程可以选择处理来自站点本地 CA 的客户端 X.509 证书。现在的 exports(5) 选项可以要求客户端提供有效的 X.509 证书。如果证书中存储了“用户名”并且用于将所有 RPC 凭证映射到该用户，则可能违反了互联网草案。只有在提供 `-u` 命令行选项时，rpc.tlsservd(8) 才启用此功能。
...
设置用于测试的系统仍然需要从最近的 head/FreeBSD13 源代码构建带有 `options KERN_TLS` 的自定义内核，并安装守护进程的 Port，如上述文档所述。
...
在 10 月份，Clang/LLVM 更新至 11.0.0，随后从上游获得了一些 bug 修复，包括对多个 Tier-2 架构的改进。我们还启用了 `-fstack-clash-protection` 标志，以启用编译程序对“栈冲突”漏洞的缓解，并与上游协调。
...
英特尔无线驱动程序更新项目旨在支持更新的芯片组，并且在第一阶段将站点端升级至 11ac。

在过去几个月中，已实现 net80211 与 Linux 驱动程序 KPI 之间的连接代码，并且扫描功能已正常工作。目前，重点是从一个状态机驱动另一个状态机，并在 net80211 和 Linux 兼容代码之间同步状态。
...
FreeBSD CSPRNG 小组 <[csprng@FreeBSD.org](mailto:csprng@FreeBSD.org)>
...
- *Fenestras X* 具有每个 CPU 的生成器，且由根生成器播种。*Fortuna* 只有根生成器。这一变化消除了多个核心上运行的 `random(4)` 读取器之间的锁争用。
...
pf 的性能并不如预期。在使用 hwpmc 工具进行一些调查后，最终发现缓存行为非常差。longest_lat_cache.miss 事件提供了非常有用的信息。

事实证明，这是由于 pf 在状态、规则和接口中进行数据包和字节计数造成的。
...
IRC #freebsd-riscv 在 freenode 上
...
- 创建了 `sysutils/u-boot-sifive-fu540`，适用于 HiFive Unleashed 的 u-boot port
- 改进了 SBI 扩展支持
...
- 最大的新闻是，得益于 FreeBSD 基金会（以及 Philip Paeps 的协助），FreeBSD 现在拥有两台新的 aarch64 机器，替代了之前那些老化严重的机器。从 8 月以来，我们首次再次拥有了最新的 aarch64 软件包。
...
- Piotr 还开始了将 powerpc64le（小端）与 powerpc64 对齐的工作。尽管许多 powerpc64 的源代码提交者（包括本文作者）偏爱大端，但事实是，要让图形能力接近 x86 水平，最可行的路径是小端架构。
- Mark Linimon (linimon@) 已开始在 riscv64 上进行自己的 Port 测试构建，以确定整体可构建性。令人惊讶的是，许多 Port 确实可以构建。感谢许多已在 riscv64 上工作的贡献者，包括 John Baldwin (jhb@) 提供的 LLVM 修复，现在我们已经能够构建大约 20,000 个软件包。注意：这些软件包是 *非官方的*，不保证其可用性。
...
- 我们需要 riscv64 的用户实际测试已经构建的软件包（目前它们仅被测试为可构建性）。有兴趣的用户请联系 linimon@。
- 如果有人仍然在使用 mips/mips64 进行除最简单任务外的其他工作，我们欢迎补丁。

### 从 Ports 中移除 Python 2.7
...
- 协调与邮件管理员的合作，隔离或迁移 mail/mailman
...
- 新增了面板插件 `statustray`，结合了 StatusNotifier 和传统 Systray 项目
...
警告：不幸的是，这次更新可能会暴露出 pkg 中的一个 bug，导致升级后 libexo 软件包中的文件丢失。为避免此问题，请在升级其他软件包之前，单独先升级 libexo 软件包，如 UPDATING 记录 20210102 所述。
...
### FreeBSD Aarch64 在 VMWare ESXi-ARM Fling 下运行
...
其他人基于此工作的一个例子是 Twitter 用户 astr0baby 在新的 Apple M1 系统上的 QEMU 中成功运行了 FreeBSD。
...
更多关于 [Bastille 发布](https://github.com/BastilleBSD/bastille/releases) 的详细信息。上游已更新为 `0.8.202010101`（最新版本）。ports（`sysutils/bastille`）已更新为 `0.7.20200414`。
...
`#helloSystem` 在 `irc.freenode.net`，并已镜像到 [Matrix 上的 `#helloSystem:matrix.org`](https://matrix.to/#/%23helloSystem:matrix.org?via=matrix.org)
...
Kirill Ponomarev <[krion@FreeBSD.org](mailto:krion@FreeBSD.org)>
