...
- 原文：[FreeBSD Quarterly Status Report 2nd Quarter 2020](https://www.freebsd.org/status/report-2020-04-2020-06.html)
- 最后作者：Daniel Ebdrup Jensen

## 引言

本报告将涵盖 4 月至 6 月之间与 FreeBSD 相关的项目，涉及的主题多种多样，从内核更新到用户空间和 Port，以及第三方工作。

通过掷骰子选出的亮点包括但不限于：当底层介质变得不可访问时，强制卸载 UFS 的功能、增加对蓝牙低功耗的初步支持、FreeBSD 办公时间的推出、以及软件集合仓库 potluck，供 pot 工具安装使用，还有许多其他内容。

作为一点小福利，读者还能看到季度团队难得提交的报告。

最后，我代表季度团队，向 salvadore@ 表达我们最深的感激之情，他决定退出并结束他的工作。他不仅对季度报告本身做出了贡献，还为许多工具的开发工作提供了极大便利，使工作效率成倍提升，这些贡献是无法估量的。
...
像其他组织一样，我们为所有员工制定了远程办公政策，并对员工的差旅进行了临时禁令。我们继续支持社区和项目的工作，但由于一些优先事项的变化以及部分员工育儿照护受限，我们的一些工作和响应可能会有所延迟。
...
上个季度我们筹集了 $268,400！感谢那些支持我们工作的个人和组织。我们要特别感谢 Netflix、Nginx 员工、Beckhoff 自动化和 Mozilla 基金会在上个季度的巨大贡献，这些帮助使我们的 2020 年筹款总额达到了 $339,000。我们希望其他组织能够效仿并回馈，帮助我们继续支持 FreeBSD。
...
除了大型独立项目外，工作人员还在许多其他方面投入了大量精力。这些包括协助代码审查、漏洞报告分类、处理安全报告和建议、处理 syzkaller 报告以及在功能领域的持续维护和漏洞修复，例如工具链、开发者工具、虚拟内存内核子系统、低级 x86 基础设施、套接字和协议等。

#### 滑铁卢大学合作项目

基金会的合作学生 Colin、Tiger 和 Yang 在第二季度完成了他们的 2020 年冬季工作期，并继续在各自的学业中进入下一学期。尽管疫情造成了独特的挑战，并促使工作期过半时迅速转为远程工作，但三位学生都学到了很多，并为 FreeBSD 项目和基金会做出了积极贡献。
...
在 2020 年第二季度，基金会工作人员继续改进项目的 CI 基础设施，监控回归问题，并与贡献者合作修复构建和测试用例失败的情况。CI 作业的虚拟机主机和暂存环境的设置正在进行中。我们还与项目中的其他团队合作，满足他们的测试需求。例如，我们为非 x86 架构添加了完整测试作业。我们还与许多外部项目和公司合作，以提高它们对 FreeBSD 的支持。
...
- 每年一次的 FreeBSD Day，6 月 19 日。今年的庆祝活动因支持 Juneteenth 而被推迟。然而，围绕 FreeBSD 日的活动已经转变为一系列持续的在线会议。有关更多信息，请参见下方的 *FreeBSD 星期五*。
...
- 宣布了 *FreeBSD 星期五*：一系列旨在帮助你入门 FreeBSD 的 101 课程。有关更多信息，请查看 [公告](https://www.freebsdfoundation.org/blog/announcing-freebsd-fridays-a-series-of-101-classes/)。
- 作为谷歌编程之夏 2020 的管理员参与。
- 参与了新的 FreeBSD Office Hours 系列，其中包括举办我们基金会主办的办公时间。会议视频可以在 [项目的 YouTube 频道](https://www.youtube.com/channel/UCxLxR_oW-NAmChIcSkAyZGQ) 上找到。你可以 [在这里](https://www.youtube.com/watch?v=Ji4ux4FWpRU) 观看我们的视频。
...
- Deb 请求关于基金会如何支持社区的指导。核心团队和基金会成员认为，需要更多的开发者支持，以弥补商业客户未提供支持的领域。最明显的例子是桌面体验，包括图形和无线支持。与以往的请求不同的是，这次请求并非针对一次性项目，而是需要持续的职位，以保证桌面体验的一贯高质量。
  “FreeBSD 没法运行在你的笔记本电脑上，是走向无关紧要的第一步。” — Ed Maste
...
FreeBSD 发布工程团队负责制定和发布 FreeBSD 官方项目的发布计划、公告代码冻结（slush）以及维护相关分支等工作。
...
- 构建 aarch64 和 powerpc64 软件包的基础设施
  ...
  - 由 FreeBSD 基金会提供的硬盘升级测试（SSD），适用于 pkg 构建器。
  - aarch64 参考机器的启动问题。
...
Ports 管理团队负责监督 Ports 树的整体方向、构建软件包以及人员事务。以下是上个季度的工作总结。
...
- 它们在 Capsicum 沙盒中运行
- 它们遵循 rootdir，即 pkg 用作安装所有软件包的起始点的目录
...
在上个季度，antoine@ 进行了 55 次 exp-runs 测试，测试了 Port 版本更新、使 liblzma 使用 libmd、为 **devel/scons** 和 Lua Port 添加 flavor、在基本系统中添加和更新库函数、使 `malloc.h` 再次可用、从基本系统中移除 **as(1)**、增强了 **sed(1)** 的选项 `-f` 。
...
季度状态报告 <[quarterly@FreeBSD.org](mailto:quarterly@FreeBSD.org)>
...
跨多个类别的项目，从内核和用户空间到 Ports Collection 或外部项目。

### FreeBSD 在微软 HyperV 和 Azure 上

**链接**：

[FreeBSD 在微软 Azure 维基](https://wiki.freebsd.org/MicrosoftAzure)
...
尽管 NFS 一直能够使用 sec=krb5p 来加密传输中的数据，但这需要一个 Kerberos 环境，因此并未广泛采用。它还要求加密/解密在软件中进行，因为只有 RPC 消息中的 NFS 参数被加密。由于内核 TLS 可以使用硬件加速来提高性能，并且不需要 Kerberos，因此一旦有可用的实现，NFS over TLS 可能会得到更广泛的采用。
...
添加了对 AUX 设备的支持（如辅助放大器等设备，这些设备不是主 CODEC 芯片的一部分）。
为了验证框架设计，添加了以下音频驱动程序：RT5640 CODEC（Firefly-RK3399）的录音支持、Allwinner I2S、Allwinner 的 sun8i 和 A64 CODEC（Pine A64+），包括录音和播放功能。
...
- 在队列访问周围添加锁
- 合并 CQ 更新
- 基于声明的 MDTS 调整 pci_nvme_ioreq 大小

你可以通过测试/评论代码审查来提供帮助。

### 蓝牙支持

**联系方式**：Marc Veldman <[marc@bumblingdork.com](mailto:marc@bumblingdork.com)>
...
- 修复 Rx 充填问题，改善低内存条件下的稳定性（也已作为 FreeBSD-12.1 的勘误通知发布）
...
Kirk McKusick <[mckusick@mckusick.com](mailto:mckusick@mckusick.com)>
...
FreeBSD 已经提供了对超级页面的优秀支持，且对应用程序完全透明。它尝试主动防止碎片化，为受管对象的线性分配保留连续的物理页面范围，并在小页面形成完整的超级页面时自动提升这些小页面。
...
最初的路由 KPI（内核编程接口）在 1980 年代引入。那时它是一个很好的通用方法，因为没人知道协议会如何发展。它取得了巨大的成功，因为它能够维持超过 20 年。
...
此外，本季度已开始将 CheriBSD 移植到 ARM 的 Morello SoC。Morello 是一块原型展示板，它为 ARMv8-A 添加了 CHERI 扩展。
...
该项目的目标是在 FreeBSD 支持的其他架构上运行回归测试，包括 ARM、ARM64、MIPS、POWER 和 RISC-V。这些测试使用通用的、主要与机器无关的脚本运行。为了使 QEMU 也能作为 bhyve（用于 x86 测试的虚拟机监控器）之外的另一种选择，做了一些必要的修改。**sysutils/u-boot-qemu-arm** 和 **sysutils/u-boot-qemu-arm64** Port 已添加到 Ports Collection 中。最后，针对每种架构都需要一些调整，以满足不同的配置需求——例如 MIPS 内核不支持 VirtIO 磁盘，甚至不支持 AHCI，而在 ARM64 上，U-Boot 对多个 VirtIO 磁盘的处理会出现混乱。
...
Stefan Esser <[se@FreeBSD.org](mailto:se@FreeBSD.org)>

Gavin D. Howard <[yzena.tech@gmail.com](mailto:yzena.tech@gmail.com)>
...
GNU as 工具曾被基本系统和 Port 共同使用。与 objdump 一样，依赖 GNU as 的 Port 一般已更新为依赖 binutils。在迁移到使用 Clang 集成汇编器（IAS）的过程中，基本系统中的一个文件（skein_block_asm.s）曾遇到问题。然而，在更新到 Clang 10 后（并对源代码进行了一些微小修改），IAS 可以成功地汇编该文件。
...
在 link_map 的工作过程中，变得明显的是，rtld 有时需要报告一些无法仅通过运行时符号存在或函数行为测试来推断的功能。为此，设计了一个通过统一命名的符号报告特性方案——详见 **rtld(1)** 手册页（在 CURRENT 中）进行说明。

FreeBSD 上的位置无关（PIE）二进制文件现在标记为 DF_1_PIE DT_FLAG1 标志。否则，这些二进制文件只是 ET_DYN 对象，并且很难区分适当的动态共享对象（DSO）与 PIE 二进制文件。问题在于，对于二进制文件，静态链接器会剥离一些信息，这些信息是正确加载为 DSO 所必需的；此外，二进制文件包含像 copy-relocations 这样的重定位，这些重定位对于非主二进制文件无法处理。
...
影响 Ports Collection 的更改，无论是涉及 Ports 树的广泛更改，还是单个 Port 本身的更改。
...
本季度情况非常特殊。虽然我们习惯了远程工作，通过互联网协作更新 Ports 树，但当整个世界封锁时，这种情况变得 *质* 上不同。与此同时，软件继续发布，因此本季度 kde@ 团队做了以下工作：
...
- KDE 应用程序 -- KDE 发布服务，提供库、应用程序和插件 -- 进行了一次大型发布，20.04.1 版本于 5 月中旬进入 Ports 树，20.04.2 的月度更新则在 6 月中旬发布。
...
**联系方式**：Gleb Popov <[haskell@FreeBSD.org](mailto:haskell@FreeBSD.org)>
...
### rtsx - 从 OpenBSD 移植 Realtek SD 卡读卡器驱动
...
**联系方式**：Henri Hennebert <[hlh@restart.be](mailto:hlh@restart.be)>

已从 OpenBSD 移植 Realtek SD 卡读卡器的 rtsx 驱动。可以通过 Port [**sysutils/rtsx-kmod**](http://freshports.org/sysutils/rtsx-kmod/) 获取其开发快照。
...
Paul Floyd <[paulf@free.fr](mailto:paulf@free.fr)>
...
FreshPorts 跟踪提交并从 Port Makefile 中提取数据，创建一个对 Port 开发者和 Port 用户都很有用的信息数据库。
...
- 查看默认配置选项
- 显示安装特定文件的软件包（例如 bin/unzip）
- 查找某人维护的 Port
- 查找包含 bunzip 引用的 Makefile
- 搜索结果可以是包含 Port foo/bar 的纯文本列表
- 搜索页面上的最大努力复选框无效
- 提交者可以在提交后收到基本合理性测试失败的通知
- 基于 SVN 修订号查找提交，举例：<https://www.freshports.org/commit.php?revision=352332>

#### 需要 JavaScript 帮助

我们 [最近更新](https://github.com/FreshPorts/freshports/commit/f620270161dd7818272dfebacacaaf26df5f37e6) 了一些过时的 JavaScript 模块。此更新破坏了我们基于 JavaScript 的图表功能 [基于 JavaScript 的图表](https://www.freshports.org/graphs2.php)。我们需要一些帮助来修复它。起点列表已列出在该 URL 中。如果你需要一个可用的网站来玩耍，请通过 SSH 公钥与我联系。

**赞助者**：由 iXsystems 提供硬件

### 在 Intel 上使用 bhyve 进行 PCI 直通，并支持 OpenBSD 虚拟机
...
Anatoli <[me@anatoli.ws](mailto:me@anatoli.ws)>

Callum <[callum@aitchison.org](mailto:callum@aitchison.org)>

Peter Grehan <[grehan@freebsd.org](mailto:grehan@freebsd.org)>

**bhyve(8)** 是一款虚拟机监控程序，支持在虚拟机中运行多种客户操作系统。**bhyve(8)** 包括对 PCI 设备直通的支持，这是一种将物理机的 PCI 设备传递给虚拟机以供其独占控制和使用的技术。
...
网络适配器直通后，FreeBSD 物理机不仅看不到它，因此不会处理网络流量，它甚至不需要初始化该适配器（例如，对于 WiFi 卡，由虚拟机加载固件）。

简单来说，物理机只会将设备中断从硬件原样传递给虚拟机。所有与设备管理相关的操作都发生在虚拟机内部，因此不会存在网络流量利用物理机网络栈中的问题，从而导致物理机崩溃或以其他方式异常的风险。

### SageMath

**链接**：[SageMath 官网](https://www.sagemath.org/)
...
这是一个复杂的 Port，具有许多依赖项，且中断了一段时间。上游正在努力简化其打包过程，许多先前捆绑的应用程序现在可以通过外部包进行替换。
...
### chaifi - 简化连接公共 WiFi 网络的工具

**链接**：[chaifi](https://github.com/gonzoua/chaifi)
...
**链接**：[mixertui](https://gitlab.com/alfix/mixertui)
...
与手动配置相比，这应简化许多包含多个软件包和 Port 的复杂软件的设置：Potluck 旨在提供一个内容库，作为现有基础设施（如 pkg）之上的额外抽象层，而这些正是 pot 所能提供的。
