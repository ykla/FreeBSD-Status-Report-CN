...
FreeBSD 基金会是一家 501(c)(3) 非营利组织，致力于支持和推广全球范围内的 FreeBSD 项目和社区。资金来自个人和企业捐款，用于资助和管理软件开发项目、会议和开发者峰会，并为 FreeBSD 贡献者提供差旅补助。基金会采购并支持硬件以改善和维护 FreeBSD 基础设施，并提供资源以提升安全性、质量保证和发布工程工作；出版营销材料以推广、教育和支持 FreeBSD 项目；促进商业厂商和 FreeBSD 开发者之间的合作；最终，代表 FreeBSD 项目执行合同、许可协议和其他需要合法实体的法律安排。
...
我们目前正在安排第四季度的 Zoom 公司会议，如果您希望与我们安排会议，请联系我们。

#### 筹款努力

上季度我们筹集了 $192,874.43！感谢那些伸出援手的个人和组织，帮助我们资助我们的努力。我们特别感谢 Arm 在上季度的慷慨捐助，帮助我们将 2020 年的筹款总额提升至 $521,000。我们希望其他组织能效仿 Arm，回馈支持我们继续支持 FreeBSD。
...
我们还提供了合作伙伴计划，为我们的较大商业捐助者提供更多福利。了解更多关于 [合作伙伴计划](https://www.freebsdfoundation.org/FreeBSD-foundation-partnership-program/) 的信息，并与您的公司分享！

#### 操作系统改进

许多 FreeBSD 基金会的资助项目在第三季度启动、继续进行或完成了项目，包括：
...
基金会为改善持续集成、自动化测试以及 FreeBSD 项目的整体质量保证工作，提供了一名全职工作人员，并资助相关项目。
...
请访问 [FreeBSD 基金会的网站](http://www.freebsdfoundation.org/) 了解我们如何支持 FreeBSD 以及我们如何帮助您！
...
这项工作由 Rubicon Communications, LLC（netgate.com）和 FreeBSD 基金会资助。
...
- 构建 aarch64 和 powerpc64 软件包的基础设施。
  ...
  - FreeBSD 基金会资助的 pkg 构建机驱动升级测试（SSD）。
  - Aarch64 参考机器的启动问题。
...
我们在 [GitHub](https://github.com/FreeBSDDesktop) 上也有一个团队区域，您可以在其中找到我们的工作仓库。
...
尽管在 Linux 和 macOS 上构建的支持仍应视为实验性，但在许多情况下应该能够正常工作。如果您想尝试一下，以下命令行应能在安装了 LLVM 10（或更新版本）包的 Linux 和 macOS 系统上成功构建 amd64 world：`MAKEOBJDIRPREFIX=/somewhere ./tools/build/make.py TARGET=amd64 TARGET_ARCH=amd64 buildworld`。必须使用 `./tools/build/make.py` 包装脚本来执行构建，因为大多数 Linux 和 macOS 系统不附带适当版本的 bmake。如果您遇到任何问题，请告诉我。
...
您也可以查看 Git 转换工具仓库中的 wiki、问题、README 和其他文档：[Git 转换工具仓库](https://github.com/freebsd/git_conv)。
...
生成的二进制文件 `grub-bhyve` 将位于 `grub-core/` 目录中。如果您成功使用或遇到问题，请告知我。
...
Doc New Generation 项目旨在将网站和所有现有文档转换为 Hugo/AsciiDoctor。目前，几乎所有内容都已转换，您可以在仓库中查看。
...
pot 是一个监狱管理工具，[还支持通过 nomad 进行编排](https://www.freebsd.org/news/status/report-2020-01-2020-03.html#pot-and-the-nomad-pot-driver)。
