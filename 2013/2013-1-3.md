...
本状态报告的亮点包括 8.4-RELEASE 的忙碌准备工作、二进制包构建的恢复、多个移植工作的稳步进展，如 FreeBSD 对 xorg、GNOME、KDE 和 Xfce 的 Ports 工作，将 FreeBSD 移植到 Cubieboard 和 Hackberry 板卡、ARM 和 AMD GPU 支持的开发、UFS/FFS 和 callout 性能的提升以及网络堆栈中的多路径 TCP 实现。
...
在对崩溃进程进行事后分析时，有时非常有用的是获取关于进程崩溃时的内核信息，例如打开的文件描述符或资源限制。对于活跃进程，可以通过 sysctl(3) 接口获取此信息，例如使用 procstat(1)。
...
该项目的重点扩展至包括新的 iSCSI 启动器。与旧版本相比，它更加可靠、更加用户友好，并且速度略快。它使用与旧版本完全相同的配置文件格式，以便更容易迁移。

在目标端方面，已经验证其可以与主要启动器（FreeBSD、Linux、Solaris、Windows 和 VMWare ESX）正常工作。
...
和往常一样，任何有兴趣帮助处理 PR 队列的人都可以加入我们，参与 EFnet 上的 #freebsd-bugbusters 频道。我们始终欢迎更多的帮助，无论你是对处理接收到的 PR 感兴趣，生成修复现有问题的补丁，还是帮助进行数据库的整理（识别重复的 PR，已解决的 PR 等）。这是更深入参与 FreeBSD 的一个好方法！
...
鉴于安全事件，核心团队与安全团队之间的联络角色已恢复，由 Gavin Atkinson 承担此角色。核心团队在与 Ports 管理团队、集群管理员及 FreeBSD 基金会董事会的紧密合作下，致力于解决当前的二进制包构建集群问题及相关的安全问题。[妥协页面](http://www.freebsd.org/news/2012-compromise.html) 已保持更新。

FreeBSD 项目再次提交了谷歌编程之夏计划的申请。

本季度为 2 位新提交者授予了访问权限，并收回了 1 位提交者的提交权限以保管。
...
Ports 包含大约 24,300 个 Ports，而 PR 数量仍接近 1600。

在第一季度，我们增加了 4 位新提交者，收回了 1 位提交者的提交权限以保管，并恢复了 1 位提交者的提交权限。

在 2 月，Mark Linimon（linimon）辞去了团队的职责。Mark 是团队中第二长时间服役的成员。Mark 曾花费大量时间重构和文档记录 portbuild 软件，以确保可以恢复 pointyhat 服务。
...
Bryan Drewery（bdrewery）于 3 月加入了 Ports 管理团队，带来了他在维护 portupgrade、portmaster、协助 pkgng 以及共同开发 poudriere 方面的丰富经验。
...
- J.R. Oldroyd 开始着手将驱动程序回移植到 9-STABLE 版本，并且现在已经运行得相当好。他不得不从虚拟内存子系统中回移植一些特性，这些特性可能还需要虚拟内存方面进一步的完善。

[Yakaz](http://www.yakaz.com/) 借给 Jean-Sébastien 一台计算机，使他能够测试基于 RV630 的独立显卡，并将来能够测试其他 PCIe 显卡。已经有几位用户热心地测试了这个驱动程序，感谢所有这些贡献者！
...
如果线程或信号处理程序调用 fork() 和 exec()，文件描述符可能会不希望地传递到子进程，这可能导致挂起（如果管道未关闭）、超出文件描述符限制和安全问题（如果子进程权限较低）。一种解决方案是通过各种新的 API，在分配文件描述符时原子地设置关闭执行标志。如果存在这些新特性，一些现有软件将使用它们，甚至可能在没有这些特性时拒绝编译。
...
在 FreeBSD 中，定时器由 callout 功能提供，该功能允许注册一个带有参数的函数，在指定的未来时间调用该函数。该子系统存在一些问题，如无法处理高分辨率事件或其固有的周期性结构，这可能导致虚假唤醒和更高的功耗。一些使用者，如高速网络、VoIP 和其他实时应用，需要比当前允许的更高的精度。此外，尤其是随着近年来笔记本电脑的普及，由于中断唤醒 CPU 使其从睡眠状态中恢复，可能会浪费大量能量。最近对该子系统的更改解决了这些长期存在的问题，并引入了新的编程接口，以利用这些新特性。
...
Clang 已被设为 ARM 的默认编译程序。许多与 LLVM 和 clang 相关的问题已经被发现、报告并在上游修复。
...
我们自豪地宣布 FreeBSD Haskell 团队已经将现有的 Ports 更新为最新的稳定版本。我们还增加了一些新的 Port，使 FreeBSD Ports 中的 Haskell Ports 数量超过 400，涉及了许多流行的软件，如 xmonad、git-annex、pandoc 或各种 Web 框架实现。所有这些更新将在即将发布的 8.4-RELEASE 中提供。我们还了解到，Haskell Ports 在 DragonFlyBSD 的 dPorts 中也成功使用。

在我们的开发仓库中，增加了一些基于 LLVM 的代码生成的可选支持，使用的是 GHC LLVM 后端。这在 FreeBSD 上也大多可以工作，尽管一些 Ports 仍需修复，因此仍被视为实验性。

#### 待办事项

1. 尝试使用 clang（作为系统编译程序）构建 GHC。
2. 将待提交的 Haskell Ports 提交到 FreeBSD Ports。
...
GNOME/FreeBSD 团队最近将 Glib 2.34、Gtk+ 2.24.17 和 Gtk+ 3.6.4 合并到 Ports 中，C++ 绑定也得到了更新。除此之外，一些 "低级" GNOME Ports 也得到了更新，例如 libsoup、gobject-introspection、atk 和 vala 等。Telepathy 堆栈和 Empathy 也已更新。
...
- Gustau Perez i Querol 接手并开始更新旧的 GNOME 3.4 Ports 到 3.6。撰写时这些更新尚未在 GNOME 开发仓库中提供。为了表彰他的努力，他被授予了 FreeBSD GNOME 团队成员资格。
- Jeremy Messenger (mezz) 完成了 Mate 1.6 的工作，待稳定后将加入 Ports 中。
...
2. 将更多更新的 Ports 从 MC 合并到 Ports 中。
3. 测试最新的 Glib/Gtk 版本与现有 Ports 的兼容性，并在准备好后将其导入 Ports 中。
...
1. 更新过时的 Ports，请查看 PortScout 获取列表。
...
Xfce FreeBSD 团队已更新了许多 Ports，特别是：
...
在此期间的大部分工作集中在更新、测试和稳定化开发仓库。一些 xorg 应用程序和其他一些叶子 Ports 已作为此工作的一部分提交。之后，发布了 CFT（Call For Testing），请求帮助测试开发中的剩余部分，包括所有主要库和 xorg-server 的更新。
...
2. 继续测试和更新与 xorg 相关的 Ports。需要移植和最终集成到 Ports 中的有新版本的 xserver、MESA 以及相关的 OpenGL 库。
