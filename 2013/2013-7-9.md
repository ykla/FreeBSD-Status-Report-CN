...
- 原文：[FreeBSD Quarterly Status Report 3rd Quarter 2013](https://www.freebsd.org/status/report-2013-07-2013-09.html)
- 最后作者：FreeBSD 项目

## 引言

本报告涵盖 2013 年 7 月至 9 月期间与 FreeBSD 相关的项目。这是 2013 年计划发布的四个报告中的第三个。

在 FreeBSD 世界中，我们度过了又一个非常活跃的三个月，其中包括两次开发者峰会（BSDCam 和 EuroBSDcon），这些将会在单独的状态报告中详细介绍。FreeBSD 在安全性方面继续加大力度，改进了随机数生成的性能和可靠性，并对基本系统中的程序进行了更多隔离。对于开发者来说，正在开发新的现代化调试器。此外，除了基本系统中已经具备的 C++11 和 C11 支持外，FreeBSD 还在通过 Port 支持 Objective-C 和 Ada 语言方面进行显著的现代化，使 FreeBSD 成为开发这两种语言的第一流平台。服务器用户将会看到 iSCSI 栈和可扩展性的改进，允许在普通硬件上每秒超过一百万次 I/O 操作，而桌面用户则会看到对新 GPU 的 X 支持和可能的 X 替代方案的改进。

感谢所有报告者的出色工作！本报告包含 30 个条目，希望你阅读愉快。
...
在 2013 年第三季度，核心团队集中精力正式推出了 **pkg.freebsd.org**，这是项目的官方 **pkg(8)** 仓库，合作伙伴包括 Ports 团队、安全团队和集群管理团队。同时，计划逐步淘汰旧版 **pkg_add(1)** 的使用，使 **pkg(8)** 成为 FreeBSD 默认的二进制软件包管理解决方案，并在 10.0-RELEASE 版本中推出。Thomas Abthorpe 担任核心团队与 Ports 团队之间的联络员，以提高合作效率。

David Chisnall 加入了发布季度状态报告的团队，并编写了一份关于 BSDCan 2013 开发者峰会结果的特别报告。David 还主导了组织在英国剑桥举行的非定期开发者峰会，最终该会议在 8 月底举行。关于剑桥会议讨论的内容，详细报告的准备工作仍在进行中。

今年 7 月，Gavin Atkinson 对仓库访问文件进行了大规模清理，结果是发放了 5 个新的源代码提交权限，并将大部分 12 个月以上未使用的源代码提交权限移交到安全存档中。

### FreeBSD Ports 团队

**联系方式**：FreeBSD Ports 团队 <[portmgr@FreeBSD.org](mailto:portmgr@FreeBSD.org)>

Ports 树包含约 24,400 个 Port，而 PR（问题报告）数量超过 1,900。在第三季度，我们增加了四名新的提交者，并备份保存了六个提交权限。

大量的工作集中在调整和改进基础设施，以现代化更新，为 **pkg(8)** 替换旧的 **pkg_add(1)** 基础设施做准备，同时也为 FreeBSD 10.0 做准备，采用 Clang 作为默认编译器，**libc++** 作为默认 C++ 标准库，以及将 **iconv(1)** 集成到 `libc` 中。

实现了自动化质量保证程序，特别是 **pkg-fallout**。建议所有 Port 开发者订阅相关的邮件列表（请参见链接），并为 **pkg(8)** 和 Clang 准备工作贡献力量。

进行了多次测试，确保尽可能多的软件包可以在 9.2 版本发布时可用。
...
**联系方式**：FreeBSD 邮件管理团队 <[postmaster@FreeBSD.org](mailto:postmaster@FreeBSD.org)>
...
- 创建了 **freebsd-users-jp** 邮件列表，由佐藤広生提出。
- 停用了 **freebsd-mozilla** 邮件列表，由 Florian Smeets 提出。
- 与 FreeBSD 集群管理员合作，为进出邮件服务器启用了 TLS 支持。
- 开始讨论和探索当前及可能的未来邮件和垃圾邮件过滤方案。
- 启动了停用 **aic7xxx** 邮件列表的流程，预计将于 2013 年 10 月 12 日完成。

### FreeBSD 发行工程团队

**联系方式**：FreeBSD 发行工程团队 <[re@FreeBSD.org](mailto:re@FreeBSD.org)>

FreeBSD 发行工程团队已完成 9.2-RELEASE 版本的发布过程。发布周期因最后时刻新增 9.2-RC4 而有所变化。9.2-RELEASE 于 9 月 30 日发布，推迟了四周。

FreeBSD 10.0-RELEASE 的发布周期已开始，强烈建议进行测试。测试版本的安装镜像和虚拟机镜像已上传到 FreeBSD 项目的 FTP 服务器。
...
由于我们自己（旧且不稳定的）Coverity Prevent 实例已消失，我们现在已完全过渡到由 Coverity 运营的 Scan 项目（请参见链接），开源项目可以使用该项目来发现其源代码中可能存在的缺陷。
...
我们正在寻找共同管理员来管理这两个项目，以增加服务的生存机会和容错能力。名利等着你！
...
对 OpenCrypto（内核的加密框架）中 AES-NI 实现的改进已提交，显著提高了 AES-XTS 和 AES-CBC 解密性能。使用 AES-XTS 的 **geli(8)** 在 **gnop(8)** 上的性能比旧代码提高了大约三倍。
...
**链接**：[Newcons 项目分支](http://svn.freebsd.org/base/user/ed/newcons/)
...
该项目最初由 Ed Schouten 发起，且在 2013 年已具备以下功能（仅列举部分）：

- 支持拉丁文、西里尔文及其他简单字符集的 Unicode 字体。
- 支持 Unicode 输出。
- 支持图形模式。
- 支持文本模式。
...
1. 实现通用帧缓冲接口，提供通过 **/dev/fb*** 直接访问帧缓冲区，并由 Newcons 自动管理虚拟终端。
2. 支持鼠标操作，使用 **sysmouse(4)** 实现复制/粘贴。
3. 改进锁定机制。
4. 修复错误。
5. 将其集成到 FreeBSD `head`。
6. 将其集成到 FreeBSD 10.0 中。
7. 实现将键盘输入的非 ASCII 字符映射到 Unicode。
8. 调整现有的屏幕保护程序。
9. 最后但同样重要的是，欢迎进行测试！

### GEOM 直接调度与精细粒度 CAM 锁定
...
现有的 GEOM 设计假设大多数 I/O 处理仅由两个内核线程（`g_up()` 和 `g_down()`）完成。这在某些情况下简化了锁定，但限制了 SMP 可扩展性，并产生了额外的调度开销。该项目为 GEOM 引入了直接 I/O 调度的概念，适用于已知是安全且高效的情况。这意味着通过使用一个或两个新标志标记一些 GEOM 消费者和提供者，声明在某些情况下可以使用直接函数调用，而不是正常的请求排队。这允许避免在 GEOM 内部进行任何上下文切换，从而同时处理来自多个调用线程的多个 I/O 请求。

让 GEOM 通过多个并发调用传递到底层层次暴露了 CAM 中的重大锁拥塞。在现有的 CAM 设计中，所有连接到同一 ATA/SCSI 控制器的设备共享一个锁，而这个锁可能由于多个控制器硬件访问或代码逻辑而变得非常忙碌。实验表明，仅仅应用上述 GEOM 直接调度更改会导致系统 CPU 时间的 60% 或更多用于多个调用者试图获取这些锁，从而消耗掉 GEOM 直接调度的任何好处。
...
这些更改已经多人测试，并将在 FreeBSD `head` 中提交，之后将在 FreeBSD 10.0 发布周期结束后合并到 **stable/10**。
...
由于 10.0-RELEASE 即将发布，原生 iSCSI 栈的优先级有所调整，从性能优化转向确保新栈的可靠性、功能完整性，并能够与各种实现正确互操作。大量时间投入到了测试和调试，主要是在启动器端，以确保它与其他目标（如 Solaris COMSTAR）正确工作，并在连接问题等边缘条件下表现正常。尽管如此，仍然实现了一些基本的优化，如支持即时数据。文档也有所改进，并将在 FreeBSD 手册中新增一节，介绍新栈的使用方法。

新栈已提交至 `head`，并将作为 10.0-RELEASE 的一部分发布。针对早期用户报告的问题，仍在进行修复工作。
...
**联系方式**：Mark Murray <[markm@freebsd.org](mailto:markm@freebsd.org)>

**联系方式**：Arthur Mesh <[arthurmesh@gmail.com](mailto:arthurmesh@gmail.com)>

**联系方式**：Dag-Erling Smørgrav <[des@freebsd.org](mailto:des@freebsd.org)>

随机数需要比直觉上认为的更复杂的思考和准备。对于模拟，通常需要重复但足够混乱的数字序列，以实现所需的实验重复能力，许多程序员都熟悉这些。对于加密，至关重要的是攻击者不能预测或猜测输出序列，从而为口令或“密钥材料”等安全关键的秘密材料提供来源。

FreeBSD 的随机数生成器，通过伪文件 **/dev/random** 提供不可预测的数字，旨在用于加密，因此它是加密安全伪随机数生成器（CSPRNG）。其安全性由输出生成器的精心设计（基于块加密）和输入熵积累队列提供。后者使用哈希来积累从内核各处收集的随机信息，以为生成器提供高度不可预测的输入。执行此操作的算法 Yarrow，由 Schneier 等人设计，可通过网络搜索找到。

FreeBSD 的 CSPRNG 还允许某些视为“高质量”的随机源直接为 **random(4)** 设备提供输入，而无需经过 Yarrow。随着近期有关政府监控和参与选择这些“高质量”源的揭露，人们认为这些源不再可信，因此也必须通过 Yarrow 进行处理。

这个问题在 8 月的剑桥开发者峰会和 9 月的 EuroBSDcon 2013 上进行了不同层次的讨论。

此项工作现已完成，**random(4)** CSPRNG 已提升到更为偏执、现代化的标准，针对其熵源采取不信任的策略。还进行了基础设施工作，以便系统管理员更方便地选择熵源。
...
SDIO 是作为现有 SD 卡标准的扩展设计的接口，用于通过标准 SD 控制器将不同外设连接到主机。目前市场上销售的外设包括 WLAN/BT 模块、摄像头、指纹识别器和条形码扫描仪。该驱动程序作为现有 MMC 总线的扩展实现，增加了许多新的 SDIO 特定总线方法。用于 Marvell SDIO WLAN/BT（Avastar 88W8787）模块的驱动原型也正在开发中，参考了现有的 Linux 驱动程序。

SDIO 卡检测和初始化已经完成，大多数需要的总线方法已实现并测试过。正在进行的工作是设计良好的锁定模型。WiFi 驱动程序能够加载固件到卡中并初始化它。

#### 待办事项

1. SDIO 栈：设计锁定模型，定义如何处理中断（在 SDIO 控制器级别、MMC 栈级别和子驱动程序级别）。
2. Marvell SDIO WiFi：连接到 FreeBSD 网络栈，编写代码实现所需的功能（如发送和接收数据、网络扫描等）。
3. 实现分离路径。由于开发所用的 DreamPlug 不具备外部支持 SDIO 的插槽，因此无法进行测试。

### VirtIO 网络多队列

**链接**：<http://svnweb.freebsd.org/changeset/base/255112>

**联系方式**：Bryan Venteicher <[bryanv@freebsd.org](mailto:bryanv@freebsd.org)>

VirtIO 网络驱动程序 **vtnet(4)** 供 FreeBSD 系统在 **bhyve(4)** 和 Linux 的 KVM 等虚拟机监控程序中运行。最近，它增加了对多个队列的支持，并进行了显著的清理，并支持一些附加功能。
...
为 VMware 虚拟机移植的 OpenBSD **vmx(4)** 以太网驱动程序已提交。该驱动程序可以代替 VMware Tools `vmxnet3` 驱动程序，后者目前不支持 10.0-RELEASE（或任何高于 9.0-RELEASE 的版本）。
...
### Cubieboard2 上的 FreeBSD

**链接**：<http://svnweb.freebsd.org/changeset/base/254056>

**联系方式**：Ganbold Tsagaankhuu <[ganbold@FreeBSD.org](mailto:ganbold@FreeBSD.org)>

Allwinner A20 SoC 的初步支持已提交至 `head`。Cubieboard2 上的 A20 SoC 与 Cubieboard1 上的 A10 在引脚上是兼容的，FreeBSD 支持以下外设：
...
3. 提供某种机制通过 EC2 `user-data` 配置实例。这可能涉及使用 `cloud-init`，或可能是新的系统。

### FreeBSD/pseries

**链接**：<http://svnweb.freebsd.org/changeset/base/255643>

**联系方式**：Andreas Tobler <[andreast@freebsd.org](mailto:andreast@freebsd.org)>

**联系方式**：Nathan Whitehorn <[nwhitehorn@freebsd.org](mailto:nwhitehorn@freebsd.org)>

从 FreeBSD 10.0-ALPHA4 开始，`projects/pseries` 分支已合并到 FreeBSD `head`。这使得 FreeBSD/powerpc64 可以在 IBM POWER 逻辑分区上运行，并支持某些类别的旧款 IBM 类型的 PowerPC 硬件。
...
在解决了所有剩余问题并从 9.2-RELEASE 开始，针对该架构的发布在 FreeBSD 项目集群上进行交叉构建。正如大家可能已经注意到的，这意味着从现在起，sparc64 安装集和镜像（包括 ALPHA、BETA 和 RC 构建）将与 FreeBSD 支持的其他平台的镜像一起提供。从 2013 年 8 月开始，自动交叉构建的 FreeBSD/sparc64 每月快照将通过官方项目镜像分发。希望这很快能通过 **freebsd-update(8)** 支持扩展到 sparc64。
...
- 执行了广泛的测试和基准测试：
  ...
  - LMbench：内存延迟降低了 38%。
  - 自托管 `buildworld`：使用 GCC 时构建时间缩短了 20%。
- 最终集成到 FreeBSD `head`。
...
Capsicum 是 FreeBSD 的沙盒子系统，为程序员提供了能力模块，允许将权限细粒度地分配给较低权限的进程。Casper 是友好的守护进程，提供服务给沙盒进程，允许基于策略的访问到特权服务，如 DNS 解析。

Capsicum 及相关项目（如 Casper、`libnv` 等）的工作进展顺利。`cap_rights_t` 的重构已提交到 FreeBSD `head`，并将包含在 10.0 中。这使我们可以在文件描述符上拥有比以前 64 个权限限制更多的权限，之前的限制几乎达到。这一变化不向后兼容，因此将其包含在 10.0 中非常重要。

`libnv`，作为 Casper 服务与消费者之间的通信工具，但希望能更广泛使用，已完成，并附带了一套很好的回归测试。

使用 Capsicum 框架进行沙盒化的应用程序数量正在增加。目前已有大约 10 个在基本系统中，更多的尚未提交。
...
2. 为 Casper 服务实现回归测试。
3. 完成 `libnv` 的文档。
4. 开始使 `libc` 更适合沙盒使用，即修改当前在首次使用时打开文件的函数，如 **strerror(3)**、**strsignal(3)**、**localtime(3)**、`login_get*()`、**getservent(3)**、**getprotent(3)** 和 **getrpcent(3)**，如果已经在能力模式沙盒中，这些操作可能太晚。
...
自上次状态更新以来，LLDB 移植已做出多项改进。单元测试失败已排查，并在 LLDB 的错误跟踪器中输入了缺陷。结合 `lldb` 构建机器人，可以迅速识别其他正在进行的开发所引入的新失败。还添加了核心文件支持。

LLDB 快照已导入到 FreeBSD 基本系统，且从 SVN 修订版 255722 起可用。默认情况下尚未构建，但可以通过在 **src.conf(5)** 中添加 `WITH_LLDB=` 启用。
...
随着第一个新的 Ada 编译器 Port 的引入，基于 GCC 4.6 的 `lang/gnat-aux`，GNAT 编程工作室（多语言集成开发环境）、XML/Ada 和 GtkAda 是首批复兴的 Ada Port 之一。

使用最新的编译器，基于 GCC 4.7 的 `lang/gcc-aux`，创建了完整的 Ada 框架，并采用了新的 `USES=` 框架。目前约有 20 个 Port 已成为此框架的一部分，包括 Florist、ASIS、GPRbuild、QtAda、AdaControl、AdaBrowse、PolyOrb 和 AWS（Ada Web 服务器）。
...
已完成支持不同 Python Port 安装软件包的任务。新的元 Port 结构替代了原始的 Python Port 行为，并将在未来几个月中增强，以支持同时安装不同 Python 版本的软件包。

Python Port 得到了增强，增加了自动打包列表创建和替换宏，这改善了与多个 Python 版本的兼容性，并减少了打包列表的大小。
...
Glib 2.36 和 Gtk 3.8 已导入到 Ports 中。GNOME 团队目前正在致力于提高 GNOME 3.6 的质量。随 GNOME 3 提供的 `multimedia/cheese` 版本现在可以使用 **devd(8)** 通过 `multimedia/webcamd` 找到摄像头。对 `www/webkit-gtk3` Port 进行了一些构建改进，但它仍然比较脆弱。

MATE，从现在不再维护的 GNOME 2 代码库分支出来的桌面环境，接近完成。

GNOME 2 将在不久的将来移除。具体的移除方式和时间尚不清楚。
...
GNUstep 是开源实现的 Objective-C API，基于 OpenStep 规范，即 Apple 以 Cocoa 品牌化的规范。由于 FreeBSD 和 OS X 的 libc 之间的相似性，FreeBSD 成为吸引人的平台，适合移植 OS X 应用程序，并通过添加 GNUstep 来实现。

FreeBSD 中的 GNUstep Port 现已更新到最新版本，且默认使用 GNUstep 的 Objective-C 运行时和 Clang 3.3 构建，支持非脆弱的 ABI。这意味着支持 Objective-C 的所有现代特性，包括自动引用计数（ARC）和最近的语法改进。

`devel/gnustep` 元 Port 将安装所有核心 GNUstep 库，准备进行开发。`x11/gnustep-app` 元 Port 将安装 Ports 中所有基于 GNUstep 的应用程序和库。这些应用程序和库中的许多已经很老，且未经过最新版 GNUstep 的良好测试，因此目前可以认为它们是实验性的。我们目前正在更新它们，包括将一些弃用的上游位置迁移到 GNUstep 应用程序项目（GAP），该项目接管了多个旧版 GNUstep 程序的维护。
...
`radeonkms` 驱动程序已于 8 月底提交到 FreeBSD `head`，并将包含在 10.0-RELEASE 中。自初次提交以来，已进行了多个修复，现在看起来相当稳定。然而，缺少的主要特性是对挂起/恢复的支持：测试计算机在恢复时几乎总是会导致 GPU 锁死。

感谢 Mesa 更新以及 `x11-drivers/xf86-video-ati` 更新至 Ports 中的 7.2.0，现在所有组件已就绪，允许用户使用最新的 AMD 显卡（最多支持 HD7000，可能包括某些 HD8000）。

该驱动程序今后只接收错误修复，重点将转向更新 DRM 通用代码和 `i915` 驱动程序。

`i915kms` 和 `radeonkms` 视频驱动程序共享的通用 DRM 代码现在已相当陈旧。工作已开始，将其更新并与 Linux 3.8 版本同步。此代码在 GitHub 上可用。
...
实验性的 **devd(8)** 后端已添加到 Port `x11-servers/xorg-server`。这允许 X.Org 使用 **devd(8)** 动态检测和配置输入设备（例如，键盘和鼠标）。
...
1. 持续跟踪 Mesa 9.2 或更高版本和 `xorg-server` 1.14。目前这两者都受阻，但跟踪上游的进展是很有价值的。
2. 测试并报告 AMD GPU 的成功与失败。
3. Wayland 构建现已开始。正在对 Weston 进行工作，查看是否存在运行时问题。Weston 是 Wayland 的参考合成器。
...
**链接**：<http://www.freebsd.org/doc/en/books/fdp-primer/book.html>

**联系方式**：Warren Block <[wblock@FreeBSD.org](mailto:wblock@FreeBSD.org)>

FreeBSD 文档项目初学者指南（Primer）没有与文档本身的变化同步更新。一些章节已过时，其他部分则冗长且令人困惑，且对文档中新变化的描述缺失。7 月，Warren 对整个 FDP 初学者指南进行了相当深入的编辑，简化了内容并提高了清晰度。章节和部分已移到更合乎逻辑的顺序，且信息已更新，以更好地指导当前的状态。标记示例已添加并修订。风格指南也已扩展和更新。现在该指南更加一致且易于使用。像往常一样，仍有改进的空间，欢迎添加或纠正内容。

#### 待办事项

1. 编写关于使用 **mdoc(7)** 编写手册页的入门章节，将是很好的补充。

### 实体文档分支

**链接**：<http://svnweb.freebsd.org/changeset/doc/42226>

**联系方式**：René Ladan <[rene@FreeBSD.org](mailto:rene@FreeBSD.org)>

实体项目分支已成功合并到主文档分支中，合并版本为 doc 仓库的第 42226 号修订版（见链接）。该分支的目的是移除 `authors.ent` 和 `developers.ent` 中重复的作者定义。迁移内容至前者后，后者文件已删除。尽管大多数更改对最终用户不可见，但《提交者指南》已做出相应更改，以适应增加新提交者相关的变化。也已通知翻译者此更新。上一报告中提到的最大难题——处理 `<email>` 元素，已在 Gábor Kövesdán 的帮助下得到解决。
...
**联系方式**：Ambarisha Bhatlapenumarthi <[ambarisha@freebsd.org](mailto:ambarisha@freebsd.org)>

**联系方式**：李鑫 <[delphij@freebsd.org](mailto:delphij@freebsd.org)>

这是谷歌编程之夏 2013 项目，旨在用智能下载管理器服务替代基于 **fetch(1)** 的方法来获取第三方应用程序（Port）的分发文件（如源 tarball）（更多信息请参见链接）。

项目 wiki 中强调的所有模块已完成（见链接）。具体来说：

- 接收并提供下载请求的服务。它从不同的镜像站点采样下载速度，并利用这些信息选择下一个请求的最佳镜像。如果它发现从另一个镜像站点完成下载会比继续使用当前镜像站点更快，它可以将任务迁移到其他镜像。
- 客户端（`dmget`）还新增了状态转储功能，可以转储关于当前活动下载、镜像站点的下载速度等信息。
