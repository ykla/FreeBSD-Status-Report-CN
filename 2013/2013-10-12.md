...
Xen 是原生（裸金属）虚拟机监控程序，提供服务，允许多个计算机操作系统在同一计算机硬件上并发执行。Xen 4.4 将引入虚拟化模式 PVH——在 HVM（完全虚拟化）容器中的 PV（半虚拟化）。这本质上是一个使用半虚拟化驱动程序进行启动和 I/O 的半虚拟化客户端。除此之外，它还使用硬件虚拟化扩展，无需仿真。
...
输入/输出内存管理单元（IOMMU）是一种内存管理单元（MMU），它将支持直接内存访问（DMA）的 I/O 总线连接到主内存；因此，I/O 虚拟化由芯片组执行。IOMMU 的示例是图形地址重映射表（GART），该表由 AGP 和 PCIe 显卡使用。英特尔已发布了 IOMMU 技术规范“虚拟化技术”，用于定向 I/O，简称 VT-d。
