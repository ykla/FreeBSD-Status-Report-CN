...
- 原文：[FreeBSD Quarterly Status Report First Quarter 2022](https://www.freebsd.org/status/report-2022-01-2022-03/)
- 版本：2022 年 6 月 11 日
...
那么，首先让我做一下介绍，我非常高兴接受 Pau Amma 的帮助，他协助审阅工作已有一段时间；Lorenzo Salvadore 也在积极投入，为报告制作提供工具，以减少这项工作的负担；还有 Sergio Carlavilla，他将协助处理那些无法轻易自动化的工作。

本报告涉及了非常多样的主题，包括但不限于无障碍、系统启动加速、GEOM 联合体的实现、WiFi 状况的变化，以及其他许多内容。
...
[基金会新闻和活动](https://www.freebsdfoundation.org/news-and-events/)
...
如承诺所言，我们更新了 2022 年的筹款进度表。截至目前，我们已经筹集了超过 84,000 美元，目标为 1,400,000 美元。我们感谢个人和企业捐赠者对我们今年工作的支持。同时，我们还要特别感谢我们的金牌赞助商 Facebook、银牌赞助商 VMware 和 Tarsnap，以及为项目提供免费托管服务的公司：Bytemark、365 Data Centers、NYI、NextArray、Sentex 数据通信和 NCTU 计算机科学系。

你可以通过阅读本报告中关于我们在第一季度支持的内容，以及我们的春季通讯，来了解我们如何使用你的捐款。
...
SV48 是为 39 位虚拟地址空间不足的系统而设计的。此更改将用户映射的大小从 256GB 增加到 128TB。内核映射暂时保持不变。

目前，SV48 模式默认禁用，但可以通过可调参数启用。请注意，现有硬件并不支持 SV48，但 QEMU 支持。

- 在 pmap_bootstrap() 中，分配一个 L0 页并尝试启用 SV48 模式。如果对 SATP 的写入不生效，内核将继续在 SV39 模式下运行。
...
- 临时修复停滞的 USB 枚举、蓝牙和 S3 恢复。临时修复方法是避免附加到一些较新的 Intel 控制器，这些控制器需要加载固件，与 `ng_ubt_intel` 和 `iwmbtfw` 实现的方式不同，因此它们暂时无法使用。
...
基金会提供一名全职工作人员并赞助项目，以改善 FreeBSD 项目的持续集成、自动化测试和整体质量保证工作。
...
FreeBSD 基金会赞助了全球多个会议、活动和峰会。这些活动可能与 BSD 相关、开源或面向代表性不足群体的技术活动。我们支持与 FreeBSD 相关的活动，帮助提供知识共享、共同开展项目和促进开发者与商业用户之间合作的场所。这一切都有助于提供健康的生态系统。我们支持非 FreeBSD 相关的活动，目的是宣传和提高 FreeBSD 的知名度，增加 FreeBSD 在不同应用中的使用，并招募更多的贡献者。我们继续参加虚拟活动，并开始规划 2022 年 6 月的开发者峰会。
...
- 发送了 2021 年影响报告，展示了我们在去年如何支持项目 [https://freebsdfoundation.org/blog/2021-freebsd-foundation-impact-report/](https://freebsdfoundation.org/blog/2021-freebsd-foundation-impact-report/)
...
- 参加 Open Source Voices 播客 - 该期播客将于 4 月底播出 [报告团队注：该期播客已经播出，现已在 [https://www.opensourcevoices.org/29](https://www.opensourcevoices.org/29) 可供收听；遗憾的是，没有也不会提供转录。]
...
[FreeBSD 13.1 版本信息](https://www.freebsd.org/releases/13.1R/) [报告团队注：此链接在 13.1-RELEASE 发布后由状态报告团队添加]
...
我们期待在这个周期结束时推出又一个一贯稳定的版本，并希望未来能推出更多稳定版本，涵盖其他分支。
...
- 安装了额外的 aarch64 软件包构建器
  - ampere3.nyi.freebsd.org
  - 与 ampere[12].nyi.freebsd.org 规格相同
...
- 删除了关于间接使用 Python 2.7 的 Port 的弃用说明

目前，Ports 中共有 46,800 余个 Port。其中有 2,700 个开放的 Ports PR，其中 680 个未分配。上个季度，主分支共进行了 9,403 次提交，157 位提交者参与其中，2022Q1 分支进行了 683 次提交，由 63 位提交者完成。与上个季度相比，这意味着主分支的活动略有下降，而开放的 PR 数量略有增加。

在上个季度，没有新的提交者加入，portmgr 因无活动而接管了一个提交权限以妥善保管。
...
Pau Amma <[pauamma@gundo.com](mailto:pauamma@gundo.com)>

FreeBSD 无障碍性讨论 <[freebsd-accessibility@freebsd.org](mailto:freebsd-accessibility@freebsd.org)>
...
**联系方式**：Marshall Kirk McKusick <[mckusick@mckusick.com](mailto:mckusick@mckusick.com)>
...
rtw89 驱动已经能够成功发送数据包，目前在接收帧方面存在问题。待 rtw88 的问题解决后，将会进一步调查，预计 rtw89 也将及时提交到 FreeBSD 的 git 仓库中。
...
在第一季度，工作继续进行，共有大约 70 次提交。驱动和固件的更新减少了与 Linux 版本的差异，并为我们带来了错误修复和改进。对 LinuxKPI 802.11 兼容层的改动旨在避免固件崩溃和可能导致的内核恐慌，并进行了其他改进。

已提交了 LinuxKPI PCI 驱动的自动加载支持。这意味着，iwlwifi(4) 驱动将自动在启动时加载，如果检测到支持的卡，不需要用户交互。考虑到当前驱动的状态和下一个版本，已决定 iwm(4) 支持的芯片组将继续附加到 iwm(4)，只有更新的和其他不支持的芯片组才会使用 iwlwifi(4) 驱动。这一点可能会在 CURRENT 中发生变化，待 iwlwifi(4) 提供比 iwm(4) 更好的支持。
...
在过去的一个季度里，我继续改进 FreeBSD WireGuard 驱动程序的工作。在 FreeBSD 端，我增加了对 XChaCha20-Poly1305 AEAD 加密算法的支持。我还为小的平坦缓冲区添加了一个专用 API，以支持 [X]ChaCha20-Poly1305。最后，我为来自 libsodium 的 curve25519 实现添加了一个 API 包装器。

对于 WireGuard 驱动程序，我编写了一系列补丁，更新驱动程序，使用像上述提到的加密 API 来替代内部的加密算法实现。该系列还包括一个修复，避免调度过多的加密任务，以及一些其他小修复。这些补丁目前正在等待审核。
...
Dmitry Salychev <[dsl@mcusim.org](mailto:dsl@mcusim.org)>
...
在过去的一个季度里，没有授予新的文档提交权限，并收回了一个提交权限以妥善保管。
...
- 从文档和网站中删除谷歌分析。
- 在文档和网站页面中添加最后修改信息。
- 为 13.1-RELEASE 标记了 FreeBSD 文档集。
- 将首个印尼语翻译添加到文档树中。
...
- **KDE Plasma Desktop**（所有的 ports `/plasma5-`）在年初更新到了 5.23.5。由于这是在季度分支发布后不久发生的，所以已经通过 MFH 处理。长期支持版本 5.24 于二月中旬发布。FreeBSD ports 并不坚持 LTS 版本，将遵循常规发布计划。5.24.3 按时在三月发布。
...
- **x11/plasma5-plasma-workspace** 现在可以正确编辑登录和帐户信息。
...
在 2022Q1 期间，我们向季度分支推送了 LibreOffice 7.2 的维护补丁，并将最新的 7.3 版本及所有配套库（如 MDDS、libIxion 等）带入了 Ports。
...
在大约二十年的时间里维护 FreeBSD 的 ports lang/gcc* 之后，到了交接的时刻，并且大部分需要退居二线。不幸的是，尽管多次呼吁求助，接力棒基本掉到了地板上。
...
- 我们已经移除了不再维护的 ports lang/gcc9-devel 和 lang/gcc10-devel，不幸的是保留了 ports lang/gcc11-devel 和 lang/gcc12-devel，最好能看到它们即使不是每周更新，也能有些许定期更新。

### PortConfig

**链接**：

[PortConfig 仓库](https://gitlab.com/alfix/portconfig/)

**联系方式**：

Alfonso Sabato Siciliano (上游) <[asiciliano@FreeBSD.org](mailto:asiciliano@FreeBSD.org)>

Baptiste Daroussin (Port) <[bapt@FreeBSD.org](mailto:bapt@FreeBSD.org)>
...
- 配置后，客户机可以暴露 [wpa_supplicant(8)](https://man.freebsd.org/cgi/man.cgi?query=wpa_supplicant&sektion=8&format=html) 控制套接字，这使得可以直接从主机使用相关的工具，如 [wpa_cli(8)](https://man.freebsd.org/cgi/man.cgi?query=wpa_cli&sektion=8&format=html) 或 [wpa_gui(8)](https://man.freebsd.org/cgi/man.cgi?query=wpa_gui&sektion=8&format=html)，这些工具可以通过 port/软件包 `net/wpa_supplicant_gui` 安装。
- 一切都包含在一个易于安装和卸载的单一软件包中。这个软件包提供了一个 [rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 系统服务，在启动时自动启动客户机，并在关机时停止它。
...
`#helloSystem` 在 `irc.libera.chat`，镜像到 [`#helloSystem:matrix.org`](https://matrix.to/#/%23helloSystem:matrix.org?via=matrix.org) 在 Matrix
...
[Pot 项目在 GitHub 上的组织](https://github.com/bsdpot)
...
Pot 是一个 Jail 管理工具，[也支持通过 Nomad 进行编排](https://www.freebsd.org/news/status/report-2020-01-2020-03/#pot-and-the-nomad-pot-driver)。
...
Potluck 旨在成为 FreeBSD 和 pot 的平台，就像 Dockerhub 是 Linux 和 Docker 的平台一样：一个 pot flavour 和完整容器映像的仓库，可供 Pot 使用，并且在许多情况下也可供 Nomad 使用。
...
在这里，我们计划发布 ansible playbooks，允许用户从零开始基于这些工具创建基于 FreeBSD 的编排环境。
...
[项目网站和文档](https://www.fpart.org/)

[开发](https://github.com/martymac/fpart)

[Port](https://www.freshports.org/sysutils/fpart)
