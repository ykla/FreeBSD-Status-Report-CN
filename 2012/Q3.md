...
- 原文：[FreeBSD Quarterly Status Report 3rd Quarter 2012](https://www.freebsd.org/status/report-2012-07-2012-09.html)
- 最后作者：FreeBSD 项目
...
本季度的亮点包括成功参与谷歌编程之夏，源代码树和 Ports 树领域的重要工作，以及由 30 多名开发者参加的开发者峰会。

感谢所有报告者的出色工作！本报告包含 12 项内容，祝你阅读愉快。

## 项目

### FreeBSD 在 Altera FPGA 上的应用
...
除了这些软核外，我们还开发了对 Terasic 多点触控 LCD 的支持，并正在为 Terasic HDMI 发送卡提供支持。这两个设备都与常见的 Altera FPGA 开发或参考板兼容。它们确实需要额外的 IP 核，我们计划在不久的将来将其发布给开源社区。
...
Bugbusting 团队继续致力于使 GNATS PR 数据库的内容更加清晰、易于访问，并使提交者更容易找到和解决 PR，方式是通过标记 PR 来指示相关领域，并确保每个 PR 中包含足够的信息来解决每个问题。

与往常一样，任何有兴趣帮助处理 PR 队列的人都欢迎加入我们在 EFnet 上的 #freebsd-bugbusters。我们总是寻求额外的帮助，无论是分类处理传入的 PR，生成补丁来解决现有问题，还是帮助数据库维护（识别重复的 PR，已经解决的 PR 等）。这是更深入参与 FreeBSD 的好方法！

#### 待办任务

1. 进一步研究适合替代 GNATS 的工具。
2. 让更多的用户参与到 PR 的分类处理中来。
3. 协助提交者关闭 PR。

### FreeBSD 基金会

**链接**：[半年通讯](http://www.freebsdfoundation.org/press/2012Jul-newsletter.shtml)
...
我们谈判并监督了基金会资助的项目：分布式安全审计日志、Capsicum 组件框架、原生 iSCSI 目标范围界定和在线扩展 UFS 文件系统。
...
随着核心团队成员的变化，几个相关角色也发生了变化。Gábor Páli 接替 Gavin Atkinson 担任核心秘书，David Chisnall 接替 Robert Watson 担任 FreeBSD 基金会的联络人。核心团队认为不再需要正式的安全团队联络人，因此该角色被取消。

在第三季度，核心团队为 2 名新提交者授予了访问权限，并保管了 2 个提交位。

核心团队与 Ports 管理团队和集群管理员合作，设定了停止为 Port 仓库提供 CVS 导出的日期，即 2013 年 2 月 28 日。与此同时，9.1-RELEASE 的 CVS 导出已恢复。

## 内核

### FreeBSD 在 ARMv6/ARMv7 上的支持
...
网页（htdocs）：英文版本中的新闻快报和其他一些更新已翻译，以保持其最新。特别是“FreeBSD 基础设施上的安全事件”已及时翻译并发布。

FreeBSD 手册：“高级网络”章节完成了大规模更新。通过此次更新，将来自日语文档项目本地仓库的手册翻译结果合并到主仓库中。该章节仍然过时，需要进一步完善。其他部分也在不断更新。尤其，新增了“使用 pkgng 进行二进制软件包管理”子章节到“ports”部分，并在“mirrors”部分中新增了“使用 subversion”子章节。
...
该团队还发布了许多新版本，并向上游推送了大量修复和补丁。最新的发布包括：

- KDE SC: 4.9.1 (area51) 和 4.8.4 (Port)
- Qt: 4.8.3 (area51)
- PyQt: 4.9.4 (area51); QScintilla 2.6.2 (area51); SIP: 4.13.3 (area51)
- Calligra: 2.4.3, 2.5-RC2, 2.5.0. 2.5.1, 2.5.2 (area51) 和 2.4.3, 2.5.0, 2.5.1 (Port)
- Amarok: 2.6.0 (area51)
- CMake: 2.8.9 (Port)
- Digikam (及 KIPI 插件): 2.7.0, 2.8.0, 2.9.0 (area51) 和 2.7.0, 2.9.0 (Port)
...
Ports 树接近 24,000 个 Port，而 PR 数量仍超过 1000。

在第三季度，我们增加了 2 位新的提交者，并保管了 2 个提交位。

Ports 管理团队执行了多个 `-exp` 运行，验证基本系统更新如何影响 Ports 树，并为主要 Port 更新提供 QA 运行。

Beat Gaetzi 接管了发送失败邮件的角色，这一角色此前由 Pav Lucistnik 承担。Beat 还承担了将 Ports 树从 CVS 转换为 Subversion 的任务。

Florent Thoumie 卸下了在 portmgr 中的职务，他在维护遗留的 `pkg_*` 代码方面发挥了重要作用。

#### 待办任务

1. 大多数 Port PR 已经分配，现在需要集中精力测试、提交和关闭。

## 其他事项

### FreeBSD 开发者峰会，英国剑桥

**链接**：[开发者峰会主页](https://wiki.freebsd.org/201208DevSummit)

**联系方式**：Robert Watson <[rwatson@FreeBSD.org](mailto:rwatson@FreeBSD.org)>

在 8 月底，英国剑桥大学计算机实验室举行了一场“淡季”开发者峰会。此次活动为期三天，前一天安排了文档峰会。三天的主活动分为三个会议，每个会议都有两个分会场。一些会议甚至涉及来自周边地区的 ARM 开发者，证明这些会议是富有成效的，并促成了 FreeBSD 社区与 ARM 之间的进一步合作。

第一天定下了最终的日程，提出了大量的讨论主题，然后分组进行讨论。每个小组的简短总结在最后的会议中展示，并发布在 FreeBSD 维基的活动主页上。此次峰会为制定切换到默认使用 clang 编译程序的计划做出了重要贡献。此事在邮件列表中进一步讨论，现在已经完成，使我们离无 GPL 的 FreeBSD 10 更进一步。作为程序的一部分，剑桥大学计算机实验室的研究人员在下午进行了简短的讲座，内容涉及一般性的操作系统工作或 FreeBSD 本身。Robert Watson 展示了一款运行 FreeBSD 的平板电脑，该平板电脑在 Altera FPGA 上的 MIPS 兼容软核处理器上运行。
...
我们能够接受广泛的提案，涵盖了基本系统和 Port 基础设施。学生们在文件系统、文件完整性检查和 Port 的并行化方面开展工作。有学生在内核基础设施方面工作，包括一项支持用户、进程和 jail 上的 CPU 资源限制的项目，以及一名学生改进了 BSD **callout(9)** 和计时器功能。两名学生参与了 ARM 平台的工作，这一平台广泛应用于嵌入式系统和智能手机；一名学生对扁平化设备树（Flattened Device Tree）的实现代码作了重要的清理和改进，另一名学生将 FreeBSD 移植到了基于 OMAP3 的 BeagleBoard-xM 设备上。一名学生致力于改善用户空间工具中的 IPv6 支持，另一名学生则为 FreeBSD 10 新增的 BHyVE BSD 许可证虚拟机监控程序进行 BIOS 模拟。其他学生则在 EFI 启动支持、用户空间锁定分析和自动化内核崩溃报告系统方面开展工作。
