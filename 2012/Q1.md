...
- 原文：[FreeBSD Quarterly Status Report 1st Quarter 2012](https://www.freebsd.org/status/report-2012-01-2012-03.html)
- 最后作者：FreeBSD 项目

## 引言

本报告涵盖了 2012 年 1 月至 3 月期间与 FreeBSD 相关的项目。这是计划中的四个报告中的第一个。这个季度的亮点是 FreeBSD 9.0 的发布，它最终在 2012 年 1 月初发布。FreeBSD 项目将 FreeBSD 9.0-RELEASE 献给 UNIX 操作系统的创始人之一 Dennis M. Ritchie。我们的发布工程团队还忙于准备 8.3-RELEASE，该版本已于 4 月公开发布。

感谢所有报告者的出色工作！本报告包含 27 个条目，我们希望你喜欢阅读。
...
经过一段时间的调整和换工作，我终于回到了这个项目（也感谢 Julian Fagir 的多次提交），新版本上传了，并附有简短的描述页面。当前版本支持更多选项、配置文件以及更新的 rc.d 脚本。它还包括手册页面的更新和可选的调试模式。
...
xlocale 实现（libc++ 所需）通过了一系列最初为 Darwin 实现编写的 Port 进行测试，并修复了这些测试中发现的错误。这将在 9.1 中发布。
...
我们仍然需要替换 libgcc_s 的某些部分以及链接器，但我们正在朝着在 10.0 中实现 BSD 许可证的 C++ 栈的方向前进。
...
所有功能都实现了，并正在完善、审核并合并到 HEAD 中。

该项目由 FreeBSD 基金会赞助。

#### 待办事项

1. 已实现的写挂起基础设施（`/dev/ufssuspend`）使得调整大小成为可能，同时也使得实现在线的 **tunefs(8)** 和 **fsck(8)** 成为可能。
...
### Clang 替代 GCC 成为系统编译程序

**链接**：[使用 Clang 构建 FreeBSD](http://wiki.freebsd.org/BuildingFreeBSDWithClang)
...
此外，从 r232322 开始，**src.conf(5)** 中新增了选项 `WITH_CLANG_IS_CC`，这将使 Clang 安装为 `/usr/bin/cc`、`/usr/bin/c++` 和 `/usr/bin/cpp`，将其作为默认的系统编译程序。除非同时使用选项 `WITHOUT_GCC`，否则 gcc 仍然会作为 `/usr/bin/gcc`、`/usr/bin/g++` 和 `/usr/bin/gcpp` 可用。

我们的目标是尽早启用此选项，以便开始准备在 10.0-RELEASE 中将 Clang 作为默认的系统编译程序，并逐步弃用 gcc。

另外，由于上游 LLVM/Clang 宣布了将在 2012 年 4 月 16 日分支 3.1 版本，我们将很快导入新的 Clang 快照。实际的 3.1 版本可能会在几周后发布，然后我们会再进行一次导入。

最后，很多开发者正在为确保我们的 Port 能在 Clang 下正常编译而努力。现在定期检查和提交了修复补丁，并且也定期进行了完整的 Clang -exp 运行。当然，仍然存在一些难题，尤其是对于一些非常旧的软件，连新版 gcc 都无法编译，更不用说 clang 了。

#### 待办任务

1. 当前最重要的任务是尽可能在 Clang 下构建和运行整个 FreeBSD 系统。任何编译时或运行时的问题应报告到相应的邮件列表，或者提交为 PR。如果有补丁或解决方法，当然更好。
2. Clang 在 3.0 之后应该获得了更好的交叉编译支持，因此待导入 3.1 版本，我们将需要探索如何使 FreeBSD 的系统和内核进行交叉编译。这主要适用于 ARM 和 MIPS 等架构，这些架构通常不适合在本地编译。
3. 帮助使无法使用 Clang 编译的 Port 能够正常编译也是始终需要并且非常感激的任务。请将补丁发送给你喜欢的 Port 的维护者，或者提交 PR。
...
自上次报告以来，TRE 的优化取得了显著进展。多模式启发式代码基本完成了，它通过区分多个不同的情况来加速模式匹配。它从原始模式中提取字面片段，并使用多模式匹配算法查找任何出现。GNU grep 使用 Commentz-Walter 算法，这是一种基于自动机的算法，而在本项目中，决定使用 Wu-Manber 算法，它更加高效且更容易实现。在当前状态下，它还没有完全实现，某些情况，如 REG_ICASE 标志，尚未覆盖。这是完成多模式接口的下一步主要任务。在开发分支中，BSD grep 修改为了使用这个新接口，以便用于测试和调试。
...
**bsdconfig(8)** 工具的 **sh(1)** 代码行数接近 20,000 行，完成度约为 70%。项目完成后，**bsdconfig(8)** 将与现有的 **bsdinstall(8)** 一同，提供一套完整的工具，能够有目的地逐步淘汰 FreeBSD 9 及更高版本中的 **sysinstall(8)**。这是 Ron McDowell 和我花费超过 90 天时间的心血之作，现已接近完成这一令人激动的工具。

#### 待办任务

1. 完成用于获取/安装二进制软件包和附加发行版集的“安装程序套件”模块。启动服务模块。
...
代表 FreeBSD 项目，发布工程团队很高兴宣布 FreeBSD 8.3-RELEASE 已于 2012 年 4 月 18 日发布。

随着 FreeBSD 8.3 发布周期的完成，我们的重点转向准备 FreeBSD 9.1-RELEASE。计划将很快公布，预计发布日期为 2012 年 7 月中旬。

### FreeBSD 基金会团队报告

**链接**：<https://www.FreeBSDFoundation.org>

**联系方式**：Deb Goodkin <[deb@FreeBSDFoundation.org](mailto:deb@FreeBSDFoundation.org)>

基金会赞助了 2012 年的 AsiaBSDCon，该活动于 3 月 22-25 日在日本东京举行。我们还在 1 月 21 日参加了 SCALE 会议，并在 3 月 17 日参加了 NELF 会议。这个季度，我们计划参加 4 月 14 日的 ILF（印第安纳州 LinuxFest），5 月 11-12 日的 BSDCan 会议，以及 6 月 9 日的 SELF（东南 LinuxFest）。
...
FreeBSD 9.0 发布了，我们很自豪地表示，我们资助了其中的 7 个新功能！

## 内核

### linux 兼容层的 DTrace 探针
...
截至本文撰写时，我正在将测试机更新到更近期的 -current 版本，以准备提交。

### HDA 声卡驱动（snd_hda）中的 HDMI/DisplayPort 音频支持
...
上述功能在 NVIDIA GT210 和 GT520 显卡上通过 nvidia-driver-290.10 驱动程序成功测试。旧款 NVIDIA ION 和 Geforce 8300 板上的 HDMI 音频仍无法正常工作，原因尚不清楚。关于最新的基于 KMS 的 Intel 显卡驱动程序也有成功报告。ATI 显卡的支持仅限于旧款显卡，因为支持新显卡的驱动程序不支持 HDMI 音频。

该代码提交到了 HEAD 并合并到 9-STABLE 分支。
...
802.11n 站点和 hostap 支持现在完全可用了，除了 hostap 侧的省电功能尚未正确实现。TX 聚合和 TX BAR 处理实现了。
...
**链接**：[基准测试结果](http://people.freebsd.org/~bz/bench/)
...
作为项目的一部分，创建了基准测试框架，可以执行各种测试，包括在运行之间进行重启，并在不需要用户干预的情况下可重复地收集结果。它允许通过最小配置进行定期基准测试，并且便于未来扩展以支持更多基准测试。
...
在物理接口和环回接口上的性能，使用 TCP/IPv6 卸载支持时，与 IPv4 相当了，这是一项巨大的改进。IPv6 上的 UDP 和非卸载性能虽然有所提升，但仍低于 IPv4，仍需进一步工作以赶上 IPv4 的十年基准测试和代码路径优化。然而，IPv6 最小大小的 UDP 发送路径数据包每秒（pps）数超过了 IPv4，尤其是在向本地丢弃设备发送时。

这一进展使得我们接近能够默认优先使用 IPv6，而不会导致环回性能回归。对于物理接口，**cxgb(4)** 在 HEAD 中支持了 IPv6 TCP 卸载，且添加了 LRO/v6 支持。为了在不同硬件上获得更多测试结果，**ixgbe(4)** 和 **cxgbe(4)** 也更新了，以支持 TSO6 和 IPv6 上的 LRO。
...
通过开始解决其中最大的功能对等性差距，我将继续改进 IPv6 代码路径，并希望很快能看到社区的更多贡献和独立结果。

#### 待办任务

1. 小心地将代码更改合并到 SVN 中。

### Multi-FIB：IPv6 支持与其他增强功能

**链接**：[SVN multi-FIB IPv6 项目区域](http://svnweb.freebsd.org/base/projects/multi-fibv6/)

**联系方式**：Bjoern A. Zeeb <[bz@FreeBSD.org](mailto:bz@FreeBSD.org)>

**联系方式**：Alexander V. Chernikov <[melifaro@FreeBSD.org](mailto:melifaro@FreeBSD.org)>

在 2008 年，针对 IPv4 引入了多转发信息库（FIB）功能，允许在内核中最多使用 16 个独立的转发（“路由”）表。得益于 Cisco Systems, Inc. 的赞助，这项功能现在也可用于 IPv6，填补了其中一个较大的 IPv6 功能对等性差距。这些更改集成到了 HEAD，并回归到 stable/9 和 stable/8 分支，将作为这些分支未来发布的一部分。也在项目分支中为 stable/7 提供了回溯。如果请求多个 FIB，IPv6 FIB 将与额外的 IPv4 FIB 一起添加，而无需任何特殊配置，并且像 netstat 和 setfib 这样的程序，以及 ipfw 等，也扩展以无缝支持这项多 FIB 功能，适用于两个地址族。
...
最后，路由套接字现在意识到了 FIB，并且只会显示目标 FIB 上的路由消息。这允许路由监视器或路由守护进程仅获取特定 FIB 的选择性更新。
...
和以前一样，**www/ja** 子目录中的过时内容更新为与英文版本对应的最新版本。**www/ja** 子目录中过时翻译的更新工作几乎完成。只有旧版本的发布文档的翻译可能会过时。

在此期间，我们翻译了 9.0-RELEASE 发布公告，并及时发布了它。看来，发布公告的日文版本对日本人来说非常重要，因为该页面被频繁引用。

对于 FreeBSD 手册，“cutting-edge” 部分的翻译工作仍在进行中。“printing” 和 “linuxemu” 部分的一些更新也完成了。
...
### FreeBSD/arm 在各种 TI 开发板上的支持

**链接**：<http://svnweb.freebsd.org/base/projects/armv6/sys/arm/ti/>
...
该项目的目标是让 FreeBSD 在使用基于 TI 的 SoC（系统级芯片）的各种流行开发板上运行，如 OMAP3、OMAP4、AM335x。项目涵盖一些 ARM 通用的 Cortex-A 组件：GIC（通用中断控制器）、PL310 L2 缓存控制器和 SCU。
...
这项工作旨在将 FreeBSD 移植到 Freescale QorIQ 数据路径加速架构 (DPAA) 系统芯片，并为集成外设提供设备驱动程序。自上次状态报告以来，增加了以下支持：
...
### NAND 文件系统、NAND Flash 框架、NAND 模拟器

**链接**：[NAND 分支](http://svnweb.freebsd.org/base/projects/nand/)

**联系方式**：Grzegorz Bernacki <[gjb@semihalf.com](mailto:gjb@semihalf.com)>

**联系方式**：Mateusz Guzik <[mjg@semihalf.com](mailto:mjg@semihalf.com)>

NAND Flash 堆栈包括 NAND 控制器和内存芯片的驱动框架、NAND 设备模拟器以及容错的日志结构文件系统，伴随有工具、实用程序和文档。

- NAND FS 支持合并到了 “nand” 项目分支

  - NAND FS 文件系统
  - NAND FS 用户空间工具

- NAND 框架和 NAND 模拟器合并到了 “nand” 项目分支
...
### 将 DTrace 移植到 MIPS 和 ARM

**联系方式**：Oleksandr Tymoshenko <[gonzo@FreeBSD.org](mailto:gonzo@FreeBSD.org)>

DTrace 的主要部分移植到了 MIPS 平台。支持的 ABI：o32 和 n64。n32 尚未测试。MIPS 实现通过了 DTrace 测试套件中的 927 个测试中的 853 个。
...
### 基于 CentOS 的新 Port linux_base

**联系方式**：Alexander Leidinger <[netchild@FreeBSD.org](mailto:netchild@FreeBSD.org)>

我们收到了一个基于 CentOS 6 的 Port linux_base 的 PR。目前，这只能用作测试环境，因为它依赖于比 linuxulator 提供的更高版本的 Linux 内核。
...
2. 在其他 Port linux_base 中添加冲突。
3. 提交基于 CentOS 的版本。
4. 一些清理工作。

### BSD 许可的 sort 实用工具（替代 GNU sort）

**链接**：

[FreeBSD 的 Port BSD sort](http://www.freebsd.org/cgi/cvsweb.cgi/ports/textproc/bsdsort/)

[IEEE Std 1003.1-2008 sort 规范](http://pubs.opengroup.org/onlinepubs/9699919799/utilities/sort.html)

**联系方式**：Oleg Moskalenko <[oleg.moskalenko@citrix.com](mailto:oleg.moskalenko@citrix.com)>

**联系方式**：Gábor Kövesdán <[gabor@FreeBSD.org](mailto:gabor@FreeBSD.org)>

目前，BSD sort 达到了可用的稳定阶段。它非常稳定，速度与 GNU sort 相同，并且支持多字节语言环境（这是 GNU sort 做得不正确的）。BSD sort 具有 GNU sort 5.3.0 的所有功能（包括在 FreeBSD 中的版本），并附带一些额外功能和错误修复。
...
团队发布了多个版本并推送到上游许多修复和补丁。最近的发布版本包括：

- KDE SC: 4.7.4（在 Port 中）和 4.8.0, 4.8.1, 4.8.2（在 area51 中）
...
由于 Ports 冻结期的延长，KDE 团队无法更新 KDE Port，因为这被视为侵入性更改。
...
1. 测试 KDE SC 4.8.2。
2. 测试 KDE PIM 4.8.2。
3. 测试 phonon-gstreamer 和 phonon-vlc，因为 phonon-xine 后端已弃用（但暂时仍会保留在 Port 中）。
4. 测试 Calligra beta 版本（在 area51 仓库中）。

### Perl Port 测试

**链接**：<http://wiki.freebsd.org/Perl#Test_Dependencies>

**联系方式**：Steve Wills <[swills@FreeBSD.org](mailto:swills@FreeBSD.org)>

许多 Perl 模块在 Port 中包含了随源代码一起提供的测试用例。该项目的目标是确保所有这些测试通过。该项目取得了显著进展。提交了构建 Perl 时使用 `-pthread` 的更改，并且没有报告问题。许多 Port 添加了缺失的依赖项或进行了其他更改，约 90% 的 `p5-*` Port 通过了测试。正在努力将测试支持从 Port tinderbox 中移出。
...
我们很高兴地宣布，FreeBSD Haskell 团队提交了 Haskell Platform 2011.4.0.0 更新，GHC 7.0.4 更新，现有 Port 的更新以及由于 9.0-RELEASE 冻结而未提交的新 Port。一些提交的新 Port 包括 Yesod、Happstack、wxHaskell、gitit、Threadscope 等，目前 FreeBSD Ports 中的 Haskell Port 数量接近了 300 个。所有这些更新将在即将发布的 8.3-RELEASE 中提供。
...
Ports 的 Port 数量缓慢增加，超过了 23,000 个。目前的 PR 数量仍然约为 1100 个。

在第一季度，我们新增了 2 名提交者，保管了 2 个提交位，并且有一名提交者返回了 Ports 工作。

Ports 管理团队一直在持续运行 -exp 运行，验证基本系统更新如何影响 Ports，同时为主要 Port 更新提供质量保证测试。值得注意的是，-exp 运行完成了以下内容：
...
在此期间，许多精力集中在为 FreeBSD 8.3 准备 Ports 的工作上，包括为发布版本准备软件包文件。

Beat Gaetzi 一直在对 Ports 进行持续测试，以确保从 CVS 向 Subversion 的平稳过渡。

#### 待办事项

1. 寻求帮助，使 Ports 能与 clang 构建。
2. 寻求帮助，支持 Tier-2 架构。
3. 由源代码更改引起的 Port 故障。
...
6. 标记为 BROKEN 的 Port。
7. 该 Port 何时出现故障？
8. 大多数 Port PR 分配了，我们现在需要专注于测试、提交和关闭。
