...
- 原文：[FreeBSD Quarterly Status Report 1st Quarter 2006](https://www.freebsd.org/status/report-2006-01-2006-03.html)
- 最后作者：FreeBSD 项目

## 引言

本季度报告的亮点无疑是 [FreeBSD 基金会](http://www.freebsdfoundation.org/) 提供的原生 Java 二进制文件已可用，同时在 Xen 支持和 Sun 的 Ultrasparc T1 方面也取得了进展。此外，我们期待着 FreeBSD 6.1 的发布，且 TrustedBSD 审计支持已导入 FreeBSD 7-CURRENT。总的来说，这是 2006 年令人兴奋的开端。
...
自上次报告以来，BSDInstaller 的集成工作取得了进展。后端已更改为新的 Lua 版本，以确保我们使用的版本将继续得到维护。发布的 Makefile 现在使用 Lua 软件包，而不是 Perforce 中的本地副本。为了避免将 Lua 引入基本系统，所需的模块也正在创建相应的 Port。
...
**链接**：[pfSense 网站](http://www.pfsense.com/)

**联系方式**：Scott Ullrich <[sullrich@gmail.com](mailto:sullrich@gmail.com)>

pfSense 继续发展并修复 bug。自上次报告以来，我们已扩展到 14 位开发者，部分和全职参与者将 pfSense 推向 1.0 版本。计划于 2006 年 4 月 15 日发布 Beta 3。
...
符号版本控制库使我们能够在不增加库版本号的情况下维护二进制兼容性。最近，libc、libpthread、libthread_db 和 libm 的符号版本控制已提交到 -current。默认情况下，它是禁用的，可以通过将 `"SYMVER_ENABLED=true"` 添加到 **/etc/make.conf** 来启用。在默认启用此功能之前，应为 libc 和其他受影响的库（可能是所有库）做最终版本更新。
...
3. 识别并符号版本化任何其他应做符号版本控制的库。如果有人有建议，请告诉我。
...
过去三个月中，TrustedBSD CAPP 审计实现已合并到 FreeBSD 7-CURRENT 开发树中，并已为与 6.X 合并奠定了基础。OpenBSM 是 Sun 的基本安全模块（BSM）API 和文件格式的 BSD 许可实现，并扩展以支持入侵检测应用程序。新特性包括支持审计管道，伪设备，提供入侵检测应用程序的实时审计记录接口，以及审计过滤守护进程，允许插件模块监控实时事件。
...
**链接**：[TrustedBSD OpenBSM 网页](http://www.openbsm.org/)
...
已开始实施快速生成树协议（RSTP），它取代了传统的 STP。与 STP 需要 30-60 秒的链路故障恢复时间相比，RSTP 的恢复时间大约为 1 秒，这对于现代网络非常重要。目前已有一些进展，但很快需要一台支持 RSTP 的交换机以继续推进，详情请见 [http://www.freebsd.org/donations/wantlist.html](http://www.freebsd.org/donations/wantlist.html)。
...
`PF_KEY` 代码的内核与用户空间分离工作已完成并提交到 CVS。

基于 IPv4 的 Kame IPSec 与 IPv4 `FAST_IPSEC` 之间的差异未显示出明显问题。

接下来的工作重点是使 `FAST_IPSEC` 支持 IPv6，并能够运行以下几种内核变体：
...
2. 构建更好的 TAHI 测试框架。TAHI 测试框架将不再维护，而且它并不是容易使用和理解的系统。更好的测试框架是可能且必要的，它对其他网络项目也非常重要。如果有时间并希望参与此项工作，请联系 gnn@，他有一些代码和想法可以作为起点。
...
**链接**：[FreeBSD Netperf 项目](http://www.freebsd.org/projects/netperf/)

**联系方式**：Robert Watson <[rwatson@FreeBSD.org](mailto:rwatson@FreeBSD.org)>

FreeBSD 的 netperf 项目最近专注于修订套接字和协议控制块的引用计数，以定义和强制执行引用和内存管理的不变量，从而消除不必要的检查、错误处理和锁定。所有网络协议（包括 netipx、netnatm、netatalk、netinet、netinet6、netgraph 等）的套接字发送和接收路径中，已消除了对全局 pcbinfo 锁的使用。检查通常已被断言取代；因此，`so_pcb` 现在保证不为 `NULL`。这应通过减少锁竞争和不必要的检查来提高性能，并促进后续工作，以通过适当的引用计数消除 TCP 输入路径中 pcbinfo 锁的长时间持有。这些更改已提交到 FreeBSD 7-CURRENT，等它们稳定后将在几个月后合并。
...
该项目实现了内核模块 **hwpmc(4)**、应用程序编程接口 **pmc(3)** 和一些简单的应用程序（**pmcstat(8)** 和 **pmccontrol(8)**），用于使用现代 CPU 中的事件监控硬件来测量系统性能。

自上次状态报告以来的新特性：

- 增加了对动态加载的内核和用户对象做剖析的支持。
- **pmcstat(8)** 现在支持命令行语法以记录到网络套接字。

### 改进声音子系统

**链接**：[开始支持 Intel HDA](http://www.leidinger.net/FreeBSD/hdac-20060313.tbz)

**联系方式**：Multimedia Mailinglist <[freebsd-multimedia@FreeBSD.org](mailto:freebsd-multimedia@FreeBSD.org)>
...
- 添加多通道支持/处理。
- 添加 Intel HDA 支持。已有一些代码可以查看（见上述链接），但还远未对终端用户可用（我们需要一些程序员，但目前不需要测试人员，因为还没有用户可测试的部分）。有兴趣的志愿者可以联系多媒体邮件列表。

这项工作的一部分可能已包含在 6.1 中，但截至本文撰写时，仍有很大一部分甚至尚未进入 -current。

#### 待办事项

1. 清理 **Style(9)**，至少在 `WARNS=2` 下生效。
2. 查看与声音相关的想法列表中的条目。
3. 重写部分代码（例如：新的具有 OSS 兼容性的混音子系统）。
4. **sndctl(1)**：工具，用于用户控制声音系统的非混音部分（例如：SPDIF 切换、虚拟 3D 效果）（而不是使用 -current 中的 sysctl 方法）；**pcmplay(1)**，**pcmrec(1)**，**pcmutil(1)**。
...
过去几个月的工作主要集中在稳定 ATA 以便为 6.1-RELEASE 做准备，并增加对新芯片组的支持。在这方面，JMicron 为厂商树立了标杆，因为他们不仅提供了硬件，还提供了关于硬件和软件 RAID 实现的文档，使得添加对其优秀产品的支持变得非常轻松。其他厂商也可以加入进来。:) 除此之外，我始终需要时间和资源来继续推进这项工作。

ATA 已添加了 USB 后端，这样就可以直接使用闪存驱动器和外部硬盘/CD/DVD 驱动器，而无需通过 atapicam/CAM 等。这在资源有限的嵌入式系统中非常有用，尤其是在内核空间稀缺的情况下。**burncd(8)** 正在更新，以支持此功能，并添加对 SATA ATAPI 设备的支持，如果时间允许，还会增加对 DVD 的支持。

接下来的几个月将用于（希望能够）解决 ATA 在大于 4GB 内存系统中的兼容性问题，并利用支持的控制器的 64 位寻址。对 ataraid 的 RAID5 支持也在计划中，同时对 RAID 子系统做加固，以帮助保护数据安全。

### FreeBSD 的 TMPFS 文件系统
...
**联系方式**：Rohit Jalan <[rohitj@purpe.com](mailto:rohitj@purpe.com)>

迄今为止已发布了三个测试版。代码已可以使用，并且看起来很稳定，但它还不是 MPSAFE。
...
FreeBSD 的项目和志愿者想法列表进展顺利。多个项目已由志愿者接手，并成功合并到项目中。其他项目正在审查或进行。
...
**链接**：[鼠标框架项目](http://www.semicomplete.com/projects/newpsm)

**联系方式**：Jordan Sissel <[jls@csh.rit.edu](mailto:jls@csh.rit.edu)>

当前的鼠标系统混乱不堪，moused、`psm`、`ums` 和 `mse` 各自独立支持多种鼠标类型。该项目旨在将所有驱动程序支持转移到用户空间中的 moused 模块中。此外，现有鼠标基础设施中缺失的许多功能正在添加。希望这个新系统能够简化鼠标的使用和驱动程序的编写。
...
HEAD 中的所有 dhclient 变更已合并到 6-STABLE 中，以便用于 6.1-RELEASE。目前正在测试的新补丁包括对启动脚本的支持，支持完全异步启动 dhclient，消除了启动时等待连接的问题，并且在未指定系统主机名时，支持将系统主机名发送给服务器。
...
这是更新，说明 TS-7200 已在最近的 -current 版本上成功构建并运行。

我一直在努力让 FreeBSD/arm 在 TS-7200 上运行。目前，主板已能够启动，并且以太网部分能够工作（尽管存在一些无法解释的数据包丢失问题）。我可以从 FreeBSD/i386 机器上做网络启动，并且能够在 CF 上挂载 msdosfs。
...
我们本希望能在 FreeBSD 6.1 发布之前完成 Xen DomU 和可能的 Dom0 原型。主要工作集中在将 Xen 集成到 FreeBSD 的 `newbus` 框架中。不幸的是，FreeBSD 中的架构性问题阻碍了进展。Xen 依赖于子域和父域之间的消息传递来配置设备，而这要求在启动早期，tsleep 和 wakeup 必须能够正常工作。当前似乎无法实现这一点，也不清楚需要做什么才能让它工作。没有完成 newbus 的工作，就很难完成 Dom0 代码，并且不可能支持 Xen 3.0 中的功能，如域挂起。
...
**联系方式**：John Gurney <[jmg@FreeBSD.org](mailto:jmg@FreeBSD.org)>

FreeBSD 已移植到 T1，这是 Sun 最新的处理器。目前，FreeBSD 支持多用户 SMP。JMG 正在积极改进设备支持。
...
**联系方式**：Andrew Turner <[andrew@fubar.geek.nz](mailto:andrew@fubar.geek.nz)>

Libpkg 是使用 libarchive 提取软件包文件的软件包管理库。它可以下载、安装并列出已安装的软件包。目前已开始实施基本系统中的软件包工具。已实现大部分 pkg_info，也已开始实现 pkg_add。
...
在此期间，Ports PR 的数量从上个季度末令人印象深刻的低点急剧增加。这主要是因为节假日、5.5/6.1 发布周期的冻结，以及一些提交者为解决长期存在的过时 distfiles、过时的 WWW 网站、只在 i386 上工作的 Port 等问题所做的积极工作。此外，还添加了 200 多个新 Port。统计数据未能完全反映 Ports 的状态，尽管 PR 数量增加，但仍在持续改善。

目前，我们有 3 位合格人员可以运行 5-exp 回归测试。正因为如此，我们能够做几轮测试，并做了多次提交，解决了 3 打以上的 portmgr PR。虽然在一次提交过程中发生了一些预期外的后果，但破损在不到一天的时间内得到修复。值得注意的变化包括新增了物理类别 net-p2p 和虚拟类别 hamradio 和 rubygems。5.5 和 6.1 发布后，portmgr 希望能更频繁地做回归测试。
...
2. 我们有 4,000 多个无人维护的 Port（例如，见 [portsmon 上的列表](http://portsmon.freebsd.org/portsconcordanceformaintainer.py?maintainer=ports@FreeBSD.org)）。我们始终在寻找愿意至少接管一些 Port 的志愿者。

### 更新 Ports 中的 Linux 基础设施

**联系方式**：Emulation Mailinglist <[freebsd-emulation@FreeBSD.org](mailto:freebsd-emulation@FreeBSD.org)>

**联系方式**：Alexander Leidinger <[netchild@FreeBSD.org](mailto:netchild@FreeBSD.org)>

**联系方式**：Boris Samorodov <[bsam@ipt.ru](mailto:bsam@ipt.ru)>

目前正在做更新，将新的 linux_base-fc3 作为默认的 Linux 基础。由于在此之前需要做一些基础设施工作，因此这项工作不会在 FreeBSD 5.5 和 6.1 发布之前完成。与此同时，基于 X.org 的新的 Linux Port 将取代过时的基于 XFree86 的 Linux X11 Port。

选择使用 fc3 而不是 fc4 或 fc5，是为了确保过渡尽可能顺利，减少破坏。我们当前默认的 linux_base-8 已使用了几个 fc3 的 RPM 软件包，因此不应该有太多问题需要解决。
...
**联系方式**：Anish Mistry <[amistry@am-productions.biz](mailto:amistry@am-productions.biz)>

HP 的 hplip 软件的初步版本已移植，支持其打印机和多功能设备。可查看来自打印机的状态信息，如墨水水平、错误消息和队列信息。如果你使用的是 Officejet，你还可以发送传真和扫描。照片卡和复印功能尚未测试。
...
**链接**：[FreeBSD 基金会 Java 首页](http://www.freebsdfoundation.org/downloads/java.shtml)

**联系方式**：Deb Goodkin <[deb@freebsd.org](mailto:deb@freebsd.org)>

FreeBSD 基金会发布了官方认证的 JDK 和 JRE 1.5 二进制文件，适用于 i386 平台的官方 FreeBSD 5.4 和 FreeBSD 6.0 版本。我们通过聘请一名承包商运行 Sun 认证测试，修复发现的问题，成功完成了这一目标。没有 BSD Java 团队的支持，这一过程是不可能完成的。
...
自上次报告以来，已修复了几个 bug，这些修复将在 FreeBSD 6.1/5.5 中提供。强烈建议 pf 用户升级到 RELENG_6，因为 RELENG_5 中的版本已过时。
...
BSDCan 是 2006 年最大的 BSD 事件。询问曾参加过的其他人，他们在渥太华度过了多愉快的时光。问问他们遇见了谁，和谁交谈了，建立了哪些联系，学到了哪些信息。
...
自 2003 年以来，我推出了（现在相当广泛使用的）FreeBSD Update 和 Portsnap 工具，但由于时间限制，很少有机会对其做改进或添加请求的功能。因此，在 3 月 30 日，我向 freebsd-hackers、freebsd-security 和 freebsd-announce 列表发送了电子邮件，宣布我正在寻求资金，以便让我能够在夏季全职从事这些工作并担任 FreeBSD 安全官。如果一些支票按预期到达，我已达到了捐赠目标，并将在 5 月初开始工作。
