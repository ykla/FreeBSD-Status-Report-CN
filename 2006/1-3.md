...
FreeBSD 的 netperf 项目最近专注于修订套接字和协议控制块的引用计数，以定义和强制执行引用和内存管理的不变量，从而消除不必要的检查、错误处理和锁定。所有网络协议（包括 netipx、netnatm、netatalk、netinet、netinet6、netgraph 等）的套接字发送和接收路径中，已消除了对全局 pcbinfo 锁的使用。检查通常已被断言取代；因此，`so_pcb` 现在保证不为 `NULL`。这应通过减少锁竞争和不必要的检查来提高性能，并促进后续工作，以通过适当的引用计数消除 TCP 输入路径中 pcbinfo 锁的长时间持有。这些更改已经提交到 FreeBSD 7-CURRENT，等它们稳定后将在几个月后合并。
...
这项工作的一部分可能已经包含在 6.1 中，但截至本文撰写时，仍有很大一部分甚至尚未进入 -current。
...
接下来的几个月将用于（希望能够）解决 ATA 在大于 4GB 内存系统中的兼容性问题，并利用支持的控制器的 64 位寻址。对 ataraid 的 RAID5 支持也在计划中，同时对 RAID 子系统进行加固，以帮助保护数据安全。
...
Libpkg 是使用 libarchive 提取软件包文件的软件包管理库。它可以下载、安装并列出已安装的软件包。目前已经开始实施基本系统中的软件包工具。已经实现大部分 pkg_info，也已经开始实现 pkg_add。
...
目前正在进行更新，将新的 linux_base-fc3 作为默认的 Linux 基础。由于在此之前需要进行一些基础设施工作，因此这项工作不会在 FreeBSD 5.5 和 6.1 发布之前完成。与此同时，基于 X.org 的新的 Linux Port 将取代过时的基于 XFree86 的 Linux X11 Port。

选择使用 fc3 而不是 fc4 或 fc5，是为了确保过渡尽可能顺利，减少破坏。我们当前默认的 linux_base-8 已经使用了几个 fc3 的 RPM 软件包，因此不应该有太多问题需要解决。
