...
- 原文：[FreeBSD Quarterly Status Report 2nd-3rd Quarter 2006](https://www.freebsd.org/status/report-2006-06-2006-10.html)
- 最后作者：FreeBSD 项目

## 引言

本报告涉及了 2006 年 6 月至 10 月期间与 FreeBSD 相关的项目。这包括今年谷歌编程之夏的结束，共有 13 名学生成功完成了项目。一些去年和今年的编程之夏参与者也已加入提交者行列，继续致力于他们的项目，并全面改善 FreeBSD。
...
**链接**：[编程之夏学生 Wiki](http://wiki.freebsd.org/Interrupts)

**联系方式**：Paolo Pisati <[pisati@FreeBSD.org](mailto:pisati@FreeBSD.org)>

**联系方式**：John Baldwin <[jhb@FreeBSD.org](mailto:jhb@FreeBSD.org)>

该项目旨在改进 FreeBSD 中的中断处理系统：在保持与以前模型（FAST 和 ITHREAD）向后兼容的同时，新增了方法“中断过滤”。在中断过滤中，中断处理程序被分为两部分：过滤器（检查实际中断是否属于该设备）和 ithread（如果需要执行阻塞操作，则调度该线程）。中断过滤的主要好处包括：
...
**链接**：[编程之夏学生 Wiki](http://wiki.freebsd.org/MarkusBoelter)

**联系方式**：Markus Boelter <[m@FreeBSD.org](mailto:m@FreeBSD.org)>

**联系方式**：Paul Saab <[ps@FreeBSD.org](mailto:ps@FreeBSD.org)>

对我来说，谷歌编程之夏是一次全新的、非常激动人心的经历。我积极参与了开源软件的开发，并为社区贡献力量。项目中的挑战使我深入了解了 FreeBSD。在这个过程中，我对整个项目有了更好的理解。直接与许多开发者合作，也为谷歌编程之夏增添了非常特别的氛围。
...
**链接**：[gvirstor 首页](http://wiki.freebsd.org/gvirstor)
...
**联系方式**：Clement Lecigne <[clem1@FreeBSD.org](mailto:clem1@FreeBSD.org)>
...
**链接**：[编程之夏学生 Wiki](http://wiki.freebsd.org/JailResourceLimits)
...
**联系方式**：Hajimu UMEMOTO <[ume@FreeBSD.org](mailto:ume@FreeBSD.org)>
...
3. **重写 nss_ldap**：虽然这一任务在原始提案中没有明确提到，但在编程之夏期间，我们发现直接导入 PADL 的 `nss_ldap` 并不如从头重写它来得容易（许可问题是其中的主要原因）。最终的模块行为类似于 PADL 的模块，但具有更灵活的架构。虽然基本完成，但仍需要实现 PADL 的 nss_ldap 中的一些有用功能。尽管缺少某些功能，但该任务可视为已成功完成。缺失的功能将尽快实现，预计在九月完成。
...
尽管这些代码尚未提交到官方的 FreeBSD 树中，但根据我去年的经验，这种情况是正常的。我希望这些代码能在接下来的几个月里进行审核并提交。

### 将 SEREF 策略和 setools 移植到 SEBSD

**链接**：

[编程之夏学生 Wiki](http://wiki.freebsd.org/DongmeiLiu)

**联系方式**：Dongmei Liu <[dongmei@freebsd.org](mailto:dongmei@freebsd.org)>
...
**联系方式**：Jue Yuan <[yuanjue@FreeBSD.org](mailto:yuanjue@FreeBSD.org)>
...
**联系方式**：Emulation Mailinglist <[emulation@FreeBSD.org](mailto:emulation@FreeBSD.org)>
...
CScout 是一款重构编辑器和源代码浏览器，适用于 C 代码集合。该项目的目的是使 FreeBSD 开发者能够轻松使用 CScout，并通过基于 CScout 的查询和重构来提升 FreeBSD 源代码的质量。
...
**联系方式**：Damiel Braniss <[danny@cs.huji.ac.il](mailto:danny@cs.huji.ac.il)>
...
我的工作进展较慢。我相信 ZVOL 已经完全功能齐全（我最近修复了 zvol 上的快照和克隆），这意味着你可以在 RAID-Z 卷上放置 UFS，快照该卷，必要时进行克隆等，非常酷。最难的部分是 ZPL 层，我仍在处理它。大多数文件系统方法已经可用，但可能需要详细审查和多次修复。最近我大部分时间都在正确实现 **mmap(2)**，它在简单的测试中基本上是有效的，但在 `fsx` 程序下失败。另一方面，`fsx -RW` 非常稳定可靠。其他不使用 **mmap(2)** 的测试程序也运行得相当好。仍有很多工作要做，主要是在 ZPL 领域，许多清理工作等。有些功能（如 ACL）我甚至还没有尝试接触。
...
我在 5 月至 8 月期间致力于改进 Portsnap、FreeBSD Update，并花更多时间在我持续的安全官员职责上。FreeBSD Update 现在已是 FreeBSD 基本系统的一部分，并且得到了 FreeBSD 安全团队的完全支持；目前只为 i386 架构构建更新，但很快会提供 AMD64 更新。
...
* OpenBSM 1.0 alpha 8 到 alpha 12 已发布并合并到 FreeBSD CVS 中。更改包括大量的错误修复、文档改进和功能增强。这些增强包括基于正则表达式的 auditreduce 匹配，auditd 管理内核审计策略（如最大日志文件大小），增强了多种令牌（包括 execve 参数支持）的打印支持。
...
linimon@ 还在进行一项工作，为 portsmon 增加了分析成功的软件包（不仅仅是失败的软件包）的能力，从而可以执行例如“显示在 i386 上构建但在 amd64 上不构建的软件包”以及“显示为什么依赖软件包 foo 在 bar 上未构建”等查询。该功能目前处于 alpha 测试阶段。
...
2. 我们有近 4400 个无人维护的 Port（例如，请参见 [portsmon 上的列表](http://portsmon.freebsd.org/portsconcordanceformaintainer.py?maintainer=ports@FreeBSD.org)）。虽然最近新维护者的涌现降低了这一比例至不到 28%，但我们仍然需要更多的帮助。
3. 对 Ports 执行 `gcc4.1` 测试时，出现了大约 1000 个新的构建错误。Kris@ 已发布了一些结果，以便人们现在开始处理这些问题。特别是，似乎某些旧版本的 GCC 无法使用 GCC 4.1 构建，因此依赖这些旧版本的 Port 也需要进行修复。尽管 GCC 4.1 不会很快导入到 `-CURRENT`，现在就该开始规划了。
4. AMD64 和 sparc64 上的软件包的状态明显落后于 i386。在许多情况下，由于使用了 `NOT_FOR_ARCH`，而不是更准确地基于 `ARCH` 设置 `BROKEN`，因此并未尝试构建软件包。（`pointyhat` 可以强制构建标记为 `BROKEN` 的软件包，但不能构建标记为 `NOT_FOR_ARCH` 的软件包）。`NOT_FOR_ARCH` 应该只表示“在此架构上永远无法工作”。虽然我们有一些志愿者表示对 sparc64（和 ia64）感兴趣，但我们需要更多使用 amd64（特别是作为桌面）的人员来帮助我们让更多的软件包能够正常工作。
...
[启用 FAST_IPSEC 和 IPv6 的 CURRENT 补丁](http://www.freebsd.org/~gnn/fast_ipv6.patch)
...
### 高度改进的 sendfile(2)、sosend_*() 和 soreceive_stream() 实现
...
**联系方式**：Randall Stewart <[randall@freebsd.org](mailto:randall@freebsd.org)>
...
**联系方式**：Ulf Lilleengen <[lulf@pvv.ntnu.no](mailto:lulf@pvv.ntnu.no)>
...
**联系方式**：Bernd Walter <[tisco@freebsd.org](mailto:tisco@freebsd.org)>
...
该栈目前处于实验阶段。它被用作这些嵌入式项目的根文件系统。目前还没有对卡的热插拔提供支持（因为板卡没有连接必要的引脚，而且根文件系统消失是非常糟糕的）。目前仍有很多粗糙的地方。
...
**联系方式**：Ryan Beasley <[ryanb@FreeBSD.org](mailto:ryanb@FreeBSD.org)>

**联系方式**：多媒体邮件列表 <[multimedia@FreeBSD.org](mailto:multimedia@FreeBSD.org)>

自上次状态报告以来，我们为 envy24ht 芯片添加了基础支持，已将 emu10kx 驱动程序导入到基本系统中，并为高定义音频（HDA）兼容芯片添加了支持。

此外，Ryan Beasley 作为 2006 年谷歌编程之夏的一部分所做的工作已经提交。这为 Open Sound System（OSS）v4 API 添加了兼容性，尽可能地扩展了功能。这使得可以编写更复杂的程序。例如，现在可以同步多个声音通道的启动。驱动程序还可以支持不止 AC97 混音设备，但目前还没有驱动程序扩展以支持这一点。更多信息可以在 Wiki 和官方 OSS 文档中找到。
...
上个月，我在开发一个驱动/模块，用于更新支持微代码更新的 Intel 或 AMD CPU 的微代码。如你所知，这些处理器是由微代码驱动的，并且可以更新固件。Intel 经常发布微代码更新，AMD 的微代码更新可以在 BIOS 程序中找到。工作几乎完成，我只需要找到一点时间在 AMD64 系统上进行测试并进行代码清理。该驱动程序还提供了一种让用户空间程序访问特定 CPU 的机器特定寄存器（MSR）和 CPUID 信息的方法。这能让一些程序（如 `x86info`）提供关于 SMP 系统中 CPU 的更准确信息，并根据 MSR 的内容做出假设。
...
### FreeBSD/arm 在 Atmel AT91RM9200 上的移植
...
### Ports 中的 Enlightenment DR17 支持
...
**链接**：[FreshPorts——Ports 的聚集地](http://www.freshports.org/)
...
### 改善 FreeBSD Ports 基础设施

**链接**：[Gábor 的维基页面](http://wiki.freebsd.org/G%C3%A1borK%C3%B6vesd%C3%A1n)
...
### OCaml 语言支持在 Ports 中的集成

**链接**：[框架包含文件](http://www.freebsd.org/cgi/cvsweb.cgi/~checkout~/ports/lang/ocaml/bsd.ocaml.mk?rev=1.3&content-type=text/plain)

**联系方式**：Stanislav Sedov <[stas@FreeBSD.org](mailto:stas@FreeBSD.org)>

我们在树中有多个 OCaml Port，每个 Port 都在维护 OCaml 的 `ld.conf` 文件，保持正确的状态，安装/删除它们的文件/条目等。为了简化 OCaml 语言 Port 的创建，开发了一个特殊的框架（`bsd.ocaml.mk`），并将大部分 Port 转为使用该框架。这使得很多重复的代码得以移除。这个新框架处理了安装 OCaml 语言库并正确注册所需的所有事项。`bsd.ocaml.mk` 还包含了用于处理 findlib 支持的库、以正确方式修改 `ld.conf` 等的控制选项。此外，还添加了许多新的与 OCaml 相关的 Port。

## 杂项

### AsiaBSDCon 2007

**链接**：[会议网站](http://www.asiabsdcon.org/)

**联系方式**：佐藤広生 <[hrs@freebsd.org](mailto:hrs@freebsd.org)>

**联系方式**：George Neville-Neil <[gnn@freebsd.org](mailto:gnn@freebsd.org)>

**联系方式**：<[secretary@asiabsdcon.org](mailto:secretary@asiabsdcon.org)>
...
**联系方式**：EuroBSDCon 组委会 <[info@eurobsdcon.org](mailto:info@eurobsdcon.org)>
