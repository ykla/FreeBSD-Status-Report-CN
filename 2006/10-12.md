...
- 原文：[FreeBSD Quarterly Status Report 4th Quarter 2006](https://www.freebsd.org/status/report-2006-10-2006-12.html)
- 最后作者：FreeBSD 项目

## 引言

新年快乐！本报告涉及了 FreeBSD 开发令人兴奋的 2006 年最后一个季度。FreeBSD 6.2 终于发布，而 FreeBSD 7.0 的开发工作也在加紧进行。本报告中的一些项目将成为这项工作的一部分，其他项目则已进入源代码树中。许多项目需要你的帮助，包括测试和其他方面。有关更多信息，请参见“待办事项”部分。
...
**联系方式**：FreeSBIE 工作组 <[staff@FreeSBIE.org](mailto:staff@FreeSBIE.org)>

**联系方式**：FreeSBIE 邮件列表 <[freesbie@gufi.org](mailto:freesbie@gufi.org)>

FreeSBIE 正接近 2.0-RELEASE。第一个发布候选版本证明已足够好，但可能会发布第二个候选版本。一位外部开发者正在努力将 BSDInstaller 集成到 FreeSBIE 2.0 中，这可能会稍微延迟发布日期。版本说明已编写，并需要更新当前的软件包列表。已添加一个可以开关 Tor+Privoxy 的脚本，并记录了其使用方法。2.0-RELEASE 即将发布，预计在 1 月底前，但这也取决于 FreeBSD 6.2-RELEASE 的发布日期。
...
尽管它仍在开发中，但现在支持更多目标，具有登录 CHAP 认证和头部/数据摘要功能。它还能够从丢失的连接中恢复——大多数时候。
...
在过去的三个月里，USB 项目的活动并不多。一些回归问题已被报告并修复。Bernd Walter 报告说，他已将新的 USB 堆栈在 ARM 处理器上运行，只做了一些小的调整。Markus Brueffer 报告说，他正在处理 USB HID 解析器和支持。目前，新的 USB 堆栈存在一个问题，即 EHCI 驱动程序在 Sparc64 架构上无法工作。如果有人在 Sparc64 上运行 FreeBSD `7-CURRENT`，并愿意借出 root 口令、串口控制台和 USB 测试设备（例如 USB 存储棒），我们将不胜感激。另一个未解决的问题是 **ural(4)** USB 设备驱动程序并非总是有效。这个问题目前正在处理。
...
现在 ZFS 文件系统在 FreeBSD 上运行得相当不错。首个补丁集已发布到 [freebsd-fs@FreeBSD.org 邮件列表](mailto:freebsd-fs@FreeBSD.org)。

所有的文件系统方法已实现（除了与 ACL 相关的部分）。基本上，我尝试的所有压力测试都能通过，即使是在非常高负载下。仍然存在一个内存分配问题，可能会失控，但根据我所知，SUN 的开发人员也在处理这个问题。

最近，我一直在开发一个文件系统回归测试套件。据我发现，目前没有免费的此类测试套件。我已经有了超过 3000 个测试，并且正在测试大多数与文件系统相关的系统调用的正确性（`chflags`、`chmod`、`chown`、`link`、`mkdir`、`mkfifo`、`open`、`rename`、`rmdir`、`symlink`、`truncate`、`unlink`）。我还在努力使其可以在其他操作系统上使用（如 Solaris，已可以工作，和 Linux）。

几天前，我也（几乎）完成了 NFS 支持。虽然目前还不能使用 `zfs share` 命令，但你可以通过 **/etc/exports** 导出文件系统，并且也可以访问快照。这相当困难，因为快照是独立的文件系统，在导出主文件系统后，我们还需要在其下提供快照数据。
...
FreeBSD 6.2-RELEASE 已发布，这是首个提供实验性审计支持的 FreeBSD 版本。计划是在 FreeBSD 6.3-RELEASE 中使审计成为完整的生产功能，并默认编译 `options AUDIT`。一个待办事项列表已发布到 trustedbsd-audit。

OpenBSM 1.0 alpha 13 发布了，支持 XML 记录打印、额外的 64 位令牌类型、更多的审计事件以及更强的跨平台构建支持。OpenBSM 1.0 alpha 14 将在不久后发布，增加了使用 gcc 4.1 进行无警告干净构建的支持。新的 OpenBSM 版本将在 1 月底或 2 月初合并到 FreeBSD CVS。
...
**链接**：[TrustedBSD 项目](http://www.trustedbsd.org/mac.html)

**联系方式**：Robert Watson <[rwatson@FreeBSD.org](mailto:rwatson@FreeBSD.org)>

**联系方式**：<[trustedbsd-discuss@TrustedBSD.org](mailto:trustedbsd-discuss@TrustedBSD.org)>

在此期间，除了与 **priv(9)** 项目相关的工作外（详见另一份状态报告），MAC 框架的主要工作集中在框架结构的完善上。
...
- 优化了 MAC 框架的锁定，乐观地期望在读取锁定期间没有写锁冲突。
...
1. 完成对内核权限检查的审查，移除现已集中化的 **suser(9)** Jail 参数。
...
**联系方式**：Ceri Davies <[ceri@FreeBSD.org](mailto:ceri@FreeBSD.org)>
...
发布工程团队最近的工作集中在 FreeBSD 6.2-RELEASE 上，现已可以下载。这是 `RELENG_6` 分支的最新版本，包含许多新的性能和稳定性改进、错误修复以及新特性。FreeBSD 6.2 的发布说明和 errata 说明包含了关于该版本新特性的更具体信息。我们感谢 FreeBSD 开发者和用户社区为使这个版本成为现实所做的努力。
...
就在本周，我已让 FAST_IPSEC 和 IPv6 代码的路由功能正常运行。现在出现了内存冲突问题，接下来我们需要移除旧的 GIANT 锁。我希望在下周发布另一个补丁，届时路由代码将正常工作。
...
ipfw 中对内核 NAT、重定向和 LSNAT 的支持已提交到 `HEAD`，并且我建议大家进行测试，以便我们能迅速发现和修复问题。
...
**联系方式**：Archie Cobbs <[archie@FreeBSD.org](mailto:archie@FreeBSD.org)>

MPD 正在过渡到下一个主要版本——mpd4_0。10 月底发布了另一个 beta 版本 (4_0b5)，并计划很快发布第一个 RC。

自 3_18 和 4_0b4 以来，已修复了大量的 bug 和内部处理错误，性能有所提高，并且系统需求减少。
...
Kip Macy 最近在准备传统驱动程序以适应重新架构的 net80211 层方面取得了重大进展，该层一直在 Perforce 中搁置。经过他的努力，这些代码几乎准备好进行公开测试，测试完成后即可合并到 CVS 中。我们的目标是能够在 7.x 分支发布之前完成这一合并（否则将不得不等到 8.0 版本才会出现在公开发布中）。这个修订后的 net80211 层包括先进的站点模式功能，例如背景扫描、漫游和对 Atheros SuperG 扩展的支持。将修订后的扫描功能加入 CVS 将大大简化 Virtual AP (VAP) 代码作为补丁的公共发布，并启用 802.11n 支持的添加。

Benjamin Close 正在为常见于笔记本中的 Intel 3945 部件开发支持。这项工作正在 Perforce 仓库中进行，并会发布公共代码供测试。

Atheros PCI/Cardbus 支持已更新，配备新的 HAL，修复了一些小问题，并纠正了导致 AR2424 部件无法工作的 bug。新的 HAL 还使硬件密钥缓存的 TKIP 密钥使用更高效；在较新的硬件上，现在可以支持最多 57 个站点而无需将密钥换入缓存。对最新 802.11n 部件的支持正在开发中，这些部件通常出现在新的 Lenovo 和 Apple 笔记本（等）中；初始发布将仅支持传统操作。
...
Michael Richardson 一直在领导工作，改进用于内核各部分（包括 Fast IPSec 和 geli）的加密子系统。这项工作由 Hifn 赞助，且已在 CVS 仓库外进行。该工作的一个主要焦点是添加对高级硬件操作的支持，这可以显著提高 IPSec 和 SSL 协议的性能。
...
一个基于 GEOM 的主动/被动多路径的原型实现已完成，并存放在 Perforce 仓库中。它似乎能够正常工作。
...
中断过滤是一种新的中断处理方法，它在保持与先前模型（FAST 和 ITHREAD）向后兼容的同时，在某些方面有所改进。通过中断过滤，中断处理程序被分为两部分：过滤器（检查实际中断是否属于某个设备）和私有的每个处理程序 ithread（当需要执行阻塞工作时调度该线程）。这项工作的主要优点包括：
...
在过去的一个季度中，完成了许多改进，目前报告称 3 个架构（i386、amd64 和 arm）已可以正常工作，项目可以认为功能已完备。
...
自上次状态报告以来，针对高保真音频（HDA）兼容芯片的 emu10kx 驱动程序已得到改进。现在支持更多芯片，且已支持的芯片提供了更好的零配置体验。

通用声音代码完成了非常好的低延迟更改和修复，使其更加多通道/字节序/格式安全。目前尚未支持多通道操作，但这项工作是实现多通道操作的前提。该工作还修复了一些可能导致点击声、跳跃、截断或类似声音输出行为的 bug。
...
**链接**：[关于 Linux 兼容层环境的 Wiki 页面](http://wiki.freebsd.org/linux-kernel)
...
关于 Linux 2.6 兼容性，i386 仍有 140 个测试用例（17%）和 amd64 仍有 150 个测试用例（18%）在 `-CURRENT` 中失败。在修复一些阻碍实际应用程序的重大问题后，我们应当能够"很快"更广泛地公开 2.6 仿真，以发现更多 bug 并确定我们尚未实现的 Linux 系统调用的重要性。
...
2.6 兼容性中的一些差异是由于并非所有 i386 的更改都已合并到 amd64 代码中，还有一些测试用例已在我们的 Perforce 仓库中修复，但在提交到 `-CURRENT` 之前需要更多的审查。
...
### Bt878 音频驱动程序（即 FusionHDTV 5 Lite 驱动程序）

**链接**：[Perforce 源代码仓库](http://perforce.freebsd.org/fileSearch.cgi?FSPC=%2F%2Fdepot%2Fuser%2Fjmg%2Fbktrau%2F...&ignore=GO%21)
...
NetBSD 的 wpi 驱动程序的初步移植已完成，开发工作进展迅速，旨在让此驱动程序准备好合并到代码树中。目前，基本功能已经能够正常工作。驱动程序可以与非加密的对等设备关联，并在 11b 和 11g 模式下传输数据。仍有很多工作要做，欢迎进行更多测试。
...
### MPT LSI-Logic 主机适配器：mpt

**联系方式**：Matthew Jacob <[mjacob@FreeBSD.org](mailto:mjacob@FreeBSD.org)>

`mpt` 项目旨在支持 MPT LSI-Logic 主机适配器（SCSI、光纤通道、SAS）。

上个季度，在 Yahoo!、LSI-Logic 及许多其他公司的支持下，完成了大量更改，并且 U320 的支持得到了更好的优化。John Birrel 和 Scott Long 提供了初步的 Big Endian 支持。
...
### QLogic SCSI 和光纤通道：isp

**联系方式**：Matthew Jacob <[mjacob@FreeBSD.org](mailto:mjacob@FreeBSD.org)>

该项目旨在支持 QLogic SCSI 和光纤通道主机适配器。

上个季度，添加了 4Gb 光纤通道支持，并完整重写了 fabric 管理（目前仍在优化中）。

## 文档

### 匈牙利语网页翻译

**链接**：[匈牙利语网页](http://www.freebsd.org/hu/)
...
Gábor Kövesdán (gabor@) 提交了匈牙利语网页翻译，Giorgos Keramidas (keramida@) 已审核并提交了这些页面。初步的渲染问题也已修复，现在网页状态良好。
...
### BSNMP - 更多正在进行和即将进行的工作

**链接**：[BSNMP TODO Wiki 页面](http://wiki.freebsd.org/BsnmpTODO)
...
FreeBSD 的 BSNMP 守护进程的 BSNMP 桥接模块是在 2006 年编程之夏期间实现的，并已提交到 `HEAD`。除了 RFC 4188 单桥支持外，它还支持通过私有 MIB 监控多个桥接。自 2006 年编程之夏起，还为该模块添加了快速生成树（RSTP）支持（RSTP-MIB 在 RFC4318 中定义，并且对私有 MIB 做了扩展）。

`RELENG_6` 的补丁已发布，并将在接下来的几周内合并到 `STABLE` 分支。
...
FreeBSD 正在多种 Gateworks Avila 板上运行多用户环境，并且大多数板载设备已得到支持。这些设备包括紧凑闪存/IDE 插槽、有线网络接口、实时时钟和环境传感器。已测试了几种不同的 minipci 卡，包括由驱动程序 **ath(4)** 和 **hifn(4)** 支持的卡。剩余的需要支持的设备是板载闪存、可选的 4 端口网络交换机和可选的 USB 接口。计划对 IXP425 部件进行加密加速，但可能会在稍后的时间进行。
...
### FreeBSD/powerpc 在 Freescale MPC8555 上的移植

**联系方式**：Rafal Jaworowski <[raj@semihalf.com](mailto:raj@semihalf.com)>

**联系方式**：Marcel Moolenaar <[xcllnt@mac.com](mailto:xcllnt@mac.com)>
...
**联系方式**：FreeBSD GNOME 项目 <[gnome@FreeBSD.org](mailto:gnome@FreeBSD.org)>
...
2006 年 FreeBSD GNOME 团队最大的成就可能是移植了 [HAL](http://www.freedesktop.org/wiki/Software_2fhal)。这一努力的启动旨在为 FreeBSD 用户提供更丰富的桌面体验。自从 GNOME 2.16 发布时首次在 FreeBSD 上引入 HAL 后，它已被集成到 FreeBSD 发布的 KDE 3.5.5 和 PC-BSD 1.3 中。FreeBSD 后端也已提交到 HAL 的 git 仓库中，因此未来的 HAL 版本将会开箱即用地支持 FreeBSD。

最后，我们怀着遗憾的心情告别了团队成员之一。Adam Weinberger 从 FreeBSD GNOME 团队辞职，转而拯救生命（优先事项嘛！）。我们将会怀念他设计的启动画面和对语法的挑剔。

#### 待办事项

1. 现在 HAL 已成功移植到 FreeBSD，大家非常希望能看到 [NetworkManager](http://www.gnome.org/projects/NetworkManager/) 也能移植过来。主要工作包括将 NM 移植到使用我们的 80211 框架，并扩展一些基本工具，如 ifconfig。如果你有兴趣帮助，请联系 [marcus@FreeBSD.org](mailto:marcus@FreeBSD.org)。
...
在 2006 年的最后一个季度，FreshPorts 完成了多项改进。以下是其中的一些，链接会将你带到相关的 [FreshPorts News 网站](http://news.freshports.org/) 中的文章。
...
- 新的 [双 Opteron 服务器](http://www.freebsddiary.org/topics.php?aid=589#opteron) 已 [部署完成](http://news.freshports.org/2006/11/09/opti-has-left-the-building/)
...
由于 6.2 版本的发布周期较长，Ports 基础设施的工作进展不大。然而，我们已针对多个即将推出的功能完成了多次测试，以确保 Ports 能够与新的 gcc（4.1）版本兼容，并且不再硬编码 **/usr/X11R6**。后者的目的是将所有 Port 迁移到 `$LOCALBASE`，用户可以选择该路径。这将有助于未来的一致性，尽管这需要一次性转换。

在发布周期中，GNOME 已更新为 2.16。

此外，我们正在将 FORTRAN 的默认编译程序从 f77 更改为 gfortran。详细信息请参见 Ports 邮件列表。
...
Linimon 继续处理不再对其 Port 感兴趣的提交者，并妥善保存了一些 Port 的提交权限。这是为了确保志愿者与待办工作的最佳匹配。

#### 待办事项

1. 剩余的大多数 Port PR 都是“已分配给提交者的现有 Ports/PR”。尽管维护者超时政策有助于保持积压量较低，但我们需要做更多的工作，以确保 Port 处于理想的状态。
2. 尽管我们增加了许多维护者，但仍有许多未维护的 Port。此外，amd64 和 sparc64 上的软件包滞后。

### 更新 X.org FreeBSD Ports 到 7.2
...
**链接**：[BSDCan 2007](http://www.bsdcan.org/2007/)
...
**联系方式**：Sidsel Jensen <[info@EuroBSDCon.dk](mailto:info@EuroBSDCon.dk)>
