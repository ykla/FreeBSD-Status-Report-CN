...
Capsicum 旨在通过提供以应用程序为中心的保护模型来补充现有的系统中心强制访问控制保护，这种模型更好地支持隔离的用户程序，这些程序可以设置一个或多个沙盒来处理不可信的数据。从 tcpdump 到 Chromium 浏览器，许多应用程序已经被修改为使用沙盒技术来限制诸如解析不可信数据包和 HTML/JavaScript 渲染等高风险操作。
...
此外，Ports 开发人员也在努力使尽可能多的 Port 能够与 Clang 正常编译和运行。大多数情况下，这意味着修复 gcc 是唯一存在的编译程序的错误假设，但有时会出现更复杂的问题。对此方面的帮助非常受欢迎。
...
该项目在推动嵌入式平台方面面临的一个问题是工具链。当前 FreeBSD 中支持的编译程序、链接器等工具能够通用地支持架构，但许多硅片供应商会生产专门的工具链，以最大限度地发挥其硬件性能。目前，使用这些工具编译 FreeBSD 相当困难，因为需要进行许多手动步骤才能使其正常工作。

外部工具链项目将利用 Clang 团队为支持 Clang 在基本系统中的工作（打破对 CC=cc 的严格依赖（除了 Intel 的 CC 支持有问题））所做的部分工作。此外，构建协调（make buildworld）将发生变化，以避免引导某些工具，或根本不编译编译程序。此外，还将增加对使用替代汇编器、链接器等工具的支持。此工作将在 subversion 的 projects/xtc（eXternal Tool Chain）中进行。
...
PEFS 是一个内核级堆叠加密文件系统，即它堆叠在现有挂载的文件系统之上。支持 AES 和 Camellia 算法的 XTS 模式。自 2009 年编程之夏以来，项目已经成熟，最近几个月的主要改进包括：切换到使用 XTS 加密模式、实现稀疏文件支持、修复包括竞争条件和活锁在内的重命名 Bug、增加对 ext2 的支持。PEFS 套件包含一个 pam 模块，便于使用文件系统密钥进行用户认证，并在登录时将密钥添加到已挂载的文件系统。PEFS 通过 fsx、pjdfstest、blogbench 和 dbench 测试，在 UFS 和 ZFS 上运行通过。
...
用户空间 DTrace 支持是由 FreeBSD 基金会资助的项目，在今年夏季开发完成。该项目旨在将 OpenSolaris 上可用的用户空间 DTracing 功能引入到 FreeBSD。FreeBSD 现在支持 pid 提供程序和 usdt 探针。plockstat 可以通过单独的补丁使用。Dtruss 是一个类似于 ktrace 的 DTrace 脚本，但具有其他优势，已经被导入到 FreeBSD。mysql-server 和 postgresql-server Ports 也支持 DTrace。
...
今年已经过去四分之三，我们已筹集约 160,000 美元，目标为 350,000 美元。了解如何捐赠，请访问 [http://www.FreeBSDFoundation.org/donate/](http://www.freebsdfoundation.org/donate/)
...
该项目的目标是实现资源容器和每个 Jail 的简单资源限制机制。资源容器也是其他资源管理机制的先决条件，如分层资源限制、“对进程集（即作业）的集体限制”谷歌编程之夏 2010 项目（用于实现类似于 Linux cgroups 的机制），并且还可能用于提供精确的资源使用记录，用于管理或计费目的。目前，已经开发了一个通用的资源使用框架，并对大多数资源进行了限制执行。工作仍在进行中，正在为剩余资源添加限制，调试并改进实现。该项目由 FreeBSD 基金会赞助。
...
链接 [描述](http://wiki.ddteam.net/wiki.cgi?page=DIR-320+FreeBSD)

[Mercurial 仓库](http://my.ddteam.net/hg/BASE/)

联系方式：Aleksandr Rybalko <[ray@dlink.ua](mailto:ray@dlink.ua)>

FreeBSD/mips 已经移植到 D-Link DAP-1350，无线 AP/路由器，基于 Ralink RT3052F SoC。
...
Chromium 是一个基于 Webkit 的网页浏览器，主要采用 BSD 许可证，最近已提交到 Ports。它在 FreeBSD 上运行良好，并支持 HTML 5 视频等新特性。较新的构建版本使用了 Clang 编译程序，Clang 首次在 FreeBSD 上为 Chromium 编译了一个非调试版本，这是一个非常庞大的 C++ 项目。这个移植工作采用了一种新的混合源模型：最新的 FreeBSD 补丁部分内容在一定时间内保持封闭，新构建版本只对付费订阅者提供，而较旧的版本最终会转到 Ports 上。移植工作仍在进行中，目前我正在移植任务管理器，以使用 FreeBSD 的 libkvm，ALSA 音频后端需要移植到 OSS。移植总结中列出了其他问题，如果你希望参与，请联系我。
...
链接 [Wiki 页面](http://wiki.freebsd.org/Valgrind)
