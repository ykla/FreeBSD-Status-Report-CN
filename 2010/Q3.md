...
接着，向 bsnmplib(3)、bsnmpd(1) 和 bsnmptools 添加了 SNMPv3 认证和加密支持（当前通过 Ports 系统提供）。消息摘要和口令计算调用使用了 OpenSSL 的 crypto(3) 库中的相关加密算法实现。bsnmpd(1) 仍然可以选择不使用 crypto(3) 库进行编译，在这种情况下，只能处理未认证的纯文本 SNMPv3 PDU。
