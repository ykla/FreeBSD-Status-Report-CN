...
**snd_hda(4)** 音频驱动程序现在支持真正的多声道播放。它现在支持 4.0（四声道）、5.1 和 7.1 模拟扬声器设置。数字多声道 AC3/DTS 直通功能已早期实现。通过 HDMI 输出的数字多声道 LPCM 也有可能，但尚未测试。
...
[Subversion 分支](http://svn.freebsd.org/base/user/jmallett/octeon/)
...
- 新增 --delete-build-only 选项，用于删除运行时不需要的 Port /软件包。
