...
- 原文：[FreeBSD Quarterly Status Report 1st Quarter 2010](https://www.freebsd.org/status/report-2010-01-2010-03.html)
- 最后作者：FreeBSD 项目
...
[FreeBSD 编程之夏官网](http://socghop.appspot.com/org/home/google/gsoc2010/freebsd)

[编程之夏时间表](http://socghop.appspot.com/document/show/gsoc_program/google/gsoc2010/timeline)
...
我们再次参与谷歌编程之夏。这是我们第六年参与该项目，我们希望再次看到学生们的优异成果。目前，所有申请已提交，我们正在审核这些申请。被接受的学生将于 4 月 26 日公布，正式编程将于 5 月 24 日开始。
...
**联系方式**：sprewell <[chromium@jaggeri.com](mailto:chromium@jaggeri.com)>

Chromium 是基于 Webkit 的网络浏览器，主要采用 BSD 许可证。它在 FreeBSD 上运行得非常好，甚至支持 HTML 5 视频等新特性。我已开始提供订阅服务，为 FreeBSD 的移植工作提供资金，这些资金已用于修复 BSD-i386 上的 Chromium。我正在使用一种新的资助模型，订阅费用支付的开发工作最多保密一年，之后所有在构建中使用的补丁将根据 Chromium 的 BSD 许可证公开给订阅者。此外，部分闭源补丁会不断推送到上游，[BSD i386 修复已提交到上游](http://codereview.chromium.org/1543003)。目标是为 BSD 上的 Chromium 开发提供资金，同时不断将补丁推回 BSD 许可证下的 Chromium 项目。我很快将发布 Chromium 移植软件包，供那些不介意使用旧版、稳定构建的用户使用，这些构建不包含所有订阅版的付费功能。你可以阅读 [订阅服务支付的事项，例如将 ALSA 音频后端替换为 OSS](http://chromium.jaggeri.com/issues)，并 [了解更多关于订阅的信息](http://chromium.jaggeri.com/subscriptions)。

### Clang 替代 GCC 作为基本系统的一部分
...
Roman Divacky <[rdivacky@FreeBSD.org](mailto:rdivacky@FreeBSD.org)>

Brooks Davis <[brooks@FreeBSD.org](mailto:brooks@FreeBSD.org)>

Pawel Worach <[pawel.worach@gmail.com](mailto:pawel.worach@gmail.com)>

自上次状态报告以来，我们已能够在 i386/amd64 上使用 clang 构建整个 FreeBSD（C 和 C++ 部分）。唯一的例外是引导加载程序，它未能满足给定的大小约束。目前的工作重点就在这里。C++ 部分得到了很大的提升，现在可以编译 FreeBSD 中的所有 C++ 代码及其自身。
...
总体来说，ClangBSD 已在 i386/amd64 上实现了自托管，并且在 PowerPC/PPC 上取得了一些进展。我们还看到了一些对 Sparc64 的贡献，但这似乎停滞不前。
...
3. 对第三方应用（Port）做更多 clang 测试。
4. 讨论将 LLVM/clang 集成到 FreeBSD 中。

### FreeBSD/i386 的 EFI 支持
...
该项目现在已达到稳定且经过充分测试的状态。可以从 8.0-RELEASE 或 7.3-RELEASE 的 ISO 映像文件，或从自定义 makeworld 创建映像。

新特性是脚本 zfsinstall，它自动化了从 mfsbsd ISO 进行的 ZFS-only 安装（该脚本适用于 `8-STABLE` 和 `9-CURRENT`，示例 ISO 映像可以从项目网站下载）。
...
Grzegorz Bernacki <[gjb@semihalf.com](mailto:gjb@semihalf.com)>

Rafal Jaworowski <[raj@semihalf.com](mailto:raj@semihalf.com)>
...
基础设施的一部分是一个完整的 ONFI 兼容设备系统模拟器（NANDsim），以及一个用户空间控制应用程序。这样可以在没有真实 NAND 芯片的平台上测试框架。
...
我们目前正在对 PC-BSD 新的系统安装程序后端 pc-sysinstall 进行大量代码清理，这个工具也可以用于安装常规的 FreeBSD。已实现了一些新功能，例如：
...
Volker Werth <[vwe@FreeBSD.org](mailto:vwe@FreeBSD.org)>
...
一些关于“实验 PR 报告”页面的 bitrot 问题修复完成。
...
像往常一样，任何有兴趣帮助处理 PR 队列的人都欢迎加入我们在 EFnet 上的 #freebsd-bugbusters 频道。无论你的兴趣是 triage 新进 PR、生成补丁解决现有问题，还是简单地协助数据库维护（识别重复 PR、已解决的 PR 等），都可以加入我们。这是更深入参与 FreeBSD 的好方法！

#### 待办事项

1. 我们将在 BSDCan 举办 bugbusting 会话。如果你是开发者并且会参加会议，请过来看看。
...
我们继续开展基础设施项目，增强用于软件包构建、网络测试等的硬件。
...
2 月份，完成了处理（虚拟）网络栈资源泄漏的工作，特别是在拆除时。
...
目前正在为 Atheros 驱动程序开发 802.11n 支持。目前它可以在软件中执行 AMPDU RX，我们正在开发 TX AMPDU。该代码存储在一个私有的 Perforce 分支中，但其中的一些部分已提交到 `HEAD`。
...
**ALQ(9)** 实现和 KPI 已修改（还需要提交一个补丁）以支持可变长度的消息。此外，现在可以将其作为内核模块编译和加载。
...
**联系方式**：Rick Macklem <[rmacklem@uoguelph.ca](mailto:rmacklem@uoguelph.ca)>

尽管 NFS 版本 4 的基本支持已在 FreeBSD 8.0 中发布，但集成进展缓慢，预计在 FreeBSD 8.1 中将支持 RFC3530（NFS 版本 4.0）。

在 FreeBSD 8.1 后，我认为重点将转向代码清理，并在 svn 的项目区域做一些关于客户端激进的整个文件缓存的实验性工作。
...
- 现在可以将端口和“tos”字段用作表格的查找参数。这可能简化规则集，从而提高执行速度；
...
**链接**：<http://people.freebsd.org/~rpaulo/ratectl.diff>

**联系方式**：Rui Paulo <[rpaulo@FreeBSD.org](mailto:rpaulo@FreeBSD.org)>

`net80211`（无线）堆栈很快将支持模块化速率控制框架。此项工作的目的是减少驱动程序中的一些代码并增加更多的速率控制算法到代码库中。所有在软件中执行速率控制的驱动程序将自动受益于此项目。目前，我们正在更改所有必要的驱动程序，以便与新的框架兼容，并确保一切按预期工作。今年稍晚些时候，我们将进行必要的更改，以通过 **ifconfig(1)** 更改速率控制算法。

如果你正在研究无线网络的速率控制算法或相关项目，FreeBSD 现在是一个理想的测试平台！
...
FreeBSD 网络邮件列表 <[freebsd-net@FreeBSD.org](mailto:freebsd-net@FreeBSD.org)>

这个正在进行的项目旨在减少 tcbinfo/udbinfo 锁定和缓存行争用；这个全局锁保护对连接列表的访问，虽然它是一个读写锁，但每当处理入站数据包时（短暂地）需要获取这个锁来查找连接。该项目增加了一个新的连接组表，用于将连接分配到组中，每个组具有 CPU 亲和性，并与高端 1gbps 和大多数 10gbps 实现中的 RSS 选择队列对齐。以下任务已完成：
...
基于 CAM 的 ATA 实现的工作仍在继续。从上次报告以来，错误恢复和超时处理得到了改进，热插拔现在对主机和端口倍增器端口都有效。为了解决一些旧问题并满足 ATA 的新需求，CAM 做了系列更改。

新的驱动程序 **ahci(4)** 和 **siis(4)** 已修复并且非常稳定。使用 `options ATA_CAM` 内核选项对其他控制器的支持效果良好，使用现有的 **ata(4)** 驱动程序，因此现在可以开始弃用旧的 **ata(4)** API。

已开始为 PCI-X/PCIe 卡和 ARM 系统芯片 SATA 控制器开发新的 Marvell SATA 驱动程序。预计将支持 NCQ、带有 FIS 基础切换的端口倍增器以及其他新特性。
...
**联系方式**：Tsuyoshi Ozawa <[ozawa@t-oza.net](mailto:ozawa@t-oza.net)>
...
**链接**：<http://info.iet.unipi.it/~luigi/geom_sched/>

**联系方式**：

Luigi Rizzo <[luigi@FreeBSD.org](mailto:luigi@FreeBSD.org)>

Fabio Checconi <[fabio@FreeBSD.org](mailto:fabio@FreeBSD.org)>
...
该模块于 2009 年初开发，并作为编程之夏 2009 项目进行改进，最近已被引入 `HEAD`，并将在不久后合并到 `stable/8`。`stable/7` 也有版本，但存在一些限制。
...
**链接**：[P4 工作空间](http://p4web.freebsd.org/@md=d&cd=//&c=MNx@//depot/user/bz/noinet/src/sys/?ac=83)

**联系方式**：Bjoern A. Zeeb <[bz@FreeBSD.org](mailto:bz@FreeBSD.org)>

在 2009 年，我们完成了一项工作，使我们能够再次构建出不包含 INET 和 INET6 的 FreeBSD 内核。这项工作为原型奠定了基础，以便在今年早些时候成功地使用仅包含 INET6 而没有 INET 编译的内核来启动。
...
### HDA 声音驱动程序中的多声道播放 (snd_hda)

**联系方式**：Alexander Motin <[mav@FreeBSD.org](mailto:mav@FreeBSD.org)>

**snd_hda(4)** 音频驱动程序现在支持真正的多声道播放。它现在支持 4.0（四声道）、5.1 和 7.1 模拟扬声器设置。数字多声道 AC3/DTS 直通功能早已实现。通过 HDMI 输出的数字多声道 LPCM 也有可能，但尚未测试。
...
Pawel Jakub Dawidek <[pjd@FreeBSD.org](mailto:pjd@FreeBSD.org)>

Martin Matuska <[mm@FreeBSD.org](mailto:mm@FreeBSD.org)>

李鑫 <[delphij@FreeBSD.org](mailto:delphij@FreeBSD.org)>
...
欢迎具备德语技能的 FreeBSD 用户加入我们翻译文档或修复 bug 的工作。
...
Gábor Kövesdán <[gabor@FreeBSD.org](mailto:gabor@FreeBSD.org)>

Gábor Páli <[pgj@FreeBSD.org](mailto:pgj@FreeBSD.org)>
...
### FreeBSD 的 libunwind 移植

**链接**：<http://www.nongnu.org/libunwind/>
...
Xin ZHAO <[quakelee@geekcn.org](mailto:quakelee@geekcn.org)>
...
我们使用了几项现有工作并更新它们以使用新的 OpenLDAP API，修复了若干 bug，并将它们整合在一起。所有这些工作都采用 BSD 或类似许可证，我们的新工作将采用 2 款条款的 BSD 许可证。目前，我们支持在 LDAP 树中存储用户身份、口令和 SSH 公钥。
...
**联系方式**：Jakub Klama <[jceel@semihalf.com](mailto:jceel@semihalf.com)>
...
通过最近的多个提交，机器在软件包构建过程中的稳定性得到了提升。目前正在做一些重构工作，以便支持 WITNESS。然而，我们仍然受到同时构建的软件包数量的限制。

基于此，我们已完成首次完整的 ia64-8 软件包构建。共构建了 17187 个软件包（与最近的 i386-8 比较为 19885 个）。Mark Linimon 已审查了结果，标明了哪些软件包无法构建。已有一些修复基于此提交。

目前我们有 3 台足够稳定的机器进行软件包构建。

SGI Altix 350 的支持已开始。移植工作在两台连接 NUMAFlex 的 SGI Altix 350 机器上进行，总共有 4 个 CPU 和 24GB 的 DDR。内核可以通过 `projects/altix` 分支启动，但由于 ACPI 无法枚举 PCI 总线，因此没有发现硬件设备。由于唤醒 AP 导致机器检查，SMP 功能已被禁用。
...
### 在 D-Link DIR-320 上移植 FreeBSD/mips

**链接**：<http://wiki.ddteam.net/wiki.cgi?page=DIR-320+FreeBSD>

**联系方式**：Alexandr Rybalko <[ray@dlink.ua](mailto:ray@dlink.ua)>

FreeBSD/mips 已成功移植到 D-Link DIR-320，无线路由器，基于 BCM5354 SoC。该项目旨在提供多个为不同用途（配置文件）定制的工作镜像。目前已提供了基于 [racoon](http://ipsec-tools.sourceforge.net/) 的路由器-ipsec 镜像。
...
### FreeBSD/mips 在 Octeon 上

**链接**：[Subversion 分支](http://svn.freebsd.org/base/user/jmallett/octeon/)

**联系方式**：Juli Mallett <[jmallett@FreeBSD.org](mailto:jmallett@FreeBSD.org)>

在稳定单处理器 Octeon 移植以及在工具链、rtld、libc 和内核中添加对 o32 以外的 MIPS ABI 支持方面取得了重大进展。目前支持以 n32 ABI 构建的内核，但相关更改不会被合并，因为它们对系统中 `db_expr_t` 和 `register_t` 进行了侵入性更改，而这些类型在 n32 ABI 中比指针更大。一旦 n64 内核支持完成（包括运行 n32 用户空间的能力）并且移除了 n32 的临时解决方案，该分支将适合合并。还进行了许多相关的清理工作，特别是在 TLB 和 pmap 代码方面。

#### 待办事项

1. 计划导入 Cavium simple executive 的部分内容作为供应商代码，以便能够移除本地维护的 Cavium 头文件和垫片函数副本，其中许多已严重过时。
2. Linux opencrypto 移植中包含一个用于加密协处理器的 opencrypto 驱动程序，看起来相对容易移植。
3. SMP 支持是一个高优先级事项，将在 64 位更改稳定后解决。
4. PCI 和 USB 总线及设备支持计划在导入 simple executive 函数和头文件之后进行。
5. rgmx 以太网驱动程序目前将数据包复制到和复制出 mbuf，而不是将指向 mbuf 存储的指针放入硬件，这导致了较差的网络性能。
...
我们已完成首次完整的 powerpc-8 软件包构建。共构建了 10918 个软件包（与最近的 i386-8 比较为 19885 个），主要由于一些高影响的软件包失败（例如 lang/python25）。Mark Linimon 已审查了结果，标明了哪些软件包无法构建。已有一些修复基于此提交，我们正在对下一轮进行补丁测试。
...
**链接**：<http://dougbarton.us/portmaster-proposal.html>
...
- 支持在“尝试先使用软件包”模式下安装软件包，以及 --packages-only、--packages-if-newer 和 --packages-build 模式。
- 新增 --delete-build-only 选项，用于删除运行时不需要的 Port 或软件包。
- 更新终端标题栏，显示当前工作内容及剩余任务量。
- 支持自定义软件包仓库和 INDEX 文件定义。
...
1. 网站上列出的一些有趣且常被要求的功能我仍然希望能够实现，包括（但不限于）在开始安装前下载所有软件包，并编写可以在该机器或一组相同机器上重新运行的脚本。
...
Thomas Abthorpe <[portmgr-secretary@FreeBSD.org](mailto:portmgr-secretary@FreeBSD.org)>

Port 管理团队 <[portmgr@FreeBSD.org](mailto:portmgr@FreeBSD.org)>
...
Ion-Mihai Tetcu <[itetcu@FreeBSD.org](mailto:itetcu@FreeBSD.org)>

Josh Paetzel <[jpaetzel@FreeBSD.org](mailto:jpaetzel@FreeBSD.org)>
...
BSDCan 是在加拿大渥太华举办的 BSD 大会，迅速确立了自己作为为基于 4.4BSD 的操作系统及相关项目的开发者和使用者提供技术交流的平台。组织者找到了一个适合从初学者到高级开发者的广泛人群的绝妙模式。
...
meetBSD 是一个年度活动，汇集了 BSD 操作系统家族的用户和开发者，主要是 FreeBSD、NetBSD 和 OpenBSD。继加利福尼亚特刊和在利维尼奥举办的 meetBSD Wintercamp 之后，今年我们回到了波兰克拉科夫。
