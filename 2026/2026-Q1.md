...
[FreeBSD Foundation](https://freebsdfoundation.org/) URL: [https://freebsdfoundation.org/](https://freebsdfoundation.org/)

[Technology Roadmap](https://freebsdfoundation.org/blog/technology-roadmap/) URL: [https://freebsdfoundation.org/blog/technology-roadmap/](https://freebsdfoundation.org/blog/technology-roadmap/)

[Donate](https://freebsdfoundation.org/donate/) URL: [https://freebsdfoundation.org/donate/](https://freebsdfoundation.org/donate/)

[Foundation Partnership Program](https://freebsdfoundation.org/our-donors/freebsd-foundation-partnership-program/) URL:
...
[Foundation Events](https://freebsdfoundation.org/our-work/events/) URL: [https://freebsdfoundation.org/our-work/events/](https://freebsdfoundation.org/our-work/events/)

联系方式：Deb Goodkin [[deb@FreeBSDFoundation.org](mailto:deb@FreeBSDFoundation.org)]

FreeBSD 基金会是一家 501(c)(3) 非营利组织，致力于通过技术和非技术支持推进 FreeBSD 的发展。基金会完全由捐赠资助，支持软件开发、基础设施、安全和协作工作；组织活动和开发者峰会；提供教育资源；并在法律事务中代表 FreeBSD 项目。

#### 操作系统改进

在 2026 年第一季度，FreeBSD 基金会赞助了 555 次 `src`、83 次 `ports` 和 16 次 `doc` 提交。

请参阅以下描述大部分赞助开发工作的报告条目：
...
* [音频栈改进](https://www.freebsd.org/status/report-2026-01-2026-03/#_audio_stack_improvements)
...
* [FreeBSD 上 LLDB 的改进](https://www.freebsd.org/status/report-2026-01-2026-03/#_lldb_improvement_on_freebsd)
...
* FreeBSD 已被接受参与 [Google Summer of Code (GSoC)](https://summerofcode.withgoogle.com/)。潜在贡献者已向 Google 提交了申请，我们将于 4 月 30 日了解有多少 FreeBSD 项目将获得资助。

#### 倡导

在 2026 年第一季度，我们的倡导工作侧重于扩展教育视频内容，触达了比以往更多的观众，将社区聚集在一起参加富有成效的供应商峰会，并回顾了有助于维持和增长 FreeBSD 关注度的工作。以下是基金会在 2026 年第一季度为 FreeBSD 倡导的几种方式：
...
* 发布了 FreeBSD Journal [十月/十一月/十二月](https://freebsdfoundation.org/our-work/journal/browser-based-edition/freebsd-15-0/)刊，包含文章的 HTML 版本
...
基金会拥有 FreeBSD 商标，保护商标是我们的责任。我们还为核心团队提供法律支持，以调查出现的问题。
...
根据 INDEX，Ports 集合中目前有 37,958 个（从 37,163 增加）Port。目前大约有 2,710 个（从 3,428 减少）未解决的 Port PR，其中 798 个（从 821 减少）未分配。上季度在 main 分支上有 8,970 次（从 8,738 增加）提交，由 166 名（从 156 增加）提交者完成，在 2026Q1 分支上有 697 次（从 898 减少）提交，由 59 名（从 61 减少）提交者完成。
...
过去三个月 Ports 树中发生了很多变化，主要软件升级摘录如下：
...
我们在日本的一个镜像站点将在 2026Q2 进行完整的硬件刷新。
...
赞助者：若干匿名个人和公司 赞助者：[https://github.com/sponsors/ppaeps](https://github.com/sponsors/ppaeps)
...
跨越多个类别的项目，从内核和用户空间到 Ports 集合或外部项目。
...
在 2 月和 3 月期间，项目已从规划阶段进入积极交付阶段。内部每周协调会议现已到位，基金会签约的安全负责人 Pierre Pronchery 已加入 FreeBSD 安全团队，以使项目工作更紧密地与 FreeBSD 项目现有的安全流程对齐。
...
* 创建了一个基于 Python 的[应用程序](https://github.com/FreeBSDFoundation/freebsd-laptop-testing/blob/main/main.py)，它在 [`hw-probe`](https://github.com/linuxhw/hw-probe/blob/master/INSTALL.BSD.md) 实用程序之上构建，用于收集和聚合笔记本电脑开发者和最终用户查看的相关数据子集。
* 开发了一个[静态站点生成](https://github.com/FreeBSDFoundation/freebsd-laptop-testing/blob/main/parse.py)系统，将 Python 应用程序创建的文件解析为 HTML 片段，供网站使用
...
过去，我开发了几个带有图形界面的 shell 脚本，用于在我的笔记本电脑上安装和配置完整的桌面环境。这些界面允许用户选择他们偏好的选项。其中一个脚本特别包含了 GPU 驱动程序、桌面环境以及适用于笔记本电脑和台式机系统的工具。这些项目仍然可以在公共在线仓库中找到，但它们现在可能已经过时，因为它们已经几年没有被使用或维护了。

最近，FreeBSD 基金会启动了笔记本电脑支持与可用性项目。该项目的短期目标之一是在 [bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html)（FreeBSD 系统安装程序）中提供一个简单功能，用于安装和配置图形环境。安装程序应询问用户是否要安装桌面环境，如果是，则自动安装和配置所需的一切，只需最少或无需用户干预。重启后，用户应看到 KDE Plasma 图形登录界面。
...
Bhyvemgr 是一个用 Freepascal/Lazarus 在 FreeBSD 上编写的 bhyve 管理 GUI。它需要一组工具，大部分安装在基本系统上，一些从 Port/软件包安装。主要目标是成为一个面向桌面用户的应用程序，使桌面用户能够在 FreeBSD 主机上轻松快速地设置和运行虚拟机。
...
* x86 架构级别支持应该更健壮。虽然直接配置 CPUID 信息应该允许更改几乎所有内容，但对于 x86 架构级别，我们应该检查架构级别中定义的所有功能是否实际受硬件支持。应禁止设置 CPU 不支持的架构级别，或至少发出警告。
...
Sylve 是一个面向 FreeBSD 的现代化统一系统管理平台。它提供集成的 Web 界面，用于管理虚拟机（通过 Bhyve）、jail、其周围的网络以及 ZFS 存储。
...
* 启动/停止生命周期及关联的 UI 已显著改进，以利用我们内置的队列系统，提供更快、更流畅的用户体验。
...
* 重启/启动/停止生命周期及关联的 UI 已显著改进，以利用我们内置的队列系统，提供更快、更流畅的用户体验。
...
* PCI 直通支持已显著改进，现在包含一个"准备直通"按钮，该按钮为直通准备 PCI 设备，使其在系统重启后可用于虚拟机。
* 移除了若干 NPM 库，转而使用自定义构建的替代方案或内嵌依赖项，以降低供应链攻击的风险。
* 进行了大量性能优化，以减少前端的 RAM 和 CPU 使用。
* 将 CI 系统从 Jenkins 迁移到 GitHub Actions，现在使用 sysroot 进行构建，使我们能够实现更快的构建时间。
...
此项目最大的阻碍是缺乏同时了解 FreeBSD 内部机制和 KGDB 内部机制的审查者。如果你有时间，请对我的拉取请求提供反馈。非 x86 和非 arm64 机器上的测试者也非常欢迎。如果你发现任何问题，请提交错误并在 llvm/llvm-project 上提醒我。
...
这将允许现代 Intel 和 AMD 笔记本电脑（其中一些不支持 ACPI S3 睡眠）进入低功耗状态以延长电池寿命。
...
协同处理器性能控制（CPPC）是 ACPI 引入的一项标准，允许操作系统管理 CPU 的性能水平，反之亦然，通过抽象性能尺度管理效率水平，该尺度通常与单纯的频率水平不相关且更细粒度。Intel 和 AMD 多年来一直在提供支持 ACPI CPPC 的 CPU 实现。
...
* FreeBSD Journal 文章[已发布](https://freebsdfoundation.org/wp-content/uploads/2026/01/margolis_vox.pdf)。
...
作为 IPv6 协议栈现代化的一部分，巨型支持已被移除。据我们所知，此支持从未可以在 FreeBSD 主机上使用。
...
* 翻译 FreeBSD 手册页的项目已启动，处于非常早期的阶段。初步示例可在 [GitHub](https://github.com/freebsd-doc-ru/freebsd-man-ru)找到。
...
影响 Ports 集合的变更，无论是涉及树的大部分的大规模变更，还是单个 Port 本身。
...
### Valgrind：稳定化、FreeBSD 16 修复和新增功能
...
计划：* 请求实验性构建 * 检查最后的 Port 并创建 PR 或提交 * 提交维护者反馈超时的 PR * 可能请求另一次实验性构建 * 如果完成，增加 JAVA_VERSION
...
Wazuh 的 Makejail 已改进，现在模仿官方 Dockerfile，因此熟悉它的用户可以使用 [AppJail](https://github.com/DtxdF/AppJail) 和 [Director](https://github.com/DtxdF/director) 轻松部署所有 Wazuh 组件：
...
长期目标是在 FreeBSD Ports 集合中提供一个连贯的 HPC 软件环境，类似于用户在基于 Linux 的 HPC 系统上所期望的，同时在 FreeBSD 生态系统中保持可维护性。
...
* 继续改进 FreeBSD Ports 集合中 [net/openmpi](https://cgit.freebsd.org/ports/tree/net/openmpi/)、[net/ucx](https://cgit.freebsd.org/ports/tree/net/ucx/)、[net/pmix](https://cgit.freebsd.org/ports/tree/net/pmix/) 和 [net/prrte](https://cgit.freebsd.org/ports/tree/net/prrte/) 之间的互操作性。
...
* 继续扩展 FreeBSD Ports 集合中可用的 HPC 软件生态系统。
...
**最后修改于**：2026 年 4 月 17 日，由 [Lorenzo Salvadore](https://cgit.freebsd.org/doc/commit/?id=a6c3267d9b)
