...
在 Q3 期间，我们欢迎 Joel Bodenmann（jbo@）成为新的 port 贡献者，向已是 src 贡献者的 mizhka@ 授予了 port 提交权限，并在 knu@ 和 uqs@ 沉寂一年后保留了他们的提交权限。
...
我们计划通过在未明确指定 `priority` 功能时将其重置为 0 来提高一致性，这与 `umask` 十多年来的行为一致。
...
[net/wifibox port](https://cgit.freebsd.org/ports/tree/net/wifibox)
