...
新增了在 [namei(9)](https://man.freebsd.org/cgi/man.cgi?query=namei&sektion=9&format=html) 中指定备用 ABI 根路径的功能。此前，要动态重定向路径查找，每个需要进行路径名转换的 [linux(4)](https://man.freebsd.org/cgi/man.cgi?query=linux&sektion=4&format=html) 系统调用都需要一些丑陋的代码，并使用 `kern_alternate_path()`，该函数无法正确解析目标中以 `/` 开头的符号链接。现在，非原生 ABI（即 [linux(4)](https://man.freebsd.org/cgi/man.cgi?query=linux&sektion=4&format=html)）在进入该 ABI 的 exec 期间通过一次 `pwd_altroot()` 调用来指定其根目录（例如 /compat/ubuntu），从而无需再进行路径名转换。这使得可以 chroot 到 Ubuntu 兼容层，而无需手动修复此类符号链接。
