...
- 原文：[FreeBSD Quarterly Status Report 3rd Quarter 2017](https://www.freebsd.org/status/report-2017-07-2017-09.html)
- 最后作者：Benjamin Kaduk

## 引言

本季度的 FreeBSD 开发继续带来激动人心的进展，并为未来的发展提供了良好的前景。我个人特别喜欢手册页，因此看到我们获得了编写手册页的相关文档，尤其令人高兴（我希望这能转化为未来更多且改进的手册页！）。core@ 条目也特别值得关注，介绍了 FCP 过程，并认可了首位非提交者 FreeBSD 项目成员（以及更多）。继续阅读以了解更多信息，包括对 AMD Zen 系列处理器（如 Ryzen）的更好支持，以及更多的改进！

Benjamin Kaduk

此致

提交截止日期为 2017 年 10 月至 12 月期间的报告，截止日期为 2018 年 1 月 14 日。
...
此外，FreeBSD 发布工程团队开始了 10.4-RELEASE 的发布周期，代码暂缓从 7 月 28 日开始，预计最终的发布构建将于 9 月 29 日开始，官方公告将在季度结束时发布，截至撰写本文时一切按计划进行。[2]
...
本季度，我们迎来了 Zach Leslie (zleslie@)、Luca Pizzamiglio (pizzamig@)、Craig Leres (leres@)、Adriaan de Groot (adridg@) 和 Dave Cottlehuber (dch@) 作为新提交者。以下提交者的提交权限被暂时保管：alonso@ 在 19 个月的不活跃后，rpaulo@ 根据他的请求，以及 ache@ 在他去世后。尽管尝试了几次并更换了导师，kami@ 对完成导师任务缺乏兴趣，因此他的提交权限也被暂时保管。
...
顾问委员会已准备好《行为准则》的更新草案。核心团队目前正在审阅文本，并将很快就接受该草案进行投票。核心团队希望避免“规则律师”的陷阱。目前的感觉是，我们需要在《行为准则》中添加一段前言，以阐明项目的目标，并作为执行该准则的总指导。

本季度，在提交者和项目成员的名单变动方面，核心团队非常忙碌。我们选举了首位新项目成员：John Hixson，你可能从许多会议上熟悉他——他在这些会议上做过演讲并出色地代表 iXsystems。第二位提名的项目成员没有被核心团队接受，但仅仅是因为核心团队认为 Fedor Uporov 确实更值得拥有提交权限。
...
- Viking 软件许可证足够接近 BSD，因此可以在 FreeBSD 代码中使用其驱动程序中的魔术常量。
- 在源代码树中没有单独的记录表明哪些偏离了允许的 BSD 类许可证：任何不符合 BSD 类许可证条款的代码都可以假定已被核心团队批准。
- 目前，FreeBSD 对于在二进制形式重新分发时包含版权声明的要求是通过将 FreeBSD 源代码与详细的版权信息一同提供，作为预编译系统镜像的一部分来满足的。然而，这不一定符合基于 FreeBSD 的下游项目的需求，考虑到新的“pkgbase”，正在考虑以类似于 Ports 的方式添加每个软件包的许可元数据，作为一种替代机制。
...
Piotr Pietruszewski <[piotr.pietruszewski@intel.com](mailto:piotr.pietruszewski@intel.com)>
...
- 增强了 CPU 拓扑检测代码，以正确检测 Zen die 和 CPU 复合体。这为调度器在做出调度决策时提供了更多的本地性信息。
- 增强了 x86 拓扑分析，除了现有的包、核心和线程报告外，还报告了 die 和 CPU 复合体。新的输出示例为：FreeBSD/SMP: 1 package(s) x 2 groups x 2 cache groups x 4 core(s) x 2 hardware threads。
- 添加了用于访问 SMN（系统管理网络）寄存器的 amdsmn(4) 驱动程序。
- 在 amdtemp(4) 中添加了对 Zen 的 CPU 温度监控支持。
- 在 cpufreq(4) 中：
...
Johannes Dieterich <[jmd@freebsd.org](mailto:jmd@freebsd.org)>
...
[Puppetlab & FreeBSD Slack 频道](https://puppetcommunity.slack.com/messages/C6CK0UGB1/)
...
1. pkg 软件包提供者存在一些小问题（在未配置仓库时会导致问题，并且在 MCollective 软件包代理的上下文中无法正常工作）。
2. databases/puppetdb\[345\] 和 sysutils/puppetserver\[45\] Port 依赖于 Clojure 和 Java，并下载已编译的 jar 文件，而不是从源代码构建它们。
...
3. 为 port 维护者文档记录 convert-systemd-units，用于从 systemd 单元文件制作打包服务包。
