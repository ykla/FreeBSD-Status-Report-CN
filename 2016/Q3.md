...
- 创建 GPLv3 工具链仓库镜像的提案已提交给社区。最终，由于外部工具链 Port 和软件包的进展速度超出了预期，这个想法变得大部分冗余，尤其是对于那些 LLVM 尚未足够成熟的架构。
...
FreeBSD 提供了一个 API，供客户机操作系统访问主机上的共享文件夹，从而使内核驱动程序能够将这些文件夹暴露给来宾的用户空间。该项目旨在将此功能添加到 VirtualBox 客户端附加组件驱动程序中。
