...
- 原文：[FreeBSD Quarterly Status Report 3rd Quarter 2016](https://www.freebsd.org/status/report-2016-07-2016-09.html)
- 最后作者：Benjamin Kaduk
...
**链接**：[FreeBSD 11.0-RELEASE 发布计划](https://www.freebsd.org/releases/11.0R/schedule.html)
...
在上一季度，有四位提交权限被收回保管：erwin、miwi 和 sem 按自己的请求退出，jase 因超过 18 个月不活跃而被收回。我们迎来了两位新提交者：Tobias Berner (tcberner) 和 Joseph Mingrone (jrm)。
...
- 导入 Concurrency Kit。在与基金会的法律顾问咨询后，确定导入 Concurrency Kit 的选定部分是可以接受的，并已获得批准。
- 创建 GPLv3 工具链影子仓库的提案已提交给社区。最终，由于外部工具链 Port 和软件包的进展速度超出了预期，这个想法变得大部分冗余，尤其是对于那些 LLVM 尚未足够成熟的架构。
...
SecTeam 收到许多漏洞的优先报告，因此有严格的政策，直到发布安全公告和补丁后才公开评论。提前获得漏洞信息的前提是他们能够避免过早披露，若非如此，他们将无法在漏洞发布时立即准备好安全公告和补丁。
...
OpenSSH 上游项目已弃用 DSA 密钥。出于兼容性原因，FreeBSD 在较晚的 10.x 版本中保持了 DSA 密钥的启用，但随着 11.0 的发布，现在是与上游同步的时刻。由于 FreeBSD 集群中有大量使用 DSA 密钥的情况，因此需要进行更换密钥的操作。核心团队感谢 David Wolfskill 和账户团队，在处理大量密钥变更时表现出了极大的冷静。
...
**链接**：[FreeBSD 基金会官网](https://www.freebsdfoundation.org/)
...
基金会工作人员 Glen Barber 与发布工程团队合作，继续完成 11.0-RELEASE 周期的最终工作，这一版本的发布因处理一些最后时刻的问题而有所延迟。
...
我们为 FreeBSD 基础设施提供硬件和支持。上季度，我们购买并启用了两台 48 核 Cavium ThunderX 机器，用于为 arm64 平台构建 FreeBSD 软件包集合。我们还购买了更多服务器，以支持持续集成工作。
...
FreeBSD 基金会赞助了全球多个会议、活动和峰会。这些活动可以是与 BSD 相关的、开源的，或者是面向代表性不足的群体的技术活动。
...
**链接**：[Capsicum 维基页面](https://wiki.freebsd.org/Capsicum)
...
最近，一些开发者开始对基本系统中的额外应用程序进行沙盒化处理。该项工作进展顺利，其中一个目标是将 FreeBSD 中用于安全敏感应用的基本工具，如 freebsd-update 和 portsnap，纳入沙盒化。
...
#### ClonOS：基于 FreeBSD 的新免费/开放托管平台

**链接**：[ClonOS 主页](http://clonos.tekroutine.com/)

**联系方式**：Oleg Ginzburg  <[olevole@olevole.ru](mailto:olevole@olevole.ru)>

目前，FreeBSD 已被证明是路由器（**pfSense**、**OPNSense**、**BSDRP**）和 NAS（**FreeNAS**、**zfsGuru**、**NAS4Free**）的基础。然而，在虚拟化领域，基于 FreeBSD 的解决方案几乎完全缺失，**ClonOS** 是尝试改变这一状况的方案之一。
...
CloudABI 邮件列表  <[cloudabi-devel@googlegroups.com](mailto:cloudabi-devel@googlegroups.com)>
...
3. 你是否维护可能从强沙盒化中受益的软件？尝试使用 CloudABI 跨平台编译程序构建它们！
...
[编程之夏 2016: 将 /dev 条目链接到 Sysctl 节点](https://wiki.freebsd.org/action/recall/SummerOfCodeIdeas?action=recall&rev=67#Devices_management:_link_.2Fdev_entries_to_sysctl_nodes)
...
Johannes Lundberg  <[yohanesu75@me.com](mailto:yohanesu75@me.com)>

我们很遗憾地报告，两个谷歌编程之夏项目都未完成。libdevq 项目被放弃，因为学生失联。内核项目未完成，因为学生因个人原因无法继续工作。尽管 2016 年的谷歌编程之夏已结束，他计划恢复工作并完成任务。
...
Johannes Lundberg 一直在致力于使 Wayland 环境能够在 FreeBSD 上运行。Port Wayland 已处于工作状态，除了 Weston 合成器之外。

当前的 Port Weston（来自 DragonFlyBSD）可能会被放弃，重新从头开始创建一个新的 Port，基于上游源代码。由于使用了 libinput、libudev-devd 和 epoll-shim，差异不会很大，且更易于维护。
...
**链接**：[LLD Wiki 页面](https://wiki.freebsd.org/LLD)
...
### VirtualBox 共享文件夹文件系统

**链接**：[项目仓库](https://github.com/lwhsu/FreeBSD-vboxfs)
...
FreeBSD 提供了一个 API，供客户机操作系统访问主机上的共享文件夹，从而使内核驱动程序能够将这些文件夹暴露给客户机的用户空间。该项目旨在将此功能添加到 VirtualBox 客户端附加组件驱动程序中。
...
- 间接块的最大可能大小已从 16KB 增加到 128KB，现在会执行间接块的推测预取。
- 改进了空间分配的顺序。
...
Vladimir Kondratiev  <[wulf@cicgroup.ru](mailto:wulf@cicgroup.ru)>
...
**链接**：[Amazon AWS ENA 文档](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/enhanced-networking.html)

**联系方式**：

Jan Medala  <[jan@semihalf.com](mailto:jan@semihalf.com)>

Jakub Palider  <[jpa@semihalf.com](mailto:jpa@semihalf.com)>
...
一些并非严格交织但有一定联系的代码是时间限制的休眠实现。callout 和设置/处理超时的上半部分代码之间的竞争条件，依赖于 callout_stop(9) KPI（内核编程接口）的许多细节。特别是，竞争或不准确的 KPI 更改通常会导致“捕获所有”无法终止的线程状态，出现“-”等待链错误。休眠队列超时代码被重写，不再依赖 KPI 细节，从而去除了重复 bug 的源头，并且意外简化了代码。
...
**联系方式**：Mateusz Piotrowski  <[0mp@FreeBSD.org](mailto:0mp@FreeBSD.org)>
...
**联系方式**：Vincenzo Maffione  <[v.maffione@gmail.com](mailto:v.maffione@gmail.com)>

该项目提供了：

- 一个新的驱动程序（if_ptnet），用于半虚拟化网络设备，参考 netmap API 进行建模。该驱动程序支持多队列 netmap Port，并且能够同时在 netmap 模式和常规模式下工作。
...
### FreeBSD 在 Annapurna Labs Alpine 上的应用
...
### FreeBSD 在 Marvell Armada38x 上的应用

**联系方式**：

Marcin Wojtas  <[mw@semihalf.com](mailto:mw@semihalf.com)>

Bartosz Szczepanek  <[bsz@semihalf.com](mailto:bsz@semihalf.com)>
...
已经添加了透明超级页面支持。这使得 FreeBSD 可以创建 2MiB 的块，并通过一个单独的页表和 TLB 条目来进行管理。这在 ThunderX 机器上的 buildworld 时间上展示了小幅但显著的提升。超级页面已经在主干中启用，并合并到 stable/11，但由于缺乏测试，在 stable/11 上默认是禁用的。

移除了对 INTRNG 之前的中断框架的支持。这意味着 arm64 架构现在需要 INTRNG 才能进行编译。这也使得在与中断控制器交互的 arm64 驱动程序中可以进行多项清理工作。

引入了 Linaro 的 Cortex Strings 库。经过验证，部分内容优于之前的 C 代码，并已附加到 libc 构建中。
...
**联系方式**：KDE 在 FreeBSD 上团队  <[kde@FreeBSD.org](mailto:kde@FreeBSD.org)>
...
- 添加了 Port devel/grantlee5，并随之添加了 Uses/grantlee.mk。
- 添加了 Port multimedia/gstreamer1-qt5。
- 添加了 Port net-im/telepathy-qt5。
...
- 将 Port net-im/telepathy-qt4 更新至 0.9.7。
...
目前，不稳定版本与我们在 Ports 中提供的 Port Gtk3 兼容，但未来将移除对 3.18 版本的支持，转而支持 3.20.x。
...
### 记录 /bin 和 /sbin 中工具的历史

**链接**：

[Port igor](https://svnweb.freebsd.org/ports/head/textproc/igor)
...
**联系方式**：Sevan Janiyan  <[sevan@FreeBSD.org](mailto:sevan@FreeBSD.org)>

为了参加 EuroBSDcon，我开始研究我们操作系统家族内组件的不一致性。我的工作流程包括阅读给定工具的文档并检查版本控制系统中的历史记录，寻找 NetBSD、FreeBSD、OpenBSD 和 DragonFly BSD 中树状结构里缺失的修复或功能。
