...
- 原文：[FreeBSD Quarterly Status Report 1st Quarter 2016](https://www.freebsd.org/status/report-2016-01-2016-03.html)
- 最后作者：Warren Block
...
在 2016 年第一季度，FreeBSD 核心团队的最重要任务是回应去年发生的骚扰事件。核心团队的行动包括组建事件时间表，并在此基础上回顾当时的核心团队行动；并提出关于未来如何更好处理此类事件的建议。在此过程中，草拟的报告由案件相关人员以及 FreeBSD 社区的多位感兴趣的成员进行了审阅。核心团队对所有参与者的贡献表示感谢。
...
核心团队与 John Baldwin 一起，正在制定一个计划，创建包含 GPLv3 工具链组件的独立存储库。这将允许超越现有 GPLv2 工具链所能处理的代码进行现代化，并支持某些新架构，在这些架构上还没有可用的 copyfree 许可替代方案（例如，LLVM）。一份立场文件将很快分发给开发者征求意见。

在本季度，共发布了三个新的提交权限（commit bits），并且有一个提交权限被收回保管。请欢迎 Wojciech Macek、Jared McNeil 和 Stanislav Galabov，向 Davide Italiano 告别，尽管他因太忙而无法直接参与 FreeBSD 工作，但他仍将通过上游的 lld 和其他工具链部分做出贡献。
...
通过此更改，所有非固定映射都将应用随机化。所谓随机化，指的是映射的基地址会保证选取一定量的熵（位数）。如果映射要求对齐超页，随机化会遵循超页属性。

随机化是尽力而为的。如果由于碎片化问题阻止了熵注入，分配器会退回到首次适配策略。实现一个强模式并不复杂，其中若未能保证所请求的熵量则会导致映射请求失败，但我认为这种模式不可用。

我目前尚未对注入的熵量进行精细调整，但这只是一个量化的变化，不会影响实现。目前的熵量由 aslr_pages_rnd 控制。

为了不破坏合并优化，减少 ASLR 固有的页表碎片，并保留为 malloc 分配的内存进行临时超页提升的能力，对于匿名私有映射实现了局部性，它们会自动分组，直到碎片化出现。匿名组的初始位置当然是随机的。经过一些额外的调整后，这些措施证明是相当有效的。特别是在 i386 上，PyPy 5.0 的一个地址空间需求极高的构建成功完成，并激活了补丁的最激进功能。

默认模式下，sbrk 区域不会被其他映射填充，但可以关闭此功能，这在小地址空间架构（说来有趣，现在 32 位已被认为是小的）上提供了更多的腾挪余地。这与应用程序关于 mmap(2) 基地址的提示相关。测试表明，忽略提示不会影响常见应用程序的功能，但我预计更为复杂的代码可能会中断。默认情况下，sbrk 保持不变，mmap 提示得到满足，可以通过使用 kern.elf{32,64}.aslr_care_sbrk sysctl 进行更改（目前默认启用，供更广泛的测试）。
...
感谢 Ed Maste、Robert Watson、John Baldwin 和 Alan Cox 对补丁的讨论，以及感谢 FreeBSD 基金会的指导。
...
我开始研究 Ceph，是因为 HAST 解决方案与 CARP 和 ggate 并没有完全实现我想要的功能。但我的目标是运行一个 Ceph 存储集群，这些存储节点运行 ZFS。最终目标是运行 bhyve，使用存储在 Ceph 中的 RBD 磁盘。
...
- RBD
  Rados 块设备是在 Linux 内核中实现的。似乎最初曾有一个用户空间实现。而且也许可以使用 ggated 作为模板，因为它执行一些类似的功能，而不仅仅是两个磁盘之间。而且它有一个用户空间的对应部分。
- BlueStore
...
[编程之夏 2016：将 /dev 条目链接到 sysctl 节点](https://wiki.freebsd.org/SummerOfCodeIdeas#Devices_management:_link_.2Fdev_entries_to_sysctl_nodes)

[编程之夏 2016：重新设计 libdevq](https://wiki.freebsd.org/SummerOfCodeIdeas#Devices_management:_redesign_and_rewrite_libdevq)
...
首次，我们为 2016 年的编程之夏提出了两个想法：一是内核任务，一是重新设计 libdevq。六名学生提交了这些想法的提案，这让我们感到意外！我们现在需要决定要指导哪位学生，这个选择很难。
...
### ARM Allwinner SoC 支持

**链接**：[Allwinner FreeBSD 维基](https://wiki.freebsd.org/FreeBSD/arm/Allwinner)
...
**链接**：[ubldr 补丁](https://reviews.freebsd.org/D3180)
...
Filemon 是一个内核模块，用于跟踪一个命令创建、读取、写入或执行的文件。它允许与 bmake 的元模式结合，跟踪构建依赖关系。bmake 将 filemon 的输出存储在 .meta 文件中，连同构建命令一起，并在后续使用中，如果任何引用的文件缺失或修改，或者构建命令发生变化，触发目标的重新构建。它提供了与编译器的 -MF 标志相同的功能，但适用于所有文件。这对于构建世界的 WITH_META_MODE（即普通构建世界，但只使用 filemon）至关重要，以提供一个可靠的增量构建，甚至无需 .depend 文件或编译程序 -MF 标志。这使得 -DNO_CLEAN 始终有效。
...
Sepherosa Ziehau <[sepherosa@gmail.com](mailto:sepherosa@gmail.com)>

Howard Su <[howard0su@gmail.com](mailto:howard0su@gmail.com)>

Hongjiang Zhang <[honzhan@microsoft.com](mailto:honzhan@microsoft.com)>

Dexuan Cui <[decui@microsoft.com](mailto:decui@microsoft.com)>
...
**链接**：[测试召集](https://lists.freebsd.org/pipermail/freebsd-infiniband/2016-March/000190.html)
...
本项目的目标是使用 CAM 框架重新实现现有的 MMC/SD 栈。这将允许利用经过充分测试的 CAM 锁定模型和调试功能。还将能够处理插入卡片生成的中断，这是实现 SDIO 接口的前提。SDIO 支持对于与许多开发板上的 WiFi/BT 模块进行通信是必要的，如树莓派 3。
...
正在进行开发工作，旨在为使用 GlusterFS 作为后端的 NFS 服务器提供 pNFS 支持。这将是一个长期项目，最终目标是使 NFS 服务器能够扩展到单一服务器系统之外。希望在 2016 年春季末进行测试。pNFS 允许 NFSv4.1 客户端直接向数据服务器进行读/写，而不是通过 NFS 服务器。

#### 待办事项

1. 在开发过程中，pNFS 服务器需要进行测试，否则它将无法进展到接近生产状态。希望在 2016 年春末，在 FreeBSD 的 Subversion 项目分支中提供测试代码。

### 使用 PVS Studio 对 FreeBSD 内核进行静态分析
...
目前，大部分基本支持代码已经提交：dTSEC（以太网）、核心支持（e500mc、e5500）。在此过程中，rman（内核资源管理器）得到了增强，使用 uintmax_t 处理资源。这使得设备可以位于 32 位系统上 4GB 边界以上的物理地址。通过静态编译的设备树，它能够启动到多用户模式并通过 nfsroot 使用，并可以像正常系统一样工作（配置完成后通过串口和 SSH 登录）。
...
### Cavium ThunderX (arm64) 上的 FreeBSD

**联系方式**：

Dominik Ermel <[der@semihalf.com](mailto:der@semihalf.com)>

Wojciech Macek <[wma@semihalf.com](mailto:wma@semihalf.com)>

Zbigniew Bodek <[zbb@semihalf.com](mailto:zbb@semihalf.com)>
...
**链接**：[源代码树](https://svnweb.freebsd.org/base/projects/powerpcspe/)
...
**链接**：[ELF 工具链网站](http://elftoolchain.sourceforge.net/)
...
**链接**：[原生 PCIe 热插拔支持](https://github.com/bsdjhb/freebsd/tree/pci_hp)
...
直接使用 ptrace(2) 的新线程目标已被提交到上游，并包含在 GDB 7.11 中。该 Port 也已更新为 GDB 7.11。
...
Rafael Espíndola <[rafael.espindola@gmail.com](mailto:rafael.espindola@gmail.com)>
...
Ed 一直在运行 FreeBSD/amd64 的实验性 world 构建，将 lld 安装替换 ld.bfd 作为链接器。在当前功能缺口的解决方法下（使用 WITHOUT_SYMVER 选项禁用符号版本控制使用，并用 GNU ld 链接加载器组件），lld 现在能够链接一个正常工作的 FreeBSD 用户空间。
...
4. 将更新的 lld 快照导入到供应商区域，添加构建基础设施并将其连接到 world 构建，安装为 ld.lld。
...
[USE_GNOME Port 开发者手册章节](https://www.freebsd.org/doc/en_US.ISO8859-1/books/porters-handbook/using-gnome.html)
...
Tobias Berner 一直推动 KDE Frameworks 5 和 Plasma 5 的工作，他收到了 KDE 的提交权限，并已将 FreeBSD 的相关补丁提交到上游的多个 KDE 存储库中。另一个团队亮点是在今年初，向我们的实验存储库重新添加了一个提交者：Adriaan de Groot，这位长期的 KDE 贡献者曾在我们团队成立近十年前，负责 KDE 和 FreeBSD 的工作。欢迎回来，Ade！
...
Frederic Culot <[portmgr-secretary@FreeBSD.org](mailto:portmgr-secretary@FreeBSD.org)>
...
在第一季度，三名提交者的提交权限被收回保管，原因是超过 18 个月未活动（milki、brian），或根据提交者请求（mmoll）。我们迎来了一位回归提交者（fluffy），他的提交权限已恢复。两位新开发者被授予了 Port 提交权限（Olivier Cochard-Labbe 和 Christoph Moench-Tegeder）。
...
Michael Lucas <[mwlucas@michaelwlucas.com](mailto:mwlucas@michaelwlucas.com)>
...
Federico Caminiti <[demian.fc@gmail.com](mailto:demian.fc@gmail.com)>

Carlos J Puga Medina <[cpm@fbsd.es](mailto:cpm@fbsd.es)>

Ruey-Cherng Yu <[raycherng@gmail.com](mailto:raycherng@gmail.com)>

Warren Block <[wblock@FreeBSD.org](mailto:wblock@FreeBSD.org)>

Federico Caminiti 完成了全新的西班牙语版本翻译，涉及了 31,000 字的 [FAQ](https://www.freebsd.org/doc/en_US.ISO8859-1/books/faq/)，并在 Carlos J Puga Medina 的编辑帮助下完成了这项工作。

这一里程碑式的成就标志着首次使用新的 PO 翻译系统来翻译整本书！

Ruey-Cherng Yu 开始进行一项雄心勃勃的繁体中文（zh_TW）翻译，内容是 64,000 字的 [Porter 手册](https://www.freebsd.org/doc/en_US.ISO8859-1/books/porters-handbook/)。目前大约翻译了书中的一半字符串。
...
**链接**：[FreeBSD 德文文档项目主页](https://people.freebsd.org/~jkois/FreeBSDde/de/)
...
- 在 r296668 中将 WITH_FAST_DEPEND 设置为默认选项，并在 r297434 中将其变为唯一选项。新的依赖代码避免了 make depend 树遍历，并通过编译的副作用生成 .depend 文件。这是通过使用编译程序的 -MF 标志完成的，从而使构建速度提高了 15-35%。
- [PR 196193](http://bugs.freebsd.org/196193)：修复了 WITHOUT_CROSS_COMPILER，正确使用 --sysroot 使得该选项在更多情况下有效。尽管在进行重大编译程序升级时仍然不安全，但计划进一步改进。
...
1. 有选择地跳过 buildworld 的引导编译程序阶段。
2. 跳过 make obj 树遍历。
3. 在 buildworld 中启用 WITH_META_MODE，以通过 filemon(4) 和 bmake 的 .MAKE.MODE=meta 提供可靠的增量构建。这不应与 WITH_DIRDEPS_BUILD（旧名 WITH_META_MODE）混淆，后者是一个完全不同的构建系统，曾在 BSDCan 2014 上由 Simon Gerraty 展示。

### 树莓派上的 Qt 5.6

**链接**：[树莓派上的 Qt 5.6](http://kernelnomicon.org/?p=598)
...
基金会员工兼发行工程师 Glen Barber 负责使用 pkg(8) 实现 pkgbase，将调试文件从默认基本系统中分离，以便在安装时可以选择或取消选择，支持 FreeBSD 10.3 准时发布的准备和测试，并生成 11-CURRENT 和 10-STABLE 快照构建。
...
FreeBSD 基金会赞助了全球范围内的多个会议、活动和峰会。这些活动可以是与 BSD 相关的、开源的，或是面向代表性不足的群体的技术活动。我们为 BSD 相关的主要会议如 BSDCan、AsiaBSDCon 和 EuroBSDCon 提供财政支持，并为像 BSDays、FreeBSD 峰会、FreeBSD 工作坊、营地和黑客松等小型活动提供财政或其他支持。对于开源会议，当我们能够获得免费非营利展位时，我们也会参加。
