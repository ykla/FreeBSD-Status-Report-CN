...
- 原文：[FreeBSD Quarterly Status Report 4th Quarter 2018](https://www.freebsd.org/status/report-2018-09-2018-12.html)
- 最后作者：Daniel Ebdrup
...
i386 PAE 页表支持最高 24GB 内存、持续集成工作、ENA 驱动程序更新和图形更新、ARM 增强功能（如 RockChip、Marvell 8K 和 Broadcom 支持），以及更多 DTS 文件、更多 Capsicum 功能、改进 pfsync 等，你可以自己阅读更多内容。
...
Mateusz 在这一季度的工作包括识别并修复 FreeBSD 内核和系统库中的瓶颈。FreeBSD 基本系统构建以及通过 Poudriere 构建的 Ports 被用作驱动案例。
...
- 在科罗拉多州莱克伍德市的 落基山女性计算庆典（Rocky Mountain Celebration of Women in Computing）中举办了 FreeBSD 入门研讨会，并进行了“为什么你应该贡献给 FreeBSD”演讲
...
在过去的季度，我们迎来了 Alexandre C. Guimarães (rigoletto@) 和 Vinícius Zavam (egypcio@)。Alberto Villa (avilla@)、Lars Thegler (lth@)、Dryice Dong Liu (dryice@)、Ion-Mihai Tetcu (itetcu@)、Gábor Páli (pgj@)、Tom Judge (tj@)、Ollivier Robert (roberto@) 和 Maxim Sobolev (sobomax@) 的 port 提交位已被暂时保管。
...
Capsicum 的主要改进是引入了一个名为 Casper 的服务 `fileargs`，这为需要访问文件系统的工具提供了一种简便的沙盒化方式。应用程序如 `brandelf(1)`、`wc(1)`、`savecore(1)`、`head(1)` 和 `strings(1)` 中有多个示例展示了如何使用 `fileargs`。`fileargs` 服务还帮助将新功能引入 bhyve，例如通过 Capsicum 保护的音频设备。

另一个重要进展是引入了一个私有的 Casper 服务，并对 `rtsold(8)` 和 `rtsol(8)` 进行了沙盒化。
...
FreeBSD 现在支持 TPM 2.0 设备。TPM（可信平台模块）是一个独立的芯片，提供安全计算和安全的 NVRAM 存储。它通常与测量启动（Measured Boot）相关联，即提供启动元素（如固件镜像和启动配置）到操作系统的哈希测量。在 FreeBSD 中，TPM 可用于通过执行加密操作来增强像 Strongswan IPsec、SSH 或 TLS 等服务的安全性，这些操作在 TPM 芯片中使用嵌入的密钥进行，软件无法访问这些密钥。TPM 提供的功能，如安全 NVRAM 存储、数据封装、随机数生成等，也可以通过 IBM TSS 库供任何软件使用。

UEFI Secure Boot 是一种技术，提供对主机在启动过程中执行的镜像进行身份验证。这可以防止未经授权的恶意启动代码（如 rootkits）的持久化。UEFI 存储了允许和被列入黑名单的证书列表，并在执行启动镜像和 UEFI 应用程序之前验证它们的签名。Semihalf 开发了对 X509 证书和签名验证代码的支持，并将其集成到 EFI loader 中，使用的是最小的 BearSSL 库。允许和禁止的证书列表通过 UEFI 环境变量获取。这样，用户可以使用自签名证书签署内核二进制文件，附加签名，并让加载程序验证其真实性。
...
Luis Octavio O Souza <[loos@FreeBSD.org](mailto:loos@FreeBSD.org)>
...
得益于 Ports 中 libinput 的更新，我们终于将 Plasma Desktop 更新到了 5.12 之后的版本，并且再次与上游版本同步。
...
在后台，我们已更改 Ports 中 cmake 的默认行为，使其默认使用外部构建。
...
Official <[meetup@bsd-pl.org](mailto:meetup@bsd-pl.org)>

Konrad Witaszczyk <[def@FreeBSD.org](mailto:def@FreeBSD.org)>

Mariusz Zaborski <[oshogbo@FreeBSD.org](mailto:oshogbo@FreeBSD.org)>

Jarosław Żurek <[contact@zurek.pro](mailto:contact@zurek.pro)>
...
- 将更多编译程序工具链组件迁移到 llvm 的实现（llvm-ar、llvm-nm 和 llvm-objdump）。
- 使用链接时优化（LTO）编译应用程序。
...
[FreeBSD 二进制软件包](http://jdebp.eu/Softwares/nosh/freebsd-binary-packages.html)
...
更多信息请参见上述链接的简介和概述，以及 nosh 指南。

#### 新闻

自 2017 年的上次状态报告以来，项目已进行了大量开发。简要列出一些已完成的工作：

- 增加了多个软件包，包括运行 Bruce Guenter 的 bcron、OpenRC 的 rc-update 和 rc-service 工具的适配器，以及一些 Linux 的 util-linux 工具的便携替代品。
- 增加了很多新工具，包括 getuidgid、userenv-fromenv、setgid-fromenv、envgid、printenv、setlogin、console-decode-ecma48、console-control-sequence、console-flat-table-viewer、console-input-method 和 local-stream-socket-connect。例如：
  ...
  - [console-control-sequence](http://jdebp.eu/Softwares/nosh/guide/console-control-sequence.html) 也响应 setterm 名称，可以完成非便携的 util-linux 工具所能完成的大多数任务（不包括那些特定于非便携式 Linux ioctl() 和控制代码的功能，如显示适配器电源管理），而且还包括一些 util-linux 工具所不支持的额外标准 DEC VT 和 ECMA-48 功能（如启用和禁用删除线、计算器键盘应用模式、鼠标报告以及替代屏幕缓冲区等）。
- 增加了更多服务软件包，数量繁多，无法在此列出。可以在 1.37 和 1.38 + 1.39 的发布公告中找到它们。
- 在 nosh 指南中增加了新的章节，涉及软件包和 Port、终端的资源（如键盘映射、输入法和字体），以及 head0 用户空间虚拟终端的结构。还增加了新的手册页 - 除了所有新命令的手册页外，还有关于 [http://jdebp.eu/Softwares/nosh/guide/TerminalCapabilities.html](http://jdebp.eu/Softwares/nosh/guide/TerminalCapabilities.html) 系统的手册页。此外，还提供了 [一些 Linux 手册页的替代品](http://jdebp.eu/Proposals/linux-kvt-manual-pages.html)，这些手册页在过去十年里消失了。
