...
- 原文：[FreeBSD Quarterly Status Report 2018 January-September](https://www.freebsd.org/status/report-2018-01-2018-09.html)
- 最后作者：Daniel Ebdrup

## 引言

FreeBSD 一路发展至版本 12，现在回顾一下所有已经完成的工作或许是有益的，既包括许多显著的变化，也涵盖那些发生在幕后、确保 FreeBSD 在设计、实现和运作方面持续提供替代方案的工作。

即将呈现的内容纷繁庞杂，难以概括，但几乎涵盖了所有方面，从发布版本的最终敲定、行政工作，到优化与性能提升、功能的添加与修复，乃至许多或许会让你略感惊喜的改进。

请准备好一杯咖啡、茶、热可可，或一切你喜欢的饮料，享受这份从 2017 年 10 月起至今的累积报告。
...
1. 各个小组之间的协调，以支持即将发布的 12.0 版本。OpenSSL 1.1.1 的发布时间带来了挑战。新的 OpenSSL 版本包含了 API 更改，因此许多基本系统和 Port 需要进行相应的更改。在 12.0 中继续使用旧版 OpenSSL 并不可行，因为这意味着需要向一个上游已不再维护的 OpenSSL 版本向后移植许多更改。
...
- Matt Macy (mmacy@) 的提交权限在 Sean Bruno (sbruno@) 的辅导下恢复。
- Breno Leitao (leitao@) 在 Justin Hibbits (jhibbits@) 的辅导下，并由 Nathan Whitehorn (nwhitehorn@) 共同辅导，获得了源代码提交权限。
- Leandro Lupori (luporl@) 在 Justin Hibbits (jhibbits@) 的辅导下，并由 Nathan Whitehorn (nwhitehorn@) 共同辅导，获得了源代码提交权限。
...
- Daichi Goto (daichi@) 的提交权限在 George Neville-Neil (gnn@) 的辅导下被恢复。
- Intel 的 Ben Widawsky (bwidawsk@) 在 Ed Maste (emaste@) 的辅导下获得了厂商提交权限。

#### 2018 第三季度

- 核心团队决定开始每月召开两次会议，以应对大量新的议程事项。
- 許立文 (lwhsu@) 在 Mark Johnston (markj@) 的辅导下，并由 Ed Maste (emaste@) 共同辅导，获得了源代码提交权限。
...
使用 USES=php:phpize、php:ext、php:zend 和 php:pecl 的 Port 已实现 flavor 化，并且现在会为所有支持的 PHP 版本（5.6、7.0、7.1 或 7.2）自动构建软件包。
...
FreeBSD 11.2-RELEASE 循环于 2018 年 4 月 20 日启动，并宣布进入代码暂缓阶段。该阶段的进展持续到整个季度，并经历了代码冻结、三次 BETA 构建、三次 RC 构建，最终发布版本于 2018 年 6 月 27 日宣布。

FreeBSD 发布工程团队于 2018 年 8 月 10 日启动了 12.0-RELEASE 循环，宣布进入代码暂缓阶段。代码冻结随后于 2018 年 8 月 24 日进行。稳定/12 分支的初步发布日期预计为 2018 年 9 月 21 日。

由于必须纳入 12.0-RELEASE 的上游代码出现未预料到的情况，12.0-RELEASE 的初步发布计划不得不多次调整。上游代码更新版本中的 API 变化要求对所有与上游代码链接的基本系统实用程序进行更改。到 2018Q3 季度结束时，由于此延迟，稳定/12 分支尚未创建。

在 2018Q3 余下的时间里，发布了多个开发快照版本，面向 head、稳定/11 和稳定/10 分支。
...
- 从内核边界翻译 ioctl 命令和数据结构的模型迁移到翻译命令处理位置的模型。这是一个正确性改进（`ioctl` 命令在特定文件描述符之外没有意义），并改善了代码的可重用性（我的树外工作很快将包括一个 64 位兼容层）。
...
过去十年，随着软件大小的迅速增长，大多数 32 位 FreeBSD 架构（包括 i386）开始遇到问题。当 32 位地址空间足够满足某一任务时，32 位模式在内存流量更少和缓存使用更经济等方面仍然具有固有优势。然而，由于构建工具的体积不断增大，提供自托管的 i386 系统构建变得越来越困难。
...
由于用户地址空间不再与内核共享，copyout(9) 函数已被重写，以在需要访问时为用户空间页面创建临时映射。初始实现使用了 vm_fault_quick_hold_pages() 框架，但随后通过从跳板暂时切换到用户模式映射，并使用手写汇编例程执行更快速的小块复制操作进行了优化。
...
NVDIMM 是一种提供非易失性内存的技术，其访问特性类似于常规 DRAM（即实现主机常规内存地址空间的技术）。ACPI 和 UEFI 规范定义了平台独立的方式来检测和枚举 NVDIMM 的存在。这些规范允许检索大多数所需的数据，以便正确地使用 NVDIMM 存储。

一个新的 FreeBSD 驱动程序解析 ACPI NFIT 表，该表列出了 NVDIMM、其操作特性和 NVDIMM 内存的物理地址空间。该驱动程序将每个地址区域呈现为两个设备：一个设备允许用户空间通过 open(2) 打开一个 devfs 节点，可以从应用程序读取/写入/映射。此映射为零复制。第二个设备是一个 geom disk（9），它使得可以将 NVDIMM 用作常规 FreeBSD 文件系统（如 UFS、ZFS 或 msdosfs）的后端存储。请注意，创建在 NVDIMM 上的传统文件系统上的文件的缓冲区缓存/映射会导致不必要的双重缓冲。
...
本项目当前的补丁集可以在 [GitHub](https://github.com/arichardson/freebsd/tree/crossbuild-aug2018) 上找到。使用当前的原型，能够为使用 clang 编译程序的架构以及使用 gcc 的 MIPS64 编译 world 和 kernel。然而，像 LOCALES 这样的选项目前尚不支持，需要进一步修改才能使得启动工具在 Linux/macOS 上构建。
...
从 FreeBSD 12.0 开始，这一功能开箱即用。这意味着你可以将 BeagleBone Black（使用其 USB 客户端插座）或树莓派 Zero（使用 On-The-Go（OTG）端口）连接到笔记本电脑，你将获得虚拟 USB 串口，作为系统控制台，getty（8）会等待你登录。这意味着你不再需要寻找键盘和屏幕，或处理控制台电缆来配置系统。你还可以将其切换为提供网络接口，或作为 USB 驱动器呈现——所有内容都在 FreeBSD 手册中有详细说明。
...
**联系方式**：Michał Krawczyk <[mk@semihalf.com](mailto:mk@semihalf.com)>
...
Domagoj Stolfa <[domagoj.stolfa@cl.cam.ac.uk](mailto:domagoj.stolfa@cl.cam.ac.uk)>
...
**联系方式**：Ben Widawsky <[bwidawsk@FreeBSD.org](mailto:bwidawsk@FreeBSD.org)>
...
在一台拥有 128 个线程的机器上使用 poudriere 构建软件包时，会遇到一些瓶颈。
...
管道的使用非常频繁，例如，“du -s | sort -n”创建了一个管道，其中一个端点是 du 的标准输出，另一个端点是 sort 的标准输入。
...
### LLVM 7.0 - Sanitizers 支持改进 / 静态代码分析

**链接**：

[发布说明](http://releases.llvm.org/7.0.0/docs/ReleaseNotes.html)

**联系方式**：David Carlier <[devnexen@gmail.com](mailto:devnexen@gmail.com)>
...
**联系方式**：Matthew Macy <[mmacy@FreeBSD.org](mailto:mmacy@FreeBSD.org)>
...
保存/恢复功能是一个可以暂停和恢复虚拟机镜像的功能，已添加到 FreeBSD/amd64 的虚拟化程序 bhyve 中。bhyvectl 工具用于将虚拟机保存为三个文件：
...
Rafał Kozik <[rk@semihalf.com](mailto:rk@semihalf.com)>
...
#### ARMv6 和 ARMv7 镜像现使用 EFI 引导加载程序
...
现在，这些 DTS 文件已经为一些 arm64 开发板进行编译，因为 U-Boot 中存在的 DTS 文件并不总是最新的。

### FreeBSD 在 POWER9 上的支持
...
**赞助者**：IBM、FreeBSD 基金会、QCM Technologies、Semihalf、Limelight Networks。

该项目由 Limelight Networks（Kevin Bowling）发起并支持。

### FreeBSD 在 RISC-V 上的支持

**联系方式**：Ruslan Bukin <[br@FreeBSD.org](mailto:br@FreeBSD.org)>

FreeBSD/RISC-V 是过去一年中积极支持的项目之一。

在编译程序方面，我们已经将 FreeBSD 特定的部分提交到 GNU 工具链的上游。它已更新至 GCC 8.1 和 Binutils 2.30，并且 FreeBSD 的软件包也已提供。
...
U-Boot 邮件列表 <[uboot@FreeBSD.org](mailto:uboot@FreeBSD.org)>
...
[PuppetLab & FreeBSD slack channel](https://puppetcommunity.slack.com/messages/C6CK0UGB1/)
...
自去年上次状态报告以来，puppet@ 团队定期更新了 Port Puppet，以跟上上游发布的进度。我们还在 BSDCan 举办了一个 Puppet 讲座。
...
### scarab: CLI 工具，用于 Bugzilla 相关工作流程
...
**联系方式**：<[wiki-admin@FreeBSD.org](mailto:wiki-admin@FreeBSD.org)>
...
季度报告在几乎一年的中断后重新恢复。旧的工作流程包括用户提交 XML 格式的条目，这些条目随后会被手动组装成 DocBook，现在已被新的工作流程替代，改用 Markdown。XML 提交表单改由 GitHub 拉取请求完成。这将使提交和编辑变得更加简单和用户友好。
