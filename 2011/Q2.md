...
我们进行了一些 Port 的实验性运行，并修复了许多 Port 错误，现在我们能够使用 clang 构建超过 15000 个 Port。由于大多数问题比较简单，我们预计这一数字会迅速增长。
...
在上个季度，我们一直在保持 Chromium 浏览器的最新版本，每当上游发布新版本时，就会将其导入到 Ports 中。随着时间的推移，更多的补丁被集成或因上游代码清理而过时。版本 13 可以从 Chruetertee 仓库获取，其补丁数量比版本 12 少了 70 个。
...
**[2**：**联系方式**：Brooks Davis <[brooks@FreeBSD.org](mailto:brooks@FreeBSD.org)>
...
**[项**：目提案](http://www.google-melange.com/gsoc/proposal/review/google/gsoc2011/zy/1)
...
2. 添加测试套件以检查 GNU 特有的行为，并添加一些与区域设置相关的测试。
3. 测试并审查代码。联系作者，检查这些改进是否可以添加到上游代码中，以便更多人受益查这些改进是否可以添加到上游代码中，以便更多人受益。
2. 添加测试套件以检查 GNU 特有的行为，并添加一些与区域设置相关的测试。
3. 测试并审查代码。联系作者，检查这些改进是否可以添加到上游代码中，以便更多人受益。
