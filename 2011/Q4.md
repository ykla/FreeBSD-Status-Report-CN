...
- 原文：[FreeBSD Quarterly Status Report 4th Quarter 2011](https://www.freebsd.org/status/report-2011-10-2011-12.html)
- 最后作者：FreeBSD 项目
...
新增了两个库，libc++（提供 C++11 STL 实现）和 libcxxrt（提供 C++ ABI 规范实现）。通过在 **src.conf** 中添加 `WITH_LIBCPLUSPLUS=yes` 启用此功能。默认情况下未启用此功能，因为 libc++ 无法使用基本系统中的 gcc 版本构建，且需要使用 clang 构建。
...
3. 默认启用 libc++ 的构建（希望在 9.1 版本中，当 clang 成为默认系统编译程序时），并默认使用 libcxxrt 替代 libsupc++。
4. 更多的测试。然后是更多的测试。
5. 从基本系统中移除 libstdc++，并通过 Ports 提供以保持向后兼容性。

### pfSense

**链接**：[pfSense 官方网站](http://www.pfsense.org/)
...
**链接**：[DruidBSD 项目](http://druidbsd.sourceforge.net/)

**联系方式**：Devin Teske <[dteske@vicor.com](mailto:dteske@vicor.com)>

2011 年 12 月 31 日，**sysutils/sysrc** 添加到了 Ports 中。2012 年 1 月 6 日，**sysutils/host-setup** 添加到了 Ports 中。**sysutils/tzdialog** 的添加仍在待定中。

这些工具一起或单独工作，旨在使系统配置更加简便和高效。

**sysrc(8)** 允许你安全地修改 **rc.conf(5)**，无需担心或犹豫，使远程管理和脚本更改变得简单。在管理 Puppet 安装时也很有用。
...
在第四季度，我们增加了 4 名新的提交者，保管了 4 个提交位，并且有一名提交者重新参与了 Ports 工作。

Ports 管理团队持续进行 -exp 运行，验证基本系统更新如何影响 Ports 树，同时为主要的 Port 更新提供质量保证（QA）运行。值得注意的是，-exp 运行已完成以下工作：
...
- 持续验证 pkgng 基础设施
- 使用 clang 作为默认编译程序测试 Port

pkgng 现在支持真正的安全二进制升级，并且具有实际的完整性检查，工作开始使 Ports 树能够引导 pkgng。更多信息请参阅 [CFT 邮件](http://lists.freebsd.org/pipermail/freebsd-ports/2011-November/071631.html)。

pointyhat-west 构建机器正在向生产使用迈进，代码更新使其更加灵活，如支持交换 **make.conf** 中的信息以适配构建从属机，协助 pkgng -exp 运行的测试，并正确构建 Port linux_base。

决定将 Ports 树从 CVS 迁移到 Subversion，由 beat@ 负责该项目。更多信息请见 [wiki](http://wiki.freebsd.org/PortsSVN)。

创建了一个用于 Ports 相关公告的受控邮件列表，[http://lists.FreeBSD.org/mailman/listinfo/freebsd-ports-announce](http://lists.freebsd.org/mailman/listinfo/freebsd-ports-announce)，该列表旨在，也不限于，作为传达 portmgr@ 公告、测试请求及其他相关信息的方式，供我们的提交者和 Port 维护者社区使用。
...
最令人激动的消息是，我们通过筹款活动筹集了 426,000 美元。我们对 FreeBSD 社区的慷慨深感震撼。感谢所有通过财务捐赠或在项目中志愿服务为 FreeBSD 做出贡献的人。
...
基金会资助的墨尔本大学 Feed-Forward 时钟同步算法项目已完成。我们批准了两个 2012 年的新项目，它们分别是由 Bjoern Zeeb 进行的 FreeBSD IPv6 栈性能分析和由 Pawel Jakub Dawidek 实现的 auditdistd 守护进程。

我们为 FreeBSD 的共同托管中心在 Sentex、NYI 和 ISC 购买了更多服务器和其他硬件。

以上工作以及我们为 FreeBSD 项目所做的许多其他任务，离不开捐赠的支持。请通过捐款或请求你的公司捐款来帮助我们。我们很乐意向你或你的公司发送营销资料。请访问我们的 [捐赠页面](http://www.freebsdfoundation.org/donate/) 了解如何捐赠。
...
**链接**：<http://lists.freebsd.org/pipermail/freebsd-current/2012-January/031007.html>

**联系方式**：Ken Merry <[ken@FreeBSD.org](mailto:ken@FreeBSD.org)>

CAM 目标层 (CTL) 现在包含在 FreeBSD/head 中。
...
### FreeBSD 无 IPv4（“仅 IPv6”）支持

**链接**：[FreeBSD 无 IPv4 支持](http://www.freebsd.org/ipv6/ipv6only.html)

**联系方式**：Bjoern A. Zeeb <[bz@FreeBSD.org](mailto:bz@FreeBSD.org)>

“无 IPv4”（前称“仅 IPv6”）项目最初在 p4 中进行原型设计，并在 2011 年初通过 FreeBSD 基金会和 iXsystems 的支持合并到 FreeBSD 主流版本，作为世界 IPv6 日的一部分，随后继续作为一个自由时间项目进行。感谢一位匿名来源的帮助，我们设置了专用的 i386 和 amd64 构建机器以及一个分发节点，以支持持续构建快照，并希望将来能扩展这些快照的支持，提供更多服务。
...
我要特别感谢佐藤広生/allbsd.org 为亚洲社区在日本提供镜像支持，除了我在欧洲的镜像外。
...
**链接**：[已提交到 HEAD 的补丁](http://people.FreeBSD.org/~mav/gmultipath5.patch)
...
这些更改现在提交到了 FreeBSD HEAD 分支。计划在 9.0 发布后将其合并到 9-STABLE 和 8-STABLE 中。
...
### HDA 声音驱动程序 (snd_hda) 改进

**链接**：[最新补丁](http://people.freebsd.org/~mav/hda.rewrite2.patch)
...
- 增加了对多声道录音的支持。虽然我从未见过默认配置，但 UAA 规范表明这是可能的。现在，如规范所定义，驱动程序会检查具有序列号 14 和 15 的引脚的输入关联，如果找到了（通常会找到）——像以前一样将信号混合在一起。如果没有，它会将输入关联配置为多声道。虽然我发现一些 CODEC 在配置为多声道录音时表现异常，但也有一些成功工作的例子。
...
- 实现了新的控制器流预留机制。这使得可以拥有比控制器支持的流更多的 pcm 设备（通常每个方向最多 4 个流）。现在它仅限制同时传输的音频流数量，这个数量很少达到，且达到时会正确报告。
...
2. 需要对许多硬件（主要是笔记本电脑）进行兼容性调整。应该收集一些统计数据，以便实现其中一些调整，避免过度膨胀的代码。

### LSI 支持的 mps(4) SAS 驱动程序

**链接**：<http://lists.freebsd.org/pipermail/freebsd-current/2012-January/031358.html>

**联系方式**：Ken Merry <[ken@FreeBSD.org](mailto:ken@FreeBSD.org)>

**联系方式**：Kashyap Desai <[Kashyap.Desai@lsi.com](mailto:Kashyap.Desai@lsi.com)>

LSI 支持的 **mps(4)** 驱动程序在 FreeBSD/head 中可用，支持其 6Gb SAS 控制器和 WarpDrive 固态硬盘。
...
代码提交到了 HEAD 分支，并计划在一段时间后合并到 8/9-STABLE。
...
新的 NFS 客户端和服务器不再视为实验性，并且现在成为 FreeBSD 9.0 的默认设置。包括对 NFSv4.0、NFSv3 和 NFSv2 的较为完整的支持。NFSv4.0 的委派功能在服务器端默认未启用，因为服务器上对本地系统调用的处理尚未实现。到目前为止，过渡似乎还算顺利，仅有一些小的晦涩难懂的问题未在 FreeBSD 9.0 中修复。有关这些问题的补丁可以在 [http://people.FreeBSD.org/~rmacklem](http://people.freebsd.org/~rmacklem) 找到。
...
此外，还有一个针对我所称为 packrats 的补丁，这个补丁让线程在 NFSv4.0 客户端中对委托文件进行积极的磁盘缓存。它目前似乎能够正常工作，但尚未实现客户端重启恢复功能，因此目前只能作为实验性功能使用。这个补丁可以在 [http://people.FreeBSD.org/~rmacklem/packrat-patches](http://people.freebsd.org/~rmacklem/packrat-patches) 找到。

### 新 CARP

**链接**：[主要提交](http://svnweb.freebsd.org/base?view=revision&revision=228571)
...
经过显著更新的 CARP 协议提交到了 head/ 分支。我预计新代码将更容易维护且更少错误，因为它在网络栈中使用了更少的 hack。
...
德文文档项目很高兴地报告，在过去的季度中有两个重要章节翻译完成。第一个更新是防火墙章节，涵盖了完整的 IPFW 部分。此翻译由 Christopher J. Ruwe 贡献，虽然有一些语言风格上的修正工作，但他做了大部分翻译工作。我们非常感谢 Christopher 的贡献。

另一个翻译的章节是新的 bsdinstall 部分。Benedict Reuschling 完成了该章节的工作，他尝试保持与 sysinstall 章节相同的标题（至少在可能的情况下）。

建议讲德语的用户阅读这两个章节并报告拼写错误或语法错误，以便我们进行修正。
...
在此期间，**www/ja** 子树中的许多过时内容更新为英文版本的最新内容。手册中的 “bsdinstall” 部分翻译完成，“cutting-edge” 部分目前正在进行中。
...
**链接**：[ARMv6 分支](http://svnweb.freebsd.org/base/projects/armv6/)
...
### FreeBSD/powerpc Freescale QorIQ DPAA
...
### 改进对 Intel SandyBridge CPU 新特性支持

**联系方式**：Konstantin Belousov <[kib@FreeBSD.org](mailto:kib@FreeBSD.org)>

对 Intel SandyBridge CPU 新特性的支持正在逐步推进。

提交的补丁允许查询并启用扩展的 FPU 状态，这使得支持 YMM 寄存器和 AVX 指令集的处理器得以使用。待办事项包括在计划在一个月后将变更合并到 stable/9 之前，进行更广泛的测试，并开始使用 XSAVEOPT 指令来优化上下文切换时间。

开发了启用并使用每进程 TLB 的补丁。最新版本可通过 [http://people.freebsd.org/~kib/misc/pcid.2.patch](http://people.freebsd.org/~kib/misc/pcid.2.patch) 获取。文档中将该功能称为 PCID，它通过将 PID 标签应用到每个非全局 TLB 条目，避免在上下文切换时刷新 TLB。在 SandyBridge 上，测量表明，补丁和原生内核在上下文切换延迟上并无明显差异。

即将推出的 IvyBridge CPU 承诺提供以 INVPCID 指令为形式的优化，允许优化 TLB 清除处理程序。上面的补丁在支持的 CPU 上使用了该指令。待办事项包括获取 IvyBridge 处理器并进行基准测试。
...
我们很高兴宣布，FreeBSD Haskell 团队将 Haskell 平台更新为 2011.4.0.0，并且将 GHC 更新为 7.0.4，更新内容提交到了 FreeBSD Haskell Ports 仓库中。我们还增加了若干新的 Haskell Port，当前总数超过了 300 个。其中一些新添加的 Port 包括 Yesod、Happstack（Haskell 中流行的 Web 开发框架）、ThreadScope（用于并行 Haskell 程序的图形分析工具）。

由于 9.0-RELEASE 发布时的 Ports 仓库冻结，这些更新目前尚未出现在官方 Ports 树中。它们将在仓库解冻后提交到官方树中，在此期间，可以通过 FreeBSD Haskell Ports 仓库访问。

#### 待办事项

1. 将待提交的 Haskell Port 提交到 FreeBSD Ports 仓库。
...
我们将再次尝试将默认 Ruby 从 1.8 更改为 1.9。这个过渡过程中会有一些不可避免的牺牲。**sysutils/rubygem-chef-server** Port 由 RideCharge Inc / Taxi Magic 提供，现在他们专门使用它。
...
该团队还发布了多个版本并将许多修复和补丁推送到上游。最近发布的版本包括：
...
1. 测试 KDE SC 4.8.0。
2. 测试 KDE PIM 4.7.4。
3. 测试 phonon-gstreamer 和 phonon-vlc，因为 phonon-xine 后端不再维护（但目前仍将保留在 Ports 中）。
...
- [multimedia/xbmc-pvr](http://freshports.org/multimedia/xbmc-pvr) 最近提交了，此外，[multimedia/vdr](http://wiki.freebsd.org/VDR) Ports 也支持观看/录制数字电视，此外，像 kaffeine、mplayer 或 vlc 等其他应用也能使用。
...
许多 Perl 模块在 Ports 中自带测试用例。这个项目的目标是确保所有这些测试都能通过。向 Ports tinderbox 添加了补丁，以便安装与测试相关的依赖项，许多 Port 现在包含了 TEST_DEPENDS。希望提供帮助的用户可以使用补丁启用测试功能。所有 `p5-*` Port 都构建了并尝试运行测试。目前，约有 61% 的 Perl Port 测试通过。许多 Port 更新了，包含了缺失的依赖项或进行了其他修改，使测试能够通过。长期目标包括为测试 Port 提供一个更通用的框架，并在更新 Port 时执行自动化测试。

#### 待办事项

1. 许多未通过测试的 Perl Port 仍待修复。
...
Redports 是一项免费服务，为 FreeBSD 的 Port 维护者和提交者提供自动构建测试服务，支持多种 FreeBSD 版本和架构。之所以创建这个平台，是因为有许多人无法访问 Ports Tinderboxes，而现有的 Tinderboxes 通常是为某个特定团队专用的。
...
**链接**：<http://wiki.freebsd.org/Xorg>

**联系方式**：X11 FreeBSD 邮件列表 <[freebsd-x11@FreeBSD.org](mailto:freebsd-x11@FreeBSD.org)>

X11 团队开始着手更新 X.Org Ports 的下一个主要版本。你可能已经注意到，X.Org 栈中的库和 Port proto 更新了。目前，在我们的开发仓库中，我们有了包括 mesa 和 xf86-video-intel 在内的许多 Port 的最新版本。
