...
新的 NFS 客户端和服务器不再被视为实验性，并且已成为 FreeBSD 9.0 的默认设置。包括对 NFSv4.0、NFSv3 和 NFSv2 的较为完整的支持。NFSv4.0 的委派功能在服务器端默认未启用，因为服务器上对本地系统调用的处理尚未实现。到目前为止，过渡似乎还算顺利，仅有一些小的晦涩难懂的问题未在 FreeBSD 9.0 中修复。有关这些问题的补丁可以在 [http://people.FreeBSD.org/~rmacklem](http://people.freebsd.org/~rmacklem) 找到。
...
我们将再次尝试将默认 Ruby 从 1.8 更改为 1.9。这个过渡过程中会有一些不可避免的牺牲。sysutils/rubygem-chef-server port 由 RideCharge Inc / Taxi Magic 提供，现在他们已经专门使用它。
