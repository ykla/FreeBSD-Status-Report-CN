...
- 原文：[FreeBSD Quarterly Status Report 1st Quarter 2011](https://www.freebsd.org/status/report-2011-01-2011-03.html)
- 最后作者：FreeBSD 项目

## 引言

本报告涵盖了 2011 年 1 月至 3 月期间与 FreeBSD 相关的项目。这是计划于 2011 年发布的四个报告中的第一个。在本季度，工作重点是发布 FreeBSD 的新次要版本 7.4 和 8.2，这两个版本于 2011 年 2 月发布。目前，项目开始着手下一版主要版本 9.0 的开发。

感谢所有报告者的出色工作！本报告包含了 34 项条目，希望你会喜欢阅读。
...
### 基于 GEOM 的 **ataraid(4)** 替代方案 — geom_raid

**联系方式**：Alexander Motin <[mav@FreeBSD.org](mailto:mav@FreeBSD.org)>

**联系方式**：M. Warner Losh <[imp@FreeBSD.org](mailto:imp@FreeBSD.org)>

FreeBSD 9-CURRENT 中新增了一个 RAID GEOM 类（geom_raid），用于替代 **ataraid(4)**，以支持各种基于 BIOS 的软件 RAID。与 **ataraid(4)** 不同，这种实现不依赖于传统的 **ata(4)** 子系统，并且可以与任何磁盘驱动程序一起使用，包括新的基于 CAM 的驱动程序（**ahci(4)**，**siis(4)**，**mvs(4)** 和带有 `options ATA_CAM` 的 **ata(4)**）。为了使代码更具可读性和可扩展性，这一实现遵循了模块化设计，包括核心部分和两个模块集，分别实现对不同元数据格式和 RAID 级别的支持。

目前实现了对以下流行元数据格式的支持：Intel、JMicron、NVIDIA、Promise（也被 AMD/ATI 使用）和 SiliconImage。
...
**赞助者**：Cisco Systems, Inc. 和 iXsystems, Inc.
...
HAST 开发进展顺利。Mikolaj Golub 为 HAST 做出了贡献，现在成为 FreeBSD 的 src 提交者。从上次报告以来，有一些值得注意的变化：
...
在 3 月 14 日，sysinstall 被新的模块化安装程序 BSDInstall 取代，出现在 9.0 快照媒体中。这个新安装程序为多种新特性提供了支持，同时简化了安装过程。在 9.0 发布之前的测试非常欢迎——来自不同平台的 CD 和 U 盘镜像可以在 BSDInstall wiki 页面找到。
...
AFS 是一个分布式网络文件系统，起源于卡内基梅隆大学的 Andrew 项目。自 FreeBSD 4.X 版本以来，OpenAFS 客户端在 FreeBSD 上的实现一直不太实用。先前报告中所涵盖的工作使 OpenAFS 客户端在 9.0-CURRENT 上达到了可用的状态，尽管仍有一些粗糙的地方。自上次报告以来，我们修复了几个影响可用性的 bug，并预计即将发布的 1.6.0 版本将适用于常规客户端工作负载（尽管不适合重负载）。因此，我们提交了包含在 Ports 中的打包（PR ports/152467）。
...
**链接**：[pfSense 主页](http://www.pfsense.org/)

**联系方式**：Scott Ullrich <[sullrich@FreeBSD.org](mailto:sullrich@FreeBSD.org)>

**联系方式**：Chris Buechler <[cmb@pfsense.org](mailto:cmb@pfsense.org)>

**联系方式**：Ermal Luci <[eri@FreeBSD.org](mailto:eri@FreeBSD.org)>
...
大部分代码合并到 CURRENT。还有两个问题，我希望在 9.0-RELEASE 之前解决——见下文——但除此之外，代码稳定；请进行测试并报告任何问题。你将需要使用 `options RACCT` 和 `options RCTL` 重新构建内核。**rctl(8)** 手册页应当是一个很好的入门指南。

该项目由 FreeBSD 基金会赞助。

#### 待办事项

1. 重新实现 `%CPU` 记账和 CPU 节流。
2. 使 Jail 规则持久化——目前，无法在创建 Jail 之前添加 Jail 规则，这使得无法将它们放入 **/etc/rctl.conf**；此外，当 Jail 被销毁时，规则会消失。

### ZFSv28 在 FreeBSD 9-CURRENT 中可用

**联系方式**：Pawel Jakub Dawidek <[pjd@FreeBSD.org](mailto:pjd@FreeBSD.org)>

**联系方式**：Martin Matuska <[mm@FreeBSD.org](mailto:mm@FreeBSD.org)>

ZFS v28 现在进入主线！请多多测试。新特性包括：
...
Bugmeister 团队很高兴欢迎 Eitan Adler (eadler@) 成为最新的仅 GNATS 贡献者。Eitan 一直在帮助筛选新提交的 bug，同时也在处理许多较旧的 bug，关闭重复和过时的 bug，并在必要时联系提交者以获取额外信息。很长时间以来第一次，我们成功地将未关闭的 PR 数量降到了 6000 以下，这在很大程度上归功于 Eitan 的努力。欢迎加入！

PR 在到达时继续分类，通过在主题行中添加“标签”来标记相关的内核子系统，或者针对用户空间 PR 添加手册页引用。我们每晚生成这些报告，将相关的 PR 按标签或手册页分组在一起。这使得在一个特定领域或子系统工作的人能够轻松找到相关的 bug 和问题，并且这种方法证明在关闭一些较旧的 bug 报告方面非常有效。所有这些报告可以通过点击上面第三个链接找到。
...
1. 寻找方法让更多的提交者帮助我们关闭那些被团队分析过的 PR。

### FreeBSD NYI 管理员状态报告

**联系方式**：NYI 管理员团队 <[nyi-admin@FreeBSD.org](mailto:nyi-admin@FreeBSD.org)>
...
我们很荣幸成为 2011 年东京 AsiaBSDCon 的赞助商。我们还承诺赞助 2011 年 5 月的 BSDCan 以及 2011 年 10 月的 EuroBSDCon。基金会还代表 FreeBSD 参与了洛杉矶 SCALE、印第安纳波利斯 LinuxFest 和芝加哥 Flourish 展会。

完成的基金会资助项目：由 Swinburne 大学进行的 [五种新的 TCP 拥塞控制算法项目](http://www.freebsdfoundation.org/announcements.shtml#Swinburne) 和由 Edward Napierala 进行的资源容器项目。
...
我们继续进行基础设施项目的工作，以增强软件包构建、网络测试等硬件。这包括购买设备以及管理设备捐赠。
...
**联系方式**：Sebastian Zander <[szander@swin.edu.au](mailto:szander@swin.edu.au)>
...
使用 DIFFUSE，IPFW 可以计算观察到的流的统计数据（如数据包长度或数据包间隔时间），并使用机器学习（ML）对流进行分类。除了传统的数据包检查规则外，IPFW 规则现在还可以通过流量统计数据或 ML 分类识别的类别来表达。当直接检查数据包存在问题时（例如出于管理原因，或者因为端口号无法可靠地识别应用程序），这种方法特别有用。
...
一个候选补丁准备好测试，旨在将模块化拥塞控制框架合并到 8-STABLE 分支中，补丁地址为 [这里](http://people.freebsd.org/~lstewart/patches/5cc/modularcc_mfc_8.x.r219091.patch)。如果你尝试了该补丁，请将你的经验（无论好坏）反馈给 Lawrence Stewart。
...
所有已知的与有日志软更新相关的问题在主干中修复。如果在运行带有有日志软更新时遇到任何问题，请向我们报告。

我们解决了多个被提出的性能问题。如果在使用有日志软更新时遇到任何性能问题，请向我们报告。

我们改进了在小型（根）文件系统上使用软更新时的资源恢复。我们预计可以在 9.0 系统中使用软更新作为根文件系统。

我们预计在 9.0 系统中默认启用有日志软更新。我们建议 -CURRENT 的用户启用有日志软更新，以帮助发现任何剩余的性能问题和错误。

### Linux 兼容层 - DVB 和 V4L2 支持

**链接**：<http://people.freebsd.org/~nox/dvb/>
...
2. 将 DVB 支持做成一个 Port，因为有关于 LGPL 许可的头文件问题的顾虑，尽管我将代码分离到额外的 kld（linux_dvbwrapper.ko）。
3. 打磨并提交补丁。:)（在它们提交之前，你可以查看我的 [DVB 页面](http://people.freebsd.org/~nox/dvb/) 和 freebsd-emulation@ 邮件列表获取更新。）

## 文档

### 新的 FreeBSD 手册部分：涉及 HAST

**链接**：<http://www.freebsd.org/doc/en_US.ISO8859-1/books/handbook/disks-hast.html>

**联系方式**：Daniel Gerzo <[danger@FreeBSD.org](mailto:danger@FreeBSD.org)>
...
Benedict Reuschling 贡献了关于 HAST 的新手册部分的翻译，而 Benjamin Lukas 则致力于首次翻译手册中的防火墙章节。德语文档项目的提交者忙于保持现有的德语文档的更新。网站翻译也与 FreeBSD.org 上的版本保持同步。
...
**www/ja** 和 **doc/ja_JP.eucJP/books/handbook** 一直在不断更新。在此期间，手册安装页面的翻译完成。以下章节现在与英文版本同步：
...
将来自 www 树的翻译结果合并到单独的仓库进行翻译工作并同步到主树也完成。由于过时或未翻译的文档仍然保留在 **doc/ja_JP.eucJP** 和 **www** 中，进一步的翻译工作仍然是必要的。在此期间，Porter 手册的翻译也有所进展。

### 网络摄像头和 DVB 兼容性列表

**链接**：<http://wiki.freebsd.org/WebcamCompat>

**联系方式**：Matthias Apitz <[guru@unixarea.de](mailto:guru@unixarea.de)>

#### 网络摄像头和 DVB 兼容性列表

这是 FreeBSD 网络摄像头、DVB 和遥控器兼容性列表。该页面的主要目标是准确回答哪个应用程序与给定的摄像头或 DVB 配合使用。表中提到的硬件和软件组合已知可以正常工作。

请向表中添加更多行，或者通过发送邮件告知我添加你的摄像头/DVB 信息。请注意：你应仅添加你实际看到过有效的信息，而不是你认为可能有效的或想象中可能有效的。联系方式（姓名或电子邮件地址）是可选的。
...
### 在 Marvell Raid-on-Chip 上的 FreeBSD/arm
...
**链接**：[FreeBSD/EC2 状态页面](http://www.daemonology.net/freebsd-on-ec2/)

**联系方式**：Colin Percival <[cpercival@FreeBSD.org](mailto:cpercival@FreeBSD.org)>
...
### 在 Freescale QorIQ 上的 FreeBSD/powerpc
...
### MIPS/Octeon 支持和 bootinfo

**联系方式**：Andrew Duane <[aduane@juniper.net](mailto:aduane@juniper.net)>

目前正在改进对 Octeon 处理器的支持，并与其他 MIPS 处理器系列进行集成。目前正在为标准的 MIPS bootinfo 结构提供支持，作为引导 API（以补充/替换 Cavium 特定的结构）。其他对 Octeon 的改进（包括对 CF 和 USB 驱动程序的清理工作）即将到来。

## Ports

### 作为家庭影院 PC 的 FreeBSD

**链接**：[](http://wiki.freebsd.org/HTPC)

**联系方式**：Bernhard Froehlich <[decke@FreeBSD.org](mailto:decke@FreeBSD.org)>

**联系方式**：Juergen Lock <[nox@FreeBSD.org](mailto:nox@FreeBSD.org)>

FreeBSD 作为家庭影院 PC 的平台，可以比现在好得多。我们正在专注于通过扩展主要的 Port（MythTV、VDR、XBMC）并创建一些文档来指导感兴趣的人，来改进对媒体中心应用程序的支持。

在过去的几个月里，我们继续在 HTPC 相关的 Port 上工作，改进了 lirc 和 **multimedia/webcamd** 遥控支持。最后一个缺失的主要 HTPC 应用程序 VDR（视频磁盘录像机）终于作为 **multimedia/vdr** 包含在 portstree 中，并提供了 17 个 vdr 插件 Port。
...
**联系方式**：FreeBSD Chromium 团队 <[chromium@FreeBSD.org](mailto:chromium@FreeBSD.org)>

感谢 FreeBSD 社区、OpenBSD 社区和 Chromium 开发者的出色协作，Chromium 在 Ports 中更新。
...
我们很高兴宣布，FreeBSD Haskell 团队将 GHC 更新到 7.0.3，并将所有现有的 Haskell Port 更新到最新的稳定版本，并添加了新 Port。FreeBSD 仓库中的 Haskell Port 总数超过 200 个。当前这些 Port 仍在等待提交。它们暂时可以通过 [FreeBSD Haskell Ports 仓库](https://github.com/freebsd-haskell/freebsd-haskell) 获取。任何希望尽早访问这些 Port 的用户，请参考 [FreeBSD Haskell Ports 测试请求](http://www.haskell.org/pipermail/freebsd-haskell/2011-April/000278.html)。
...
**链接**：[KDE-FreeBSD](http://freebsd.kde.org/)
...
**链接**：[如何：为 FreeBSD linuxulator 创建你自己的更新版 linux RPM](http://www.leidinger.net/blog/2011/02/25/howto-creating-your-own-updated-linux-rpm-for-the-freebsd-linuxulator/)

**联系方式**：Alexander Leidinger <[netchild@FreeBSD.org](mailto:netchild@FreeBSD.org)>

**联系方式**：仿真邮件列表 <[freebsd-emulation@FreeBSD.org](mailto:freebsd-emulation@FreeBSD.org)>

旧版的 Port linux_base（所有默认未在某些发布版中使用的 Port）已标记为弃用，且有较短的过期时间。原因是这些 Port 过了生命周期结束的时间，并且不再接收安全更新。不幸的是，这对于仍然在发布版中默认使用的 Port linux_base 也适用，但目前没有替代方案（见待办任务）。

Port linux-f10-pango 已更新到一个更近期的版本，该版本通过在虚拟机中生成一个以“FreeBSD”作为厂商的 linux-RPM 来解决了安全问题（参见链接部分的 HOWTO）。

#### 待办事项

1. 决定接下来为 Port linux_base 追踪哪个基于 RPM 的 Linux 发行版及其版本，创建 Port 并测试与我们的内核代码的兼容性。
...
- 新参数 `--update-if-newer`，接受 Port 列表或命令行上的通配符模式，只更新那些过时的 Port。此功能非常有用，可以确保系统更新所需的软件包在软件包构建系统中都可用并且是最新的。
- portmaster.rc 文件现在可以与脚本本身存储在同一目录中，这有助于共享访问脚本（例如，通过 NFS 挂载）
- 更多功能现在与 `--index-only` 一起工作（或工作得更好），包括 `--check-depends`

#### 待办事项

1. 我收到了关于上述网页中 E.2 和 E.3 项目的支持，因此我将在接下来的几个月中投入更多精力到这些领域。我还计划将“fetch”代码拆分为独立的脚本，部分支持目标 E.2，并在下载多个 distfile 时实现更高效的并行化（尤其是对于下载相同 distfile 的多个 Port）。这也将允许我设置并行抓取的全局限制，这应该有助于慢速链接的用户。
...
3. 大多数 Port PR 分配完成，现在我们需要专注于测试、提交和关闭。
...
**联系方式**：Philip Gollucci <pgollucci@FreeBSD.org>

**联系方式**：Olli Hauer <ohauer@FreeBSD.org>

**联系方式**：Apache Apache <apache@FreeBSD.org>

完成了 95%，待最终 -exp 测试运行，并切换。预告通知已发送至相关邮件列表。这将适用于 8.3/9.0。
...
我们的演讲名单确定，日程安排几乎完成。现在还有时间加入进行中的工作会话。
...
**联系方式**：Zheng Liu <lz@FreeBSD.org>

我在 ext2fs 中实现了 reallocblks，就像在 ffs 中一样，并向邮件列表提交了补丁文件。接下来，我将尝试在 ext2fs 中实现 htree 目录索引。
...
FreeBSD 很荣幸第七次参加谷歌编程之夏 2011。我们于 4 月 25 日星期一接受了 17 个提案，整体质量非常优秀。可以在谷歌编程之夏网站找到所有被接受提案的完整列表。我们期待在夏季与这些学生共同工作。
