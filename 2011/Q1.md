...
将来自 www 树的翻译结果合并到单独的仓库进行翻译工作并同步到主树也已完成。由于过时或未翻译的文档仍然保留在 doc/ja_JP.eucJP 和 www 中，进一步的翻译工作仍然是必要的。在此期间，Porter's Handbook 的翻译也有所进展。
...
- 新参数 `--update-if-newer`，接受 Port 列表或命令行上的通配符模式，只更新那些过时的 Port。此功能非常有用，可以确保系统更新所需的包在包构建系统中都可用并且是最新的。
...
FreeBSD 很荣幸第七次参加 2011 谷歌编程之夏。我们已于 4 月 25 日星期一接受了 17 个提案，整体质量非常优秀。可以在谷歌编程之夏网站找到所有被接受提案的完整列表。我们期待在夏季与这些学生共同工作。
