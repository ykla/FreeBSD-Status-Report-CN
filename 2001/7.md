...
联系方式：Jeroen Ruigrok <[asmodai@FreeBSD.org](mailto:asmodai@FreeBSD.org)>
...
联系方式：Paul Richards <[paul@freebsd-services.co.uk](mailto:paul@freebsd-services.co.uk)>

联系方式：Josef Karthauser <[joe@tao.org.uk](mailto:joe@tao.org.uk)>
...
联系方式：Justin Gibbs <[gibbs@FreeBSD.org](mailto:gibbs@FreeBSD.org)>
...
联系方式：Chris Costello <[chris@FreeBSD.org](mailto:chris@FreeBSD.org)>
...
sparc64 加载器功能已经足够，能够使用现有的 openfirmware 库从 UFS 文件系统启动 ELF 二进制文件，已经修订该库，使其完美适用于 32 位和 64 位架构。接下来将实现对网路启动和模块的支持，随后是更好的 openfirmware 映射策略。
...
联系方式：Sheldon Hearn <[sheldonh@FreeBSD.org](mailto:sheldonh@FreeBSD.org)>
...
该项目的目标是通过 **netgraph(4)** 框架实现 ATM 信令和其他 ATM 协议。这应该为在 FreeBSD 上使用 ATM 提供一款易于扩展的架构。目前，完整的 UNI4.0 栈（除了 LIJ 功能）已经实现，包括 ILMI 和针对 UNI 的 ATM Forum API 的初始版本。还实现了 ATM 上的经典 IP（Classical IP over ATM）。已经为 Fore PCA200E 和 Fore HE-155 卡实现了驱动程序。
...
NGPT 是由 IBM 工程师主导的一个项目，旨在实现 MxN 线程（也称为多个用户线程到一个内核线程映射）在 Linux 上的实现。我已经将其移植到 FreeBSD，并使用了 **rfork(2)**。
...
联系方式：Arun Sharma <[arun@sharmas.dhs.org](mailto:arun@sharmas.dhs.org)>

联系方式：eGroups: ORP <[orp@egroups.com](mailto:orp@egroups.com)>
...
在功能方面，强制访问控制（MAC）、对象标签和改进内核访问控制机制的一致性的工作仍在继续，尤其是在进程间授权和凭证管理方面。现在已开始将 NAI Labs 的低水位强制访问控制（LOMAC）方案从 Linux 移植到 FreeBSD，并且已重新授权为 BSD 许可证。我们希望能够在今年晚些时候的 5.0-RELEASE 版本之前，完成初步移植。
