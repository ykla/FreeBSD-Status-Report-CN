...
FreeBSD 项目在 2001 年 8 月取得了显著进展，既包括继续开发 RELENG_4 系列（4.x-STABLE 和 4.x-RELEASE），也包括在 5.0-CURRENT 主开发分支上的进展。在本月，决定将 5.0-CURRENT 的发布推迟，以便 KSE（对细粒度用户线程的支持）能够及时完成，而不是将该支持推迟到 6.0 版本。因此，RELENG_4 系列的生命周期将被延长，并将继续将新特性回溯到该分支。本月 4.4-RELEASE 进入最终测试版，且将很快发布。

本月的状态报告由 Nik Clayton 和 Chris Costello 协助撰写。

## 后续提交

下个月的提交程序保持不变：报告应在一到两段之间，通过电子邮件提交，格式大致应与本月的提交格式（项目、联系人、URL 和正文）相同。提醒将在截止日期前至少一周通过邮件发送到hackers@FreeBSD.org和developers@FreeBSD.org邮件列表；完整的提交说明将在提醒邮件中提供。
...
aac 驱动程序最近得到了诸多关注，现在已经几乎完成了所有功能。更改包括崩溃转储支持、正确处理控制器发起的命令，以及更完整的管理接口支持。来自 Dell 和 HP 的 Linux RAID 管理工具现在完全可用；FreeBSD 原生版本的该工具也在开发中。这些更改已提交到-current，并将在 4.4 发布后出现在 -stable 中。



## [ARM 移植](https://www.freebsd.org/status/report-2001-08.html#ARM-port)
**链接**
...
该项目目前处于无限期暂停状态。我们与 NVIDIA 的联系人似乎对该项目很有热情，并且回复电子邮件也很迅速，但当我们发现一些问题，无法在不更改二进制组件或需要解码的错误代码的情况下进行移植时，Nick（联系人）表示他会调查一下，但之后再也没有回复。第一个主要问题是 ioctl 接口，NVIDIA 驱动程序传递一个指针并依赖内核侧复制正确的数据量，而 FreeBSD 则期望参数正确，且复制操作由子系统执行。通过使用 Dave Rufinos 的“ioctl 隧道”方法解决了这个问题。之后，我们发现 X 拒绝加载，并追踪到一个在二进制组件中定义的 ioctl 发生错误。我们无法知道这个 ioctl 是什么，被告知无法签署该组件源代码的 NDA，并且已经等了一个月，Nick 仍然没有“调查”。因此，在不违反许可证的情况下，不可能有进展，我们认为这些缺陷使得该驱动程序无法移植到除 Linux 之外的任何 `* nix` 系统。
...
核心秘书职位由 Alan Clegg <abc@FreeBSD.org> 填补。第一份核心秘书报告将在 9 月的第二周发布，并将涵盖核心小组在 2001 年 8 月讨论的问题。
...
许多公司捐赠了设备、网络访问或资金来资助这些活动，包括 Compaq、Yahoo!、Wind River Systems 等。



## [FreeBSD/sparc64 移植](https://www.freebsd.org/status/report-2001-08.html#FreeBSD/sparc64-port)
...
## [ia64 移植](https://www.freebsd.org/status/report-2001-08.html#ia64-Port)
...
2001 年 3 月中旬，Guardent 的 Tim Newsham 发现 FreeBSD（以及其他操作系统）初始序列号生成方案可能遭受攻击。为了防范这一威胁，一个来自 OpenBSD 的随机化序列号生成方案已被移植并包含在 4.3-release 中。不幸的是，非单调生成方式被发现会导致一些应用程序在启动大量快速连接到单一主机时出现严重问题。
...
里程碑 2 意味着内核源代码在资源的使用上已经一致地按照每线程和每进程的资源来进行引用，就像当每个进程有多个线程时需要的那样。但关于调度程序、fork 和 exit 等的逻辑更改尚未进行，以允许创建多个线程（也没有添加新的线程系统调用）。这是一个重要的里程碑，因为它代表了内核中仅包含“机械性”变化的最后时刻。要继续前进，我们必须开始添加新的算法和功能。
...
这是一个实验性工作，强烈鼓励并欢迎对 API 和功能提出建议。尚不清楚是否能够（或是否有必要）提供与通过套接字接口执行的操作完全相同的操作集。
...
## [OLDCARD 改进](https://www.freebsd.org/status/report-2001-08.html#OLDCARD-improvements)

联系人: Warner Losh <[imp@FreeBSD.org](mailto:imp@FreeBSD.org)>

OLDCARD 的改进已经完成，除了几个老款笔记本（使用 CL-PD6729/30 芯片）和一些 PCI BIOS 问题的边缘案例。虽然仍会进行一些小的工作，但在 4.4R 发布后，只有少数剩余的 bug 会被修复，然后作者将转向 NEWCARD 开发的更广阔领域。
...
可在上述 URL 上找到 RELNOTESng 的 CURRENT 和 4-STABLE 版本的快照（包括 HTML、文本和 PDF 格式），并不定期但频繁更新。Dima Dorfman <dd@FreeBSD.org> 和 Nik Clayton <nik@FreeBSD.org> 一直在努力使 FreeBSD 官方网站自动生成快照。
...
能力（Capabilities）支持正在提交到 FreeBSD 的基本系统源码中——用户空间库已完全提交，内核基本设施正在进行集成。
