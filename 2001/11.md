...
已从 `-current` 合并 VLAN 克隆的支持，并将与 4.5-RELEASE 一起发布。此外，还增加了在启动时克隆接口的 `rc.conf` 支持。正在继续进行工作，旨在对 `stf` 和 `faith` 克隆进行 `MFC`，并为 `ppp` 设备添加克隆功能，同时增强 VLAN 模块化。
