...
Ben Smithurst 编写了一个名为“diskcheckd”的守护进程，该进程可以在设定的时间内读取磁盘上的所有扇区。随着磁盘容量的不断增加，在发生致命故障之前未发现磁盘读取错误的可能性也在增加。希望该守护进程能促使磁盘固件在坏扇区变得不可读之前重新分配它们。目前，该代码已提交至 5.0-CURRENT 分支。
