...
- 原文：[FreeBSD Status Report September 2001](https://www.freebsd.org/status/report-2001-09.html)
- 最后作者：FreeBSD 项目

## 引言

在 9 月，FreeBSD 项目继续投资于长期项目，包括继续进行细粒度 SMP 实现的工作、对内核可调度实体（KSE）的支持，以支持高效线程，以及扩展对现代硬件平台的支持，包括 Intel 新的 IA64 架构、UltraSparc 和 PowerPC。此外，还重点关注了发布过程，包括发布说明基础设施的工作、DVD 发布的支持以及二进制更新工具的工作。

由于 9 月的报告推迟发布，11 月的状态报告也将包括 10 月。在 11 月期间，我们期待着 BSDCon 欧洲大会，这是首个在美国本土以外举办的此类活动。USENIX 会议论文提交截止日期也在 11 月，建议 FreeBSD 用户和开发者向 general 和 FREENIX 主题提交论文。更多信息请参见 <https://www.usenix.org>。
...
该项目有邮件列表：`freebsd-binup@FreeBSD.org`，并且源代码树已移入 FreeBSD CVS 仓库的目录 **projects/**。当前的工作重点是扩展 FreeBSD 的软件包框架，客户端库应该在年底之前重写并完成。
...
由于同步缓存代码的提交滞后，该项目的开发进度放缓，因为这部分工作依赖于该代码。
...
两张光盘的版本已经制作完成，送去压制。这次发布有一些惊喜，详细信息将在官方公告中给出（在 BSDConEurope 上）。
...
构建过程现在会创建四个不同版本的库，其中包括对 TVision、Qt 的支持，或两者，或者都不支持。我已经从现有的 Port 创建了一些初始软件包，并且仅使用 libh 的工具安装了这些 libh 软件包，包括在软件包数据库中注册所有文件，记录它们的校验和等。对磁盘编辑器的补丁已经提交，包括写入 fdisk 部分更改的功能，并初步支持磁盘标签编辑器。我们很快就会有一位新的提交者。
...
我很高兴地宣布，自 10 月 19 日星期五凌晨 1 点起，sparc64 平台已成功启动到单用户模式。基本系统的几个二进制文件已被构建并验证可正常工作。许多这方面的工作仍在审查中，但会尽快集成到 CVS 树中。从 NetBSD 移植了 EBus 支持，编写了 ISA 支持。PCI 主桥代码已稳定，busdma 现在也能正常工作。sio 驱动程序添加了 EBus 支持，修改了 ATA 驱动程序，使其能够在大端系统上工作，并使用 busdma API。通过这些更改，现在可以从 sparc64 上的 ATA 磁盘成功挂载根文件系统，即使是在 DMA 模式下。从 NetBSD 移植的 gem 驱动程序支持 Sun GEM 和 ERI 以及 Apple GMAC 和 GMAC2 以太网适配器，但尚未经过充分的测试。
...
好了，伙计们，我昨晚终于动起来，整理了迁移到新的 **rc.d** 初始化脚本的路线图，这些脚本很久以前就从 NetBSD 导入了，但一直放在源代码树中。
...
希望有几个人能看看这个，然后如果没有太多反对意见，我将提交一个 PR。我预计里程碑 2 会引发不少争论，但嘿，我已经从清洁工那拿回了我那光亮的石棉衣。

## Intel Gigabit 驱动

**联系方式**：Jonathan Lemon <[jlemon@FreeBSD.org](mailto:jlemon@FreeBSD.org)>

终于把 gx 驱动程序提交到了源代码树中。该驱动程序支持 Intel PRO/1000 卡，包括光纤和铜缆两种变体。驱动程序支持 VLAN 标签和 TCP/IP 校验和卸载。

## Intel 千兆以太网

**联系方式**：Matthew Jacob <[mjacob@FreeBSD.org](mailto:mjacob@FreeBSD.org)>

这个驱动程序进行了大量的清理，修复了错误并进行了一些性能增强。然而，该驱动程序可能很快会被移除，并由 Intel 自己的驱动程序替换。
...
上个月，除了对 8 月份的大规模提交进行整理外，其他进展不大。主要是因为我在工作中的工作量突增，以及需要把时间花在其他地方。然而，设计工作还是在进行中。API 已经逐渐稳定下来，几个人正在阅读已经完成的部分，以便能在下一个阶段提供帮助。

里程碑 3 将实现每个进程生成和移除多个线程/KSE 的能力。里程碑 3 不要求这样做是安全的（尤其是在 SMP 系统中），即锁定问题将不会完全解决，所以虽然可以进行一些测试，但在此模式下无法承载任何负载。
...
我们确定是否达到里程碑 3 的标准是：在一个无负载的系统上，简单的进程能够可靠地执行一系列阻塞系统调用。例如：打开 2 个套接字，在其中一个上发送数据，读取另一个后，再按相同方式‘响应’。
...
代码已完成，只需做些最后的打磨以清理一些粗糙的部分。控制台输出可以同时发送到 `syscons` 和 `sio`（以及网络），在 ddb 中时，可以从任何输入源获取输入。一个小的控制程序能动态地添加和移除控制台。

## Netgraph ATM

**联系方式**：Harti Brandt <[brandt@fokus.gmd.de](mailto:brandt@fokus.gmd.de)>

ATM-Forum LAN 仿真版本 2.0（不支持 QoS）已经实现并测试。ILMI 守护进程已被模块化为一个通用的 `mini-SNMP` 守护进程、ILMI 模块和尚未完成的 IPOA（IP over ATM）模块。

## 网络控制台

**联系方式**：Jonathan Lemon <[jlemon@FreeBSD.org](mailto:jlemon@FreeBSD.org)>

该项目的目标是为 FreeBSD 添加底层网络功能。初步目标是为远程调试提供网络控制台，支持 ddb 或 gdb。次要目标是利用此代码执行网络崩溃转储。设计假设网卡和驱动程序正常工作，但不依赖于内核的其他部分。

初步开发进展相当迅速，并且已经编写了一个最精简的 TCP/IP 堆栈。目前，可以通过 telnet 连接到处于 `ddb>` 提示符下的机器，并与调试器进行交互。
...
此外，更新了 ifconfig，现在能在配置设备时接受别名。

## 网络 SMP 锁定

**联系方式**：Jonathan Lemon <[jlemon@FreeBSD.org](mailto:jlemon@FreeBSD.org)>

本月进展不大，其他项目占用了大部分时间。然而，审查所有代码和数据结构有个意外的好处：为 `inet` 地址添加了哈希表。这将显著加快在接口别名众多的情况下对接口地址的查找速度。
...
## 将 ppp 移植到 hurd & linux

**联系方式**：Brian Somers <[brian@freebsd-services.com](mailto:brian@freebsd-services.com)>

自上月以来，状态没有变化。已经提交了补丁，以使 ppp 能在 Hurd 下运行，并在大部分 Linux 下运行。GPL 版权问题仍待解决。预计在提交 ppp IPv6 支持后会出现许多冲突。

## PowerPC 平台

**联系方式**：Benno Rice <[benno@FreeBSD.org](mailto:benno@FreeBSD.org)>

在本月的 PowerPC 移植工作中，取得了一些重要进展。Mark Peek 成功地在 FreeBSD 上交叉编译了 FreeBSD/PowerPC 内核，并使其在 PSIM 模拟器（现在在 **/usr/ports/emulators/psim-freebsd**）下启动。我也成功地使 FreeBSD 引导加载程序能够使用 Apple Macintosh 硬件上的 OpenFirmware 加载和执行内核。Mark 现在正在完成一些启动和 `pmap` 代码，而我则利用模拟器来处理一些中断和设备问题。

## PPP IPv6 支持

**联系方式**：Brian Somers <[brian@freebsd-services.com](mailto:brian@freebsd-services.com)>

该软件已经提交到 `-current`，并且似乎功能是正常的。待解决的问题包括处理 IPV6CP 事件（`linkup` 和 `linkdown` 脚本）以及分配站点本地和全局地址（目前，`iface add` 是唯一实际使用链路的方式）。在 `-STABLE` 中有个 bug（运行尚未 MFC 的 ppp 代码），即路由条目会在一段时间后消失（大约 12 到 24 小时）。目前没有更多详细信息。
...
FreeBSD 4.4-RELEASE 是 FreeBSD 发布的第一版新的发布文档格式。我们为这些文档创建了英文和日文版本。现在可以在网站上找到 `-CURRENT` 和 `4-STABLE` 发布文档的定期构建快照，但它们需要一些 HTML 基础设施来使其更易于查看。我打算继续更新我的快照站点，至少暂时如此。
...
本月在进程锁定方面取得了一些进展。此外，还定义了一个新的 `LOCK_DEBUG` 宏，来提高某些锁定基础设施的效率。现在，内核仅在需要时包含调用互斥锁（`mutex`）、共享/独占锁（`sx`）或信号量锁（`semaphore`）操作所在文件的文件名。如果启用了任何调试选项，互斥锁操作也不再内联。`ucred` API 也进行了重构，可更好地支持锁定。一个小组还开始研究 `tty` 子系统，设计并可能实现一个锁定策略。
