...
在 FreeBSD 5.1 发布之后，我们达到了两个里程碑。第一个是能够支持 Intel 和 HP 的机器，且源代码已经放入 CVS。这要归功于全新的串口（UART）驱动程序。不幸的是，这仍然意味着 syscons 尚未配置。这是另一个待办任务，但可以关注 KGI/FreeBSD 项目……第二个里程碑是完成了 KSE 支持。M:N 和 1:1 线程在 ia64 上均已正常工作，旧的 libc_r 库已被废弃。测试表明，KSE（即 M:N）可能会成为默认的线程模型，发展前景良好。
...
我们希望最终能将其集成到基础系统中。在此之前，我们需要解决与 tcpdump 和 kame 的一些问题。
...
4.9 版本即将发布，提供了 x86 的物理地址扩展（PAE），以及 4-STABLE 系列所期望的世界级稳定性和性能。像往常一样，别忘了从您喜爱的 FreeBSD 供应商那里购买 CD 套件。
...
**链接**
 ...
 [www.wifibsd.org](http://www.wifibsd.org/ "http://www.wifibsd.org") 
