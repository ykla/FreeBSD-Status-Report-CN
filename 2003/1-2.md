...
已锁定的子系统包括 IP、UDP、TCP、ifaddr 引用计数、syncache、ifnet 列表、路由基数树和 ARP。这些已被提交到树中。此外，原始 IP、转发套接字处理和 Unix 域套接字的 SMP 锁定也已完成并经过测试。目前正在一些子系统中进行工作，以使并行网络处理在 SMP 系统上安全。
