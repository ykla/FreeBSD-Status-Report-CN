...
2. 我们所赞助的项目并未在项目官网上提及。我们在自己的网站上记录这些项目，因为我们希望展示捐赠者的资金是如何被使用的。我们意识到还需要在 FreeBSD.org 上添加这些项目的文档，因此我们将调查如何更好地将我们的软件开发工作与 FreeBSD 项目关联起来。
...
- 基金会技术团队的其他成员也为 FreeBSD 开发做出了贡献。例如：
  ...
  - 許立文确保了我们的 Jenkins ports 跟踪最新的上游版本，并进行了一些 Port 的更新。
...
FreeBSD 测试套件通过 [kyua(1)](https://man.freebsd.org/cgi/man.cgi?query=kyua&sektion=1&format=html) 工具执行。Kyua 支持并行执行测试，使用 `kyua -v parallelism=<n> test` 命令。然而，许多网络测试利用了 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 特性，如 [VNET(9)](https://man.freebsd.org/cgi/man.cgi?query=VNET&sektion=9&format=html)，并且在 jail 命名和网络配置上相互冲突。因此，这些测试被标记为 `is_exclusive=true` 元数据属性，以防它们同时运行并相互干扰。这造成了一个困境：当一个项目旨在增加测试覆盖率时，独占测试的增加会成比例地增加执行时间，从而在多个方面影响开发过程。
