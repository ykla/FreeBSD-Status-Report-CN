...
以下是 2024 年的第四份状态报告亦是最后一份，共包含 44 份条目。  
...
回到财务状况方面，我们将在 2024 年第一季度发布财务文件和报告。将在 1 月底发布 2024 年第一至第三季度的更新财务报告，以更好地匹配预算格式。将在 2024 年第二季度初发布最终的 2024 年财务报告。将来，我们的预算和财务报告会更加详细地说明资金如何分配给主要的软件开发项目。例如，我们会包括每季度在笔记本电脑项目上的支出。我们正在与会计师合作，以改进我们的会计系统，使资金使用情况更加透明。  

我们对 2025 年及 FreeBSD 后续的发展机遇感到兴奋，我们正在扩展团队，以支持抓住这些机遇所需的工作。然而，我们需要你的帮助来维持这一切。我们的投资最多只能支撑一到两年。如果你的公司希望 FreeBSD 保持长期可持续发展，请考虑提供资金支持，确保 FreeBSD 继续成为你所依赖的安全、可靠和创新的平台。不知道该如何申请资助？请联系我们，我们可以帮助你完成流程。  
...
在上个季度，pkgmgr@ 团队进行了 14 次实验性构建（exp-run），以测试各类 Ports 升级及对 `bsd.port.mk` 的改动。
...
FreeBSD 音频栈是个并未像其他领域那样吸引大量关注和开发的领域，因为它在很大程度上欠缺维护，尽管其质量较高，但仍有改进空间——从缺乏音频开发框架，到缺失的用户空间工具，以及与内核驱动相关的 bug。这个项目旨在触及所有这些领域，因此，它更像是一个通用的改进项目，而不是一个特定功能的实现。
...
目前，AMD 上的所有设备电源约束已经可以解析，系统能进入低功耗状态。
...
此协议的首个实现是在 2016 年为 OpenBSD 完成的，命名为 umb(4)。我本人在 2019 年将其移植到 NetBSD，并且之后被雇佣使其与 OPNSense 兼容，并获准在 2022 年将其作为开源发布。不幸的是，到了这个时候，FreeBSD 中的一些更改实际上使驱动程序无法使用，直到修复才能合并。
...
目前有多个无线项目正在进行，本报告重点介绍在 FreeBSD 上尽可能直接使用宽松许可的 Linux 无线驱动的工作。
...
已经移植了驱动程序 [rtw89(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw89&sektion=4&format=html) ，它进入源代码中有一段时间了，并已成功连接到构建中。感谢社区的努力，之前妨碍其工作的两个 bug 已被解决。
...
目前，OpenBSD 上的 iwx 驱动支持以站点（station）模式运行 80211abgn 和 ac，但尚不支持 ax 速率。该项目的目标是从 OpenBSD 引入一款可维护的驱动程序，并逐步增加支持，直到我们在 FreeBSD 中拥有一个原生驱动，支持 80211ac（并可能支持 80211ax）。
...
如果你维护着任何受影响的 Port ，或想帮助准备和测试一些补丁，可以考虑尝试在受影响的 Port 中将 `-fpermissive` 添加到 `CFLAGS` 作为临时解决方案：GCC 14 将一些警告转化为错误，这是导致许多构建失败的原因。参数 `-fpermissive` 会将这些错误转换回警告。然而，更好的方法是上游更新其代码，以完全消除这些警告，从而不再需要 `-fpermissive`，可能需要 FreeBSD Ports 维护人员的支持。如果上游不再维护代码，可能是时候弃用该 Port 了。
...
2024年11月16日，LDWG 举行了首次虚拟会议。来自开发者、贡献者、Discord 社区管理员、用户和 FreeBSD 基金会成员的多样化参与者表明 FreeBSD 在笔记本电脑和台式机上的广泛兴趣。会议幻灯片、会议纪要和录音可在工作组的 [维基页面](https://wiki.freebsd.org/LaptopDesktopWorkingGroup) 查阅。
...
工作组正在进行一项[在线调查](https://docs.google.com/forms/d/e/1FAIpQLSd6-AOHpnae4o40CSr0tt6GlmfgfP7A9REdulJmPw9Nn0Uo_w/viewform?usp=sf_link)，以收集非参与者的反馈。调查将持续到1月的下次电话会议，届时将呈现并讨论结果。
...
Pot 是一款 jail 管理工具，[还支持通过 Nomad 进行编排](https://www.freebsd.org/news/status/report-2020-01-2020-03/#pot-and-the-nomad-pot-driver)。Potluck 旨在成为 FreeBSD 和 Pot 的 Dockerhub，就像 Dockerhub 对 Linux 和 Docker 的作用一样：一个 Pot 风味和完整容器镜像的仓库，供 Pot 使用，在许多情况下也可以与 Nomad 一起使用。
