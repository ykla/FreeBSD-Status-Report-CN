...
基金会技术团队的其他成员也对 FreeBSD 的开发工作做出了贡献。例如，Mark Johnston 与 Andrew Turner 一起编写了构建 arm64 bhyve 客户机的扁平化设备树（FDT）的基本例程。FDT 描述了客户机操作系统需要了解的各种硬件组件，如 CPU、内存、UART、PCIe 控制器、中断控制器和平台计时器。Ed Maste 提交了多项源代码贡献，包括现代化 [tzsetup(8)](https://man.freebsd.org/cgi/man.cgi?query=tzsetup&sektion=8&format=html) 并修正了 [diff(1)](https://man.freebsd.org/cgi/man.cgi?query=diff&sektion=1&format=html) 选项的问题。Li-Wen Hsu 和 Joe Mingrone 在平衡日常职责的同时，也为多个 ports 提供了更新和修复，其中包括处理紧急的安全问题。
...
作为我们不懈改进 FreeBSD 项目的一部分，基金会支持一名全职员工专门负责改进项目的持续集成系统和测试基础设施。

#### 合作与研究

在第二季度，基金会合作负责人 Greg Wallace 参加了在西雅图举行的开源峰会活动。在那里，他与 Doug Rabson 一起参加了关于 FreeBSD OCI Runtime Extension 工作组工作的演讲。[你可以在这里查看](https://youtu.be/pggcc6fi-ow?si=0veLZJ5J4rXCWAHY)。Greg 还利用此次活动与多家重要科技公司建立联系，以推动重大联合技术项目。[Greg 对这次活动的撰文在此](https://www.freebsd.org/status/report-2024-04-2024-06/)。

其他高度需求的功能工作也在继续。RG Nets 和其他公司在将 CUDA 及相关 AI 栈组件引入 FreeBSD 方面取得了很大进展。基金会正在寻求与 FreeBSD 用户协调的方法，以支持在 FreeBSD 上提供多种 AI 技术的支持。一个想法是推出一个 FreeBSD AI 实验室，从支持者那里筹集资金，以全面支持 FreeBSD 上的 CUDA 并完善 DPU 驱动程序支持。[如果你想支持这样的倡议，请联系我们](mailto:partnerships@freebsdfoundation.org)。

我们还在继续利用 FreeBSD 社区的英雄工作，使 .NET 能够在 FreeBSD 上得到支持，从而使其下游依赖项能够更好地支持 FreeBSD。关于这方面的更多信息即将发布。

感谢 Alpha-Omega 的慷慨资助，FreeBSD 基金会委托 Synactiv 对重要子系统进行了两次代码审计。Alpha-Omega 是一个开源项目，其使命是通过推动对最关键的开源软件项目和生态系统的可持续安全改进来保护社会。[我们最近的每月更新可以在这里找到](https://github.com/khorben/alpha-omega/blob/freebsd/alpha/engagements/2024/FreeBSD/update-2024-06.md)。代码审计将在七月完成，然后我们将进行过程审计，并启动一个双重身份验证（2FA）试点项目。

在第一季度和第二季度，Greg 参与了几次关于各种政府法规的会议。今年三月，他代表 FreeBSD 参加了 [CISA 为期两天的开源软件（OSS）安全峰会](https://www.cisa.gov/news-events/news/cisa-announces-new-efforts-help-secure-open-source-ecosystem)，与其他 [开放政策联盟成员](https://opensource.org/programs/open-policy-alliance) 一起参会。此前，Greg 与 OPA 合作提交了关于美国政府如何支持开源安全性和可持续性的意见，并在六月，FreeBSD 基金会加入了 Eclipse 基金会的 [开放监管合规工作组](https://outreach.eclipse.foundation/open-regulatory-compliance)。该工作组旨在加速开发所需的网络安全流程，以实现法规遵从，同时提供一个中立的环境，进行与整个开源社区的技术讨论。

我们非常高兴地欢迎 Alice Sowerby 作为兼职合同合作项目经理加入。Alice 是一位经验丰富的多技能领导者，活跃于多个开源领域。她是 CHAOSS 播客的联合主持人，也是 OSPO 书籍评审团队的 TODO 小组主席。Alice 为合作倡议，如 Alpha-Omega、OCI FreeBSD Runtime Extension WG 和企业工作组提供项目和项目管理。

#### 宣传

在 2024 年第二季度，我们不懈地提升着 FreeBSD 项目的关注度，倡导项目的发展，展示用户，并为 FreeBSD 社区带来教育内容。以下是我们的一些努力成果：
...
我们始终欢迎新的官方镜像加入。我们注意到在全球的互联网交换点托管单一镜像服务器的好处，例如我们在澳大利亚、巴西和南非的现有镜像。如果你与有意愿赞助单一镜像服务器的组织有联系，或知道这样的组织，请联系我们。我们特别关注美国西海岸和整个欧洲的地点。
