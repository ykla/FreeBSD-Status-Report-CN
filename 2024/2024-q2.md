...
根据索引，目前 Ports 中共有 32,471 个 ports。目前约有 3,497 个正在进行的 ports PR。上季度，在主分支上有 160 位提交者进行了 10,525 次提交，在 2024Q2 分支上有 107 位提交者进行了 1,771 次提交。同上一季度相比，对 main 分支的提交数量大幅减少（从 12,991 次下降），对季度分支的反向移植也有所减少（从 888 次下降）。ports 数量则有所增加（32,244）。
