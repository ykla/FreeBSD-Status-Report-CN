...
- [Philip Paeps](mailto:philip@FreeBSD.org) 正按一份每月 20 小时、为期六个月的合同，持续使 FreeBSD 集群现代化。
...
其他的基金会技术团队成员也为 FreeBSD 的开发做出了贡献。例如，Mark Johnston 与 Andrew Turner 一起编写了构建 arm64 bhyve 客户机的扁平化设备树（FDT）的基本例程。FDT 描述了客户机操作系统需要了解的各种硬件组件，如 CPU、内存、串口、PCIe 控制器、中断控制器和平台计时器。Ed Maste 贡献了多项源代码修改，包括现代化 [tzsetup(8)](https://man.freebsd.org/cgi/man.cgi?query=tzsetup&sektion=8&format=html) ，修正了 [diff(1)](https://man.freebsd.org/cgi/man.cgi?query=diff&sektion=1&format=html) 选项的问题。許立文和 Joe Mingrone 在兼顾日常职责之余，也为多个 port 提供了更新和修复，处理了紧急安全问题。
...
根据索引，目前 Ports 中共有 32,471 个 port。目前约有 3,497 个正在进行的 ports PR。上季度，在主分支上有 160 位提交者进行了 10,525 次提交，在 2024Q2 分支上有 107 位提交者进行了 1,771 次提交。同上一季度相比，对 main 分支的提交数量大幅减少（12,991），对季度分支的反向移植也有所减少（888 次）。ports 数量则有所增加（32,244）。
...
待一切调整好，我们希望能通过宣传来感召贡献者，至少在基本系统上，要使用 GitHub PR。我们需要更多开发人员来审查 FreeBSD 的 GitHub PR。待自动化成熟并全面展开，我们还将需要更多开发人员来审查和合并 PR。我们真切希望以这一努力，改善 FreeBSD 的贡献体验，并从社区收到有用的修复。
...
可以使用六个新属性来配置限制：`limit_bw_read`、`limit_bw_write`、`limit_bw_total`、`limit_op_read`、`limit_op_write`、`limit_op_total` `limit_bw_*` 属性分别限制数据集及其子数据集可以消耗的读取、写入或总带宽。这些限制以 B/s 为单位。`limit_op_*` 属性限制数据集及其子数据集可以生成的读取、写入或总的元数据操作数量。这些限制以每秒运算次数（operations per second，OPS）为单位。
...
与之前报告中的计划相反，本季度在现有 GCC Port 的错误修复工作不怎么多。这是因为，我发现上游对在 FreeBSD 上支持 GCC 的兴趣有所增加，但 GCC 的源代码在我们的平台上仍无法开箱即用，需大量补丁，这些补丁在我们的 Ports 中。因此，有必要集中精力将所有这些补丁推送到上游，最好一次性进行，而不是像之前那样逐个处理，再定期运行测试套件，来修复所有失败的测试。同时，我正在开发一个新的自动化设置，以尽可能高效地构建和测试所有受支持的 GCC 版本，包括从源代码（不使用 Port）构建 GCC。所有这些都显著减缓了以往的 GCC Port 维护（-devel Port 的用户可能已经注意到更新频率降低），但我相信，这最终会带来回报。
...
FreeBSD 项目的 KDE 负责把 CMake、Qt 及 KDE 社区的软件打包到 FreeBSD Ports。这些软件包括了一款完整的桌面环境，即 KDE Plasma（支持 X11 和 Wayland），以及数百个可在 FreeBSD 机器上使用的应用程序。[KDE 团队](mailto:kde@FreeBSD.org) 是 [desktop@](mailto:desktop@FreeBSD.org) 的一部分，构建软件堆栈，使 FreeBSD 成为美观且可用的日常图形桌面工作站。
