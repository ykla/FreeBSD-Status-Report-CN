...
新的一年给我们带来了许多新的有趣项目，比如将系统调用与工作分开的新 libsys ，或者在 FreeBSD 上制作图形安装程序，这将有助于使我们的操作系统更加用户友好。当然，像云初始化、OpenStack 或 GCC ports的常规项目仍在继续。像往常一样，我们的主要团队与我们分享他们的进展。
...
* [ FreeBSD 团队报告](https://www.freebsd.org/status/report-2024-01-2024-03/#FreeBSD-Team-Reports)

  * [ FreeBSD 核心团队](https://www.freebsd.org/status/report-2024-01-2024-03/#_freebsd_core_team)
  * [ FreeBSD 基金会](https://www.freebsd.org/status/report-2024-01-2024-03/#_freebsd_foundation)
  * [FreeBSD 发行工程团队](https://www.freebsd.org/status/report-2024-01-2024-03/#_freebsd_release_engineering_team)
  * [集群管理团队](https://www.freebsd.org/status/report-2024-01-2024-03/#_cluster_administration_team)
  * [ 持续集成](https://www.freebsd.org/status/report-2024-01-2024-03/#_continuous_integration)
  * [ Ports 集合](https://www.freebsd.org/status/report-2024-01-2024-03/#_ports_collection)
* [ 项目](https://www.freebsd.org/status/report-2024-01-2024-03/#projects)

  * [音频堆栈改进](https://www.freebsd.org/status/report-2024-01-2024-03/#_audio_stack_improvements)
  * [ Bhyve 改进](https://www.freebsd.org/status/report-2024-01-2024-03/#_bhyve_improvements)
  * [自由 BSD 的图形化安装程序](https://www.freebsd.org/status/report-2024-01-2024-03/#_graphical_installer_for_freebsd)
* [ 用户空间](https://www.freebsd.org/status/report-2024-01-2024-03/#userland)

  * [libsys](https://www.freebsd.org/status/report-2024-01-2024-03/#_libsys)
  * [FreeBSD pkg 的 PackageKit 后端](https://www.freebsd.org/status/report-2024-01-2024-03/#_packagekit_backend_for_freebsd_pkg)
* [ 内核](https://www.freebsd.org/status/report-2024-01-2024-03/#kernel)

  * [13.3-RELEASE 的 iwlwifi(4) 和无线](https://www.freebsd.org/status/report-2024-01-2024-03/#_iwlwifi4_and_wireless_for_13_3_release)
* [ 结构](https://www.freebsd.org/status/report-2024-01-2024-03/#architectures)

  * [Ten64，WHLE-LS1 和 HoneyComb](https://www.freebsd.org/status/report-2024-01-2024-03/#_ten64_whle_ls1_and_honeycomb)
* [ 云](https://www.freebsd.org/status/report-2024-01-2024-03/#cloud)

  * [在 Microsoft HyperV 和 Azure 上使用 FreeBSD](https://www.freebsd.org/status/report-2024-01-2024-03/#_freebsd_on_microsoft_hyperv_and_azure)
  * [将 FreeBSD 作为一级 cloud-init 平台](https://www.freebsd.org/status/report-2024-01-2024-03/#_freebsd_as_a_tier_1_cloud_init_platform)
  * [ 在 FreeBSD 上使用 OpenStack](https://www.freebsd.org/status/report-2024-01-2024-03/#_openstack_on_freebsd)
* [ 文档](https://www.freebsd.org/status/report-2024-01-2024-03/#documentation)

  * [文档工程团队](https://www.freebsd.org/status/report-2024-01-2024-03/#_documentation_engineering_team)
* [Ports](https://www.freebsd.org/status/report-2024-01-2024-03/#ports)

  * [FreshPorts：新软件包通知](https://www.freebsd.org/status/report-2024-01-2024-03/#_freshports_notification_of_new_packages)
  * [ FreeBSD 上的 GCC](https://www.freebsd.org/status/report-2024-01-2024-03/#_gcc_on_freebsd)
  * [Valgrind：port 到 arm64 的途中](https://www.freebsd.org/status/report-2024-01-2024-03/#_valgrind_port_to_arm64_on_its_way)
* [ 第三方项目](https://www.freebsd.org/status/report-2024-01-2024-03/#third-Party-Projects)

  * [容器和 FreeBSD：Pot、Potluck 和 Potman](https://www.freebsd.org/status/report-2024-01-2024-03/#_containers_and_freebsd_pot_potluck_and_potman)
...
2024 年第一季度期间，180 个源代码、65 个ports和 18 个文档树提交将 FreeBSD 基金会确定为赞助商。
...
最近一个季度，ports树发生了很多事情，以下是主要软件升级的摘要：
...
跨越多个类别的项目，涵盖从内核和用户空间到Ports Collection 或外部项目。
...
链接: bhyve 生产用户调用 URL: https://callfortesting.org FreeBSD Wiki - 企业工作组 URL: https://wiki.freebsd.org/EnterpriseWorkingGroup FreeBSD Wiki - EWG - bhyve 和jails管理工具 URL: https://wiki.freebsd.org/ChrisMoerz/bhyve_management Jan Bramkamp 关于 s6rc 的工作 URL: http://static.bultmann.eu/s6-talk/ GitHub 上的 vmstated URL: https://github.com/christian-moerz/vmstated YouTube - vmstated 解释 URL: https://www.youtube.com/watch?v=f60NCrunXyw

联系人: Chris Moerz < freebsd@ny-central.org>

#### bhyve I/O 性能测量

最近，每周的 bhyve 生产用户电话会议的参与者讨论了 bhyve 的 I/O 性能。提出了各种测量和比较的方法，然而很快就清楚目前没有关于此的正式分析和报告。因此，我们开始了这项工作，希望更好地了解客户机配置选项对其 I/O 性能的各种影响。我们创建了一组shell脚本，利用 FreeBSD 客户机运行基准测试/fio I/O 性能测量，测试不同配置下的性能。这使我们能够比较带宽、延迟、IOPS 等多个标准。
...
去年，FreeBSD 基金会的 Greg Wallace 成立了企业工作小组，其具体目标是解决 FreeBSD 企业用户的痛点。其中形成了一个围绕 bhyve 和jails管理工具的工作小组。在收集了一组所需功能和功能之后，bhyve 的一个主要关键点浮出水面：希望像jails那样为 bhyve 提供配置概念和工具。
...
vmstated 通过 UCL 配置文件进行配置（类似于jails.conf），结合 bhyve_config(5)配置文件，已经提供了配置虚拟机的最高灵活性。vmstatedctl 提供了类似jail命令集，用于启动、停止和检索有关客户机状态的信息。状态转换可以通过shell脚本轻松挂钩，并允许在相关状态变化发生时运行附加命令来进行网络或存储设置和拆卸。

一个初始版本已经在ports上作为 sysutils/vmstated 发布，并且更新正在等待提交；但是，最新版本可以在 GitHub 上找到。我们正在考虑扩展这项工作；我们也想邀请任何感兴趣的人加入我们的工作！补丁、建议、反馈等都非常受欢迎！
...
在实践中，FreeBSD 已经被不同项目衍生为面向桌面的操作系统。其中，我只发现 GhostBSD 作为一个维护的项目，提供了一个图形化的安装系统的过程。这里的目标是考虑一个 FreeBSD 可以采纳为其基本系统的过程，以便像当前的安装程序一样提供一个图形化安装程序。然而，GhostBSD 的安装程序依赖于用 Python 驱动的 Gtk+界面，这意味着在采用 FreeBSD 通常的镜像生成过程时会对安装媒体产生沉重的负担。这还意味着将新项目导入并维护到ports树中。

相反，借助当前的 bsdinstall(8)和 bsdconfig(8)实用程序的知识，我设想了一个符合 BSD 许可证的替代 Xdialog(1)的方案。就像在调用带有-X 开关的 bsdconfig 进行图形模式安装时一样，它可以替换 bsddialog(1)，并允许图形化安装 - 同时共享当前安装程序的基础设施。为了避免与 x11/xdialog port中的 Xdialog 的当前实现混淆，我将其替换命名为 gbsddialog(1)。也必须指出，Xdialog 已经相当过时（最新版本发布于 2006 年），这在视觉上也表现出来。
...
这个项目最重要的目标是改进 bsdinstall，不管这个集成的成功与否。从上面的项目中可以看出，D44279、D44280、D44670 都希望改进安装程序的整体外观和感觉，即使在文本模式下也是如此。同样，D44671 和 D44672 改善了安装程序在脚本化或定制化时的整体多功能性。D44673 和 D44674 使其与 bsdconfig-X 相媲美，甚至允许jails的图形安装。
...
* libsys ABI 尚未被视为稳定（可以安全地假设 __sys_foo() 将受支持，因此语言运行时现在可以使用它）。
...
PackageKit 是一个小的 D-Bus 守护程序，用作“应用商店”类型应用的后端-最显著的是 Plasma Discover 和 Gnome 软件中心。最新的 PackageKit 发布功能了一个 libpkg 后端，这意味着您现在可以在 FreeBSD 上使用 PackageKit 启用的程序来管理软件。Plasma Discover 已经切换到使用 PackageKit，因此一旦更新您的ports/packages，您将可以立即开始使用。
...
* 我还为 OpenBSD 启动了一个官方port，但该工作已经停滞不前。

未来的工作大致如下：

* 完成 FreeBSD VirtIO 套接字驱动程序。
* 修复 Go 的运行时以支持在 FreeBSD 上的 VirtIO。
* 将Port lxd-agent 的依赖项添加到 FreeBSD。
...
展望未来，我们下一季度的重点包括确认实现利用 mac(4)和 priv(9)的 FreeBSD 特权管理用户空间工具的可行性，通过过渡到 FreeBSD ports简化安装步骤，并将 OpenStack Ironic 移植到 FreeBSD。这些任务将增强项目的功能和兼容性。
...
影响Ports Collection 的变化，无论是触及大部分树木的大规模变化，还是单个ports本身。
...
数据库很少变化，与应用程序和操作系统相比，需要的维护很少。网站基本上可以自行运行。偶尔，对 FreeBSD ports基础设施的更改会导致某些问题或需要修改，但很少有任何紧迫性需要修复。这不需要太多时间。有很多学习。虽然 FreshPorts 不是一个复杂的应用程序，但也不是微不足道的。
...
作为前一份报告中承诺的，下一个目标是减少 GCC ports的未解决错误数量。针对现有错误的一些工作已经开始。

特别是，lang/gcc14-devel 由于构建port时没有任何 BOOTSTRAP 选项而长期处于过时状态。在其他开发人员和贡献者的帮助下（特别感谢 Mark Millard），根据官方文档构建 GCC 而无需引导需要一个工作的 GCC 二进制文件，因此我将 lang/gcc14-devel 切换为需要设置 BOOTSTRAP 选项。然而，有人后来指出使用 clang 和 libc++来引导 GCC 是官方支持的。但也有人指出这不是高优先级事项。

目前 lang/gcc14-devel 是唯一需要设置 BOOTSTRAP 选项的 GCC port。计划是要求所有 GCC ports版本为 14 或更高版本（即未来的 GCC ports）也需要这样一个选项：即使构建时没有引导是官方支持的，对上游来说它不是高优先级，这增加了维护 GCC ports的负担来看效果很差。如果较低版本开始在没有引导的情况下构建出现问题，我也将要求为这些版本引导。
...
Pot 是一個jail管理工具，同時還通過 Nomad 支持編篡。Potluck 的目標是成為 FreeBSD 和 Pot 的 Dockerhub：Pot 風格和完整容器映像的存儲庫，供 Pot 使用，並在許多情況下供 Nomad 使用。
