...
一如既往，由于部分重要报告延迟到达，报告的发布速度较慢。对此我们表示歉意，但大多数成员都是志愿者，我们都尽力而为。

幸运的是，志愿者人数似乎即将增加：在上个季度，我注意到许多新社区成员对参与 FreeBSD 项目表现出浓厚兴趣。这无疑将提升整个开发工作的效率，包括状态报告的制作。

许多新加入的成员都会带着同样的问题来找我们：“我该如何开始？”这里不是回答这个问题的地方，但实际上已经有一份指南专门为此编写。你可以在[这里](https://docs.freebsd.org/en/articles/contributing/)找到它。此外，时刻记住：尽可能多地使用 FreeBSD 会大有帮助——把它作为你的日常操作系统，在其上运行你的 Web 服务器，在虚拟机中安装，支持它时贡献现有软件！如果你发现了 bug，也不要忘记[报告](https://bugs.freebsd.org/bugzilla/)它们。
...
来自各官方及半官方团队的条目，来源于[管理页面](https://www.freebsd.org/administration/)。

---

### FreeBSD 核心团队

联系方式：FreeBSD 核心团队 [[core@FreeBSD.org](mailto:core@FreeBSD.org)](%[5Bcore@FreeBSD.org](mailto:5Bcore@FreeBSD.org)%5D(mailto:core@FreeBSD.org))
...
* EuroBSDCon 核心会议
  ...
  * dch 发表了题为“Core Team: Personal Perspective”的演讲。
* 从项目网站移除 X/Twitter 链接

  * 核心团队收到询问，是否可以从项目网站移除 X/Twitter 的链接。
  * 由于这些账号未被使用，核心团队认为移除是可行的。
  * 项目仍需控制这些账号，以防被他人滥用。
  * 如果项目或基金会中有人能够良好管理这些社交账号，链接可以重新添加。
* 基金会提议的隐私友好型网站分析

  * 一个想法是比较 freebsd.org 与 freebsdfoundation.org 的流量情况。
  * 核心团队与文档工程团队合作并已部署。
* 核心团队与 FreeBSD 基金会正在合作发布 2025 年社区调查结果。

#### 正在进行的工作

核心团队目前正在处理以下事项：

* 核心团队正在创建一个工作组，以重组文档工程（doceng）。
* 提交者指导方针/规则

  * 由开发者的咨询提出。
  * 核心团队发现已有部分文档，但应向 doceng、portmgr 和 srcmgr 征求更多意见。
* 核心团队选举与任期变更

  * 根据社区反馈，优选方式是分阶段的核心任期。
  * 一个想法是每年评估核心团队的一半，而不是每两年可能轮换整个核心团队，以免正在进行的工作和经验丢失。
  * 一些开发者对此有强烈意见（例如每年轮换三分之一核心成员，而不是每两年轮换全部核心成员），并希望参与讨论。
  * 为此需要修改章程，标准要求较高。
  * 社区中有人建议仅修改章程，因为核心团队在法律上不代表任何人，这真的是好主意吗？
  * 此议题在 2025 年 BSDCan 开发者峰会的核心会议上讨论过。
  * 核心团队正持续努力，以建立分阶段核心任期。
* 项目连续性

  * 由开发者的咨询提出。
  * 需要（更新的）文档说明如何重建关键系统，以确保 FreeBSD 项目的业务连续性。涵盖三个方面：

    * 项目自身的业务连续性（关键人员或基础设施丢失）
    * FreeBSD 终端用户的复制方案
    * 发布工程的工作流程，以及各团队受影响情况
  * 目前过多知识依赖口传记忆，需要收集、整理并记录。
  * STA 工作的一个成果是发布文档已经撰写（在文档库的 releng 文章旁），但某些边缘情况仍缺失。
* 提交者 GECOS 字段与化名

  * 有开发者提出是否可以使用化名作为提交者名称的问题。
  * 更高的关注点在于如何处理版权问题。
  * 核心团队认为使用化名并不被支持，因为会增加文书工作难度和集群访问恢复难度，同时建议强调使用真实姓名的好处。
  * 化名可能带来的另一个问题是身份共享，例如供应商提交权限情况下，谁将成为发言人？
* AI 政策

  * 核心团队正在起草关于使用 AI 和大型语言模型（LLM）的政策。
  * 此过程中还计划咨询其他团体，例如基金会法律顾问，以及 Linux 和其他开源社区中从事相关工作的人员。
  * 开发者和外部贡献者对版权、代码来源保障、最佳实践和“常识”存在多种问题，并讨论了 AI（辅助）工具可以做什么与不可以做什么。
  * BSDCan 和 EuroBSDCon 后有许多后续讨论，核心团队正在组织并总结这些讨论。
...
FreeBSD 基金会是一个 501(c)(3) 非营利组织，致力于通过技术和非技术支持推动 FreeBSD 发展。基金会完全依靠捐赠资助，支持软件开发、基础设施、安全和协作工作；组织活动和开发者峰会；提供教育资源；并在法律事务上代表 FreeBSD 项目。
...
* [挂起/恢复改进](https://www.freebsd.org/status/report-2025-07-2025-09/#_suspendresume_improvement)
* [LinuxKPI 802.11 和原生无线更新](https://www.freebsd.org/status/report-2025-07-2025-09/#_linuxkpi_802_11_and_native_wireless_update)
* [音频栈改进](https://www.freebsd.org/status/report-2025-07-2025-09/#_audio_stack_improvements)
...
* [USB 内核调试改进](https://www.freebsd.org/status/report-2025-07-2025-09/#_usb_kernel_debugging_improvements)
* [DRM 驱动延迟与卡死修复](https://www.freebsd.org/status/report-2025-07-2025-09/#_drm_drivers_slowdowns_and_freezes_fixes)
...
基金会还继续支持两个重大项目：笔记本支持与可用性项目（与 [Quantum Leap Research](https://www.ql-research.com/) 合作）以及由 [Sovereign Tech Agency](https://www.sovereign.tech/) 委托的[基础设施现代化项目](https://www.freebsd.org/status/report-2025-07-2025-09/#_infrastructure_modernization)。有关这两个项目的背景，请参阅[2025Q1 季度状态报告](https://www.freebsd.org/status/report-2025-01-2025-03/#_freebsd_foundation)。
...
其中四位 GSoC 参与者贡献了报告条目：

* [ACPI Lua 绑定](https://www.freebsd.org/status/report-2025-07-2025-09/#_acpi_lua_bindings)
* [Sockstat 用户界面改进](https://www.freebsd.org/status/report-2025-07-2025-09/#_sockstat_ui_improvements)
* [Geomman 发布](https://www.freebsd.org/status/report-2025-07-2025-09/#_geomman_release)
* [mac_do(4) 与 mdo(1) 改进](https://www.freebsd.org/status/report-2025-07-2025-09/#_mac_do4_and_mdo1_improvements)

#### 宣传工作

2025 年第三季度的宣传工作包括在开源活动中代表 FreeBSD、制作更多技术教程，以及为即将于 2025 年 11 月举行的 FreeBSD Vendor Summit 做准备。以下是基金会在 2025 年第三季度为 FreeBSD 宣传所采取的一些举措：
...
* 发布了 [2025 年 4/5/6 月](https://freebsdfoundation.org/our-work/journal/browser-based-edition/networking-3/) 期 FreeBSD Journal，并提供文章的 HTML 版本。
...
基金会拥有 FreeBSD 商标，并负责保护这些商标。同时为核心团队提供法律支持，以调查相关问题。
...
联系方式：Tobias C. Berner [[portmgr-secretary@FreeBSD.org](mailto:portmgr-secretary@FreeBSD.org)](%[5Bportmgr-secretary@FreeBSD.org](mailto:5Bportmgr-secretary@FreeBSD.org)%5D(mailto:portmgr-secretary@FreeBSD.org))
联系方式：FreeBSD Ports 管理团队 [[portmgr@FreeBSD.org](mailto:portmgr@FreeBSD.org)](%[5Bportmgr@FreeBSD.org](mailto:5Bportmgr@FreeBSD.org)%5D(mailto:portmgr@FreeBSD.org))

Ports 管理团队负责监督 Ports Tree 的整体方向、软件包构建和人员事务。以下是上季度的情况：

上季度，我们欢迎 Älven (alven@) 和 Tiago Gashiba (tiga@) 成为新的 ports 提交者，并告别了六位提交者。同时，在成功完成 lurker 计划后，Dan Langille (dvl@) 晋升为正式 portmgr 成员。

根据 INDEX，目前 Ports Collection 中共有 37,163 个 ports（上季度为 36,605）。当前大约有 3,428 个（上季度为 3,330）开放 ports PR，其中 821 个尚未分配。上季度，main 分支由 156 位（上季度 157 位）提交者完成 8,738 个（上季度 10,924）提交；2025Q3 分支由 61 位（上季度 56 位）提交者完成 898 个（上季度 770）提交。
...
在过去三个月中，Ports Tree 发生了大量变化，主要软件升级包括：
...
在上季度，pkgmgr@ 运行了 12 次实验性构建，以测试源代码更改和各类 ports 升级。

---

## 项目

涵盖多个类别的项目，从内核与用户空间到 Ports Collection 或外部项目。
...
[跟踪票：Framework Laptop 功能支持、缺陷和改进](https://bugs.freebsd.org/262152) URL: [https://bugs.freebsd.org/262152](https://bugs.freebsd.org/262152)
...
基金会继续改进整体笔记本支持，Framework 笔记本是[笔记本支持与可用性项目](https://github.com/FreeBSDFoundation/proj-laptop/)的[目标平台](https://github.com/FreeBSDFoundation/proj-laptop/blob/main/supported/laptops.md)之一。

2025 年第二和第三季度，在 Framework 台北办公室举办了两次黑客马拉松，以测试新发布和开发中的产品上的 FreeBSD。

* 4 月：继续测试 Framework Laptop 12 和 Framework Desktop 对开发中的 15.0 支持，包括以太网支持、串行控制台访问等。
* 9 月：测试 Framework Laptop 16 AMD Ryzen AI 300 系列，包括 NVIDIA 图形模块。ShengYi [修复了该型号的声音支持](https://cgit.freebsd.org/src/commit/?id=7f81b2519aebcf90d7e027122ca99b628ca81ed9)。
...
* Li-Wen 和 ShengYi 的工作由 FreeBSD 基金会赞助
* Daniel 的工作由 Framework Computer Inc. 赞助，包括硬件和场地支持
...
该项目于 2024 年第三季度启动，由 Sovereign Tech Agency 委托，预算为 745,000 美元，使用期限至 2025 年底。主要目标是改进基础系统、Ports 和软件包的安全工具，更新项目基础设施以加快开发速度，增强构建安全性，并让新开发者更容易上手。
...
截至 2025 年 9 月，该工作包已完成。项目与 FreeBSD 项目源代码管理团队的建立同步进行，团队创建并嵌入了新的流程，使 Bug 管理更容易、更可持续。委托的 Bug 积压仪表板仍可使用，有助于更清晰地理解积压情况。

8 月，在 [Open Source Summit Europe](https://osseu2025.sched.com/event/25VsE/from-backlog-to-breakthrough-how-freebsd-and-bitergia-tackled-7k+-bugs-with-data-driven-dashboards-alice-sowerby-freebsd-foundation-contractor-daniel-izquierdo-cortazar-miguel-angel-fernandez-sanchez-bitergia-moin-rahman-freebsd-project) 举办专题讨论，将该工作向更广泛的观众分享。基金会两名项目人员（Alice Sowerby 和 Moin Rahman）参与讨论，与两位来自 Bitergia 的代表共同介绍了 GrimoireLab 的实现。（FreeBSD 项目源代码管理团队成员未能参加。）

基金会将持续与源代码管理团队定期沟通，至少直到 2025 年底，以确保理解项目的长期价值。

该工作范围由 srcmgr@ 共同制定。具体工作项如下：
...
该工作包旨在改进工具和流程，以支持 FreeBSD 的零信任构建。通过扩展现有组件，使项目能够在不需要任何特殊权限的情况下构建发布产物（如软件包集、ISO 镜像等）。

详细范围由 core@、srcmgr@、secteam@ 共同制定。工作项如下：

* 必须完成（Must）

  * 所有源码发布构建案例/产物无需 root 权限（已完成）
  * 源码产物可重复构建（进行中）
  * 对 `make world` 和 `release.sh` 进行规范化和文档化（审查中）
* 应该完成（Should）
...
该工作包旨在改进 CI/CD 自动化，通过现代化和安全化现有 CI/CD 系统，并扩展到 FreeBSD Ports Collection 的第三方软件包，从而简化新旧软件的交付和运维。

详细范围由 core@、srcmgr@、portmgr@、doceng@ 共同制定。
...
该工作包旨在现代化和扩展 FreeBSD Ports 与软件包集合的安全控制，包括：从 VuXML 漏洞数据库迁移到 OSV 或类似现代格式；开发软件包审计后台和服务器，以可靠地从全球机构数据库获取漏洞数据（任何格式，如 JSON - NIST）并生成分析；改进 FreeBSD Ports 的 CI 工具。

详细范围由 core@、portmgr@、pkgmgr@、secteam@ 共同制定。
...
详细范围由 core@、portmgr@、pkgmgr@、secteam@、releng@ 共同制定。

* 必须完成（Must）

  * 评估更广泛生态系统中可用的项目/解决方案（进行中）
  * 提出 SBOM 目标解决方案（进行中）
  * 在 CI 中生成 SBOM（例如每周构建）（进行中）
  * 在发布流程中生成 SBOM 作为产物（进行中）
  * 按需生成 SBOM 产物（进行中）
  * 汇总现有数据（进行中）
  * 记录并说明所作决策（进行中）
* 可选完成（Could）
...
委托单位：Sovereign Tech Agency

### Alpha-Omega 海滩清理项目

链接：
[Alpha-Omega — Linux 基金会项目](https://alpha-omega.dev/) URL: [https://alpha-omega.dev](https://alpha-omega.dev)
[Alpha-Omega 在 GitHub](https://github.com/ossf/alpha-omega) URL: [https://github.com/ossf/alpha-omega](https://github.com/ossf/alpha-omega)
[FreeBSD Foundation](https://freebsdfoundation.org/) URL: [https://freebsdfoundation.org](https://freebsdfoundation.org/)
[FreeBSD Foundation 项目仓库](https://github.com/FreeBSDFoundation/alpha-omega-beach-cleaning) URL: [https://github.com/FreeBSDFoundation/alpha-omega-beach-cleaning](https://github.com/FreeBSDFoundation/alpha-omega-beach-cleaning)

联系人：Pierre Pronchery [[pierre@freebsdfoundation.org](mailto:pierre@freebsdfoundation.org)](%[5Bpierre@freebsdfoundation.org](mailto:5Bpierre@freebsdfoundation.org)%5D(mailto:pierre@freebsdfoundation.org))

Alpha-Omega 的使命是推动关键开源项目和生态系统的可持续安全改进。在 2024 年与 FreeBSD Foundation 成功合作——审计 bhyve 虚拟机管理程序和 Capsicum 沙箱框架之后——Alpha-Omega 再次选择 FreeBSD，启动本次 Alpha-Omega 海滩清理项目。该新资助旨在普遍改善 FreeBSD 基础系统中第三方软件的安全性和维护性。FreeBSD Foundation 收到资助后负责管理和执行该项目。
...
欢迎帮助完善已收集的信息，或改进项目的其他方面！

最终，每月报告会提交并在 [GitHub](https://github.com/ossf/alpha-omega/tree/main/alpha/engagements/2025/FreeBSD) 上公开。

赞助：Alpha-Omega，FreeBSD Foundation

### STA 工作包 C：CI/CD 自动化

联系人：Siva Mahadevan [[smahadevan@freebsdfoundation.org](mailto:smahadevan@freebsdfoundation.org)]

在本季度，作为主权技术机构（STA）委托的基础设施现代化工作的一部分，我一直在处理 in-tree CI Makefile 目标。我还致力于在我们的一级架构（amd64 和 aarch64）上使 CI 测试报告保持干净状态。本报告是对总体 STA 状态报告的补充，将更详细地描述已完成的工作。
...
* [tests/ci: 添加 KYUA_TEST_FILTERS 以允许用户选择特定测试](https://cgit.freebsd.org/src/commit/?id=92a7f2d577630d670643f855a1d123a2260102af)
...
FreeBSD 15.0 快照安装镜像（disc1.iso、dvd1.iso）现在包含离线 pkgbase 包，而非传统的发行版 tarball。这使得在无网络环境下也能安装 pkgbase 系统成为可能。
...
Sylve 是一个现代化、统一的 FreeBSD 系统管理平台，灵感来自 Proxmox。我们的目标是提供一个集成的 Web 界面，用于管理虚拟机（通过 Bhyve）、Jails、ZFS 存储、网络和防火墙。后端使用 Go 实现，前端采用 SvelteKit 搭配 Tailwind CSS 和 ShadCN UI 组件。
...
* 完整支持厚 Jails（创建、编辑、查看）。
* 已实现 CPU 和 RAM 资源限制。
* Jails 网络支持继承配置和基于交换机的（手动/标准）设置。
...
赞助方：FreeBSD Foundation 与 Alchemilla（开发和基础设施支持）
...
FreeBSD Git Weekly 是针对 [FreeBSD 源代码树](https://cgit.freebsd.org/src/) 的每周提交列表，按“文档”、“硬件支持”、“测试”和“内核”等类别进行分类。

我们的目标是让开发者和用户更方便地跟踪对自己相关的 FreeBSD 变更，而无需订阅 dev-commits-src-main 邮件列表并查看每一条提交。

未来，这些报告可能会包含摘要或更多信息，但目前我们重点是确定哪种分类方式对 FreeBSD 社区最有用。

每周报告自 2025-09-01 起开始发布。

赞助方：Tarsnap Backup Inc.

### 2025 年 7 月 FreeBSD Hackathon，德国柏林
...
我们受邀在柏林 Chaos Computer Club 的会场举办为期两天的 Hackathon。完整报告可见 [此处](https://pad.lorenz.lu/Hackathon202507)。
...
在 2025 年谷歌编程之夏中，我在 [Warner Losh](mailto:imp@FreeBSD.org) 的指导下进行 ACPI Lua 绑定项目开发。ACPI（高级电源与配置接口）用于在操作系统层管理电源，而非仅在 BIOS 中。项目目标是在 amd64 平台的 bootloader 中将 ACPI 功能暴露给 Lua，并计划未来支持 arm64 平台。

#### 成果

* Lua 是一种更简单、高级的脚本语言，可加速开发。
* 减少了 bootloader 中与 ACPI 相关的猜测工作。
* 允许用户在进入内核前查询和操作 ACPI 数据，从而在引导加载阶段即可控制配置。

#### 备注

如果在 loader 中存在任何我提供的接口无法支持的 ACPI 专用用例，请联系我，我会尽力提供支持。目前，这是我计划在 GSoC 中提交到源代码树的接口，因此任何额外的工作都需在之后进行，但我对此非常感兴趣（并鼓励大家进行扩展）。
...
Geomman 是一个基于 [sade(8)](https://man.freebsd.org/cgi/man.cgi?query=sade&sektion=8&format=html) 的分区工具（TUI），提供了更多功能，例如复制、粘贴分区，创建 ext 文件系统，或使用 [geli(8)](https://man.freebsd.org/cgi/man.cgi?query=geli&sektion=8&format=html) 加密分区。
...
最后，在 [Robert Clausecker](mailto:fuz@FreeBSD.org) 的帮助下，我们发布了 [geomman Port](https://www.freshports.org/sysutils/geomman)，让用户可以尝试使用。
...
Sockstat 是 FreeBSD 中的一个命令行工具，用于显示打开的网络套接字的详细信息。然而，其传统的固定宽度格式在处理长地址时不易阅读。本项目通过引入动态列宽和 libxo 的结构化输出支持，对 sockstat 进行了改进。通过这些增强功能，用户可以更轻松地在终端中阅读输出，或将数据导出为 JSON、XML 或 HTML 等格式，以便脚本处理和与外部工具集成。

sockstat 的改进分为两个阶段。第一阶段，将固定宽度布局替换为动态列宽，采用双遍算法确保对齐和可读性，包括处理长套接字地址的情况。`-w` 选项被重新定义，用于启用宽输出并自动调整列宽，同时保持与默认终端显示的兼容性。第二阶段，集成了 libxo 支持，通过将 `printf()` 调用替换为 `xo_emit()`，应用适当的字段标记，并使用 libxo 工具验证输出。手册页也更新以记录 libxo 集成。此改动在保留传统可读格式的同时，提供了机器可读的结构化输出。

在 sockstat 转换之后，发现 `parse_ports` 函数存在问题。由于其复杂性，为确保行为正确，对该函数添加了单元测试。问题通过修复函数并引入单元测试集得到解决。此外，代码库重构，将 `sockstat.c` 重命名为 `main.c`。

本项目属于 2025 谷歌编程之夏 的一部分，对 sockstat 进行了现代化改造，使其更易用，并更适合自动化工作流的集成。
...
* 应用程序应在使用 [setgroups(2)] 或 [initgroups(3)] 的同时使用 [setgid(2)] 或 [setegid(2)] 来设置有效组 ID。
  如果没有这样做，则必须添加这些调用，否则受影响的应用程序在 FreeBSD 15 及以后将无法设置有效组 ID。
* 使用 [getgroups(2)] 的应用程序不应将返回数组的第一个元素视为特殊元素，而应像对待其他补充组一样处理。
  如果仍然这样做，需要修改为通过 [getegid(2)] 获取有效组 ID，并将 [getgroups(2)] 返回的所有组仅作为补充组处理。

所有更改函数的手册页已在 `stable/14` 和 `stable/15` 中更新，以描述和对比新旧行为，并新增了 `SECURITY CONSIDERATIONS` 节，说明变更原因及注意事项。
...
[vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html) 控制台字体 “gallant” 在 -CURRENT 和 14-STABLE 中进行了扩展。Unicode 字形数量从 502 个增加到超过 4500 个。老用户们熟知并喜爱这款字体，因为曾在 Sun SPARCstations 启动时使用过它。
...
* 将 `virtual_oss` 审核为 base port：[D52308](https://reviews.freebsd.org/D52308)、[D52365](https://reviews.freebsd.org/D52365)、[D52366](https://reviews.freebsd.org/D52366)、[D52426](https://reviews.freebsd.org/D52426) 等。
* [snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 声音重定向补丁 ([D50070](https://reviews.freebsd.org/D50070)) 接近完成。
* 更多 [sound(4)](https://man.freebsd.org/cgi/man.cgi?query=sound&sektion=4&format=html) 清理、修复与改进。
* 增加了更多开箱即用的笔记本支持。
* 为 FreeBSD Journal 撰写了 BSDCan 2025 旅行报告。
* 正在撰写 FreeBSD Journal 音频专题文章。
...
DRM 驱动是**用于集成和独立 GPU 的内核驱动**，它们在 Linux 内核中维护，我们将其移植到 FreeBSD。截至本报告，移植的仅有 AMD 和 Intel DRM 驱动（NVIDIA FreeBSD 驱动为专有，由 NVIDIA 提供）。

我们一次移植一个 Linux 版本，这样可以更频繁地发布更新，也方便移植和调试，因为相比跨越多个版本跳跃，版本差异更小。
...
GitHub 上的 pull request 及相关 freebsd-src 分支可能尚未包含最新版本，直到编译和加载失败的问题解决为止。

这些更新目前针对 FreeBSD 16-CURRENT 开发分支，但 `linuxkpi` 的更改应回移植到 FreeBSD 15-STABLE。

以下 pull request 目前尚未获得足够的评审和测试，因此仍在进行中：

* [`DMA_BUF_IOCTL_EXPORT_SYNC_FILE` 和 `DMA_BUF_IOCTL_IMPORT_SYNC_FILE` ioctl](https://github.com/freebsd/drm-kmod/pull/357)
...
两个 pull request 都依赖于 `linuxkpi` 的补丁，相关补丁已在 pull request 中链接。

本工作由 FreeBSD Foundation 赞助，作为笔记本和桌面项目的一部分。
...
[drm-kmod GitHub 问题](https://github.com/freebsd/drm-kmod/issues/302) URL: [https://github.com/freebsd/drm-kmod/issues/302](https://github.com/freebsd/drm-kmod/issues/302)

联系人：Olivier Certner [[olce.freebsd.statusreports@certner.fr](mailto:olce.freebsd.statusreports@certner.fr)](%[5Bolce.freebsd.statusreports@certner.fr](mailto:5Bolce.freebsd.statusreports@certner.fr)%5D(mailto:olce[.freebsd.statusreports@certner.fr](mailto:.freebsd.statusreports@certner.fr)))

使用 `drm-kmod` Ports 中 amdgpu DRM 驱动的 AMD GPU 用户，尤其是从 v5.15 (`drm-515-kmod`) 开始，自至少 2024 年 5 月以来，一直经历逐渐的性能下降和系统卡顿。代码分析表明，近期基于 Intel 的 GPU（13 代及以上）可能也受到影响。我们很高兴地宣布，据我们所知，这些问题已经全部得到修复。

我们鼓励用户在 `main`、`stable/15` 或 `stable/14` 分支上测试最新的 FreeBSD 代码。这些修复将包含在即将发布的 15.0 和 14.4 版本中。对于 14.3，可能会发布勘误通知和补丁，以便用户无需等待 14.4（预计将于明年 3 月发布）。额外的修复也将进入 `drm-kmod` Ports（见下文）。
...
第一个问题是我们的 LinuxKPI 未处理 Linux 的 `__GFP_NORETRY` 标志，会强行尝试满足首批请求（即高阶页请求），使用昂贵的机制获取或生成连续内存，如果没有现成的可用页。Mathieu（邮箱 `sigsys@gmail`，带常规公司后缀）提出的第一次修复，移除了这一过程中对内存压缩的调用（即放弃调用 `vm_page_reclaim_contig()`）。随后，该修复进一步完成，通过阻止 VM 系统尝试打破内存保留，这些保留是一个推测性机制的一部分，旨在自动触发大页（superpages）的使用。
...
使用 `dtrace` 对性能下降进行了初步分析。结果显示，失败分配操作的相当一部分执行时间花费在尝试在同一 NUMA 域内进行多次分配上。对 VM 域集合迭代器代码的分析揭示了多处缺陷，尤其是导致在常见的单域机器情况下多次（最多 4 次）重新检查同一域，而对于新尝试没有任何额外成功保障。在此过程中还修复了一些其他 VM 域集合问题，例如确保分配请求在任何情况下都优先选择不处于低内存状态的域。至于成功的分配，它们会触发无用的页面属性更改，从而导致昂贵的 TLB 清空操作。

最后，针对 amdgpu 驱动，且仅影响基于 Carrizo、Polaris 和 Vega M 的 AMD GPU，将一个不必要的物理连续临时分配替换为常规分配，使剩余的、相对短暂但明显的卡顿消失。与上述情况不同，此更改属于 `drm-kmod` Ports 的代码，并已包含在 Ports 树的最新版本更新中。请确认你的软件包版本——`drm-510-kmod`、`drm-515-kmod`、`drm-61-kmod` 和 `drm-66-kmod`——分别大于或等于 `5.10.163.*_13`、`5.15.160.*_8`、`6.1.128.*_7` 和 `6.6.25.*_6`，其中 `*` 表示构建系统上的 `__FreeBSD_version` 值。

此工作由 FreeBSD Foundation 赞助，作为笔记本项目（Laptop Project）的一部分。

赞助方：FreeBSD Foundation

### LinuxKPI 802.11 与原生无线更新

联系人：Bjoern A. Zeeb [[bz@FreeBSD.org](mailto:bz@FreeBSD.org)](%[5Bbz@FreeBSD.org](mailto:5Bbz@FreeBSD.org)%5D(mailto:bz@FreeBSD.org))
联系人：FreeBSD 无线邮件列表 [[wireless@FreeBSD.org](mailto:wireless@FreeBSD.org)](%[5Bwireless@FreeBSD.org](mailto:5Bwireless@FreeBSD.org)%5D(mailto:wireless@FreeBSD.org))
...
作为 2025 谷歌编程之夏 的一部分，Kushagra 对 [mac_do(4)](https://man.freebsd.org/cgi/man.cgi?query=mac_do&sektion=4&format=html)（内核）和 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html)（用户空间）进行了扩展工作。

#### 已改进的 [mac_do(4)] 功能：

* **每个 jail 配置授权可执行文件**：允许管理员为每个 jail 指定允许请求凭证切换的可执行文件列表，而不局限于硬编码的 **/usr/bin/mdo**。
...
* **支持细粒度指定目标组**（`-g`、`-G`、`-s` 选项）：可继承当前凭证或密码和组数据库中某个用户的凭证，并可显式覆盖任何用户 ID、组 ID 及辅助组。
* **提供 `--print-rule` 选项**：切换到显示规则目标部分示例的模式，以匹配所请求的凭证。

其中，[mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 的新细粒度凭证切换请求功能已被提交，并将在 15.0 和 14.4 中可用。其他功能最可能在 14.4 之前进入 stable/14，但由于需要更多审查和部分修改，可能无法出现在 15.0 中。
...
Suspend-to-idle 和 S0ix 睡眠支持正在被添加到 FreeBSD 中。

这将允许现代 Intel 和 AMD 笔记本（其中一些不支持 ACPI S3 睡眠）进入低功耗状态，从而延长电池寿命。
...
这些更改进一步需要创建通用的睡眠类型，独立于 ACPI。目前并非所有更改都已提交，但将创建新的 `hw.power.*` sysctl，用于选择各种睡眠状态切换请求的睡眠类型。
...
XHCI USB 控制器提供一种模式，使其可用作系统调试接口。XHCI 调试使用一条特殊的 USB 3 数据线，其中 VBUS、D+ 和 D- 被断开。该功能可用于对 FreeBSD 内核进行实时调试，从而调查导致系统视频控制台锁定的问题，而无法使用其他替代方案（如串口控制台）时尤其有用。这种情况通常发生在调试图形驱动问题时。
...
在本季度，XHCI 调试的 Debug Host 部分已提交到 FreeBSD main 并将在 FreeBSD 15 中可用。Debug Host 驱动使 FreeBSD 机器能够调试作为 Debug Target 的 Linux 和 Windows 系统。Debug Host 驱动已从补丁系列中拆分出来，以便从 FreeBSD 15 调试 FreeBSD 16 及以后版本。
...
Banana Pi R64 是基于 MediaTek MT7622 的开发板（ARM Cortex-A53，双核约 1.35 GHz），配备 4× 千兆 LAN、1× 千兆 WAN、Wi-Fi（4×4n）、蓝牙 5.0，以及多个外设接口（UART、SPI、I²C、GPIO、SATA、mini-PCIe、eMMC 等）。
...
其他关键组件—以太网、USB、SATA、Wi-Fi 等—尚未实现。

#### 技术背景与重要性

FreeBSD 对 Banana Pi R64 的支持仍处于早期阶段—UART 和时钟驱动已存在，但 ppl clock 驱动仍在开发中，GPIO 驱动也在开发中—而大多数关键子系统尚未实现。
...
* 实现缺失的驱动程序
  ...
  * USB（XHCI/OTG）
  * SATA / AHCI
  * Wi-Fi（可能为 MediaTek MT7615）
  * GPIO 子系统

#### 结论

FreeBSD 对 Banana Pi R64 的支持仍处于早期阶段——UART 和时钟驱动已存在，但 ppl clock 驱动仍在开发中，GPIO 驱动也在开发中——大多数关键子系统尚未实现。发布可用的代码和工件，并与 FreeBSD 社区积极合作，将是使该开发板在 FreeBSD 下达到可用状态的关键。
...
受 [cloud-init](https://cloud-init.io/) 启发，nuageinit 是一个完全用 Lua 编写的脚本，用于为 FreeBSD 添加 cloud-init 兼容性。借助 [rc(8)](https://man.freebsd.org/cgi/man.cgi?query=rc&sektion=8&format=html) 框架的 `firstboot` 功能，它可以在系统启动早期运行，并且只运行一次。
...
* doceng@ 欢迎 ziaee@ 作为新成员（lurker）。
* ebrandi@ 晋升为正式 doceng@ 成员。
...
在本季度，doceng@ 维护的 Port 完成了以下工作：

* www/gohugo：更新至 0.151.0

#### 未解决的问题

在 Bugzilla 中有 1 个分配给 doceng@ 的开放 PR：
...
FreeBSD 俄语文档项目的当前目标是提供 FreeBSD 文档中最重要部分（FAQ、Handbook、网站内容）的最新俄语翻译。为讲俄语的用户提供高质量的官方技术资料，并提升操作系统在全球的使用率至关重要。我们希望此项目能获得俄语 FreeBSD 社区的支持，并推动翻译材料的增加。

在 2025 年 9 月，两名提交者获得提交权限，可将俄语翻译文档写入 FreeBSD 文档树：
...
对 Ports Collection 的更改，包括影响整个树的大范围更改，或单个 Port 的更改。
...
Bhyvemgr 是一个用 Freepascal/Lazarus 在 FreeBSD 上编写的 bhyve 管理 GUI。它需要一些工具，大部分已安装在基础系统中，部分需从 Ports/Packages 安装。其主要目标是作为桌面应用，为桌面用户提供简单快速的方式，在 FreeBSD 主机上设置和运行虚拟机。
...
**AppJail** 是一个完全用 POSIX shell 和 C 编写的开源 BSD-3 许可框架，用于使用 FreeBSD jails 创建隔离、可移植且易于部署的环境，其行为类似于应用程序。

**Director** 是一个工具，用于在 [AppJail](https://github.com/DtxdF/AppJail) 上运行多 jail 环境，通过简单的 YAML 规范进行配置。Director 文件用于定义组成应用程序的一个或多个 jail 的配置。一旦拥有 Director 文件，就可以通过单条命令 `appjail-director up` 创建并启动应用程序。
...
本项目的目标是提升 FreeBSD/amd64 和 FreeBSD/arm64 上的 OpenJDK 支持。
...
* OpenJDK 23 Port 已 [更新](https://cgit.freebsd.org/ports/commit/?id=a936fb9b16bac7cc5945213c7edf8c6a57709591)，增加了 IPv6 双协议套接字支持（与 OpenJDK 24 类似），并提供 Elastic 和 OpenSearch 的性能改进。
...
联系方式：FreeBSD 上的 KDE 邮件列表 [[kde@FreeBSD.org](mailto:kde@FreeBSD.org)](%[5Bkde@FreeBSD.org](mailto:5Bkde@FreeBSD.org)%5D(mailto:kde@FreeBSD.org))
...
[graphics/qt6-wayland](https://cgit.freebsd.org/ports/tree/graphics/qt6-wayland/) Port 已打补丁，以修复 Plasma 在 Wayland 下右键点击桌面时的崩溃问题。

Qt5 已更新至 5.15.17 的 KDE 补丁集合。Qt5 上游的标准支持已正式结束。这可能是 FreeBSD 上 Qt5 的最后一次更新。

[www/qt5-webengine](https://cgit.freebsd.org/ports/tree/www/qt5-webengine/) Port 已更新至 5.15.19，包含 Chromium 135.0.7049.95 的安全补丁。然而，Qt5 WebEngine 组件始终基于 Chromium 87.0.4280.144，这一版本已有 4 年多。底层 Chromium 的安全更新已经停止，使用需自担风险。
...
带有 Linux NetworkManager 桩实现的 [net-mgmt/kf6-networkmanager-qt](https://cgit.freebsd.org/ports/tree/net-mgmt/kf6-networkmanager-qt/) Port 已添加，以便移植第三方软件。
...
联系方式：Paul Floyd [[pjfloyd@wanadoo.fr](mailto:pjfloyd@wanadoo.fr)](%[5Bpjfloyd@wanadoo.fr](mailto:5Bpjfloyd@wanadoo.fr)%5D(mailto:pjfloyd@wanadoo.fr))
...
* 函数 `memalign` 现在接受大小为零

### 一个新的 Wi-Fi 管理工具：wutil
...
联系方式：Muhammad Saheed [[saheed@FreeBSD.org](mailto:saheed@FreeBSD.org)](%[5Bsaheed@FreeBSD.org](mailto:5Bsaheed@FreeBSD.org)%5D(mailto:saheed@FreeBSD.org))

[net/wutil](https://cgit.freebsd.org/ports/tree/net/wutil/) 是一个 Wi-Fi 管理工具，支持大部分 [wpa_supplicant(8)](https://man.freebsd.org/cgi/man.cgi?query=wpa_supplicant&sektion=8&format=html) 的站点模式操作（扫描、连接或断开无线网络，以及管理已知网络等），提供更友好的操作界面。它还会自动管理和更新 [wpa_supplicant.conf(8)](https://man.freebsd.org/cgi/man.cgi?query=wpa_supplicant.conf&sektion=8&format=html)，对包含 Unicode 字符的 SSID 也能很好地处理。

[wutil(8)](https://man.freebsd.org/cgi/man.cgi?query=wutil&sektion=8&format=html) 提供命令行界面（CLI），而 [wutui(8)](https://man.freebsd.org/cgi/man.cgi?query=wutui&sektion=8&format=html) 提供终端用户界面（TUI）。wutui 的构建没有依赖任何 TUI 库，仅通过在 uncooked 终端原始模式下拼写 ANSI 转义序列，并结合 [kqueue(2)](https://man.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2&format=html) 事件循环实现。两个工具都通过 wpa_supplicant 的控制套接字接口进行通信。此外，它还依赖 [net/libifconfig](https://cgit.freebsd.org/ports/tree/net/libifconfig/) 来处理接口相关功能。

未来，我计划支持来自 [hostapd(8)](https://man.freebsd.org/cgi/man.cgi?query=hostapd&sektion=8&format=html) 的 AP 模式操作，清理 TUI 组件，并可能逐步摆脱 wpa_supplicant，自行处理认证。
