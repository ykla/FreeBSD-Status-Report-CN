...
一如既往，因部分重要报告提交延迟，报告发布有所延后。对此我们深表歉意，但大多数成员都是志愿者，我们都尽力而为。

所幸志愿者的人数似乎即将增加：在上季度，我注意到许多新社区成员对参与 FreeBSD 项目表现出浓厚兴趣。这无疑将提升整个开发工作的效率，包括状态报告的编写。

许多新加入的成员都会带着这样的问题来找我们：“我该如何开始？”这里不是回答这个问题的地方，但实际上已经有一份指南专门为此编写。你可以在 [这里](https://docs.freebsd.org/en/articles/contributing/) 找到它。此外，时刻记住：尽可能地多使用 FreeBSD，会带来很大帮助：将它作为你的主力系统，用它运行 Web 服务器，在虚拟机上安装它，为现有软件做贡献时也支持它！如果你发现了 bug，也不要忘记 [报告](https://bugs.freebsd.org/bugzilla/) 它们。
...
来自各官方及半官方团队的条目，详见 [管理页面](https://www.freebsd.org/administration/)。
...
- EuroBSDCon 核心会议
  ...
  - dch 发表了题为“Core Team: Personal Perspective（核心团队：个人视角）”的演讲。
- 从 FreeBSD 网站移除 X/Twitter 链接
  - 核心团队收到询问，是否可以从项目网站移除 X/Twitter 的链接。
  - 由于该账号未被使用，核心团队认为移除是可行的。
  - FreeBSD 仍需控制该账号，以防被他人滥用。
  - 如果 FreeBSD 项目和基金会中有人能够良好管理这些社交账号，可以重新添加链接。
- 基金会提议的隐私友好型网站分析
  - 一个想法是比较 freebsd.org 与 freebsdfoundation.org 的流量情况。
  - 核心团队与文档工程团队合作并已部署。
- 核心团队与 FreeBSD 基金会一直在合作进行 2025 年版社区调查结果的发布工作。

#### 正在进行的工作

核心团队目前正在处理以下事项：

- 核心团队正在创建一个工作组，以重组文档工程团队（doceng）。
- 提交者指导方针/规则
  - 由开发者提出的咨询议题。
  - 核心团队发现已有部分文档，但应向文档工程团队、portmgr 和 srcmgr 征求更多意见。
- 核心团队选举与任期变更
  - 根据社区反馈，优选方式是交错的核心任期。
  - 一个想法是每年轮换一半核心成员，而不是每两年可能会轮换整个核心团队，以免正在进行的工作和经验丢失。
  - 一些开发者对此有强烈意见（例如每年轮换三分之一核心成员，而不是每两年轮换全部核心成员），并希望参与讨论。
  - 为此需要修改章程，标准要求较高。
  - 社区中有人建议仅修改章程，因为核心团队在法律上不代表任何人，这真的是个好主意吗？
...
- FreeBSD 项目连续性
  ...
  - 主权技术局工作的一个成果是发布工程文档已经撰写（在文档库的 releng 文章旁），但某些边缘情况仍缺失。
- 提交者 GECOS 字段与化名
  - 有开发者提出是否可以使用化名作为提交者名称的问题。
  - 更高的关注点在于如何处理版权问题。
  - 核心团队认为使用化名并不受支持，因为会增加文书工作难度和集群访问的恢复难度，同时建议强调使用真实姓名的好处。
  - 化名可能带来的另一个问题是身份共享，例如供应商提交权限情况下，谁将成为发言人？
- AI 政策
  - 核心团队正在起草关于使用 AI 和大型语言模型（LLM）的政策。
  - 此过程中还计划咨询其他团体，例如基金会法律顾问，以及 Linux 和其他开源社区中从事相关工作的人员。
  - 开发者和外部贡献者对版权、代码来源保障、最佳实践和“常识”，以及 AI（辅助）工具可以做什么与不可以做什么，存在多种问题。
...
FreeBSD 基金会是一家 501(c)(3) 非营利组织，致力于通过技术与非技术支持推动 FreeBSD 发展。基金会完全依靠捐赠资助，支持软件开发、基础设施、安全和协作工作；组织活动和开发者峰会；提供教育资源；并在法律事务上代表 FreeBSD 项目。
...
在基金会管理下，FreeBSD 连续第 21 年参与了 [谷歌编程之夏](https://summerofcode.withgoogle.com/) 项目。所有 [12 个项目](https://wiki.freebsd.org/SummerOfCode2025Projects) 均已成功完成。
...
- 发布了以下博客和视频，以帮助社区学习和了解 FreeBSD：
  - [如何安装和配置 Galene 视频会议服务器](https://freebsdfoundation.org/blog/how-to-install-and-configure-the-galene-video-meeting-server-on-freebsd/)
  ...
  - [FreeBSD Jail 简单易用](https://freebsdfoundation.org/blog/freebsd-jails-are-simple-and-easy/)
...
基金会持有 FreeBSD 商标，并负责保护这些商标。同时为核心团队提供法律支持，以调查相关问题。
...
[FreeBSD 发布](https://download.freebsd.org/releases/ISO-IMAGES/)
...
团队管理了 15.0-RELEASE 周期的前半段，目标是 12 月发布官方 RELEASE 版本及公告。值得注意的是，这期间有大量 pkgbase 相关工作已落地（合并入树）。
...
[FreeBSD Wiki 上的 Framework Laptop 页面](https://wiki.freebsd.org/Laptops/Framework_Laptop/)

[在 Framework 系统上安装和使用 FreeBSD 的指南](https://github.com/FrameworkComputer/freebsd-on-framework)

[跟踪工单：Framework Laptop 功能支持、Bug 和改进](https://bugs.freebsd.org/262152)

联系方式：Daniel Schaefer [dhs@frame.work](mailto:dhs@frame.work)

联系方式：許立文 [lwhsu@FreeBSD.org](mailto:lwhsu@FreeBSD.org)

联系方式：ShengYi Hong [aokblast@FreeBSD.org](mailto:aokblast@FreeBSD.org)
...
该项目启动于 2024 年第三季度，由德国主权技术局委托，预算为 745,000 美元，预计期限至 2025 年底。主要目标是改进基本系统、Ports 和软件包的安全工具，更新项目基础设施以加快开发速度，增强构建安全性，并让新开发者更容易上手。
...
详细范围由 FreeBSD 核心团队、srcmgr@、secteam@ 共同制定。工作项如下：
...
详细范围由核心团队、srcmgr@、portmgr@、文档工程团队共同制定。
...
详细范围由 FreeBSD 核心团队、portmgr@、pkgmgr@、secteam@ 共同制定。
...
详细范围由 FreeBSD 核心团队、portmgr@、pkgmgr@、secteam@、releng@ 共同制定。
...
[在 GitHub 的 Alpha-Omega](https://github.com/ossf/alpha-omega)

[FreeBSD 基金会](https://freebsdfoundation.org/)

[FreeBSD 基金会项目仓库](https://github.com/FreeBSDFoundation/alpha-omega-beach-cleaning)

联系方式：Pierre Pronchery [pierre@freebsdfoundation.org](mailto:pierre@freebsdfoundation.org)

Alpha-Omega 的使命是推动关键开源项目和生态系统的可持续安全改进。在 2024 年与 FreeBSD 基金会成功合作——审计 bhyve 虚拟机管理程序和 Capsicum 沙盒框架之后——Alpha-Omega 再次选择了 FreeBSD，启动本次 Alpha-Omega 海滩清理项目。该新资助旨在普遍改善 FreeBSD 基本系统中第三方软件的安全性和维护性。FreeBSD 基金会收到资助后负责管理和执行该项目。
...
联系方式：Siva Mahadevan [smahadevan@freebsdfoundation.org](mailto:smahadevan@freebsdfoundation.org)

在本季度，作为主权技术局（STA）委托的基础设施现代化工作的一部分，我一直在处理 in-tree CI Makefile 目标。我还致力于在我们的一级架构（amd64 和 aarch64）上使 CI 测试报告保持干净状态。本报告是对总体主权技术局状态报告的补充，将更详细地描述已完成的工作。
...
FreeBSD 15.0 快照安装镜像（disc1.iso、dvd1.iso）现在内置了离线的 pkgbase 软件包，而非传统的发行版 tar 压缩包。这使得在无网络环境下也能安装 pkgbase 系统成为可能。
...
- **网络对象**：子网、主机和 MAC 地址现在被视为一等对象。用户可以在虚拟机、Jail 和交换机间创建并复用它们。
...
赞助商：FreeBSD 基金会与 Alchemilla（开发和基础设施支持）

### FreeBSD 每周 Git

链接

[FreeBSD 每周 Git](https://freebsd-git-weekly.tarsnap.net/)
...
### 2025 年 7 月德国柏林 FreeBSD 黑客松

链接

[活动页面](https://wiki.freebsd.org/Hackathon/202507)
...
- [向 Jail 注入宿主机可执行文件](https://pad.lorenz.lu/Hackathon202507#Jan-B)
...
[GitHub](https://www.github.com/kpowkitty/freebsd-src)

[PR: libsa: 添加 isprint()](https://www.github.com/freebsd/freebsd-src/pull/1740)

[PR: loader: 移动 ACPI RSDP 检测](https://www.github.com/freebsd/freebsd-src/pull/1843)

[PR: efi: 创建 libacpi](https://www.github.com/freebsd/freebsd-src/pull/1818)

[PR: liblua: ACPICA Lua 绑定](https://www.github.com/freebsd/freebsd-src/pull/1819)

联系方式：Kayla Powell（又名 Kat）[kpowkitty@FreeBSD.org](mailto:kpowkitty@FreeBSD.org)
...
loader 旨在保持轻量并为内核做准备。为此，其 ACPI 初始化功能缩减了 130 个函数。这些函数经筛选属于在上述接口中非必要或在 loader 中不可实现。包括：
...
[geomman Port](https://www.freshports.org/sysutils/geomman)
...
- 对新分区设置标签 [glabel(8)](https://man.freebsd.org/cgi/man.cgi?query=glabel&sektion=8&format=html)
- 对新分区实施加密 [geli(8)](https://man.freebsd.org/cgi/man.cgi?query=geli&sektion=8&format=html)，支持可选“keyfile”和“passphrase”（或 passfile）
...
[Sockstat UI Improvements wiki](https://wiki.freebsd.org/SummerOfCode2025Projects/SockstatUIImprovements)

[自动列宽调整 PR](https://github.com/freebsd/freebsd-src/pull/1720)

[重新引入 -w 选项 PR](https://github.com/freebsd/freebsd-src/pull/1746)

[cap-getnameinfo 微优化 PR](https://github.com/freebsd/freebsd-src/pull/1753)

[Libxo 支持 PR](https://github.com/freebsd/freebsd-src/pull/1770)

[撤销 SCTP 连接中 path-state 的错误使用 PR](https://github.com/freebsd/freebsd-src/pull/1799)

[完成 libxo 集成 PR](https://github.com/freebsd/freebsd-src/pull/1806)

[修复 libxo 集成后的端口解析问题 PR](https://github.com/freebsd/freebsd-src/pull/1807)

联系方式：Damin Rido [rido@FreeBSD.org](mailto:rido@FreeBSD.org)
...
本项目属于谷歌编程之夏 2025 的一部分，对 sockstat 进行了现代化改造，使其更易用，并更适合自动化工作流的集成。
...
[T2 2025 状态报告](https://www.freebsd.org/status/report-2025-04-2025-06/#_ucred_group_changes_in_freebsd_15_0)

[initgroups(3)：向后兼容实现及手册页更新](https://cgit.freebsd.org/src/commit/?id=9dc1ac869196)

[修改 getgroups(2) 手册页的主要提交](https://cgit.freebsd.org/src/commit/?id=4be38acc826f)

[修改 setgroups(2) 手册页的主要提交](https://cgit.freebsd.org/src/commit/?id=6d22cd6b5f8b)
...
1. [setgroups(2)](https://man.freebsd.org/cgi/man.cgi?query=setgroups&sektion=2&format=html) 和 [getgroups(2)](https://man.freebsd.org/cgi/man.cgi?query=getgroups&sektion=2&format=html) 系统调用的行为略有变化。
   出于谨慎考虑，即便几乎所有现有应用程序仍能正常运行，我们仍建议对你维护或使用的相关应用程序进行审核，如下所述。
2. 登录时进程的组成员资格从口令和组数据库中获取的方式略有变化：登录用户的初始数值组 ID（来自口令数据库）现在会自动加入补充组，即使该用户在组数据库中并未被明确列为该组成员。
...
- 使用 [getgroups(2)](https://man.freebsd.org/cgi/man.cgi?query=getgroups&sektion=2&format=html) 的应用程序不应将返回数组的第一个元素视为特殊元素，而应像对待其他补充组一样处理。
  如果仍然这样做，需要修改为通过 [getegid(2)](https://man.freebsd.org/cgi/man.cgi?query=getegid&sektion=2&format=html) 获取有效组 ID，并将 [getgroups(2)](https://man.freebsd.org/cgi/man.cgi?query=getgroups&sektion=2&format=html) 返回的所有组仅作为补充组处理。
...
[Gallant 项目](https://github.com/NanoBillion/gallant)

联系方式：Jens Schweikhardt [schweikh@FreeBSD.org](mailto:schweikh@FreeBSD.org)
...
[更新到 Linux 6.10 DRM 驱动](https://github.com/freebsd/drm-kmod/pull/371)
...
这些更新目前针对 FreeBSD 16-CURRENT 开发分支，但 `linuxkpi` 的更改应向后移植到 FreeBSD 15-STABLE。
...
本工作承蒙 FreeBSD 基金会赞助，作为笔记本和桌面项目的一部分。
...
[主要 PR](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=277476)

[drm-kmod GitHub issue](https://github.com/freebsd/drm-kmod/issues/302)
...
[Wiki 页面](https://wiki.freebsd.org/SummerOfCode2025Projects/MacDoAndMDoImprovements)

[提交至 mdo(1) 支持细粒度凭证切换请求](https://cgit.freebsd.org/src/commit/?id=3ca1e69028ac)

联系方式：Kushagra Srivastava [thesynthax@FreeBSD.org](mailto:thesynthax@FreeBSD.org)
...
作为谷歌编程之夏 2025 的一部分，Kushagra 对 [mac_do(4)](https://man.freebsd.org/cgi/man.cgi?query=mac_do&sektion=4&format=html)（内核）和 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html)（用户空间）进行了增补工作。
...
- **支持细粒度指定目标组**（选项 `-g`、`-G`、`-s`）：可继承当前凭证或口令和组数据库中某个用户的凭证，并可显式覆盖任何用户 ID、组 ID 及辅助组。
...
[s2idle/S0ix + AMD SMU 堆栈最新提交](https://reviews.freebsd.org/D48721)

联系方式：obiwac [obiwac@FreeBSD.org](mailto:obiwac@FreeBSD.org)

挂起到 idle 和 S0ix 睡眠支持正在加入 FreeBSD 中。
...
XHCI USB 控制器提供了一种模式，使其可用作系统调试接口。XHCI 调试使用一条特殊的 USB 3 数据线，其中 VBUS、D+ 和 D- 已断开。该功能可用于对 FreeBSD 内核进行实时调试，从而研究导致系统视频控制台锁定的问题，而无法使用其他替代方案（如串口控制台）时尤其有用。这种情况通常发生在调试图形驱动问题时。
...
文档工程团队负责处理与 FreeBSD 文档项目相关的一些元项目问题；更多信息请参阅 [FreeBSD Doceng 团队章程](https://www.freebsd.org/internal/doceng/)。
...
在 2025 年 9 月，两名提交者获得了提交权限，可将俄语翻译文档写入 FreeBSD 文档树：
...
对 Ports 的变更，包括影响整个树的大范围变更，或单个 Port 的变更。
...
- 添加 IPv6 支持，利用 [dns/dnsmasq](https://cgit.freebsd.org/ports/tree/dns/dnsmasq/) 提供更完善的 IPv6 支持。启用 Use_IPv6 后，会向 dnsmasq 添加 host-record。
- 在 bhyve Manager 设置中添加桥接 IPv6 计算器，用于计算应分配给桥接接口的 IPv6。详细信息请查看 README。
- 添加从 Cloud 或 VM 镜像创建虚拟机的支持。选择“Create a virtual disk from image”选项时，会显示新页面，包括镜像下载器（支持 img.xz、raw.xz、qcow2.xz、qcow2、img 和 raw 文件）。
...
- 从“Create Virtual Machine/Image”表单选择文件配置时，支持定义静态 IPv4 地址。
...
赞助商：[https://paypal.me/alonsocbsd](https://paypal.me/alonsocbsd)
...
[AppJail on GitHub](https://github.com/DtxdF/AppJail)

[Director on GitHub](https://github.com/DtxdF/Director)

[Overlord on GitHub](https://github.com/DtxdF/Overlord)

[Makejails](https://github.com/AppJail-makejails)

联系方式：Jesús Daniel Colmenares Oviedo [dtxdf@FreeBSD.org](mailto:dtxdf@FreeBSD.org)

**AppJail** 是一款完全用 POSIX shell 和 C 编写的开源 BSD-3 许可框架，用于使用 FreeBSD Jail 创建隔离、可移植且易于部署的环境，其行为类似于应用程序。
...
[CI for libvirt/bhyve on FreeBSD](https://empt1e.blogspot.com/2025/09/ci-for-libvirtbhyve-on-freebsd.html)
...
[KDE/FreeBSD 计划](https://freebsd.kde.org/)
...
[KDE 团队](mailto:kde@FreeBSD.org) 维护了大量 Port，并根据需要进行更新。根据 [Portscout](https://portscout.freebsd.org/kde@freebsd.org.html) 的数据，过时的 Port 不到 0.7%。

KDE 团队感谢 [Dima Panov](mailto:fluffy@FreeBSD.org)、[Gleb Popov](mailto:arrowd@FreeBSD.org)、[Jason E. Hale](mailto:jhale@FreeBSD.org)、[Kenneth Raplee](mailto:kenrap@kennethraplee.com)、[Loïc Bartoletti](mailto:lbartoletti@FreeBSD.org) 和 [Max Brazhnikov](mailto:makc@FreeBSD.org) 对保持 Port 更新所做的贡献。同时，KDE 团队感谢 Harley（IRC 上的 SponiX）分享他的构建机器。
...
在本报告撰写时，[将 GCC 默认版本从 13 更新到 14 的 exp-run](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=281091) 已接近完成：只剩下最后一个 PR 未合并。希望在你阅读本文时，该更新已经完成。不过需要提醒的是，最新的 GCC 主版本是 GCC 15，因此我们仍然落后一个版本。当然，后续还会准备另一次 exp-run，将 GCC_DEFAULT 更新到 GCC 15，但不会立即执行。我会等待一段时间，以确保 `GCC_DEFAULT=14` 的更新确实如预期般生效，同时处理与 GCC Port 相关的其他问题。

另一个重要变化涉及引导构建（bootstrapping）。GCC Port 之前状态不一致：有些 Port 需要选择 bootstrap 选项，而有些则不需要。现在所有 GCC Port 都允许在不选择任何 bootstrap 选项的情况下构建，就像过去一样。

问题在于，使用 FreeBSD 默认编译程序（clang）构建 GCC 并非完全受支持。由于我知道许多用户希望在不进行 bootstrap 的情况下构建 GCC，因此我没有像最初计划的那样强制要求，而是保留了该选项，但会从无 bootstrap 构建中移除所有无法成功构建的功能。用户需自行确保不需要与无 bootstrap 构建不兼容的功能。

目前，jit 是唯一被排除在无 bootstrap 构建之外的功能。默认 bootstrap 选项为 `STANDARD_BOOTSTRAP`，因此来自官方 FreeBSD 软件包仓库的用户将能够获得完整构建，所有受支持功能均可使用。
...
[arm64 Port](https://github.com/paulfloyd/freebsdarm64_valgrind)
...
我为 Valgrind 添加了一个小工具脚本 vgstack。它的工作方式类似于 pstack（在 FreeBSD 上为 bstack）：你提供一个 PID，它会生成该进程的堆栈跟踪。如果你对 Valgrind 进程使用 bstack，你将看到 Valgrind 的调用堆栈，这对你可能没用。而使用 vgstack 对 Valgrind PID 执行，则会打印在 Valgrind 下运行的测试程序的调用堆栈。
...
- amd64：添加 `sysarch` 子命令 `AMD64_SET_TLSBASE` 和 `AMD64_GET_TLSBASE`
- 减少在静默模式（`-q`）下打印的警告
- 改进 sysctl kern.proc.pathname 检查
- 支持 `mmap` 的 MAP_STACK 和 MAP_GUARD
- 系统调用 `open*` 在尝试写入 guest exe 时现在会报错
- 系统调用 `sigwait` 和 `sigwaitinfo` 对 NULL 参数的接受过于宽松
...
[源代码](https://github.com/MainKt/wutil)

[Port](https://www.freshports.org/net/wutil)
...
[wutil(8)](https://man.freebsd.org/cgi/man.cgi?query=wutil&sektion=8&format=html) 提供了命令行界面（CLI），而 [wutui(8)](https://man.freebsd.org/cgi/man.cgi?query=wutui&sektion=8&format=html) 提供终端用户界面（TUI）。wutui 的构建未依赖任何 TUI 库，仅通过在 uncooked 终端原始模式下输出 ANSI 转义序列，并结合 [kqueue(2)](https://man.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2&format=html) 事件循环实现。两个工具都通过 wpa_supplicant 的控制套接字接口进行通信。此外，它还依赖 [net/libifconfig](https://cgit.freebsd.org/ports/tree/net/libifconfig/) 来处理接口相关功能。
...
导师：[Aymeric Wobo](mailto:obiwac@FreeBSD.org) 和 [Getz Mikalsen](mailto:getz@FreeBSD.org)
