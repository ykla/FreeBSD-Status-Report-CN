...
一如既往，由于部分重要报告延迟送达，报告的发布速度较缓。对此我们表示歉意，但大多数成员都是志愿者，我们都尽力而为。

幸运的是，志愿者人数似乎即将增加：在上个季度，我注意到许多新社区成员对参与 FreeBSD 项目表现出浓厚兴趣。这无疑将提升整个开发工作的效率，如状态报告的制作。

许多新加入的成员都会带着同样的问题来找我们：“我该如何开始？”这里不是回答这个问题的地方，但实际上已经有一份指南专门为此编写。你可以在[这里](https://docs.freebsd.org/en/articles/contributing/)找到它。此外，时刻记住：将 FreeBSD 作为主力系统使用，用 FreeBSD 来运行网络服务器，在虚拟机上安装部署，在参与 FreeBSD 开发时鼎力相助！！如果你发现了 bug，也不要忘记[报告](https://bugs.freebsd.org/bugzilla/)它们。
...
* 从项目网站移除 X/Twitter 链接
  ...
  * 项目仍需控制这些账号，以防被他人滥用。
  * 如果项目或基金会中有人能够良好管理这些社交账号，可以重新添加链接。
* 基金会提议的隐私友好型网站分析
...
联系方式：Deb Goodkin [[deb@FreeBSDFoundation.org](mailto:deb@FreeBSDFoundation.org)]([deb@FreeBSDFoundation.org](mailto:deb@FreeBSDFoundation.org)(mailto:deb@FreeBSDFoundation.org))
...
联系方式：FreeBSD 发布工程团队 [[re@FreeBSD.org](mailto:re@FreeBSD.org)]([re@FreeBSD.org](mailto:re@FreeBSD.org)(mailto:re@FreeBSD.org))
...
根据索引，目前 Ports 中共有 37,163 个 ports（上季度为 36,605）。当前大约有 3,428 个（上季度为 3,330）开放 ports PR，其中 821 个尚未分配。上季度，main 分支由 156 位（上季度 157 位）提交者完成 8,738 个（上季度 10,924）提交；2025Q3 分支由 61 位（上季度 56 位）提交者完成 898 个（上季度 770）提交。
...
涵盖多个类别的项目，从内核与用户空间到 Ports 或第三方项目。
...
Framework Computer Inc. 在多个方面大力着支持 FreeBSD 项目，包括向基金会提供工程样品用于测试和兼容性工作。
...
该项目于 2024 年第三季度启动，由德国主权技术基金委托，预算为 745,000 美元，使用期限至 2025 年底。主要目标是改进基本系统、Ports 和软件包的安全工具，更新项目基础设施以加快开发速度，增强构建安全性，并让新开发者更容易上手。
...
联系人：Pierre Pronchery [[pierre@freebsdfoundation.org](mailto:pierre@freebsdfoundation.org)]([pierre@freebsdfoundation.org](mailto:pierre@freebsdfoundation.org)(mailto:pierre@freebsdfoundation.org))
...
在本季度，作为主权技术基金（STA）委托的基础设施现代化工作的一部分，我一直在处理 in-tree CI Makefile 目标。我还致力于在我们的一级架构（amd64 和 aarch64）上使 CI 测试报告保持干净状态。本报告是对总体 STA 状态报告的补充，将更详细地描述已完成的工作。
...
FreeBSD 15.0 快照安装镜像（disc1.iso、dvd1.iso）现在内置了离线的 pkgbase 包，而非传统的发行版 tarball。这使得在无网络环境下也能安装 pkgbase 系统成为可能。
...
赞助方：FreeBSD 基金会与 Alchemilla（开发和基础设施支持）
...
FreeBSD 每周 Git是针对 [FreeBSD 源代码树](https://cgit.freebsd.org/src/) 的每周提交列表，按“文档”、“硬件支持”、“测试”和“内核”等类别进行分类。
...
在 2025 年谷歌编程之夏中，我在 [Warner Losh](mailto:imp@FreeBSD.org) 的指导下进行 ACPI Lua 绑定项目开发。ACPI（高级电源与配置接口）用于在操作系统层管理电源，而非仅在 BIOS 中。项目目标是在 amd64 平台的 bootloader 中将 ACPI 功能暴露给 Lua，并计划后续支持 arm64 平台。
...
如果在 loader 中存在任何我提供的接口无法支持的 ACPI 专用用例，请联系我，我会尽力提供支持。目前，这是我计划在谷歌编程之夏中提交到源代码树的接口，因此任何额外的工作都需在之后进行，但我对此非常感兴趣（并鼓励大家进行扩展）。
...
* 增加了更多开箱即用的笔记本支持。
* 为 FreeBSD 期刊撰写了 BSDCan 2025 旅行报告。
...
联系人：Olivier Certner [[olce.freebsd.statusreports@certner.fr](mailto:olce.freebsd.statusreports@certner.fr)]([olce.freebsd.statusreports@certner.fr](mailto:olce.freebsd.statusreports@certner.fr)(mailto:olce[.freebsd.statusreports@certner.fr](mailto:.freebsd.statusreports@certner.fr)))

使用 Ports `drm-kmod` 中 amdgpu DRM 驱动的 AMD GPU 用户，尤其是从 v5.15 (`drm-515-kmod`) 开始，自至少 2024 年 5 月以来，一直经历着缓慢的性能下降和系统卡顿。代码分析表明，近期基于 Intel 的 GPU（13 代及以上）可能也受到影响。我们很高兴地宣布，据我们所知，这些问题已经全部得到修复。
...
此工作由 FreeBSD 基金会赞助，作为笔记本项目（Laptop Project）的一部分。
...
作为 2025 谷歌编程之夏的一部分，Kushagra 对 [mac_do(4)](https://man.freebsd.org/cgi/man.cgi?query=mac_do&sektion=4&format=html)（内核）和 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html)（用户空间）进行了扩展工作。
...
Banana Pi R64 是基于 MediaTek MT7622 的开发板（ARM Cortex-A53，双核约 1.35 GHz），配备 4× 千兆 LAN、1× 千兆 WAN、WiFi（4×4n）、蓝牙 5.0，以及多个外设接口（UART、SPI、I²C、GPIO、SATA、mini-PCIe、eMMC 等）。
...
其他关键组件—以太网、USB、SATA、WiFi 等—尚未实现。

#### 技术背景与重要性

FreeBSD 对香蕉派 R64 的支持仍处于早期阶段—UART 和时钟驱动已存在，但 ppl clock 驱动仍在开发中，GPIO 驱动也在开发中—而大多数关键子系统尚未实现。
...
FreeBSD 对香蕉派 R64 的支持仍处于早期阶段——UART 和时钟驱动已存在，但 ppl clock 驱动仍在开发中，GPIO 驱动也在开发中——大多数关键子系统尚未实现。发布可用的代码和工件，并与 FreeBSD 社区积极合作，将是使该开发板在 FreeBSD 下达到可用状态的关键。
...
对 Ports 的更改，包括影响整个树的大范围更改，或单个 Port 的更改。
...
联系方式：FreeBSD 上的 KDE 邮件列表 [[kde@FreeBSD.org](mailto:kde@FreeBSD.org)]([kde@FreeBSD.org](mailto:kde@FreeBSD.org)(mailto:kde@FreeBSD.org))
...
联系方式：Lorenzo Salvadore [[salvadore@FreeBSD.org](mailto:salvadore@FreeBSD.org)]([salvadore@FreeBSD.org](mailto:salvadore@FreeBSD.org)(mailto:salvadore@FreeBSD.org))
...
联系方式：Paul Floyd [[pjfloyd@wanadoo.fr](mailto:pjfloyd@wanadoo.fr)]([pjfloyd@wanadoo.fr](mailto:pjfloyd@wanadoo.fr)(mailto:pjfloyd@wanadoo.fr))
...
联系方式：Muhammad Saheed [[saheed@FreeBSD.org](mailto:saheed@FreeBSD.org)]([saheed@FreeBSD.org](mailto:saheed@FreeBSD.org)(mailto:saheed@FreeBSD.org))
