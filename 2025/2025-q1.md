...
添加了控制 \[rtw88(4)] 和 \[rtw89(4)] 中 HT 和 VHT 支持的可调参数。硬件加密卸载（HW crypto offload）默认启用 CCMP。发现许多用户仍在使用 TKIP，相关支持正在开发，预计在第二季度初完成。
...
公告指出，iwlwififw(4) 固件将效仿 rtw88 和 rtw89 的固件管理方式，于 2025 年 4 月从基本系统移除，改用 Ports 和 fwget(8) 方式（[公告](https://lists.freebsd.org/archives/freebsd-stable/2025-March/002763.html)）。
