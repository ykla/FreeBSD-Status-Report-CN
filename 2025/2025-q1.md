...
Core 正在收集意见和建议，以起草项目的路线图。这是 core.13 认为值得从 core.12 继续推进的事项。这个路线图并不是用来限制开发者和贡献者能做什么，而是汇总项目的目标和预期，以及社区可以协作完成的事务。这也将有助于 FreeBSD 基金会更有效地支持项目，因此，这是 core 与 FreeBSD 基金会会议中的一个重要讨论议题。
...
* Core 与 FreeBSD 基金会正合作准备 2025 年版社区调查
* 基金会提出的隐私友好的网页分析服务
...
我是 Deb Goodkin。已经是第二季度了吗？上一季度我们在支持 FreeBSD 项目和社区方面活动频繁、节奏飞快。在本报告中，我们将重点介绍我们目前正在进行的工作，以确保 FreeBSD 从长远来看依然可持续且安全。
...
令人振奋的消息：Mark Phillips 加入基金会，担任技术营销经理。请期待更多实用的信息和技术内容！我们还招募了一位新的兼职开发人员，担任解决方案专家角色，具体人选将很快公布。
...
* 发布了关于我们软件开发工作的更新与公告，包括：
  ...
  * [2025 年 1 月笔记本支持与可用性项目更新](https://freebsdfoundation.org/blog/january-2025-laptop-support-and-usability-project-update/)
  * [2025 年 2 月笔记本支持与可用性项目更新](https://freebsdfoundation.org/blog/february-2025-laptop-support-and-usability-project-update/)
...
* 发布了 [2024 年 11/12 月](https://freebsdfoundation.org/our-work/journal/browser-based-edition/virtualization-2/) 的 FreeBSD Journal，提供 HTML 格式的文章。
...
* [pkgbase 自动转换工具](https://www.freebsd.org/status/report-2025-01-2025-03/#_automatic_pkgbase_conversion_tool)
...
* [Sylve — 一个统一的 FreeBSD 系统管理平台](https://www.freebsd.org/status/report-2025-01-2025-03/#_sylvea_unified_system_management_platform_for_freebsd)
...
基金会正在管理 FreeBSD 参与 [Google Summer of Code (GSoC)](https://summerofcode.withgoogle.com/) 项目。截至 2 月底，我们欣喜地获悉 FreeBSD 再次被选为 GSoC 2025 的导师组织，这是我们连续第 21 年参与该项目。我们共收到 64 份申请，最终获选项目将于 5 月 8 日公布。
...
基金会持有 FreeBSD 商标，并负责其保护工作。我们还为 core 团队提供法律支持，协助其处理各类法律问题。

请访问 [https://freebsdfoundation.org](https://freebsdfoundation.org/) 了解我们如何支持 FreeBSD，以及我们如何能帮助您。
...
我们位于台湾的镜像站点当前处于长期离线状态，恢复工作正在进行中。我们希望能在 2025 年第 2 季度恢复其上线。
...
我们始终欢迎新的官方镜像加入。我们注意到，在互联网交换点（IXP）部署单个镜像的优势明显，目前在澳大利亚、巴西和南非的镜像站点已验证了这一点。如果您所在的机构或您认识的组织愿意赞助部署单个镜像服务器，请联系我们。我们尤其欢迎在美国西海岸和欧洲各地部署新镜像。
...
[参与贡献 Ports](https://docs.freebsd.org/en/articles/contributing/#ports-contributing) URL：[https://docs.freebsd.org/en/articles/contributing/#ports-contributing](https://docs.freebsd.org/en/articles/contributing/#ports-contributing)
...
在上一季度，pkgmgr@ 共进行了 20 次 exp-run，用于测试基础设施变更和各类 Ports 升级。
...
本季度我们在基础系统（Base System）问题单方面取得了重大进展，关闭了 1,000 多个已不再适用的旧问题单。许多问题单是通过仔细检查 **ObsoleteFiles.inc** 中的所有条目发现的。
...
几乎所有包含补丁的问题单都已由 patchQA.py 处理，其中数百个补丁已被重新基于当前源码树（例如基础系统补丁相对 src 树根目录重设）。我们现在已经了解到，有不少 Ports 的补丁实际上并不是用于 FreeBSD port 本身，而是需要手动应用到 work/ 目录中某个解包后的源代码。这方面已有脚本正在 alpha 测试阶段。
...
联系方式：srcmgr [srcmgr@FreeBSD.org](mailto:srcmgr@FreeBSD.org)

srcmgr@ 的工作重点是提升 src 开发者的工作效率，并试图应对大量收到的问题单和 pull requests。该团队每两周召开一次会议，讨论与 src 相关的事务，并对问题单和 pull requests 进行分流。现任成员包括 Ed Maste、Mark Johnston、John Baldwin 和 Warner Losh。会议纪要可在 GitHub 上查看：[会议记录](https://github.com/freebsd/meetings/tree/master/srcmgr)

在 1 月和 2 月，srcmgr@ 组织了两次线上 bug 清理会，每次有约 12 名开发者参与。这些会议持续了 3 小时，专注于对新的 src 问题单进行分流。团队计划在下个月恢复这类会议。

srcmgr@ 团队计划在 6 月举行的 [FreeBSD 开发者峰会](https://wiki.freebsd.org/DevSummit/202506) 上组织一场会议，讨论议题包括弃用 32 位平台、以及 FreeBSD 15.0 即将推出的 pkgbase 支持。
...
该项目始于 2024 年第 3 季度，由 Sovereign Tech Agency 委托执行，预算为 745,000 美元，预计持续约一年。项目主要目标包括改进基础系统、Ports 和包的安全工具，升级开发基础设施以加速开发流程、增强构建安全性，并简化新开发者的入门流程。
...
FreeBSD 基金会与 FreeBSD 项目的源码管理团队（Source Management Team）正在合作，使问题单管理更易于维护和持续发展。我们目前已上线一个 [问题单积压仪表盘](https://grimoire.freebsd.org/)，在“bug 清理”会议期间帮助更直观地了解积压情况，数据也已显示关闭的问题单数超过了新增问题单数。该工具运行在 FreeBSD 上，并且其 [文档](https://github.com/chaoss/grimoirelab/blob/main/FreeBSD.md) 已上游提交至 GrimoireLab 项目，便于他人参考复用。
...
[pkgbase](https://wiki.freebsd.org/PkgBase) URL：[https://wiki.freebsd.org/PkgBase](https://wiki.freebsd.org/PkgBase)
...
Framework Computer Inc. 长期以来在多方面支持 FreeBSD 项目，包括向基金会提供工程样机进行测试并推进兼容性工作。
...
赞助方：FreeBSD 基金会（赞助 Li-Wen 和 Sheng-Yi 的工作）
赞助方：Framework Computer Inc（赞助 Daniel 的工作、硬件和场地支持）

---

### Hackathon 202503 日本东京
...
在 [AsiaBSDCon-Lite 2025](https://hackmd.io/@AsiaBSDCon/2025lite) 活动前，一些社区成员在东京聚集并举行了一次 hackathon。

感谢 Christoff Visser 和 Internet Initiative Japan Inc. 为此次活动提供场地赞助。

#### 本次 hackathon 中完成或推进的工作
...
* FreeBSD 基础系统使用 Sccache：FreeBSD 基础系统支持使用 ccache 缓存构建结果。对于跨主机构建，需要分布式缓存机制，也就是 sccache。在 hackathon 中创建了添加 sccache 支持的补丁：[https://reviews.freebsd.org/D49417](https://reviews.freebsd.org/D49417)
...
* 在收到 OPNsense 开发者的模糊 bug 报告后，与 Gleb 和 Kristof 一同在 pf 中发现并修复了一个罕见的竞态条件，可能导致 use-after-free：[8efd2acf07bc](https://cgit.freebsd.org/src/commit/?id=8efd2acf07bc0e1c3ea1f7390e0f1cfb7cf6f86c)
...
赞助方：Christoff Visser 和 Internet Initiative Japan Inc.（提供场地）
...
Sylve 是一个面向 FreeBSD 的现代统一系统管理平台，灵感来源于 Proxmox。其目标是提供一个集成的网页界面，用于管理虚拟机（基于 Bhyve）、jail、ZFS 存储、网络与防火墙。后端使用 Go 编写，前端则采用 SvelteKit，并结合 Tailwind CSS 与 ShadCN UI 组件。
...
Sylve 致力于填补 FreeBSD 生态中一个关键空白：一个用户友好、功能完整的系统管理网页界面。通过整合虚拟化、存储和网络管理，它旨在降低用户与管理员在复杂环境中部署 FreeBSD 的门槛。
...
我们欢迎各类贡献、测试与反馈。如果您有兴趣参与，可从以下方面着手：
...
链接：
[主提交](https://cgit.freebsd.org/src/commit/?id=30e6e008bc06385a66756bebb41676f4f9017eca) URL: [https://cgit.freebsd.org/src/commit/?id=30e6e008bc06385a66756bebb41676f4f9017eca](https://cgit.freebsd.org/src/commit/?id=30e6e008bc06385a66756bebb41676f4f9017eca)
...
链接：
[Linux 6.7 DRM 驱动更新](https://github.com/freebsd/drm-kmod/pull/332) URL: [https://github.com/freebsd/drm-kmod/pull/332](https://github.com/freebsd/drm-kmod/pull/332)
[Linux 6.8 DRM 驱动更新](https://github.com/freebsd/drm-kmod/pull/344) URL: [https://github.com/freebsd/drm-kmod/pull/344](https://github.com/freebsd/drm-kmod/pull/344)

联系人：Jean-Sébastien Pédron \[[dumbbell@FreeBSD.org](mailto:dumbbell@FreeBSD.org)]

DRM 驱动是针对集成和独立 GPU 的内核驱动，维护于 Linux 内核，我们将其移植到 FreeBSD。目前我们只移植 AMD 和 Intel 的 DRM 驱动（NVIDIA FreeBSD 驱动为专有，由 NVIDIA 自行提供）。
...
这些更新目前针对 FreeBSD 15-CURRENT 开发分支。一旦内核补丁获批并合并 DRM 驱动更新后，我们将评估如何将内核补丁回移植到早期的发布分支（如 14-STABLE）。
...
Syzkaller 是一个操作系统内核模糊测试工具，用于查找内核漏洞。
...
FreeBSD 移植到 Pinephone Pro 的项目正在推进。该项目目标是逐步支持 Pinephone Pro 的各个组件，使设备未来能够作为高移动性的 FreeBSD 设备使用。
...
赞助：Microsoft（对 Microsoft 内部人员及资源）
赞助：The FreeBSD Foundation（其他所有支持）
...
[新贡献者的 FreeBSD 文档项目入门](https://docs.freebsd.org/en/books/fdp-primer/)
[文档工程团队](https://www.freebsd.org/administration/#t-doceng)

联系人：FreeBSD Doceng 团队 \[[doceng@FreeBSD.org](mailto:doceng@FreeBSD.org)]

doceng@ 团队负责处理 FreeBSD 文档项目相关的元项目问题；更多信息请参见 [FreeBSD Doceng 团队章程](https://www.freebsd.org/internal/doceng/)。
...
自上次状态报告（2024Q3）以来，进展陷入停滞。我们发放了几十个新账户，但这些新作者的大部分更改仅限于个别页面，而没有改善整体结构。

Mark Linimon 仍然认为，如果有人愿意投入时间，wiki 可以成为极好的资源。但目前关于数据陈旧的投诉多于新贡献者和新想法。

可以说，现在的 wiki 处于自动驾驶状态。
...
感谢贡献者和提交者的专注努力，FreeBSD 提供了多项辅助技术。当前的持续工作重点是整理并记录这些无障碍功能，形成一本新的手册。项目目前集中于为盲人、低视力和色盲用户记录功能，涵盖 PORTS 和 BASE 系统的功能。例如：用于屏幕放大、屏幕阅读器（帮助不能看屏幕的用户）以及调整桌面环境颜色的工具。此外，BASE 系统中提升可见性的辅助功能也在记录中，带有示例和提示，比如在系统虚拟控制台 [vt(4)](https://man.freebsd.org/cgi/man.cgi?query=vt&sektion=4&format=html) 中修改颜色、字体和大小的能力。
...
此外，本季度更新了端口 [www/edbrowse](https://cgit.freebsd.org/ports/tree/www/edbrowse/)，这是一个完全命令行的网页浏览器，兼容屏幕阅读器。还在开发一套解决方案，便于在 BASE 系统中的 TUI 工具实现颜色自定义，并可能直接通过系统安装程序 [bsdinstall(8)](https://man.freebsd.org/cgi/man.cgi?query=bsdinstall&sektion=8&format=html) 设置高对比度。
...
影响 Ports 的变更，包括触及大部分树结构的重大调整，或个别端口的更新。
...
Bhyvemgr 是一个用 Freepascal/Lazarus 在 FreeBSD 上编写的 bhyve 管理 GUI。它依赖于基系统中安装的一些工具以及部分来自 Ports/包的工具。其主要目标是成为一款面向桌面用户的桌面应用，便于快速轻松地在 FreeBSD 主机上设置和运行虚拟机。
...
Bhyvemgr 仅支持 FreeBSD 15-CURRENT 上的 aarch64，及 FreeBSD 13.x 到 15-CURRENT 上的 amd64。Bhyvemgr 可从 [Ports](https://www.freshports.org/sysutils/bhyvemgr) 或 pkg 二进制包安装，支持 gtk2、qt5 或 qt6 界面。
...
**AppJail** 是一个完全用 POSIX shell 和 C 语言编写的开源 BSD-3 许可证框架，用于创建隔离、可移植且易于部署的环境，使用 FreeBSD jails，行为类似于一个应用程序。

**Director** 是一个用于在 AppJail 上运行多 jail 环境的工具，采用简单的 YAML 规范。Director 文件用于定义组成应用程序的一个或多个 jail 的配置。一旦有了 Director 文件，你可以通过一条命令 `appjail-director up` 来创建并启动你的应用程序。

**Overlord** 是一个快速的分布式 FreeBSD jail 编排器，面向 GitOps。你定义一个文件来描述集群上运行的服务，部署时间从几秒到几分钟不等。该工具使用 AppJail、Director，甚至可以用 vm-bhyve 创建虚拟机，但其理念是“用代码部署”，因此你只需创建一个文件即可多次部署。通过树状链式系统，Overlord 能在连接的系统上部署 jails，几乎无限共享它们的资源。更多信息请参阅其 [wiki](https://github.com/DtxdF/Overlord/wiki) 上的相关文章。
...
将 GCC 默认版本从 13 更新到 14 的实验运行被暂停。原因是 FreeBSD 13.4 缺少 GCC 14 链接时使用的一些符号，详情见 [问题报告 284499#c0](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=284499#c0)。不过这些符号在较高版本的 FreeBSD 中已存在。鉴于 FreeBSD 13.4 预计将于六月三十日停止支持，决定暂停实验运行直到那时，并计划在七月一日恢复。
...
[中国 FreeBSD 社区（CFC）](https://bsdcn.org/)
链接：[https://bsdcn.org/](https://bsdcn.org/)

社区成员数：QQ 群 249 人，微信 群 175 人

#### FreeBSD-Ask

链接：
[FreeBSD-Ask GitHub](https://github.com/FreeBSD-Ask/FreeBSD-Ask)
[FreeBSD-Ask 网站](https://book.bsdcn.org/)

联系人：ykla \[[yklaxds@gmail.com](mailto:yklaxds@gmail.com)]，Voosk \[[roisfrank@icloud.com](mailto:roisfrank@icloud.com)]

**FreeBSD-Ask** 是一本由中国 FreeBSD 社区（CFC）的 ykla 发起，用简体中文编写的 FreeBSD 入门指南。**FreeBSD-Ask** 于 2021 年 3 月 14 日成立。
...
* 新增内容：
  ...
  * [sysutils/bsdconfig](https://cgit.freebsd.org/ports/tree/sysutils/bsdconfig/) 系统配置工具
...
* 新增多个 GitHub Actions，如自动生成 PDF、死链检查等

欢迎一如既往地提出反馈和提交补丁。

赞助方：中国 FreeBSD 社区（CFC）
...
默认情况下，该模块对屏幕调光十分积极，在大约一秒无操作后即调暗屏幕。这种行为能立即节能，但用户可根据个人偏好进行调整。模块通过 sysctl 提供设置选项，允许用户定制行为。用户可以分别设置调暗和正常亮度等级，调整无输入信号触发调光的时间长度，并针对笔记本使用电源或电池时应用不同配置。键盘亮度控制键也可调节亮度等级。
