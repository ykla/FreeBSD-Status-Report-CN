...
基金会负责管理 FreeBSD 参与[谷歌编程之夏](https://summerofcode.withgoogle.com/)（Google Summer of Code，GSoC）项目。今年有 [12 份项目](https://wiki.freebsd.org/SummerOfCode2025Projects)被接受。
...
Geomman 是一款基于 [sade(8)](https://man.freebsd.org/cgi/man.cgi?query=sade&sektion=8&format=html) 的新型分区工具，有移动、复制和粘贴分区等功能。Geomman 属于 2025 谷歌编程之夏项目，目前可在 Gitlab 仓库访问，预计后续将成为基本系统工具。
...
现在 Ports 可以启用现代编译程序的一些安全功能以构建软件包。由于并非所有 Ports 都兼容，因此默认未启用。
...
- `WITH_ZEROREGS=yes`：在函数返回时清零调用寄存器，以提高程序安全性，缓解 ROP 攻击、防止寄存器信息泄露。依赖于特定架构的编译程序支持。Python Ports 因当前存在问题而默认禁用。

博客中详细说明了使用方法、排除特定 Ports 的方法，以及这三个新功能与 Ports 和基本系统构建中已有的构建时安全选项的详细解释。
