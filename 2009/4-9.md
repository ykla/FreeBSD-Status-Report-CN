...
- 原文：[FreeBSD Quarterly Status Report 2nd-3rd Quarter 2009](https://www.freebsd.org/status/report-2009-04-2009-09.html)
- 最后作者：FreeBSD 项目

## 引言

本报告涵盖了 2009 年 4 月至 9 月之间与 FreeBSD 相关的项目。在此期间，许多项目开展了大量开发工作，包括谷歌编程之夏项目。BSDCan 大会于 5 月在加拿大渥太华举行，EuroBSDCon 大会于 9 月在英国剑桥举行。两项活动都非常成功。FreeBSD 的新主版本 8.0 即将发布。如果你想了解这次备受期待的发布中有哪些新特性，请阅读 Ivan Voras 的精彩 [总结](http://ivoras.sharanet.org/freebsd/freebsd8.html)。
...
2009 年是 FreeBSD 项目参与谷歌编程之夏的第五年。我们总共有 17 个成功的项目。部分谷歌编程之夏代码将在 FreeBSD 8.0-RELEASE 中发布，其他项目将在未来的版本中集成。

FreeBSD 谷歌编程之夏管理团队感谢谷歌以及我们学生和导师的贡献，感谢他们的努力使这一年取得了成功！

### BSD 许可的 iconv（2009 年谷歌编程之夏）

**链接**：[FreeBSD 上的 BSDL iconv Wiki](http://wiki.freebsd.org/G%C3%A1borSoC2009)

**联系方式**：Gábor Kövesdán <[gabor@FreeBSD.org](mailto:gabor@FreeBSD.org)>

该代码已从 NetBSD 中提取，并转化为独立的共享库。基本的编码已得到良好的支持。几乎所有的正向转换（foo -> UTF-32）都与 GNU 兼容，但反向转换不太准确，因为 GNU 使用了先进的音译方法。一些额外的编码也已添加。目前有两个模块存在段错误（segfault），需要调试。我可以将这个项目作为我的本科学位论文的一部分继续工作，所以希望能够解决剩余的问题。提高 GNU 兼容性也非常重要（**iconv(1)** 的额外命令行选项、`iconvctl()`、私有接口等）。
...
### Ext2fs 状态报告（2009 年编程之夏）

**链接**：[Wiki 页面](http://wiki.freebsd.org/SOC2009AdityaSarawgi)

**联系方式**：Aditya Sarawgi <[sarawgi.aditya@gmail.com](mailto:sarawgi.aditya@gmail.com)>
...
- 在使用 **kvm(3)** 和 **sysctl(3)** 获取信息时提供更好的一致性。
...
有一个示例应用程序，叫做 **nettop(8)**，提供了一个基于 ncurses 的类似 **top(1)** 的接口，用于通过该库监控活动连接和网络缓冲区分配。还创建了一个修改版的 **netstat(1)**，尽可能多地使用 **libnetstat(3)**。

### pefs - 堆叠加密文件系统（2009 年编程之夏）
...
Pefs 是一个内核级文件系统，用于在其他文件系统（如 zfs 或 ufs）之上透明加密文件。它不会向文件中添加额外的信息（与其他文件系统不同），不需要按密码块大小的 IO 操作，支持按目录/文件的密钥和密钥链，使用唯一的每文件调整（tweak）加密。支持的算法有：AES、Camellia、Salsa20。该代码已准备好测试。
...
在过去的一年里，BSD# 团队继续跟踪 Mono 开发，**lang/mono** Port 几乎始终保持最新（我们因为该版本的回归问题，跳过了 mono-2.2）。我们的许多补丁已被合并到 Mono 的主干中，并应该包含在即将发布的 mono-2.6 中。

与此同时，更多与 .NET 相关的 Port 已被更新或添加到 FreeBSD 的 Ports 中。这些 Port 包括：
...
在春季/夏季期间发生了很多变化：amd64 得到了正确的 `mcmodel=kernel` 支持，引入了 `compiler-rt`（为 libgcc 替代铺平了道路），我们进行了两次实验性的 Port 构建，看看 clang 在这些方面的表现。C++ 支持能够解析 `devd.cc`，且没有警告。我们成功使内核在 `-O2` 下工作。FreeBSD 被提升为 LLVM 的官方支持平台。由于这些工作的结果，许多之前无法编译的 FreeBSD 部分现在能够顺利构建。

#### 待办事项

1. FreeBSD 的 `ClangBSD` 分支有些陈旧，已有一段时间没有更新了。
...
### Grand Central Dispatch - FreeBSD 移植

**链接**：[GCD / libdispatch 网页](http://libdispatch.macosforge.org/)

**联系方式**：Robert Watson <[rwatson@FreeBSD.org](mailto:rwatson@FreeBSD.org)>

**联系方式**：Stacey Son <[sson@FreeBSD.org](mailto:sson@FreeBSD.org)>

**联系方式**：libdispatch 邮件列表 <[libdispatch-dev@lists.macosforge.org](mailto:libdispatch-dev@lists.macosforge.org)>

我们已将 libdispatch，即 Apple 的 Grand Central Dispatch 事件和并发框架移植到 FreeBSD：
...
- 将 libdispatch 适配为使用可移植的 POSIX 时间例程

Jordan Hubbard 还为 FreeBSD 准备了一个支持块（blocks）的 clang 编译软件包。使用 clang 编译时，libdispatch 提供基于块和基于函数的回调。

该移植在 9 月份的 FreeBSD 开发者峰会上展示，演示文稿已上传至 devsummit wiki 页面。FreeBSD Port 现在可以在 Ports 中获取。FreeBSD 8.0-RELEASE 发布后，新的 kqueue 原语将被 MFC（合并到主干），以便 libdispatch 可以在 FreeBSD 8.1-RELEASE 上开箱即用。
...
**链接**：[libprocstat 仓库](http://svn.freebsd.org/viewvc/base/projects/libprocstat/)
...
我最近一直在为 FreeBSD 平台编写一项新的调试器。由于各种原因，我使用了一种相对不太为人知的类似 C 的语言 D 来编写它。

到目前为止，我已编写了一个相当有用（尽管还有些粗糙）的命令行调试器，支持 ELF、Dwarf 调试信息以及（目前）32 位的 FreeBSD 和 Linux。该引擎包括解析和评估任意 C 表达式，并提供常见的调试工具，如断点、源代码列出、单步执行等。所有代码都是新的，并且是 BSD 许可证的。目前，该工具支持通过 ptrace 对 i386 目标进行用户态调试，并支持同样的目标的事后核心文件调试。我很快将添加对 amd64 的支持，并可能在以后支持 GDB 的远程调试协议。
...
在 2008 年的谷歌编程之夏中，我实现了对 ZFS 和 UFS 的原生 NFSv4 ACL 支持。大部分代码已合并到 CURRENT 中。NFSv4 ACL 在 ZFS 中已无条件启用，可以使用常用工具，如 **getfacl(1)** 和 **setfacl(1)** 来查看和更改它们。我计划在本月合并剩余部分（UFS 支持）。应该可以将其 MFC（合并到主干），以便在 FreeBSD 8.1-RELEASE 发布时交付。
...
一段时间前，我开始为 FreeBSD 内核编写一项新的驱动程序，名为 **vt(4)**，它基本上是 syscons 的替代品。尽管还有很多工作需要做，但可能有必要提到它的功能（以及它的不足之处）。
...
一些随机特性：它已支持 Unicode（UTF-8）输入和输出，支持 MPSAFE，并且支持每个窗口的可变尺寸图形字体，包含几乎无限的字形（包括粗体和常规体）。

#### 待办事项

1. 需要研究 DRM 的代码库。
2. Syscons 应已迁移到 TERM=xterm，以使在驱动程序之间切换更容易。

### 在 FreeBSD 上运行 VirtualBox
...
**联系方式**：Dennis Herrmann <[dhn@FreeBSD.org](mailto:dhn@FreeBSD.org)>

**联系方式**：Juergen Lock <[nox@FreeBSD.org](mailto:nox@FreeBSD.org)>

**联系方式**：Martin Wilke <[miwi@FreeBSD.org](mailto:miwi@FreeBSD.org)>

VirtualBox 已提交到 Ports，并与 Sun 的最新 trunk 版本同步。几个已知的问题已修复，并且添加了一些新特性：
...
我们继续对提交的 PR 分类，为涉及的内核子系统或用户空间 PR 的手册页添加“标签”。这些标签反过来生成按标签和手册页排序的 PR 列表。

由 Bugbusting 团队推荐供提交者评估的 PR 列表继续增加。此列表包含 PR（大多带有补丁），这些 PR 被认为可能已准备好提交，或者在熟悉特定子系统的提交者手中可能会被轻松解决。我们邀请所有提交者在有空闲时间的情况下查看这个列表，并帮助关闭 PR。
...
1. 根据参加 EuroBSDCon DevSummit 的开发者提出的建议工作。
2. 尝试找到更多提交者来帮助我们关闭已分析过的 PR。
...
我们目前正在为 amd64-6、amd64-7、amd64-8、i386-6、i386-7、i386-8、sparc64-7 和 sparc64-8 构建软件包。i386-9 的初步构建已完成；然而，为了继续在 -9 上构建，我们需要找到一些新机器的托管位置，或者放弃对 -6 的软件包构建。后者的邮件列表讨论引起了相当大的争议。

我们增加了一些新的 i386 机器来加速构建，但这仅仅弥补了部分较老、较慢 i386 节点的磁盘故障。

我们还感谢多位提交者（包括 pgollucci@、gahr@、erwin@、Boris Kochergin 和 Craig Butler）借出的更多软件包构建机器。

portmgr@ 团队也欢迎了新成员 Ion-Mihai Tetcu (itetcu@) 和 Martin Wilke (miwi@)。我们还感谢离任成员 Kirill Ponomarew (krion@) 多年的服务。

Ion-Mihai 花了大量时间工作在系统 `QAT` 上，该系统能够自动进行新提交的质量保证。第二个 tinderbox `QATty` 帮助我们修复了许多问题，特别是涉及自定义 `PREFIX` 和 `LOCALBASE` 设置以及文档包含选项的问题。Ports 对文档功能/非默认配置的符合度将会跟进。

在 pav 和 miwi 的协作下，已完成并提交了二十多个实验性的 Port 运行。

自上次报告以来，我们增加了 5 名新的提交者，2 名旧的提交者重新加入了我们。

#### 待办事项

1. 我们正在尝试建立可以供提交者预测试的 Ports tinderboxes；能够提供机器借用的人应与 Ion-Mihai (itetcu@) 联系，提供硬件和带宽的详细信息。
2. 大多数剩余的 Ports PR 是“现有的 Port/PR 已分配给提交者”。尽管维护者超时策略有助于减少积压，但我们需要做更多工作，将 Port 调整到真正需要的状态。
3. 尽管我们增加了许多维护者，但我们仍然有近 4,700 个无人维护的 Port（例如，见 portsmon 上的列表）。这一比例已降至 22%。我们始终在寻找有意愿的志愿者来接管至少一些无人维护的 Port。此外，amd64 和 sparc64 的软件包更新滞后于 i386，我们需要更多的测试人员来帮助这些平台。
...
在公开测试期间识别出的问题仍在逐步修复。目前我们计划完成另外两次公开测试构建（RC2 和 RC3），然后在 10 月底或 11 月初发布正式版本。
...
我们是 EuroBSDCon 2009 的赞助商，并为 8 名 FreeBSD 开发者和用户提供了旅行资助。我们赞助了位于乌克兰基辅的 Kyiv BSD 2009 会议。我们也是 BSDCan 的赞助商，并资助了 7 名开发者。我们资助了三个新项目，分别是 Ed Schouten 提出的 New Console Driver、Arnar Mar Sig 提出的 AVR32 支持，以及 Rui Paulo 提出的无线网状网络支持（该项目已完成）。我们继续资助 Lawrence Stewart 改进 FreeBSD TCP 堆栈的项目。由 Edward Napierala 提出的使移除已挂载文件系统的磁盘设备变得安全的项目现已完成。

我们在 EuroBSDCon 2009 上表彰了以下 FreeBSD 开发者：Poul-Henning Kamp、Bjoern Zeeb 和 Simon Nielsen。这些开发者获得了限量版 FreeBSD 基金会马甲。
...
我还需要做一次较为入侵的更改，这将允许拥塞控制模块在 TCP 和 SCTP 堆栈之间共享。完成后，我将寻求审查和测试，希望能在 `FreeBSD-CURRENT` 中提交补丁，以便能在 8.1-RELEASE 中回溯。

#### 待办事项

1. 将拥塞控制特定的变量从 TCP 和 SCTP 控制块抽象出来，放入某新结构体中，可以传递给 API，而不是直接传递控制块。
...
在即将发布的 FreeBSD 8.0 版本中，vnets 被视为实验性功能。因此，尚不建议在生产环境中使用它们。尽管如此，很多时间已花费在了完善 vnets 的基础设施上，以便能合并更多的更改，并计划在 8.2 中使其达到生产级别。

如果你想帮助实现这一目标，请随时联系我们，支持或帮助虚拟化如两个防火墙、appletalk、netipx 等尚未虚拟化的部分，并生成回归测试。

### 流控制传输协议（SCTP）

**联系方式**：Randall Stewart <[rrs@FreeBSD](mailto:rrs@FreeBSD)>

SCTP 继续小修小补，并增加了一些新功能。首先，VIMAGE 和 SCTP 已能协同工作。这个目标是在 FreeBSD DevSummit（在英国剑桥举行）上，由 bz@、我的新学员 tuexen@ 和我共同完成的。此外，特拉华大学贡献的不可撤销 SACK 功能已修复，现在启用该功能是安全的（它是一个 sysctl）。如果你在使用带有 CMT（并行多路径传输）的 SCTP，你会想启用这个选项（CMT 也是一个 sysctl）。启用 CMT 后，你将能够向 SCTP 对等方的所有目的地发送数据。

我们欢迎了一名新的学员（不久将成为提交者）加入 FreeBSD。Michael Tuexen 现在是 rrs@ 的学员。Michael 长期以来一直在为 SCTP 工作做出贡献，并且在德国明斯特大学兼职教授（当不做 SCTP 编程时）。

## 内核

### FreeBSD/ZFS

**联系方式**：Pawel Dawidek <[pjd@FreeBSD.org](mailto:pjd@FreeBSD.org)>

我们认为 ZFS 文件系统现在在 FreeBSD 8.0 中已准备好投入生产使用。大多数（如果不是全部）报告的 bug 都已修复，ZFS 不再被标记为实验性。Perforce 中也有持续的工作，旨在将最新的 ZFS 版本（v19）引入 FreeBSD。

#### 待办事项

1. 下载 8.0 版本候选并测试、测试、测试，并将任何问题报告到 [freebsd-fs@FreeBSD.org](mailto:freebsd-fs@FreeBSD.org) 邮件列表。
...
目前正在开展板卡启动工作。我已查看了关于 MIPS 如何提供性能计数器的文档，并将很快开始添加代码。

## 文档

### FreeBSD 荷兰文档项目

**链接**：[荷兰语翻译的当前状态](http://www.freebsd.org/docproj/translations.html#dutch)

**联系方式**：René Ladan <[rene@FreeBSD.org](mailto:rene@FreeBSD.org)>

**联系方式**：Remko Lodder <[remko@FreeBSD.org](mailto:remko@FreeBSD.org)>

目前的翻译（手册和一些文章）与英文版本保持同步。网站的部分内容已翻译，更多的工作正在进行中。
...
**链接**：[Gecko TODO](https://trillian.chruetertee.ch/freebsd-gecko/wiki/TODO)

**联系方式**：Beat Gaetzi <[beat@FreeBSD.org](mailto:beat@FreeBSD.org)>

**联系方式**：Martin Wilke <[miwi@FreeBSD.org](mailto:miwi@FreeBSD.org)>

**联系方式**：Andreas Tobler <[andreast-list@fgznet.ch](mailto:andreast-list@fgznet.ch)>

Andreas Tobler 犯了经典的错误，向我们发送了大量与 powerpc 和 sparc64 相关的补丁。通常的惩罚是给他 Gecko 仓库的提交权限，现已执行。

我们当前在 Ports 中有一些过时的 Port：

- **www/mozilla** 已有 5 年的历史，不再受到上游支持，并且存在许多安全漏洞。我们可以使用 **www/seamonkey** 来替代。
- **www/xulrunner** 被 **www/libxul** 替代。

已在 `pointyhat` 上测试并准备提交的补丁包含以下更改：
...
我们还在处理 Firefox 3.6（Alpha 2）、Thunderbird 3.0（Beta 4）、新的 libxul 1.9.1.3 和 Seamonkey 2.0（Beta 2）Port。所有这些都已提交到我们的 Gecko 仓库。
...
### Portmaster - 帮助用户管理 Ports 的工具

**链接**：[Portmaster](http://dougbarton.us/portmaster.html)
...
### Valgrind 套件在 FreeBSD 上的情况

**链接**：[Valgrind Wiki 页面](http://wiki.freebsd.org/Valgrind)

**联系方式**：Stanislav Sedov <[stas@FreeBSD.org](mailto:stas@FreeBSD.org)>

FreeBSD Ports 中的 Valgrind 套件已更新到 3.5.0 版本（最新可用版本）。大多数以前版本的问题现在应已解决：我们预计 memcheck、callgrind 和 cachegrind 在 i386 和 amd64 平台上都能完全正常工作，并且 i386 二进制文件在 amd64 系统上运行时也应能正常工作。DRD/hellgrind 也应该能正常工作，尽管它们目前会生成很多假阳性，因此它们的输出有些杂乱。
...
**联系方式**：Sam Smith <[eurobsdcon@ukuug.org](mailto:eurobsdcon@ukuug.org)>
...
**链接**：<http://wiki.freebsd.org/200909DevSummit>

**联系方式**：Robert Watson <[rwatson@FreeBSD.org](mailto:rwatson@FreeBSD.org)>

约 70 名 FreeBSD 开发者和嘉宾参加了 FreeBSD 开发者峰会，该峰会在 EuroBSDCon 2009 前于英国剑桥举行。活动在剑桥大学计算机实验室举办，采用了工作坊形式，包括准备好的演示以及小组编程和讨论环节。演讲内容涵盖了包括 802.11 网状网络、虚拟网络堆栈和内核、新的 BSD 许可证调试器、基准测试、缺陷修复、NetFPGA、苹果 GCD（Grand Central Dispatch）移植到 FreeBSD、安全政策工作、加密签名、FreeBSD.org 系统管理、时间极客、新的控制台驱动程序以及 FreeBSD subversion 迁移等主题。许多演讲的幻灯片现已在 wiki 页面上提供。大家都度过了愉快的时光，还一起进行了划船活动！
...
**联系方式**：i18n 邮件列表 <[freebsd-i18n@FreeBSD.org](mailto:freebsd-i18n@FreeBSD.org)>
...
1. 目前系统尚未自托管，因为基本操作系统中缺少类似 iconv 的程序。Gabor@正在作为编程之夏项目开发 bsdiconv，待它被导入，我们将能够从 Unicode 文本格式的定义干净地安装，转化为所需的格式和字符集。
...
论坛成功的一个因素是它们在管理和版主方面采用了“BSD 风格”的方法。论坛有着强大统一的身份，整齐地划分为多个子论坛（如“网络”、“安装与升级”等），版主非常积极，防止垃圾信息，有一群非常活跃且乐于助人的成员，向 FreeBSD 的初学者、中级和专业用户传授了合计数十年的知识。
