...
- 原文：[FreeBSD Quarterly Status Report 1st Quarter 2009](https://www.freebsd.org/status/report-2009-01-2009-03.html)
- 最后作者：FreeBSD 项目
...
**链接**：<http://www.freebsd.org/~jhb/pat/>
...
**链接**：[OpenBSM 网站](http://www.openbsm.org/)
...
自上次测试发布 OpenBSM 1.1 beta 1 以来，已修复 **auditon(2)** 系统调用的 32/64 位兼容性问题。现在在 **audit_control(5)** 中设置了默认的“expire-after”值为 10MB。使用新 API，现在本地 **fcntl(2)** 参数映射到 BSM 线传输版本。已修复 **audit_submit(3)** 手册页。已添加一项新的审计事件类别，用于登录后认证和访问控制事件。
...
*Sysinfo* 是一个 shell 脚本，旨在自动收集系统信息并记录给定主机系统的硬件和软件配置。其目标是为系统操作员提供关于未知 FreeBSD 安装的描述性信息。
...
该工具现在可以通过 [sysutils/sysinfo](http://www.freshports.org/sysutils/sysinfo) Port 安装。
...
### 在 GENERIC 中的 TrustedBSD MAC 框架

**链接**：[TrustedBSD MAC 主页面](http://www.trustedbsd.org/mac.html)

**联系方式**：Robert Watson <[rwatson@FreeBSD.org](mailto:rwatson@FreeBSD.org)>

**联系方式**：TrustedBSD 讨论邮件列表 <[trustedbsd-discuss@TrustedBSD.org](mailto:trustedbsd-discuss@TrustedBSD.org)>

正致力于将 `options MAC` 包含在 8.0 的 `GENERIC` 内核中。这主要包括性能工作，以减少使用策略时的开销，并在没有配置策略时消除开销。迄今为止的工作包括：
...
一项新的 DTrace 提供程序 `dtnfsclient` 已添加到 FreeBSD `8.x` 内核，并将在 7.3 发布之前合并到 `7.x`。以下是可用的探针：
...
- `vfs:namecache:*:*` - VFS 名称缓存 `enter/enter_negative/fullpath_enter/fullpath_hit/fullpath_miss/fullpath_return/lookup_hit/lookup_hit_negative/lookup_miss/purge/purge_negative/purgevfs/zap/zap_negative` 探针
...
我们继续对 PR 分类，并为其添加标签，这些标签对应内核子系统或用户空间 PR 的手册页引用。这些标签会生成按 [标签](http://people.freebsd.org/~linimon/studies/prs/pr_tag_index.html) 和 [手册页](http://people.freebsd.org/~linimon/studies/prs/pr_manpage_index.html) 排序的 PR 列表。
...
Mark Linimon 还继续尝试定义一般性问题并研究可能的新工作流模型，届时将于 BSDCan 上就该主题发表演讲。

BugBusting 团队推荐的 [待评审 PR 列表](http://people.freebsd.org/~linimon/studies/prs/recommended_prs.html) 继续有新 PR 被添加。该列表包含 BugBusting 团队认为可能已准备好直接提交的 PR（大部分带有补丁），或者是可能仅需提交者根据特定子系统知识轻松解决的 PR。所有提交者都被邀请在有空时查看此列表，帮助关闭 PR。

自上次状态报告以来，未解决的错误数量一直徘徊在 5600 左右，虽然随着 7.2 Port 冻结，数量有所上升。

一如既往，欢迎更多的帮助，提交者和非提交者都可以加入我们，在 EFnet 上的 `#freebsd-bugbusters` 频道帮助关闭过时的 PR 或提交有效 PR 的补丁。

#### 待办事项

1. 寻找方法让更多的提交者帮助我们关闭团队已分析过的 PR。
2. 思考一种方式，让提交者仅查看已被“审核”或“确认”的 PR。
3. 为我们已完成的工作以及接下来打算做的事情做更多宣传。
4. 定义新的 PR 类别、分类和状态，以更好地匹配我们的工作流（进行中）。

## 架构

### FreeBSD/powerpc G5 支持
...
如同上次状态报告中宣布的，基于 UltraSPARC III 及更高版本的 sun4u 机器的支持已 MFC 到 `stable/7`（最后缺失的部分是 `r190297`），因此将在即将发布的 7.2-RELEASE 中出现，并且已可以通过 7.2-RC1 测试。此外，从 `r191076` 开始，**machfb(4)** 已修复，可以与 UltraSPARC III 及更高版本兼容，虽然该修复未能在 7.2-RC1 中加入，但将在最终版本中提供。X.Org 7.4 和 Firefox Port 以及其他一些基于 gecko 的 Port（如 Seamonkey）再次修复，能够在 sparc64 上工作并打包，包括在安装了 **creator(4)** 或 **machfb(4)** 驱动的 UltraSPARC III 和 UltraSPARC IIIi 机器上。Sun Cassini/Cassini+ 的驱动程序以及国民半导体 DP83065 Saturn 千兆网卡的驱动程序也在顺利进行中（例如 Fire V440 中的板载网卡及附加卡），在进入 CURRENT 之前最后一个要实现的功能是对 jumbo 帧的支持。
...
已于今年 1 月完成手册的翻译，并与英文版保持同步。此外，五篇文章和传单也已翻译完成。

目前已对网站做了一些初步翻译，但可能需要更多的翻译人员才能完全实现。
...
我们仍然希望翻译 [FDP Primer](http://www.freebsd.org/doc/hu/books/fdp-primer/) 能激励更多人帮助我们的工作。欢迎随时贡献，每一行翻译或反馈都受到欢迎并极为感激。有关如何贡献的更多信息，请阅读项目的 [介绍](http://www.freebsd.org/hu/docproj/hungarian.html)（匈牙利语）。
...
**链接**：[Perforce 仓库](http://perforce.freebsd.org/depotTreeBrowser.cgi?FSPC=//depot/projects/soc2008/gabor_textproc)

**联系方式**：Gábor Kövesdán <[gabor@FreeBSD.org](mailto:gabor@FreeBSD.org)>

目前，grep 已完成，只待 Port 构建测试。已知它基本功能完备，且比 GNU 版本小得多。

至于 sort，完全重写已有一些进展，并且还缺少一些选项。还需要性能测试。

#### 待办事项

1. 在 `pointyhat` 上测试 grep。
2. 完善 sort，添加缺少的功能。
3. 测量 sort 的性能，寻找可能的优化机会。
