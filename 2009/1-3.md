...
### [Clang 替代 GCC 作为基本系统编译程序](https://www.freebsd.org/status/report-2009-01-2009-03.html#Clang-replacing-GCC-in-the-base-system)
...
在过去的 3-4 个月里，我们与 LLVM 开发者一起合作，讨论我们在使用 Clang 编译程序前端时遇到的所有错误和问题。FreeBSD 项目正在考虑用 Clang 替代 GCC 作为系统编译程序。Clang 可以编译 99% 的 FreeBSD 系统，并且可以在 i386/amd64 上编译引导内核，但它仍然存在一些错误，C++ 支持仍不成熟。

Ed 正在维护一个补丁集，用于将 FreeBSD 源代码中的 cc(1) 替换为 Clang 二进制文件，并使用 Clang 编译程序引导几乎所有源代码。
