...
FreeBSD 的 MTRR 代码已针对 Athlon 内存类型范围寄存器中有时出现的异常值进行了增强。这些值的问题曾导致 XFree 4.2 在某些主板上无法运行。实验表明，这些未被文档记录的值可能控制 BIOS/ROM 的映射，或者与 SMM 有关。如果有人能提供这些值的含义，敬请告知，以便我们完成 MTRR 代码的编写。
