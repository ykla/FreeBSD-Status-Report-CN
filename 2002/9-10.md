...
BSDCon 2003 项目委员会邀请您提交原创和创新的论文，内容涉及与 BSD 衍生系统及开源世界相关的话题。感兴趣的主题包括但不限于：
...
在过去的两个月里，GNOME 活动频繁。GNOME 已开始发布即将推出的 GNOME 2.2 桌面开发快照。FreeBSD 的移植工作已开始，并且不再局限于主 Ports，而是在 [MarcusCom CVS 仓库](http://www.marcuscom.com:8080/cgi-bin/cvsweb.cgi)中进行。如果您有兴趣测试新的桌面环境，请按照上述 cvsweb 链接中的说明操作，并检出 `port` 模块。
...
## [PowerPC 移植](https://www.freebsd.org/status/report-2002-09-2002-10.html#PowerPC-Port)
...
发布工程（RE）团队已于 2002 年 10 月 10 日完成并发布了 FreeBSD 4.7 版本。此版本包含了多个基于系统的贡献软件程序的更新，以及 FreeBSD 4.6.2 的所有安全和 bug 修复。4.X 系列的下一个版本将是 FreeBSD 4.8，计划于 2003 年 2 月 1 日发布。
...
在这一初步阶段之后，无线网络支持将扩展以支持工作组、企业和城市（例如网状网络）环境中所需的功能。这将包括全面的电源管理支持、用于在桥接配置中运行多个 AP 的 802.1D生成树协议、QoS 支持和增强的安全协议（LEAP、AES、EAP）。还计划支持新的硬件设备。
