...
这个项目现在终于开始了，多亏了 DARPA 和 NAI 获得了赞助。基础设施代码和数据结构目前正在用户空间模拟环境中成形。基本的 MBR 和 BSD 方法已经编写完毕，已经实现并验证设备 attach/taste/detach 算法。
...
KSE 项目（旨在使用内核支持在 FreeBSD 中支持可扩展线程）已达到我所称之为“里程碑 3”。在此里程碑中，可以在单颗 CPU 上运行多线程程序，但该 CPU 上的线程完全并发。换句话说，内核支持当一个线程阻塞时，允许另一个线程在其位置运行。可以在上述网站上获取此功能的演示测试程序。
...
现在，补丁已发送至邮件列表 <freebsd-arch@FreeBSD.org>，并且未收到任何反对意见，将在近期进行提交。Poul 目前正在自己的源码中测试此补丁。提交后，将开始修改源码中的文件系统，来使用 VFS_NMOUNT 而非 VFS_MOUNT。Mount(8) 也需要进行一些修改。与此同时，正在为 nmount(2) 和 kernel_vmount(9) 创建新的手册页。
