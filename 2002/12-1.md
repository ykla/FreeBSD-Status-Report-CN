...
现在，补丁已发送至邮件列表 <freebsd-arch@FreeBSD.org>，并且未收到任何反对意见，将在近期进行提交。Poul 目前正在自己的源码中测试此补丁。提交后，将开始修改源码中的文件系统，来使用 VFS_NMOUNT 而非 VFS_MOUNT。Mount(8) 也需要进行一些修改。与此同时，正在为 mount(2) 和 kernel_vmount(9) 创建新的手册页。
