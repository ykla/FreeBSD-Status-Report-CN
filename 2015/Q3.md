...
原子操作还提供了屏障操作，允许软件控制现代处理器中乱序执行和推测执行对内存的影响，以及编译程序的优化。这一功能对多线程软件尤为重要，例如 FreeBSD 内核，在多处理器通过共享主内存通信的系统上运行时。
...
由于 x86 处理器实现的强内存模型，atomic_load_acq() 和 atomic_store_rel() 可以通过普通的加载和存储指令实现，仅需要编译程序屏障，不需要额外的排序约束。atomic_store_rel() 的这种简化在 r236456 中完成。atomic_load_acq() 的更改在 r285934 中完成，经过仔细审查内核和用户空间中所有使用的情况，以确保没有遗留依赖于更强实现的情况。
...
一些二级架构平台，如 PowerPC 和 ARM，最近在 FreeBSD 和编译程序方面有了显著改进，但由于该树在集群中的重建不够频繁，因此怀疑许多 Port 虽然标记为 `BROKEN`，但实际上现在能够成功构建和运行。
