...
- 原文：[FreeBSD Quarterly Status Report 3rd Quarter 2015](https://www.freebsd.org/status/report-2015-07-2015-09.html)
- 最后作者：Ben Kaduk
...
基金会再次在 FreeBSD 项目中发挥重要作用，不仅在会议上提供开发者和推广者的支持，还赞助了集群管理团队用于保持系统运行的大部分硬件，并赞助了许多 FreeBSD 的开发项目。本季度，我们还听取了由谷歌编程之夏 2015 赞助的一些学生项目的汇报，这些项目涉及了从启动加载程序到增强 ARM 支持等多个方面，但它们的完成进度差异较大。部分谷歌编程之夏项目的成果已经提交至代码库，而其他一些项目则需要更多关注，以帮助新贡献者在返回课堂后继续推进。
...
- 新的 32 核参考构建主机已上线。
- 内部的 admbugs 从 bugzilla 4.4 升级到 5.0，并为 bugmeister 团队提供了可用的软件包。
...
- 我们每隔几天构建一次内部基本系统快照，每天构建一次软件包。
- 我们还为非集群管理服务提供支持，包括 jenkins、reviews、portsnap、freebsd-update、bugzilla、软件包构建器、git 和 mercurial。这些支持内容从维护 SSL 前端到操作服务器、分发数据或构建运行的软件包/二进制文件不等。
...
### automtud: 更好的 Jumbo 帧支持

**链接**：[jmgurney/automtud 在 GitHub 上](https://github.com/jmgurney/automtud)

**联系方式**：John-Mark Gurney <[jmg@FreeBSD.org](mailto:jmg@FreeBSD.org)>

automtud 脚本能让 FreeBSD 机器向支持 Jumbo 帧的机器发送 Jumbo 帧，同时对其他机器使用正常大小的帧。
...
### Clang、llvm、lldb、compiler-rt 和 libc++ 更新到 3.7.0
...
在第一次 Port exp-run 中，发现了一些重大问题，其中一个是 clang 错误导致在某些情况下 `pow()` 生成浮点异常。这进而导致 libpng 无法构建，另一个问题出现在 libjpeg-turbo 中，由于未定义行为导致的错误。这两个问题花费了一些时间修复，之后进行了第二次 exp-run，这次结果约有十几个新的 Port 失败。对于几乎所有这些新的失败，已经提交了修复，并链接到原始 PR 201377 进行 exp-run。
...
**链接**：[提交替换 TCPDEBUG 的跟踪点](https://svnweb.freebsd.org/changeset/base/287759)
...
### FreeBSD 在 Acer C720 Chromebook 上运行
...
### 将 bhyve 移植到基于 ARM 的平台

**链接**：[项目 Wiki 页面](https://wiki.freebsd.org/SummerOfCode2015/PortingBhyveToArm)
...
**链接**：[用户空间代码审查](https://reviews.freebsd.org/D3693)
...
过去几个月的其他发展包括完全独立的关机支持，不再依赖外部工具集提供的关机命令，以及一整套二进制软件包。从版本 1.20 开始，可以使用预编译的二进制软件包在 FreeBSD 和 Linux 上运行一个完全由 nosh 管理的系统。
...
### Cavium LiquidIO 智能网卡驱动

**链接**：[LiquidIO 产品页面](http://www.cavium.com/LiquidIO_Application_Acceleration_Adapters.html)

**联系方式**：Stanislaw Kardach <[kda@semihalf.com](mailto:kda@semihalf.com)>

**联系方式**：Zyta Racia <[zr@semihalf.com](mailto:zr@semihalf.com)>
...
与此同时，已经开始开发一个包含跨编译的 CloudABI 实用程序和库的 Ports。该框架旨在为多种操作系统生成本地软件包，使得无论操作系统是否实际支持 CloudABI，都可以在任何操作系统上开发 CloudABI 应用程序。
...
3. CloudABI Ports 目前只有 60 个软件包。虽然这些软件包已包含一些有趣软件的构建模块，但我们始终欢迎扩展。
...
**联系方式**：Julien Grall <[julien.grall@citrix.com](mailto:julien.grall@citrix.com)>
...
4. 改进通用的回弹缓冲区代码，支持 blkfront 驱动程序的对齐要求，以处理未映射的 bio。
...
大多数 FreeBSD 的开发和测试仍然发生在 x86 机器上，结合 x86 强序内存模型，这导致了原子操作，特别是屏障的使用错误。换句话说，代码没有按照 FreeBSD 的抽象内存模型正确编写，但 x86 架构的强序性掩盖了这一事实。受到错误使用原子操作影响的架构通常不太流行或可用性有限，且因原子操作误用导致的错误很难诊断。
...
C11 的一个设施是缺失的，那就是 *栅栏*。栅栏是双向的屏障操作，现有的原子 + 屏障访问无法表达它们。它们在 r285283 中被添加。
...
### HiKey ARMv8 开发板上的 FreeBSD

**链接**：

[HiKey 维基条目](https://wiki.freebsd.org/arm64/HiKey)

[硬件说明](https://www.96boards.org/products/ce/hikey/)
...
**链接**：[FreeBSD arm64 维基页面](https://wiki.freebsd.org/arm64)
...
**联系方式**：FreeBSD pkg 团队 <[pkg@FreeBSD.org](mailto:pkg@FreeBSD.org)>
...
- 增加了生成 Graphviz 的 dot 格式表示求解器问题的能力
...
**链接**：[维基百科：SCSI 外设服务 (SES)](https://en.wikipedia.org/wiki/SCSI_Enclosure_Services)
...
- 按照 SES 插槽编号或设备名称闪烁硬盘的定位或故障 LED
- 检查整个 SES 控制器的状态
...
**链接**：[扩展 libkvm 支持跨调试 vmcore](https://reviews.freebsd.org/D3341)

**联系方式**：John Baldwin <[jhb@FreeBSD.org](mailto:jhb@FreeBSD.org)>

支持在 FreeBSD 上跟踪 fork 后的子进程已实现并合并到 GDB 的主分支，并包含在 GDB 7.10 中。

移植 kgdb 到更新版本的 gdb 的工作仍在继续。amd64、i386、powerpc、powerpc64 和 sparc64 后端已经完成移植，现在可以通过 Port devel/gdb 中的新 KGDB 选项使用。
...
2. 找出为什么 powerpc kgdb 目标无法展开初始帧之后的堆栈。
...
**联系方式**：Michael Fausten <[ports@michael-fausten.de](mailto:ports@michael-fausten.de)>
...
GNOME 3.18 已经移植。然而，在提交到 Ports 之前，还需要解决一些问题。
...
截至第三季度末，Ports 中包含超过 25,000 个 Port，PR 数量超过 2,000 个。暑期期间，Ports 的活动比前一季度减少，120 名活跃提交者完成的提交不到 7,000 次。不幸的是，关闭的问题报告数量也显著减少，第三季度修复的问题报告不到 1,500 个。

第三季度，有几个提交者的提交权限被收回保管，其中包括由于超过 18 个月未活跃（fluffy）或提交者请求保留的权限（xmj、stefan、brix）。一名新开发者获得了 Port 提交权限（Jason Unovitch，<junovitch@FreeBSD.org>），并且一名回归提交者（Babak Farrokhi）恢复了提交权限。
...
1. 我们想提醒大家，Ports 是由志愿者建立和维护的，非常感谢任何帮助。这个问题比以往更为紧迫，因为问题报告的数量似乎没有停止增长。所以，如果你使用 Ports 或软件包，请考虑参与进来，提供帮助！对于现有的 Port 维护者来说，如果你能考虑下一步，帮助指导一些新手，传授你对 Ports 的内部知识，那将非常棒。如果你已经在做这些工作，非常感谢你！

### PowerPC 上的 Ports

**联系方式**：Alexey Dokuchaev <[danfe@FreeBSD.org](mailto:danfe@FreeBSD.org)>

与一级架构相比，二级架构上的 Ports 通常收到较少的关注，尽管过去曾在不同时间进行过几次构建，并在当时将出现问题的 Port 标记为有问题。
...
**联系方式**：FreeBSD 翻译团队 <[freebsd-translators@FreeBSD.org](mailto:freebsd-translators@FreeBSD.org)>
...
**链接**：[FreeBSD 主站](https://www.freebsd.org/)
...
### Allwinner A10/A20 支持

**链接**：[Wiki 页面](https://wiki.freebsd.org/FreeBSD/arm/Allwinner)

**联系方式**：Luiz Otavio O Souza <[loos@FreeBSD.org](mailto:loos@FreeBSD.org)>

**联系方式**：Pratik Singhal <[ps06756@gmail.com](mailto:ps06756@gmail.com)>
...
**链接**：[Wiki 页面](https://wiki.freebsd.org/SummerOfCode2015/mtreeParsingLibrary)

**联系方式**：Michal Ratajsky <[michal@FreeBSD.org](mailto:michal@FreeBSD.org)>
...
**链接**：[项目 Wiki 页面](https://wiki.freebsd.org/SummerOfCode2015/MultiqueueTestingProject)
...
**链接**：[Wiki 页面](https://wiki.freebsd.org/SummerOfCode2015/UpdateFiclInBootloader)

**联系方式**：Colin Lord <[clord@FreeBSD.org](mailto:clord@FreeBSD.org)>
...
**联系方式**：Jason Edwards <[jason@zfsguru.com](mailto:jason@zfsguru.com)>
