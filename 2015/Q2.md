...
上个月，我为 FreeBSD Ports 添加了许多软件包。我们现在拥有一个完整的 C/C++ 交叉编译程序，可以非常容易地安装（[devel/cloudabi-toolchain](http://www.freshports.org/devel/cloudabi-toolchain)）。我还导入了工具 cloudabi-run，可以用来安全地启动程序，只授予程序配置文件中列出的文件和网络套接字的访问权限（[sysutils/cloudabi-utils](http://www.freshports.org/sysutils/cloudabi-utils)）。
