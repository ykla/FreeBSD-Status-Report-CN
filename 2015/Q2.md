...
- 原文：[FreeBSD Quarterly Status Report 2nd Quarter 2015](https://www.freebsd.org/status/report-2015-04-2015-06.html)
- 最后作者：Warren Block
...
FreeBSD 集群管理团队负责管理项目依赖的机器，以便其分布式工作和通信能够保持同步。在本季度，团队非常忙碌，进行了大量从 FreeBSD 基础设施外部看来既可见又不可见的工作。
...
我们还随机化了虚拟动态共享对象（VDSO）。VDSO 是内核和用户空间之间共享的一个或多个内存页面。在 amd64 上，它包含信号跳板和定时代码（例如 `gettimeofday(4)`）。
...
**链接**：[bhyve 常见问题与讲座](http://www.bhyve.org/)
...
**链接**：[FreeBSD wiki 上的仿真团队](https://wiki.freebsd.org/Emulation)
...
**链接**：[iser-freebsd 在 GitHub 上](https://github.com/sagigrimberg/iser-freebsd)
...
### FreeBSD 的多路径 TCP (Multipath TCP)

**链接**：[MPTCP 项目网站](http://caia.swin.edu.au/newtcp/mptcp)
...
**链接**：[1-Wire 基础和温度](https://reviews.freebsd.org/D2956)
...
上个月，我为 FreeBSD Ports 添加了许多软件包。我们现在拥有一个完整的 C/C++ 交叉编译程序，可以非常容易地安装（[devel/cloudabi-toolchain](http://www.freshports.org/devel/cloudabi-toolchain)）。我还导入了一个名为 cloudabi-run 的工具，可以用来安全地启动程序，只授予程序配置文件中列出的文件和网络套接字的访问权限（[sysutils/cloudabi-utils](http://www.freshports.org/sysutils/cloudabi-utils)）。
...
**链接**：[提交 r282684](https://svnweb.freebsd.org/base?view=revision&revision=282684)

**联系方式**：Konstantin Belousov <[kib@FreeBSD.org](mailto:kib@FreeBSD.org)>

进程上下文标识符（PCID）是英特尔处理器翻译后备缓冲器（TLB）的一个性能增强特性，随 Sandy Bridge 微架构引入。它允许 TLB 同时缓存多个地址空间的翻译信息，并使操作系统的上下文切换代码能够避免在进程切换时刷新 TLB。每个缓存的翻译都会带有某个上下文标识符，在上下文切换时，操作系统指示处理器哪个上下文将变为活跃。该特性通过避免刷新 TLB，略微减少了上下文切换时间，更重要的是减少了线程在上下文切换后的热身期。
...
**链接**：[提交 r282678](https://svnweb.freebsd.org/base?view=revision&revision=282678)
...
### Warner 的 ARMv6 硬浮点实验

**链接**：[将 armv6 从软浮点转为硬浮点](https://wiki.freebsd.org/armv6tohardfloat)
...
### FreeBSD 在 Cavium ThunderX (arm64) 上的应用
...
**链接**：[FreeBSD arm64 Wiki](https://wiki.freebsd.org/arm64)
...
**链接**：[软件包状态](http://pkg-status.freebsd.org/)
...
本季度还发布了第二个季度分支，即 2015Q2。在此分支上，39 名提交者应用了 305 个补丁，是上个季度更新数量的两倍多。
...
我们的 Ports 开发树仍然保留 Mesa 10.6 的更新，并进行了一些清理和修复。（在上一季度报告中为 10.5。）最初我们计划在 7 月初提交更新，但由于 FreeBSD 8.4-RELEASE 的 EOL 被推迟到 7 月 31 日，因此我们将在 7 月底发布测试人员邀请，更新将在 8 月初提交。当然，用户仍然可以通过使用 mesa-next 分支从 Ports 开发树直接获取并进行测试。
...
本季度对 Port wine-devel 进行了七次更新，该 Port 紧密跟踪上游开发，同时更新了其辅助 Port（wine-gecko-devel 和 wine-mono-devel）：

- 稳定版发布：1.6.2（1 次 Port 修订）
- 开发版发布：1.7.40 至 1.7.46

Port i386-wine-devel 为 FreeBSD 8.4、9.1+、10.1+ 和 CURRENT 构建了 amd64 包。
...
- 为 Port i386-wine 添加对 pkg 1.5 的支持（目前库冲突阻止了支持）。
- 为 Windows 64 位上的 Windows 32 位（WoW64）添加支持：

  - 将 Port i386-wine 简化为仅包含 WoW64 所需的组件。
...
在 BSDCan 前两天举行的开发者峰会期间，举行了一个文档工作组会议。我们讨论了文档团队可以抓住的一些最大机会。
...
**链接**：[闰秒文章](https://www.freebsd.org/doc/en_US.ISO8859-1/articles/leap-seconds/article.html)
...
作为今年谷歌编程之夏的一部分，我们为 libc 添加了 `_FORTIFY_SOURCE` 扩展的支持。此扩展利用 GCC 的 `builtin_object_size` 信息，防止现有代码中的缓冲区溢出。编译程序和 C 库可以有效地检测出一些常见的编程错误。
...
**链接**：[多队列测试项目](https://wiki.freebsd.org/SummerOfCode2015/MultiqueueTestingProject)
...
方式：San Janiyan <[venture37@geeklan.co.uk](mailto:venture37@geeklan.co.uk)>
...
- 项目开发工作
  ...
  - 在参加 BSDCan 2015 时，Glen 与多个开发者和团队就多项事项进行了合作，例如讨论使用 pkg(8) 实现 pkgbase、将 FreeBSD 服务器迁移到基金会为 NYI 设施购买的新机器，并讨论 FreeBSD 构建基础设施的未来增强。
  - 启动了 10.2-RELEASE 发布周期。

### ZFS 对 UEFI 启动/加载器的支持
