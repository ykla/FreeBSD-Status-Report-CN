...
3 月份，讨论了两个可能具有分裂性的议题。是否应当提交并 MFC ZFS ARC 响应性补丁，作为 10.1-RELEASE 性能问题的务实修复，尽管这不是理想的解决方案，且需要进一步重做？是否应停止支持旧版（C89 或更早版本）编译程序在内核代码中的使用，直接采用 C11 标准？从这个最后一点扩展：我们是否应当有一个正式机制来决定哪些系统中的组件已不再使用，并且应何时移除？
