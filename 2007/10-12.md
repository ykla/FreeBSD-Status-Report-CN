...
Juniper Networks 已捐赠了一款通用的 MIPS FreeBSD 平台。此移植不在任何实际硬件上运行，但包含了在理想化的 MIPS 硬件上运行所需的部分。FreeBSD/mips 工作组一直在将当前的基础代码和 Juniper 的代码合并成统一的基础代码。此外，Cavium Networks 也捐赠了支持其多核 mips64r2 平台的代码。这些代码也在合并并清理中。合并后的代码库目前已进入第一次（或许是第二次）调用 `cpu_switch` 之前，但仍会崩溃。该领域的活跃工作仍在进行中。
...
除此之外，软件包的状态比过去一段时间要好。linimon 继续为 portsmon 开发软件包分析工具。
