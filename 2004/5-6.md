...
- 原文：[FreeBSD Quarterly Status Report 2nd Quarter 2004](https://www.freebsd.org/status/report-2004-05-2004-06.html)
- 最后作者：FreeBSD 项目
...
从更严肃的角度看，必须牢记 FreeBSD 5.3 的代码冻结将于 2004 年 8 月 15 日开始。这仅仅是几周后的事情，还有很多工作需要完成。发布的 TODO 列表可以在 <http://www.freebsd.org/releases/5.3R/todo.html> 中找到。如果你在寻找参与发布的方式，这个 TODO 列表中有几个项目是紧急且需要关注的。测试同样非常重要。过去几周，代码树出现了一些稳定性问题，但已经有了可以让每个人继续测试和使用 FreeBSD 的解决方法。我们必须确保 FreeBSD 5.3 是一个坚如磐石的版本，因此每一分贡献都有帮助！

谢谢，

Scott Long

## FreeBSD 的蓝牙协议栈（Netgraph 实现）

**联系方式**：Maksim Yevmenkin <[m_evmenkin@yahoo.com](mailto:m_evmenkin@yahoo.com)>

蓝牙代码已标记为非 i386 特定。现在可以在所有受支持的平台上构建。请帮助测试。除此之外，过去几个月的进展不大。我一直很忙于现实生活中的事务。
...
ALTQ 框架是 KAME 的一部分，已经有 4 年多的历史，并且在 NetBSD 和 OpenBSD 中已经使用了 3 年多。它提供了管理外发数据包的手段，用于实现 QoS 和带宽限制。OpenBSD 开发了一种不同的方式来与 ALTQ 交互，使用 `pf`，KAME 将其采纳为“日常使用的默认方式”。
...
`buf-junta` 项目正在进行，我正在尝试将代码拆分，以便我们获得一个 `struct bufobj`，作为缓冲区缓存对象的句柄和方法承载体。所有的 `vnode` 都包含一个 `bufobj`，但随着文件系统迁移到 GEOM 后端，可能会存在没有关联 `vnode` 的 `bufobj`。该工作仍在进行。
...
**链接**：[Cronyx WAN 适配器](http://www.cronyx.ru/hardware/wan.html)
...
## EuroBSDCon 2004 注册现已开放

**链接**：[EuroBSDCon 2004 官方网站](http://www.eurobsdcon2004.de/)

**联系方式**：Patrick M. Hausen <[hausen@punkt.de](mailto:hausen@punkt.de)>

EuroBSDCon 2004 注册现已开放，会议将于 10 月 29 日至 31 日在德国卡尔斯鲁厄举行。对于在 8 月 15 日之前注册的所有人员，将提供早鸟优惠。有关详细信息，请参阅会议官网。
...
**联系方式**：DOC-BR 讨论组 <[doc@fugspbr.org](mailto:doc@fugspbr.org)>

FreeBSD 巴西文档项目是巴西 FreeBSD 用户组（FUG-BR）致力于将可用文档翻译成葡萄牙语（巴西）的努力。我们很高兴地宣布，我们已完成《手册》和《FDP Primer》的翻译工作，目前正在审阅。这两份文档很快将集成到 FreeBSD 的 CVS 仓库中。

还有许多其他文章正在翻译，你可以在我们的网站上查看它们的状态。如果你想提供帮助，请在 BerliOS 创建一个账户，因为我们的 CVS 仓库托管在那里，并通过我们的邮件列表与我们联系。任何帮助都欢迎！
...
FreeBSD 荷兰文档项目是一个持续进行的项目，旨在将 FreeBSD 手册（及其他文档）翻译成荷兰语。我们仍在寻找翻译人员和愿意检查当前 HTML 文档的人。如果你有兴趣，请通过上述邮件地址与我联系。目前，我们正在做一些检查，然后将第一批文档插入到文档树中。

## FreeBSD 手册，第三版，第二卷：管理员指南

**链接**：[FreeBSD 手册第三版任务列表](http://www.freebsd.org/docproj/handbook3.html)

**联系方式**：Murray Stokely <[murray@FreeBSD.org](mailto:murray@FreeBSD.org)>

FreeBSD 手册的第三版已分为两卷。第一卷，即用户指南，已经发布。第二卷的工作正在进行。第二卷包括以下章节：高级网络、网络服务器、配置、启动、前沿技术、磁盘、本地化、Mac、邮件、PPP 和 SLIP、安全、串行通信、用户、Vinum、电子资源、参考书目、镜像。请参阅任务列表以了解剩余的工作内容。除了技术和语法审查外，还需要纠正文档中关于 HTML 输出的若干假设。

## FreeBSD ports 监控系统

**链接**：[FreeBSD ports 监控系统](http://portsmon.firepipe.net/index.html)
...
由于从 `bento` 到 `pointyhat` 的过渡，修复了若干个错误。在大约两周的时间里，构建错误的链接是错误的。现已修正（但请注意，某些 `pointyhat` 汇总页面仍然显示断开的链接）。
...
- 我们在已分配给 FreeBSD 提交者的现有 Port PR 数量上取得了一些进展，从 400 个减少到大约 350 个。这部分是由于一些提交者清理了数据库，将旧 PR 设置为“反馈”状态，然后再触发上述的“维护者超时”规则。（在某些情况下，PR 现在已经太旧，不再适用，这些会直接关闭。）
...
FreeBSD profile.sh 是对 FreeBSD 5 `rcng` 启动系统的增强，专为笔记本电脑设计。用户可以配置多个网络环境（例如，家庭、工作、大学）。在初始配置完成后，笔记本会自动检测启动时所在的环境，并相应地配置。不仅网络设置，几乎 **/etc** 下的所有内容都可以按环境配置。还可以在一个环境中挂起机器，并在另一个环境中唤醒，重新配置将自动发生。

## FreeBSD/arm

**联系方式**：Olivier Houchard <[cognet@FreeBSD.org](mailto:cognet@FreeBSD.org)>

没有太多报告，Xscale 支持正在进行，并且应该很快能在 Intel IQ31244 评估板上至少启动单用户模式。
...
在过去的两个月里，出现了许多可以对 FreeBSD/MIPS 做大量工作的机会，FreeBSD/MIPS 中的一些重大问题，如上下文切换、时钟、中断和内核虚拟内存，已得到解决。修复了一些与缓存相关的问题，但这些工作远未完成，最后检查时，运行缓存时出现了一些问题，可能会导致无法启动。由于工具链的问题，当前的内核在真实硬件上已经无法启动。

一个 64 位的 MIPS 模拟器已经出现，提供了在模拟器中测试和调试的能力，并且做了大量测试。它已加入到 FreeBSD 的 Ports 中，且该 Port 将尽可能积极地跟踪主代码库。一般来说，FreeBSD/MIPS 内核应该能在其中良好运行。

在工具链和缓存问题之前，第一个内核线程能够运行，总线和一些设备能够附加，系统会启动到 `mountroot` 提示符。

## HP Network Scanjet 5

**链接**：[HP Network Scanjet 5 运行 FreeBSD](http://berklix.com/scanjet/)

**联系方式**：Julian Stacey <[jhs@FreeBSD.org](mailto:jhs@FreeBSD.org)>

HP Network Scanjet 5 可以在扫描仪内部不引人注意地运行 FreeBSD。那些在工作中怀念 Unix 的人可以拥有一台 FreeBSD 主机，不被那些限制大楼内任何非微软 PC 的狭隘经理注意和阻拦。<http://berklix.com/scanjet/>
...
现在，可以通过 `loader` 可调参数在启动时启用或禁用混合模式。

## 改进的多字节/宽字符支持

**联系方式**：Tim Robbins <[tjr@FreeBSD.org](mailto:tjr@FreeBSD.org)>

FreeBSD 基本系统中的许多文本处理工具已更新，以支持多字节字符，包括 `comm`、`cut`、`expand`、`fold`、`join`、`paste`、`unexpand` 和 `uniq`。新版本的 GNU `grep` 和 GNU `sort`（来自 `coreutils`）已导入，并且来自 IBM 和 Red Hat 开发人员的多字节支持补丁也已应用。

未来的工作将集中在修改正则表达式函数以支持多字节字符、提高 C 库例程的性能，以及更新其余的工具（`sed` 和 `tr` 是两个仍然需要更新的重要工具）。

## IPFilter 升级到 3.4.35

**链接**：[IPFilter 官方主页](http://coombs.anu.edu.au/~avalon/ip-filter.html)

**联系方式**：Darren Reed <[darrenr@FreeBSD.org](mailto:darrenr@FreeBSD.org)>
...
**联系方式**：Michael Nottebrock <[lofi@FreeBSD.org](mailto:lofi@FreeBSD.org)>

目前，正在将主要 KDE 模块的 Port 中存在的构建开关/选项转换为独立的 Port，以便为它们提供的软件/功能提供软件包。KOffice 1.3.2 的移植工作几乎完成。由 KDE/FreeBSD 团队成员运营和维护的瑞典 FreeBSD 快照服务器 <http://snapshots.se.freebsd.org/> 已恢复运行，且运行良好。额外的 amd64 硬件已添加，amd64 快照将在不久后提供。
...
KGI 的进展较慢，但稳定。`KGI/Linux` 加速器的移植工作正在进行。这不过是一个双缓冲 API，用于将图形命令传递给硬件引擎。
...
**链接**：[FreeBSD 上的最佳性能监控系统，建立在现代 CPU 的硬件性能监控功能之上](http://people.freebsd.org/~jkoshy/projects/perf-measurement/)

**联系方式**：Joseph Koshy <[jkoshy@FreeBSD.org](mailto:jkoshy@FreeBSD.org)>

当前设计尝试支持每个进程和全系统范围的统计分析以及每个进程的“虚拟”性能计数器。用户空间 API **libpmc(3)** 已相当稳定，但内核模块的设计正在重新设计，以更好地处理多处理器（MP）。初步开发的目标是支持 AMD Athlon CPU，但意图是支持所有 FreeBSD 支持的 CPU。

一个早期原型已在 Perforce 上提供，路径为 **//depot/user/jkoshy/projects/pmc/**。

## 网络接口命名变化

**联系方式**：Brooks Davis <[brooks@FreeBSD.org](mailto:brooks@FreeBSD.org)>

已提交增强的网络接口克隆 API。它允许接口支持比当前的 `name#` 风格更复杂的名称。这一功能已用于启用一些有趣的克隆器，如自动配置的 `vlan` 接口。其他功能包括克隆结构的锁定以及驱动程序拒绝销毁请求的能力。

有关该功能的用户空间支持工作仍在进行。

## 网络栈锁定

**链接**：[Netperf 网页](http://www.watson.org/~robert/freebsd/netperf/)
...
工作继续推进，旨在提高网络栈锁定（和性能）的成熟度和完整性，特别是针对 5.3 版本。网络栈开发分支已更新到最新的 CVS `HEAD`，以及以下内容和其他更多改动。报告编写时，这些更改中的许多已合并到 FreeBSD CVS 代码树中。完整的详细信息和更多小改动记录在 Netperf 网页的 `README` 文件中。

- 添加了针对套接字相关锁、路由锁、接口锁、文件描述符锁、`SLIP` 以及各种协议（UDP、TCP、UNIX 域套接字）的 `PCB` 锁的硬编码 `WITNESS` 锁定顺序（已合并）。
...
- 清理了与文件描述符关闭和 `Giant` 要求相关的锁定，尝试减少此处的锁定。
...
- 通过添加 `MSG_NBIO` 解决了在 `Fifofs` 中非原子设置 `SS_NBIO` 相关的问题（已合并）。
...
- 重写了 `soreceive()` 和 `sosend()` 部分，以提高 MP 安全性，已合并到 CVS 中，包括修改以确保在释放锁时套接字缓冲区缓存状态一致。添加了 `sockbuf_pushsync()` 以确保缓存指针的一致性（已合并）。
...
- 清理了 `IP` 栈管理 I/O 控制和锁定顺序问题（已合并）。
- 清理并注释了 `sorflush()` 在刷新时使用临时栈持有的套接字缓冲区的做法（已合并）。
- 大规模清理了套接字唤醒机制，以提前释放锁，避免在上行调用时持有锁，并断言锁定状态正确（已合并）。
...
- 重构了 `soreserve()` 和 `sbreserve()`，以改善锁定和一致性。`tcp_mss()` 中的保留函数也做了类似的清理（已合并）。
- 在 `sbappend*()` 中减少了锁定成本（已合并）。
- 为多个 `Netgraph` 模块添加了全局锁定，包括 `ng_iface`、`ng_ppp`、`ng_socket`、`ng_pppoe`、`ng_frame_relay`、`ng_tty`、`ng_eiface`（已合并）。
- IPv6 `inpcb` 锁定。由此产生的 `inpcb` 锁定断言清理，并在默认情况下启用 `inpcb` 锁定断言，即使已编译 IPv6。
- `if_xl` 现在是 `MPSAFE`（已合并）。
- `soreceive()` 非内联 OOB 支持已放入自己的函数中（已合并）。
- NFS 客户端套接字锁定（已合并）。
- `SLIP` 现在使用异步任务队列来防止 TTY 代码的 `Giant`-free 进入。
- 向 `current@` 发送了电子邮件，提供了 `Giant`-free 操作指南和详细信息。

## 数据包过滤器 - pf

**链接**：[pf 首页](http://www.benzedrine.cx/pf.html)
...
此移植项目稳步推进。已有报告显示 `buildworld` 可以原生运行。`make release` 的工作几乎完成，未来不久将会发布可启动的 CD 镜像。
...
除了在网络栈锁定工作方面（这是一项大规模的工作），SMPng 方面没有太多进展。已修正了各个软件中断线程的优先级，并改进了任务队列的锁定。`sema_timedwait()` 函数的返回值也做了调整，使其与 `cv_timedwait()` 更一致。`sleepqueue` 代码也做了小幅修复，以缩短唤醒线程时 `sleepqueue` 链锁持有的时间。为用于 `sleep` 队列和 `turnstile` 代码的哈希表性能分析添加了一些简单的调试代码。这将允许开发者衡量任何对哈希表大小或哈希算法调整的影响。

## 同步协议（Netgraph 和 SPPP）

**链接**：[当前代码、想法、问题](http://www.freebsd.org/~rik)

**联系方式**：Roman Kurakin <[rik@FreeBSD.org](mailto:rik@FreeBSD.org)>

目前我在两个方向上开展工作：`if_spppfr.c` 和 `sppp` 锁定（代表 `netperf`）。在撰写此报告时，`sppp` 锁定尚未完成，但预计几天内会完成。作为这项工作的一部分，你还可以看到一些针对 `rwatson` `netperf` 代码的用户空间修复（仅限我在编译 `world` 时能够捕获的部分。如果你知道其他问题，请告诉我，我会尽力修复）。
...
将 `tty` 子系统从 `Giant` 下解耦的努力，已转变为一项更广泛的工作，旨在消除许多错误地复制和粘贴到设备驱动程序中的代码。在理想情况下，`tty` 驱动程序永远不应接触 `cdevsw`，但由于某些驱动程序不仅仅是 `tty` 驱动程序（例如同步驱动程序），因此必须达成一个更合理的妥协。该工作正在进行。

## 各种 GEOM 类和 geom(8) 工具

**联系方式**：Pawel Jakub Dawidek <[pjd@FreeBSD.org](mailto:pjd@FreeBSD.org)>

我正在处理各种 GEOM 类。其中一些已经提交并准备好使用（`GATE`、`CONCAT`、`STRIPE`、`LABEL`、`NOP`）。`MIRROR` 类已经完成了 90%，很快会提交。接下来，我将开始处理 `RAID3` 和 `RAID5` 的实现。用于控制 GEOM 类的用户空间工具（**geom(8)**）已经在代码树中。
...
`portaudit` 工具目前是 FreeBSD 的一个附加组件，旨在让管理员和用户提前了解第三方软件中的安全漏洞。VuXML 数据库记录了这些安全漏洞以及内部的安全漏洞。安装后，`portaudit` 工具会定期下载一个已知问题的数据库，并检查所有已安装的 Port 或软件包；如果发现安装了有漏洞的软件，管理员或用户将在 `periodic` 脚本的每日运行输出中收到通知。

这些工具被认为是生产级别的，当前正在讨论是否应将它们纳入基本系统。所有 Port 提交者均应在发现漏洞时添加条目；任何问题可以发送到 `eik@` 或我本人。
