...
2004 年继续取得显著进展。网络栈锁定工作继续进行，ACPI（高级配置与电源接口）取得了巨大进展，ARM 平台出现在代码库中，FreeBSD 4.10 发布周期也已完成。一旦 4.10 发布，接下来的重点将是 FreeBSD 5.3。我们预计这将是 5-STABLE 分支的开始，这意味着它不仅会稳定适用于生产环境，还将在内部 API 方面大体上完成并稳定。我们预计将在夏季中旬发布 5.3 版，并鼓励大家下载最新的快照版本以预览。
...
## [ACPI](https://www.freebsd.org/status/report-2004-03-2004-04.html#ACPI)
 ...
 [ACPI TODO](http://www.root.org/~nate/freebsd/ "http://www.root.org/~nate/freebsd/")   
...
当前的 TCP 发送和接收缓冲区是静态的，并设置为保守的值，以节省内核内存。这对于带宽和延迟积较高的连接来说是次优的，因为 TCP 发送缓冲区的大小决定了发送窗口的最大值。对于高带宽的跨洲链接，这严重限制了每个 TCP 连接的最大传输速度。有关此问题的更详细描述，请参见上面的链接。
...
进展情况在 FreeBSD Perforce 服务器的 camlock 分支中进行跟踪。一旦进展到足以进行合理测试的阶段，我将公开发布补丁。目前，前两个项目正在进行中。
...
**链接**
 ...
 [文档的状态和下载（尚未完成）](http://www.evilcoder.org/index.cgi?i=nav&t=freebsd "http://www.evilcoder.org/index.cgi?i=nav&t=freebsd") 
...
**链接**
 ...
 [关于 TLS 的基本数据](http://people.freebsd.org/~marcel/tls.html "http://people.freebsd.org/~marcel/tls.html") 
...
新地区设置：Unicode UTF-8 地区设置已被添加到基础系统中。所有 FreeBSD 先前支持的地区设置现在都具有对应的 UTF-8 版本，并且还新增了一到两个新的地区设置，总共是 53 个。
...
经过 Martin Blapp（mbr@FreeBSD.org）和其他贡献者近三年的努力，OpenOffice.org 1.0.x 和 1.1.0 已成功移植到 FreeBSD，现在在端口树中有四个版本的 OpenOffice.org（OOo）。  
...
我的主要构建环境是 5.2.1-RELEASE，且我没有访问 4 系列的权限，因此已报告了一些针对 5-current 和 4-stable 的构建问题，但这些问题现在似乎已经解决。请确保您的 Java 和/或内核是最新的。
...
此补丁适用于 if_wi 当前版本。它使您能够禁用 SSID 广播，并且还允许您禁用客户端使用空白 SSID 进行连接。
