...
- 原文：[FreeBSD Quarterly Status Report 2nd-3rd Quarter 2004](https://www.freebsd.org/status/report-2004-07-2004-12.html)
- 最后作者：FreeBSD 项目
...
**链接**：<http://people.freebsd.org/~mlaier/CARP/>

**联系方式**：Max Laier <[mlaier@FreeBSD.org](mailto:mlaier@FreeBSD.org)>

CARP 是 VRRP 的替代方案。与 VRRP 不同，它完全支持 IPv6，并使用加密技术来保护通告信息。OpenBSD 开发 CARP 的初衷是担心 VRRP 可能涉及 HSRP 专利，而 CISCO 可能会维护其专利权。从那时起，CARP 在 VRRP 的基础上做了许多改进。
...
**链接**：[网络协议栈清理项目](http://www.freebsd.org/projects/dingo/index.html)

**联系方式**：George Neville-Neil <[gnn@FreeBSD.org](mailto:gnn@FreeBSD.org)>

在过去的一个月里，我们建立了上述项目页面，并创建了一个 `p4` 分支，供我们在 `CVS` 之外用 `p4` 做开发工作。

### FreeBSD profile.sh

**链接**：[FreeBSD profile.sh 站点](https://projects.fsck.ch/profile)

**联系方式**：Tobias Roth <[ports@fsck.ch](mailto:ports@fsck.ch)>

FreeBSD profile.sh 主要面向笔记本电脑。它能让用户定义多个网络环境（例如家庭、工作场所），并可自动检测笔记本电脑启动时所处的环境，并据此相应配置。系统中几乎所有位于 **/etc** 下的配置都可以按环境设定，用户只需定义与默认 **/etc** 配置不同的部分。此外，它还支持在一个环境中挂起（suspend）并在另一个环境中恢复（resume）。

目前正在妥善集成到 ACPI/APM 中，以及一些小的改进。此外，还需要针对不同的系统配置做更多测试。

### FreeBSD 发行工程

**链接**：<http://www.freebsd.org/releng>

**联系方式**：Scott Long <[re@FreeBSD.org](mailto:re@FreeBSD.org)>
...
与此同时，FreeBSD 4.11 的发行工程也在进行。这将是 4.x 系列的最后一个版本，主要包含增量的 bug 修复和少量功能更新。其中值得注意的是，IBM ServeRAID“IPS”驱动现已支持 4.x，并将包含在此版本中。此外，Linux 模拟层已更新，以支持 RedHat 8.0 用户空间。预计该版本将在 2005 年 1 月 24 日发布。

展望未来，5.x 系列将在 2005 年迎来多个新版本。FreeBSD 5.4 版本的发行工程将于 3 月开始，而 5.5 版本的工程预计将在 6 月启动。这些版本将比以往的 5.x 版本更加保守，并遵循以往 `-STABLE` 分支的理念，即在保持 API 稳定性的前提下修复 bug 并做增量改进。

对于 `6-CURRENT` 开发分支以及所有未来的开发和稳定分支，我们计划采用固定时间表，以避免 5.x 版本中出现的进度不确定性和频繁的日程变动。这意味着主要分支将以 18 个月为周期发布，而在这些分支上的版本将以 4 个月为间隔发布。此外，在每个分支的初期，我们会做专门的测试和 bug 修复，然后再发布该分支的首个版本。通过缩短并明确发布周期，我们希望减少因版本延迟导致的新功能无法及时交付给用户的问题，这种情况在 5.x 版本中发生得过于频繁。这一策略变更具有重要意义，我们期待它带来的好处。该计划将从 2005 年 6 月的 `RELENG_6` 分支启动，随后在 2005 年 8 月发布 FreeBSD 6.0。

此外，我们计划合并原本用于 Live-ISO 的 `disk2` 和安装发行版的 `disk1`，将它们整合为一个既可用于安装，也可用于系统恢复的光盘。原本存放在 `disk1` 上的第三方软件包将转移至新的 `disk2`，使其专用于软件包存储。这一调整不仅能更好地应对不断增长的软件包体积，还能为希望添加自定义软件包的厂商提供更大的灵活性。同时，这也为未来用更先进的安装程序替换 `sysinstall` 打开了大门。如果有兴趣参与该项目的开发，请随时与我们联系。

### 无线网络支持

**联系方式**：Sam Leffler <[sam@FreeBSD.org](mailto:sam@FreeBSD.org)>

无线网络层已更新，以支持 802.1x、WPA 和 802.11i 安全协议，以及 WME/WMM 多媒体协议。作为此项工作的一部分，为加密方法、认证和访问控制添加了可扩展框架。扩展以可加载内核模块的形式实现，挂钩到 `net80211` 层。这一机制已用于实现 WEP、TKIP 和 CCMP 加密协议。Atheros 驱动（`ath`）目前是唯一使用完整功能集的驱动。为其他驱动添加支持很简单，但正在等待志愿者。`wpa_supplicant` 和 `hostapd` 程序的移植使得使用新的安全协议成为可能。

重构了对在 `bss`（受管或 adhoc）中跟踪站点以及扫描时发现的站点的支持。现在使用多个表，每个表具有不同的管理策略，引用计数现在保持一致，非活动处理更加智能（例如，关联的站点在移除前会被探测）。这是迈向正确的漫游支持和其他高级功能的第一步。

添加了 AP 省电支持。关联的站点现在可以在省电模式下运行；发送给它们的帧将在其休眠时缓冲，而多播流量将延迟到下一个信标之后（根据 802.11 协议）。省电支持在符合标准的接入点中是必需的。目前只有 `ath` 驱动实现了省电支持。

正在积极添加 Atheros 的 SuperG 功能、WDS，以及在单个设备上支持多 `bss`（`ssid` 和/或 `bssid`）。

#### 待办事项

1. `ath` 以外的驱动需要更新以支持新的安全协议。
2. `hostapd` 需要改进以支持 IAPP 和 802.11i 预认证协议（这些是对现有 Linux 代码的简单转换）。
...
**联系方式**：FreeSBIE 团队 <[staff@FreeSBIE.org](mailto:staff@FreeSBIE.org)>
...
**联系方式**：Tim Robbins <[tjr@FreeBSD.org](mailto:tjr@FreeBSD.org)>

多字节字符支持已扩展到 FreeBSD 基本系统中的多个工具，包括 `basename`、`col`、`colcrt`、`colrm`、`column`、`fmt`、`look`、`nl`、`od`、`rev`、`sed`、`tr` 和 `ul`。此外，由于 C 语言库的改进（详见下文），大多数涉及正则表达式匹配或路径通配的工具现在也支持多字节字符。
...
### Project Frenzy（基于 FreeBSD 的 Live-CD）
...
**联系方式**：Sergei Mozhaisky <[technix@ukrpost.com.ua](mailto:technix@ukrpost.com.ua)>
...
下一个版本的计划任务包括：编写硬盘安装脚本、开发统一的系统配置工具、更新软件集合。
...
`FreeBSD Update`——我开发的安全高效的二进制更新工具（用于安全/勘误分支的跟踪）—— 仍广泛使用，目前每天约有 100 台机器下载安全或修正补丁。
...
**联系方式**：Simon L. Nielsen <[simon@FreeBSD.org](mailto:simon@FreeBSD.org)>

**联系方式**：Christian Brueffer <[brueffer@FreeBSD.org](mailto:brueffer@FreeBSD.org)>

FreeBSD 硬件支持文档现已（大部分）改为直接从驱动程序的手册页自动生成。这大大简化了维护工作，使硬件支持列表更加准确。FreeBSD 5.3 的硬件支持文档已采用这一新系统生成。
...
FreeBSD 荷兰语文档项目是一项持续进行的项目，旨在将 FreeBSD 文档翻译为荷兰语。目前，我们的主要精力集中在《手册》的翻译上，进展顺利。然而，在完成“完整”翻译之前，仍有大量内容需要翻译和校对。因此，如果你愿意提供帮助，请访问我们的网站或直接联系我。
...
**联系方式**：Søren Schmidt <[sos@FreeBSD.org](mailto:sos@FreeBSD.org)>

ATA 驱动正在经历多项重要变更，主要是将其模块化，使其可以随时加载或卸载，并且用户只需保留所需的功能模块。

这次重构也促使 `ata-raid` 彻底重写了。目前，BIOS 预定义的 RAID 阵列元数据读取功能已基本完成，并且支持了更多新的元数据格式。这也为 `ataraid` 最终能够利用某些新控制器的 RAID 功能铺平了道路。然而，这仍需要进一步开发，但至少现在已经具备了实现的可能性。
...
`ath` 驱动已更新，以支持 `net80211` 层新增的所有功能。作为此项工作的一部分，引入了新版本的硬件访问层（HAL）模块；该版本支持 PCI 和 Cardbus 产品中所有可用的 Atheros 部件。此外，adhoc 模式现在应该可用了，天线管理得到了显著改进，软 LED 支持现在可以识别流量模式。

发送速率控制算法已从驱动程序中分离出来，成为一个独立的模块。目前有两种不同的算法可用，未来（希望）会添加更多算法。

正在积极开发中，以添加 Atheros 的 SuperG 功能。

### 新 DHCP 客户端

**联系方式**：Sam Leffler <[sam@FreeBSD.org](mailto:sam@FreeBSD.org)>

OpenBSD 的 `dhcp` 客户端程序已移植并增强，可以监听来自内核的 802.11 相关事件。这使得在漫游时能够立即获取 IP 地址（不同于旧代码的轮询方式）。与之前客户端的主要区别在于，现在每个接口有一个 `dhclient` 进程，而不是整个系统一个。这需要更改系统启动脚本。

将其纳入基本系统正在等待一位志愿者，该志愿者将负责引导这些更改进入代码树并处理 bug。
...
**联系方式**：Nicholas Souchu <[nsouch@FreeBSD.org](mailto:nsouch@FreeBSD.org)>

该项目近期较为沉寂（但仍在进行！），主要依靠志愿者测试。新的文档可在 [http://wiki.daemon.li/moin.cgi/KGI](http://wiki.daemon.li/moin.cgi/KGI) 找到。
...
**链接**：[基于现代 CPU 硬件性能监控功能的 FreeBSD 高级性能监控系统](http://people.freebsd.org/~jkoshy/projects/perf-measurement/)
...
**联系方式**：Qing Li <[qingli@speackeasy.net](mailto:qingli@speackeasy.net)>

目前，**ARP**（IP 地址到 MAC 地址的映射）存储在 **路由表（FIB）** 中，但它本不应该存在于此。因此，我们计划将其移至 **独立的基于哈希的结构**，并在每个 **802.1 广播域** 中实例化该结构。

这一变更将使多个网络接口能够共享同一个 **IP 子网** 和 **第二层广播域**，同时大幅简化 **ARP 处理** 和 **路由表管理**。此外，该改动还将引入 **基于 MAC 地址的完整统计** 功能。
...
由于 Netperf 项目在 2004 年 7 月至 12 月期间做了大量工作，本文仅能涵盖部分内容，更多详细信息可通过上述链接获取。主要成果如下：
...
- **多个 UDP 相关错误修复**（已合并）
- **多个网络接口标记为 MPSAFE**
- **Unix 域套接字加锁重构**，以保护 so_pcb 指针
- **MP_WATCHDOG 机制**，将额外的 HTT 逻辑 CPU 用作看门狗 CPU（已合并）
- **Unix 域套接字加锁注释合并至 FreeBSD CVS**
- **kqueue 加锁**（由 John-Mark Gurney 开发并合并）
- **netinet6 加锁任务列表创建**
- **消除 kqueue 和套接字缓冲区相关的条件加锁**（已合并）
- **NFS 服务器加锁 bug 修复**（已合并）
- **IPv6 代码 in6_prefix 由 George Neville-Neil 移除**，降低 netinet6 的工作负载（已合并）
- **移除 netinet6 中未使用的随机 tick 代码**（已合并）
- **ng_tty、IPX、KAME IPSEC 现在使用 NET_NEEDS_GIANT 编译时标记**，允许内核检测不安全组件并在需要时自动获取 Giant 锁（已合并）
- **熵收集代码的额外加锁优化**（已合并）
...
- **TCP 计时器加锁、注释、断言**（已合并）
- **IP 套接字选项加锁及清理**（已合并）
- **Netgraph ISR 标记为 MPSAFE**
- **Netatalk ISR 标记为 MPSAFE**（已合并）
- **部分接口列表加锁清理**（已合并）
...
- **FreeBSD 项目 Netperf 页面启动**
- **性能微基准测试显示**，5.x 的 IP 转发延迟在 UP 上结合可选的网络栈直接派发时明显优于 4.x
- **多个 NFS 服务器加锁 bug 修复**（已合并）
- **新 mbufqueue 原语开发，优化网络协议栈与设备驱动之间的交互**（与 Sandvine, Inc 合作）
- **Linux TCP_INFO API 添加**，支持用户态 TCP 状态监控（已合并）
- **SMPng 任务列表更新**
- **UDP 静态/全局修复合并至 RELENG_5**
...
- **socket/pipe 等的无 Giant 关闭合并至 FreeBSD CVS**
- **优化 netisr 处理 mbuf 队列的批量出队**
- **netrate 测试工具合并至 RELENG_5**
- **TCP 加锁修复合并至 RELENG_5**
- **DDB 增加 `show alllocks` 命令**（已合并）
- **IPX 加锁 bug 修复**（已合并）
- **IPX/SPX __packed 修复**（已合并）
- **IPX/SPX 移至 queue(9)**（已合并）
- **TCP 加锁修复和注释合并至 FreeBSD CVS**
- **IPX/SPX 全局变量和 pcb 加锁**（已合并）
- **IPX/SPX 标记为 MPSAFE**（已合并）
- **IP 套接字选项加锁合并至 FreeBSD**
- **SPPP 代码加锁（由 Roman Kurakin 完成）**（已合并）
- **Unix 域套接字加锁修复（由 Alan Cox 完成）**（已合并）

该项目仍在持续推进，包括 **netinet6** 进一步加锁、**mbuf 队列** 设施和操作、基准测试、为 **UMA per-CPU 缓存** 转向临界区或 per-CPU 互斥锁、为 **malloc(9)** 统计转向临界区或 per-CPU 互斥锁、消除单独的 mbuf 分配器统计、额外的接口加锁、大量清理和加锁文档，以及广泛的优化。

### 新的模块化输入设备层

**链接**：<http://lists.freebsd.org/pipermail/cvs-src/2004-November/035462.html>

**联系方式**：Philip Paeps <[philip@FreeBSD.org](mailto:philip@FreeBSD.org)>

在多个邮件列表上做了一系列讨论后，FreeBSD 正在开发新的模块化输入设备层。其目标主要有两个：
...
FreeBSD 网络栈内部发生了大量变更，具体情况可能会在另一份报告中介绍。除此之外，内核其他部分也做了一些改动，包括 `proc` 结构的锁定调整、内核线程调度器变为可抢占式、修复调度器中的多个优先级反转问题，以及对 `mutex` 实现做一些性能优化。
...
内核线程调度器现已支持线程抢占。当一个线程使另一个线程变为可运行状态时，如果新线程的优先级更高，则当前线程可能会让出 CPU 以执行该线程。之前，只有中断线程可以抢占其他线程，并且原有实现偶尔会触发错误的上下文切换。此更改已暴露出内核其他部分的 Bug，因此在 `RELENG_5` 分支中默认关闭。目前，只有 `i386`、`amd64` 和 `alpha` 平台支持原生抢占式调度。

此外，还修复了一些由于 SMPng 项目修改内核而导致的调度器中的优先级反转 Bug。其中大部分修复工作由新提交者 Stephan Uphoff 完成，包括：修复 `turnstile` 唤醒代码中的竞争条件、修改 `sleep queue` 代码以按 FIFO 顺序存储线程，使 `sleep queue` 唤醒代码能够正确处理线程优先级变更，并抽象出优先级借用机制，使调度器能够正确跟踪优先级继承，并处理 `turnstile` 阻塞线程的优先级变更。

正在进行的工作包括将临界区与自旋锁进一步分离，以降低临界区的开销，同时继续调整各种 ABI 兼容层，使其使用内核版本的系统调用，从而减少 `stackgap` 使用并使系统调用封装 MPSAFE。

### 同步协议（SPPP 和 NETGRAPH）

**链接**：[我的 FreeBSD 主页，其中包含部分工作成果，但更新较少。](http://www.freebsd.org/~rik)
...
此外，还添加了对 FrameRelay AnnexD 的支持，作为一项历史性的提交。许多 Cronyx 用户期待这一改动已久，并且大多数用户仍然更喜欢使用 `SPPP` 而不是 `netgraph`，因为其配置更加简单（特别是对于 `PPP`（相较于 `mpd`）和 `FR`（相较于多个 `netgraph` 模块））。此改动合并到 `MFC` 后，我将最终关闭 2000 年 10 月 5 日提交的 PR 21771。

### TCP 清理与优化

**链接**：<http://people.freebsd.org/~andre/tcpcleanup.html>

**联系方式**：Andre Oppermann <[andre@FreeBSD.org](mailto:andre@FreeBSD.org)>

自 1994 年从 4.4BSD-Lite2 分支以来，FreeBSD 的 TCP 代码经历了大规模演进，主要是由于新的特性和 TCP 规范的不断完善。

现在，TCP 代码需要全面重构和清理，以提升其可读性、可维护性和可扩展性。此外，在重构过程中，还可以做许多小优化，使 FreeBSD 重新成为性能最优的 TCP/IP 协议栈之一，就像 90 年代长期占据领先地位时那样。
...
目前，TCP 片段重组是通过一个链表来实现的。然而，在当今的高带宽网络环境下，尤其是具有较大带宽*延迟积的场景中，这种方式的可扩展性和性能表现均不佳。

此次重写将优化 TCP 片段重组过程的多个方面。例如，新到达的片段很可能需要连接到重组队列的末尾，因此我们首先检查该情况。其次，我们检查它是否是缺失的片段，或者是否应连接到重组队列的起始位置。第三，连续的片段会合并（逻辑上的），并在线性搜索时一次性跳过，而不是逐个遍历所有片段。

进一步的优化方案包括在 `mbuf` 层面合并连续的片段，而不仅仅是逻辑合并。这预计会带来显著的性能提升。新的重组队列还会追踪队列中的所有“空洞”（缺失片段），未来可能会与 SACK 的缓存区集成，以提高性能。

Andrew Gallatin 在 Myrinet 双 2Gbps 网卡上测试，即便在固件 Bug 导致的严重数据包乱序情况下，新的 TCP 重组代码仍然能够实现 3.7Gbps 的 TCP 传输性能。详细信息请参考第二个链接。
...
根据 RFC1644 标准，旧版 TTCP 存在安全性差、侵入性强、复杂等问题，并且已从 FreeBSD 5.3 版本及之后的版本中移除。尽管如此，其背后的理念和语义仍然有效且合理。

重写后的版本采用了更简单且更安全的系统，使用 24 位长的客户端和服务器 cookie，并通过 TCP 选项传输。客户端 cookie 防止各种盲目注入攻击，并可用于提高 TCP 会话的整体安全性（例如用于 BGP）。服务器 cookie 仅在 SYN-SYN/ACK 阶段交换，允许服务器确保它曾与特定客户端通信过。第一次连接总是执行三次握手（3WHS）并分配一个服务器 cookie 给客户端。后续连接则可以将 cookie 返回给服务器，从而跳过三次握手，直接进入 SYN->OPEN 阶段。

TTCPv2 可以通过 `setsockopt()` 系统调用按每个套接字完全配置。不支持 TTCPv2 的客户端和服务器仍然可以正常工作，继续使用常规的三次握手，而不会造成延迟或其他复杂问题。
...
**联系方式**：Kip Macy <[kmacy@fsmware.com](mailto:kmacy@fsmware.com)>

FreeBSD 5.2.1 在 Xen 稳定分支中作为来宾操作时已稳定运行。FreeBSD 5.3 在 Xen 稳定分支中也能运行，但有几个 bug 需要追查。
...
**链接**：[FreeBSD/arm 项目主页](http://www.freebsd.org/platforms/arm)

**联系方式**：Olivier Houchard <[cognet@FreeBSD.org](mailto:cognet@FreeBSD.org)>

FreeBSD/arm 取得了巨大进展。目前，它可以启动多用户模式，并能够在 IQ31244 开发板上运行如 `make world` 和 Perl 等程序。现在它还支持多种功能，包括 `DDB`、`KTR`、`ptrace` 和内核模块。一个早期的 `gdb` 支持补丁已发布，`libpthread` 也几乎可以正常工作。
...
一个本地构建的 6.0-CURRENT miniinst ISO 文件已可以通过上述链接下载。它在 G4 Powermac 上表现最好，但也可能在其他 Newworld 机器上运行。具体细节请参见发布说明。

和往常一样，仍然需要大量帮助。这非常适合那些希望深入了解 FreeBSD 内核的开发者。

## Ports

### FreeBSD GNOME 项目状态报告

**链接**：[FreeBSD GNOME 项目](http://www.freebsd.org/gnome/)

**联系方式**：Joe Marcus <[marcus@FreeBSD.org](mailto:marcus@FreeBSD.org)>
...
[GNOME Tinderbox](http://www.freebsd.org/gnome/docs/faq2.html#q21) 仍在运行，并为所有支持的 i386 版本的 FreeBSD 生成稳定版和开发版的 GNOME 软件包。

感谢 Michael Johnson，FreeBSD GNOME 团队最近获得了 [使用 Firefox 和 Thunderbird 名称](http://people.freebsd.org/~ahze/firefox_thunderbird-approved.txt)、官方图标的许可，并可以生成官方品牌的构建版本。Mozilla 也非常希望将我们本地的补丁合并回官方代码树。这将大大提高 Firefox 和 Thunderbird 在 FreeBSD 上的质量。
...
**联系方式**：Maho Nakata <[maho@FreeBSD.org](mailto:maho@FreeBSD.org)>
...
- 目前我们有三个外部 Port：**lang/gcc-ooo**、**devel/bison-devel** 和 **devel/epm**。为了避免 `gcc` 的回归和错误，我们使用与 Hamburg 团队（前 StarDivision）相同的 `gcc` 版本。我们需要使用 1.785a 版本以上的 `bison`。请注意，该 Port 与 **devel/bison** 存在冲突。`Epm` 是 OpenOffice.org 现在使用的软件包管理器。

OpenOffice.org 1.1 状态

- 1.1.4 版本已移植并提交到 Ports。
- 软件包可以在 [http://ooomisc.services.openoffice.org/pub/OpenOffice.org/ooomisc/FreeBSD/](http://ooomisc.services.openoffice.org/pub/OpenOffice.org/ooomisc/FreeBSD/) 获取。
- 现在可以识别 Linux 版本的 Java JDK。

一般情况

- 从命令行调用 OpenOffice.org 的方式有所改变。现在必须使用 `.org` 后缀。例如，`openoffice-1.1.4` -> `openoffice.org-1.1.4`。由于软件名称是 OpenOffice.org 而不是 OpenOffice，我们也在考虑更改 Port 的名称（**/usr/ports/editors/openoffice-2.0-devel** -> `openoffice.org2-devel` 等）。

- 目前标记为 `BROKEN` 的 OOo Port 适用于早于 5.3-RELEASE 和 4.11-RELEASE 的版本。这些 Port 一直受到 FreeBSD 和 Linux、Solaris、NetBSD 之间 `rtld.c` 的小实现差异的影响。我们一直在应用一个补丁来添加 `_end` 到 mapfile。我们需要这个补丁，因为 `rtld` 在 `obj_from_addr_end` 中依赖于 `_end` 符号的存在。不幸的是，这似乎引发了难以解决的错误。kan 取得了重大进展，`rtld` 现在不再依赖 `_end`。该修复已于 2004 年 2 月 25 日提交，[http://www.freebsd.org/cgi/cvsweb.cgi/src/libexec/rtld-elf/rtld.c.diff?r1=1.91&r2=1.92&f=h](http://www.freebsd.org/cgi/cvsweb.cgi/src/libexec/rtld-elf/rtld.c.diff?r1=1.91&r2=1.92&f=h) 。

- 性能测试！构建 OOo 需要大量资源。我们只是想了解构建所需的时间，以便评估机器在高负载工作下的调优程度。[http://porting.openoffice.org/freebsd/benchmark.html](http://porting.openoffice.org/freebsd/benchmark.html)。目前，GOTO daichi（daichi）的 Pentium 4 3.0GHz 机器构建速度最快。第二次构建 OOo 1.1.4 仅需 1 小时 25 分 22.42 秒，使用了 `ccache`。

- SDK 教程可在 [http://porting.openoffice.org/freebsd/sdk.html](http://porting.openoffice.org/freebsd/sdk.html) 获得。

- 仍然没有做实现测试和质量保证，也没有为 FreeBSD 提供系统化的文档。[http://porting.openoffice.org/freebsd/testing.html](http://porting.openoffice.org/freebsd/testing.html) 和 [http://porting.openoffice.org/freebsd/QA.html](http://porting.openoffice.org/freebsd/QA.html) 有详细信息。
...
大部分精力投入到了两个版本的发布中。5.3 版本经历了特别长的冻结期，但由于 Ports 状况良好，4.11 的冻结期保持在最低限度。新基础设施的几次迭代已在集群中测试并提交。现在，集群也为 `6-CURRENT` 版本构建软件包，从而将不同构建环境的数量增加到 10。

此外，我们还对 Ports 做了几次清理，以统一不同 Port 中使用的变量及其值，例如 `BROKEN`、`IGNORE`、`DEPRECATED`、`USE_GCC` 等。

从技术角度来看，最大的变化是将 `X.org` 代码库作为默认的 X11 实现。同时，还提交了可以选择使用 `X.org`/`XFree86` 代码的功能，后者也在此期间更新了。由于 Eric Anholt、Dejan Lesjak（新提交者）和 Joe Marcus Clarke 的辛勤工作，这一切进展比预期的要顺利得多。

此外，GNOME 和 KDE、Perl 以及 Java 框架在此期间也得到了更新。《Porter 手册》也做了一些更新，但更多部分仍需要更新以包括近期的实践变更。在此期间，Bill Fenner 还修复了他在 [distfile 调查](http://people.freebsd.org/~fenner/portsurvey) 中的一个错误。

在 4.11 版本发布前，我们现有的 `linux_base` 因安全问题标记为禁止使用。大量工作投入到将默认版本从 7 升级到 8，以便在 4.11 版本发布时提供一个可用的 `linuxolator`。

由于 4 月至 5 月期间的稳定性问题，Alpha 架构的软件包构建暂停。在 Ken Smith 等人对 Alpha 做了一些工作后，`4.X` 版本的软件包构建于 2004 年底重新启用。

Ports 质量保证提醒 —— `portmgr` 团队现在定期发送关于 Ports 问题的电子邮件。当前提醒包括：

- 公开列出所有因安全问题、构建失败或一般过时而将移除的 Port，除非它们先修复
- 向受影响 Port 的所有维护者发送私人邮件（包括依赖上述 Port 的 Port）
- 向所有标记为 `BROKEN` 或 `FORBIDDEN` 的 Port 维护者发送私人邮件
- 向非提交者的维护者发送私人邮件，提醒他们有针对其 Port 的 PR（以标记可能从未抄送给他们的 PR）
- 关于导致 `INDEX` 构建失败的 Port 提交的公开邮件
- 关于导致版本修订元数据倒退的 Port 提交的公开邮件（从而干扰 `portupgrade` 等工具）
...
2. 下一个最多的构建错误是由于代码无法在我们的 64 位架构上构建，原因是假设“全世界都是 PC”。[这是完整的列表](http://portsmon.firepipe.net/ploticus/uniqueerrorcounts.html)；每个单独的条形图是可点击的。随着 `amd64` 平台提升为一级支持状态，这个问题将变得越来越重要。
3. 我们在整治安装文件超出批准目录或卸载不干净的 Port 方面取得了很多进展（请参见 [pointyhat](http://pointyhat.freebsd.org/errorlogs/) 上的“`PLIST` 中未列出的额外文件”），这将继续是我们的重点领域。

### Linux 用户空间基础设施更新

**联系方式**：Alexander Leidinger <[netchild@FreeBSD.org](mailto:netchild@FreeBSD.org)>

由于 **emulators/linux_base** 存在安全问题，默认的 Port `linux_base` 从基于 RedHat 7 的 **emulators/linux_base** 更改为基于 RedHat 8 的 **emulators/linux_base-8**，恰好在 FreeBSD 4.11 版本发布时完成。在转换过程中，修复了几个 Linux Port 中的问题。

RedHat 7 和 8 都已进入生命周期末期，因此预计将来会更新为更近期的 Linux 发行版。出于 QA 原因，此更新未安排在 FreeBSD 4.11 发布前完成。
...
`ALTQ` 是 FreeBSD 5.3 版本的一部分，可以与 `PF` 一起用于流量整形和分类。在 `CURRENT` 版本中，`IPFW` 也获得了做 `ALTQ` 分类的功能。越来越多的网卡驱动程序已转换为支持 `ALTQ`。详情请参见 **ALTQ(4)** 手册页。
...
**链接**：[Cronyx 软件下载页面](http://www.cronyx.ru/software)

**联系方式**：Roman Kurakin <[rik@FreeBSD.org](mailto:rik@FreeBSD.org)>

目前 FreeBSD 支持三种类型的 Cronyx 同步适配器：Tau-PCI - **cp(4)**，Tau-ISA - **ctau(4)** 和 Sigma - **cx(4)**。所有这些驱动程序在 `6-CURRENT` 版本中均已更新，现在它们不再依赖 `Giant`。然而，这只对 **sppp(4)** 有效。如果你使用 `Netgraph` 或异步模式（对于 Sigma），可能需要通过适当的内核变量关闭该驱动程序的 `mpsafenet`。
...
**链接**：[PF4FreeBSD 主页](http://pf4freebsd.love2party.net/)

**联系方式**：Max Laier <[mlaier@FreeBSD.org](mailto:mlaier@FreeBSD.org)>

**联系方式**：Daniel Hartmeier <[dhartmei@FreeBSD.org](mailto:dhartmei@FreeBSD.org)>

FreeBSD 5.3 是第一个包含 `PF` 的版本。该版本发布时没有问题，但一些错误发现得太晚，未能及时收录到 CD 中。建议更新 **src/sys/contrib/pf** 到 `RELENG_5`。具体修复的内容如下：
...
《手册》中的防火墙部分作了修改，提到 `PF` 作为 `IPFW` 和 `IPF` 的替代方案。
...
### EuroBSDCon 2004 提交的论文已在线发布

**链接**：[论文/演讲下载页面](http://www.eurobsdcon2004.de/papers.html)

**联系方式**：Patrick M. Hausen <[hausen@punkt.de](mailto:hausen@punkt.de)>

所有的论文和演讲资料现已上传到我们的会议网站，可以下载。再次感谢所有助力 EuroBSDCon 2004 成功举办的人。

### ifconfig 重构

**联系方式**：Sam Leffler <[sam@FreeBSD.org](mailto:sam@FreeBSD.org)>

用于配置网络接口的 `ifconfig` 程序经过了重构。多年来，`ifconfig` 已发展为一个复杂且经常扭曲的软件，难以理解和维护。此项工作的主要动机是实现最小化配置（用于嵌入式用途），而无需更改代码，并以模块化方式支持未来的扩展。功能现在拆分到独立的文件中，操作通过中央 `ifconfig` 代码库注册。只需指定构建程序时要包含的代码，即可配置功能。

未来的计划是通过动态库让 `ifconfig` 自动加载功能。这一机制将允许，例如，第三方软件包提供内核服务和 `ifconfig` 附加代码，而无需更改基本系统。

### EuroBSDCon 2005 —— 巴塞尔/瑞士

**链接**：[EuroBSDCon 官方主页](http://www.eurobsdcon.org/)

**联系方式**：Max Laier <[mlaier@FreeBSD.org](mailto:mlaier@FreeBSD.org)>

今年的 EuroBSDCon 将于 11 月 25 日至 27 日在瑞士巴塞尔大学举行。征集论文的通知将很快发布。请考虑参加或甚至演讲。更多信息请访问会议主页。
...
安全官员由副安全官员和安全团队支持。4 月份，Chris Faulhaber 辞去了副安全官员职务，Dag-Erling Smorgrav 出任新的副安全官员。在这一年中，以下团队成员辞职：Julian Elischer、Bill Fumerola、Daniel Harris、Trevor Johnson、Kris Kennaway、Mark Murray、Wes Peters、Bruce Simpson 和 Bill Swingle；而以下成员成为新成员：Josef El-Rayes、Simon L. Nielsen、Colin Percival 和 Tom Rhodes。向所有过去和现在的成员表示衷心的感谢！当前的安全团队成员名单已在网站上发布。

随着 FreeBSD 4.8 的发布，安全官员开始为一些 FreeBSD 版本及其相应的安全分支提供扩展支持。“早期采用者”分支，如 FreeBSD 5.0（`RELENG_5_0`），至少支持六个月。“正常”分支至少支持一年。“扩展”分支，如 FreeBSD 5.3（`RELENG_5_3`），至少支持两年。当前支持的分支及其估计的“生命周期结束”（EoL）日期已发布在 FreeBSD 安全信息网页上。在 2004 年，四个版本“过期”：4.7、4.9、5.1 和 5.2。

随着 FreeBSD 4.10 和 5.3 的发布，安全官员和发布工程团队扩展了安全分支的范围，纳入了与安全问题无关的关键 bug 修复。目前，这些修复发布为单独的勘误通知。未来，安全通知和勘误通知将合并并统一处理。

2004 年发布了 17 条安全通知，涉及了 8 个 FreeBSD 特有的问题和 9 个通用问题。

2004 年还引入了漏洞和暴露标记语言（VuXML）。VuXML 是一种用于记录单个软件包集合中安全问题的标记语言。安全团队和其他提交者已在 FreeBSD 项目的 VuXML 文档中记录了 Ports 中的超过 325 个安全问题。该文档目前维护在 Ports 仓库中，路径为 `ports/security/vuxml/vuln.xml`。文档的内容以可读形式提供，用户可以在 FreeBSD VuXML 网站上查看。`portaudit` 工具可用于审计本地系统是否存在列出的安全问题。从 11 月开始，流行的 FreshPorts.org 网站也开始跟踪 VuXML 中记录的问题。

### FreeBSD 源代码仓库镜像用于 svn/svk
...
你可以用 `svk` 镜像整个仓库，并在离线时提交。它还提供历史感知的分支、合并和补丁功能。非提交者可以轻松地维护自己的分支，并在补丁审查期间跟踪上游的变更。

### 包含新软件的 Wiki

**链接**：[Wiki](http://wiki.freebsd.org/)
