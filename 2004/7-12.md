...
FreeBSD 状态报告在短暂的间歇后再次回归。2004 年下半年异常忙碌，FreeBSD 5.3 版本发布，6-CURRENT 开发分支启动，EuroBSDCon 2004 取得巨大成功，这些只是其中的一部分大事件。本期报告包含了令人印象深刻的 44 份提交，是迄今为止提交数量最多的一次！
...
| [网络协议栈清理项目](http://www.freebsd.org/projects/dingo/index.html "http://www.freebsd.org/projects/dingo/index.html") |
...
| [FreeBSD profile.sh 站点](https://projects.fsck.ch/profile "https://projects.fsck.ch/profile") |

联系人：Tobias Roth <[ports@fsck.ch](mailto:ports@fsck.ch)>

FreeBSD profile.sh 主要面向笔记本电脑。它允许用户定义多个网络环境（例如家庭、工作场所），并可自动检测笔记本电脑启动时所处的环境，并据此进行相应配置。系统中几乎所有位于 /etc 下的配置都可以按环境进行设定，用户只需定义与默认 /etc 配置不同的部分。此外，它还支持在一个环境中挂起（suspend）并在另一个环境中恢复（resume）。
...
| [FreeSBIE 官方网站](http://www.freesbie.org/ "http://www.FreeSBIE.org") |
| [FreeSBIE 邮件列表](http://liste.gufi.org/mailman/listinfo/freesbie "http://liste.gufi.org/mailman/listinfo/freesbie") |
...
## [FreeBSD 内核开发资助计划](https://www.freebsd.org/status/report-2004-07-2004-12.html#Funded-FreeBSD-kernel-development)

| 相关链接 |

| [详细状态报告](http://lists.freebsd.org/pipermail/freebsd-announce/2004-December/000971.html "http://lists.freebsd.org/pipermail/freebsd-announce/2004-December/000971.html") |
...
| [官方网站](http://frenzy.osdn.org.ua/ "http://frenzy.osdn.org.ua/") |
| [英文版](http://frenzy.osdn.org.ua/eng/ "http://frenzy.osdn.org.ua/eng/") |
...
| [Portsnap](http://www.daemonology.net/portsnap/ "http://www.daemonology.net/portsnap/") |
| [FreeBSD Update](http://www.daemonology.net/freebsd-update/ "http://www.daemonology.net/freebsd-update/") |

联系人：Colin Percival <[cperciva@FreeBSD.org](mailto:cperciva@FreeBSD.org)>

在持续提升 FreeBSD 用户系统更新安全性的过程中，我新增了一个工具：**Portsnap**。它已作为 `sysutils/portsnap` 收录于 ports 树中。该工具可以安全地下载并更新压缩格式的 ports 树快照，随后用户可从中提取或更新未压缩的 ports 树。借助 RSA 签名，Portsnap 在端到端传输中确保了安全性。此外，它完全基于 HTTP 运行，相较于 cvsup，频繁更新 ports 树（每周一次以上）的用户可节省 90% 以上的带宽。

**FreeBSD Update** —— 我开发的安全高效的二进制更新工具（用于 Security/Errata 分支的跟踪）—— 仍被广泛使用，目前每天约有 100 台机器下载安全或修正补丁。
...
| [FreeBSD/i386 5.3-RELEASE 硬件支持列表](http://www.freebsd.org/releases/5.3R/hardware-i386.html "http://www.FreeBSD.org/releases/5.3R/hardware-i386.html") |
| [FreeBSD/i386 6.0-CURRENT 硬件支持列表](http://www.freebsd.org/relnotes/CURRENT/hardware/i386/article.html "http://www.freebsd.org/relnotes/CURRENT/hardware/i386/article.html") |
...
| [项目网页](http://www.evilcoder.org/content/section/6/39/ "http://www.evilcoder.org/content/section/6/39/") |
| [官方发布的文档](http://www.freebsd.org/doc/nl/books/handbook/ "http://www.FreeBSD.org/doc/nl/books/handbook/") |
| [文档预览](http://www.evilcoder.org/freebsd_html/ "http://www.evilcoder.org/freebsd_html/") |

联系人：Remko Lodder <[Remko@FreeBSD.org](mailto:Remko@FreeBSD.org)>

FreeBSD 荷兰语文档项目是一个持续进行的项目，旨在将 FreeBSD 文档翻译为荷兰语。目前，我们的主要精力集中在 **Handbook** 的翻译上，进展顺利。然而，在完成“完整”翻译之前，仍有大量内容需要翻译和校对。因此，如果你愿意提供帮助，请访问我们的网站或直接联系我。

### 待完成任务：

1. 翻译 **Handbook**
2. 校对荷兰语 **Handbook** 的语法
3. 翻译其余文档
...
| [主页](http://www.freebsd.org/~nsouch/kgi4BSD "http://www.freebsd.org/~nsouch/kgi4BSD") |
...
| [基于现代 CPU 硬件性能监控功能的 FreeBSD 高级性能监控系统](http://people.freebsd.org/~jkoshy/projects/perf-measurement/ "http://people.freebsd.org/~jkoshy/projects/perf-measurement/") |
...
| [FreeBSD 项目 Netperf 主页](http://www.freebsd.org/projects/netperf/ "http://www.freebsd.org/projects/netperf/") |
| [Robert Watson 的 Netperf 个人主页](http://www.watson.org/~robert/freebsd/netperf/ "http://www.watson.org/~robert/freebsd/netperf/") |
...
- **BPF（Berkeley Packet Filter）加锁优化**（合并）
- **VFS** 断言，检查 **VFS/网络协议栈** 边界
- **Unix 域套接字** 加锁优化
- **WITNESS** 中增加锁顺序文档
- **NFS 服务器代码** 进一步去除 **Giant Lock**
- **管道（pipe）加锁优化**，提升分配性能
- **fstat** 在 **socket 和 pipe** 上不再需要 **Giant Lock**（合并）
- **socket 和 pipe 文件描述符关闭** 不再需要 **Giant Lock**（合并）
- **新增 IFF_NEEDSGIANT 接口标志**，支持非加锁设备驱动的兼容性操作（合并）
- **Accept 过滤器加锁合并至 FreeBSD CVS**
- **uidinfo 加锁策略文档**（合并）
- **fcntl() 对 Giant 的依赖减少**

#### **2004 年 8 月**
- **UMA KTR（Kernel Trace Recorder）跟踪**（合并）
- **UDP 广播接收加锁优化**（合并）
- **TCP 加锁清理及文档完善**
...
- **i386 系统调用 KTR 支持**
- **熵收集同步优化**（合并）
- **callout(9) 采样转换为 KTR**（合并）
- **inpcb socket 选项加锁**（合并）
- **netatalk 代码中移除 GIANT_REQUIRED**（合并）
- **ADAPTIVE_GIANT 合并至 FreeBSD CVS**，显著提升内核 **IPC** 性能
- **UDP 头部预留链路层头部空间**，避免每次 **UDP** 发送时额外分配（合并）
- **多个 UDP 相关错误修复**（合并）
- **多个网络接口标记为 MPSAFE**
- **Unix 域套接字加锁重构**
- **MP_WATCHDOG 机制**（合并）
- **kqueue 加锁**（由 John-Mark Gurney 开发并合并）
- **IPv6 代码 in6_prefix 清理**（合并）
- **IPX、KAME IPSEC 现在使用 NET_NEEDS_GIANT 编译时标记**（合并）
- **熵收集代码的额外加锁优化**（合并）
- **Netperf 开发分支默认禁用 Giant**（合并）

#### **2004 年 9 月**
- **修复 Netgraph 使用内核链接器时未持有 Giant 的 bug**（合并）
- **默认移除网络协议栈的 Giant**（合并）
- **修复 netinet6 和 if_afdata 相关的竞争条件**（合并）
- **BPF 代码改用 queue(3)**（合并）
- **修复 sopoll() 竞争条件**（合并）
...
- **TCP 计时器加锁、断言**（合并）
- **IP 套接字选项加锁及清理**（合并）
- **Netgraph ISR 和 Netatalk ISR 标记为 MPSAFE**（合并）
- **修复熵收集和以太网相关的 use-after-free bug**（合并）
- **修复 soclose()/sofree() 竞争条件**（合并）
- **新增 IFF_LOCKGIANT() 和 IFF_UNLOCKGIANT()**，在非 MPSAFE 网络接口 ioctl 入口时自动获取 Giant 锁

#### **2004 年 11 月**
- **FreeBSD 5.3 发布**，首次默认使用 **MPSAFE 且无 Giant Lock** 的网络协议栈
- **文件描述符引用计数优化，提升高负载下的 send/recv 性能**（合并）
- **实验性网络轮询加锁**
- **优化互斥锁和加锁机制**（合并）
- **UMA 采用临界区保护 per-CPU 缓存，优化微基准测试性能**
- **新 mbufqueue 原语开发，优化网络协议栈与设备驱动之间的交互**（与 Sandvine, Inc 合作）
- **Linux TCP_INFO API 添加**，支持用户态 TCP 状态监控（合并）

#### **2004 年 12 月**
- **UDP 处理改进（多线程入站处理）**（合并）
- **TCP 紧急数据处理相关的 socket 缓冲区加锁修复**（合并）
- **IF_DEQUEUE() 和 IF_DRAIN() 读优化**
- **优化 netisr 处理 mbuf 队列的批量出队**
- **netrate 测试工具合并至 RELENG_5**
- **TCP 加锁修复合并至 RELENG_5**
- **DDB 增加 "show alllocks" 命令**（合并）
- **IPX/SPX 代码清理、加锁修复**（合并）
- **IPX/SPX 标记为 MPSAFE**（合并）
- **SPPP 代码加锁（由 Roman Kurakin 完成）**（合并）
- **Unix 域套接字加锁修复（由 Alan Cox 完成）**（合并）
...
| [我的 FreeBSD 主页，其中包含部分工作成果，但更新较少。](http://www.freebsd.org/~rik "http://www.freebsd.org/~rik") |
...
自 1994 年从 4.4BSD-Lite2 分支以来，FreeBSD 的 TCP 代码已经经历了显著演进，主要是由于新的特性和 TCP 规范的不断完善。
...
在 2005 年 2 月，我将正式发布资金筹集请求，提供详细的工作描述以及资金使用方式。总体而言，我可以为愿意赞助此项目的公司开具发票，以报销相关费用。此外，通过 FreeBSD 基金会，可能可以安排免税捐赠。目前已欢迎资金赞助，请通过上述邮箱联系我。

### 未完成任务：

1. 需要筹集相当于两个月全职工作的资金。
2. 如果您计划赞助 1000 美元或以上，请提前与我联系。
...
| [二进制文件 + 源代码 + 稍有过时的 HOWTO](http://www.fsmware.com/xenofreebsd/ "http://www.fsmware.com/xenofreebsd/")      |
| [Xen 项目主页](http://www.cl.cam.ac.uk/Research/SRG/netos/xen/ "http://www.cl.cam.ac.uk/Research/SRG/netos/xen/")      |
...
| [FreeBSD/arm 项目主页](http://www.freebsd.org/platforms/arm "http://www.freebsd.org/platforms/arm")      |
...
## [PowerPC 移植](https://www.freebsd.org/status/report-2004-07-2004-12.html#PowerPC-Port)

| 相关链接 |

| [Miniinst ISO 文件](http://www.freebsd.org/~grehan/miniinst.iso "http://www.freebsd.org/~grehan/miniinst.iso")      |
| [Miniinst 发布说明](http://www.freebsd.org/~grehan/miniinst.txt "http://www.freebsd.org/~grehan/miniinst.txt")      |
...
| [FreeBSD GNOME 项目](http://www.freebsd.org/gnome/ "http://www.FreeBSD.org/gnome/")      |
...
| [FreeBSD OpenOffice.org 移植状态页面](http://porting.openoffice.org/freebsd/ "http://porting.openoffice.org/freebsd/")      |
| [FreeBSD 的稳定 OpenOffice.org 包](http://ooomisc.services.openoffice.org/pub/OpenOffice.org/ooomisc/FreeBSD/ "http://ooomisc.services.openoffice.org/pub/OpenOffice.org/ooomisc/FreeBSD/")      |
| [一些不稳定的 WIP 包状态](http://sourceforge.jp/projects/waooo/files/ "http://sourceforge.jp/projects/waooo/files/")      |

联系人：Maho Nakata <[maho@FreeBSD.org](mailto:maho@FreeBSD.org)>

OpenOffice.org 2.0 状态

* OpenOffice.org 2.0 计划于 2005 年 3 月发布。目前已有开发者快照版本可用。现已将其中一个开发版本移植，并提交到 ports 树（/usr/ports/editors/openoffice-2.0-devel）。
...
* 目前我们有三个外部移植包：lang/gcc-ooo，devel/bison-devel 和 devel/epm。为了避免 gcc 的回归和错误，我们使用与汉堡团队（前 StarDivision）相同的 gcc 版本。我们需要使用 1.785a 版本以上的 bison。请注意，这个移植包与 devel/bison 存在冲突。Epm 是 OpenOffice.org 现在使用的包管理器。

OpenOffice.org 1.1 状态

* 1.1.4 版本已移植并提交到 ports 树。
...
## [Ports](https://www.freebsd.org/status/report-2004-07-2004-12.html#Ports-Collection)

**链接**

| [FreeBSD Ports](http://www.freebsd.org/ports/ "http://www.FreeBSD.org/ports/")      |
| [FreeBSD Ports 监控系统](http://portsmon.firepipe.net/index.html "http://portsmon.firepipe.net/index.html")      |
...
从技术角度来看，最大的变化是将 X.org 代码库作为默认的 X11 实现。同时，还提交了可以选择使用 X.org 或 XFree86 代码的功能，后者也在此期间进行了更新。由于 Eric Anholt、Dejan Lesjak（新提交者）和 Joe Marcus Clarke 的辛勤工作，这一切进展比预期的要顺利得多。

此外，GNOME 和 KDE、Perl 以及 Java 框架在此期间也得到了更新。Porter's Handbook 也进行了一些更新，但更多部分仍需要更新以包括近期的实践变更。在此期间，Bill Fenner 还修复了他在 [distfile 调查](http://people.freebsd.org/~fenner/portsurvey) 中的一个错误。
...
| [ALTQ(4) 手册页](http://www.freebsd.org/cgi/man.cgi?query=altq&manpath=FreeBSD+6.0-current&format=html "http://www.freebsd.org/cgi/man.cgi?query=altq&manpath=FreeBSD+6.0-current&format=html")      |
...
| [Cronyx 软件下载页面](http://www.cronyx.ru/software "http://www.cronyx.ru/software")      |

联系方式: Roman Kurakin <[rik@FreeBSD.org](mailto:rik@FreeBSD.org)>

目前 FreeBSD 支持三种类型的 Cronyx 同步适配器：Tau-PCI - cp(4)，Tau-ISA - ctau(4) 和 Sigma - cx(4)。所有这些驱动程序在 6.current 版本中都已更新，现在它们已不再依赖 Giant。然而，这只对 sppp(4) 有效。如果您使用 Netgraph 或异步模式（对于 Sigma），可能需要通过适当的内核变量关闭该驱动程序的 mpsafenet。

### 待处理任务：

1. 目前所有这些驱动程序和 sppp(4) 都使用递归锁。因此，第一个任务是使这些锁变为非递归锁。
2. 第二个任务是检查/使驱动程序在 netgraph/异步模式下可用。
3. 我正在考虑在运行时切换 sppp/netgraph 模式的功能。目前，您需要重新编译模块/内核才能更改模式。
...
| [PF4FreeBSD 官方主页](http://pf4freebsd.love2party.net/ "http://pf4freebsd.love2party.net/")      |
...
| [论文/演讲下载页面](http://www.eurobsdcon2004.de/papers.html "http://www.eurobsdcon2004.de/papers.html")      |
...
| [EuroBSDCon 官方主页](http://www.eurobsdcon.org/ "http://www.eurobsdcon.org/")      |
...
| [FreeBSD 安全信息](http://www.freebsd.org/security/ "http://www.freebsd.org/security/")      |
| [FreeBSD 安全官员章程](http://www.freebsd.org/security/charter.html "http://www.freebsd.org/security/charter.html")      |
| [FreeBSD 安全团队成员](http://www.freebsd.org/administration.html#t-secteam "http://www.freebsd.org/administration.html#t-secteam")      |
| [FreeBSD VuXML 网站](http://vuxml.freebsd.org/ "http://vuxml.freebsd.org/")      |
| [portaudit](http://cvsweb.freebsd.org/ports/security/portaudit/ "http://cvsweb.freebsd.org/ports/security/portaudit/")      |
...
随着 FreeBSD 4.10 和 5.3 的发布，安全官员和发布工程团队扩展了安全分支的范围，涵盖了与安全问题无关的关键 bug 修复。目前，这些修复发布为单独的错误通知。未来，安全通知和错误通知将合并并统一处理。

2004 年发布了 17 条安全通知，涵盖了 8 个与 FreeBSD 相关的问题和 9 个通用问题。
...
| [仓库浏览器](http://svn.clkao.org/svnweb/freebsd/ "http://svn.clkao.org/svnweb/freebsd/")      |
| [RELENG_5 提交的 RSS](http://svn.clkao.org/svnweb/freebsd/rss/fromcvs/branches/RELENG_5/ "http://svn.clkao.org/svnweb/freebsd/rss/fromcvs/branches/RELENG_5/")      |
| [CURRENT 提交的 RSS](http://svn.clkao.org/svnweb/freebsd/rss/fromcvs/trunk/ "http://svn.clkao.org/svnweb/freebsd/rss/fromcvs/trunk/")      |
| [svk 官方主页](http://svk.elixus.org/ "http://svk.elixus.org/")      |
...
svk 允许您镜像整个仓库，并在离线时进行提交。它还提供历史感知的分支、合并和补丁功能。非提交者可以轻松地维护自己的分支，并在补丁审查期间跟踪上游的变更。
...
| [Wiki](http://wiki.freebsd.org/ "http://wiki.freebsd.org/")      |

联系方式: Josef El-Rayes <[josef@FreeBSD.org](mailto:josef@FreeBSD.org)>

由于旧的 Wiki 引擎在缺乏身份验证机制的情况下遭受了垃圾邮件攻击，我不得不将其更换为更先进的软件。我们现在使用的是 moinmoin，而不再是使用的 usemod。因此，它不再仅仅是“浏览和编辑”，而是需要注册，并且必须由已在 ACL 组“developers”中的某人将您添加到该组。所以它现在是一个“仅限开发者”资源。旧的 Wiki 可以在 [http://wiki2.daemon.li](http://wiki2.daemon.li/) 找到。
