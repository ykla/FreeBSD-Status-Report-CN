...
无线网络层已更新，以支持 802.1x、WPA 和 802.11i 安全协议，以及 WME/WMM 多媒体协议。作为此项工作的一部分，为加密方法、认证和访问控制添加了可扩展框架。扩展以可加载内核模块的形式实现，挂钩到 net80211 层。这一机制被用于实现 WEP、TKIP 和 CCMP 加密协议。Atheros 驱动（ath）目前是唯一使用完整功能集的驱动。为其他驱动添加支持很简单，但正在等待志愿者。wpa_supplicant 和 hostapd 程序的移植使得使用新的安全协议成为可能。
...
OpenBSD 的 dhcp 客户端程序已被移植并增强，可以监听来自内核的 802.11 相关事件。这使得在漫游时能够立即获取 IP 地址（不同于旧代码的轮询方式）。与之前客户端的主要区别在于，现在每个接口有一个 dhclient 进程，而不是整个系统一个。这需要对系统启动脚本进行更改。
...
- 目前我们有三个外部移植软件包：lang/gcc-ooo，devel/bison-devel 和 devel/epm。为了避免 gcc 的回归和错误，我们使用与 Hamburg 团队（前 StarDivision）相同的 gcc 版本。我们需要使用 1.785a 版本以上的 bison。请注意，这个移植软件包与 devel/bison 存在冲突。Epm 是 OpenOffice.org 现在使用的软件包管理器。
...
- 目前标记为“BROKEN”的 OOo 移植包适用于早于 5.3-RELEASE 和 4.11-RELEASE 的版本。这些移植包一直受到 FreeBSD 和 Linux、Solaris、NetBSD 之间 rtld.c 的小实现差异的影响。我们一直在应用一个补丁来添加 `_end` 到 mapfile。我们需要这个补丁，因为 rtld 在 obj_from_addr_end 中依赖于 `_end` 符号的存在。不幸的是，这似乎引发了难以解决的错误。经过重大进展，rtld 现在不再依赖 `_end`。该修复已于 2004 年 2 月 25 日提交，[http://www.freebsd.org/cgi/cvsweb.cgi/src/libexec/rtld-elf/rtld.c.diff?r1=1.91&r2=1.92&f=h](http://www.freebsd.org/cgi/cvsweb.cgi/src/libexec/rtld-elf/rtld.c.diff?r1=1.91&r2=1.92&f=h) 。
...
由于 4 月至 5 月期间的稳定性问题，Alpha 架构的软件包构建被暂停。在 Ken Smith 等人对 Alpha 进行了一些工作后，4.X 版本的软件包构建于 2004 年底重新启用。
...
用于配置网络接口的 ifconfig 程序经过了重构。多年来，ifconfig 已发展为一个复杂且经常扭曲的软件，难以理解和维护。此项工作的主要动机是实现最小化配置（用于嵌入式用途），而无需更改代码，并以模块化方式支持未来的扩展。功能现在被拆分到独立的文件中，操作通过中央 ifconfig 代码库注册。只需指定构建程序时要包含的代码，即可配置功能。

未来的计划是通过动态库让 ifconfig 自动加载功能。这一机制将允许，例如，第三方软件包提供内核服务和 ifconfig 附加代码，而无需更改基本系统。

### EuroBSDCon 2005 - 瑞士巴塞尔
...
随着 FreeBSD 4.10 和 5.3 的发布，安全官员和发布工程团队扩展了安全分支的范围，涉及了与安全问题无关的关键 bug 修复。目前，这些修复发布为单独的勘误通知。未来，安全通知和勘误通知将合并并统一处理。
