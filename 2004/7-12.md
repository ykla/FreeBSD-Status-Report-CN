...
对在 `bss`（受管或 adhoc）中跟踪站点以及扫描时发现的站点的支持进行了重构。现在使用多个表，每个表具有不同的管理策略，引用计数现在一致地进行，非活动处理更加智能（例如，关联的站点在移除前会被探测）。这是迈向正确的漫游支持和其他高级功能的第一步。
...
OpenBSD 的 `dhcp` 客户端程序已移植并增强，可以监听来自内核的 802.11 相关事件。这使得在漫游时能够立即获取 IP 地址（不同于旧代码的轮询方式）。与之前客户端的主要区别在于，现在每个接口有一个 `dhclient` 进程，而不是整个系统一个。这需要对系统启动脚本进行更改。
...
该项目近期较为沉寂（但仍在进行！），主要依靠志愿者进行测试。新的文档可在 [http://wiki.daemon.li/moin.cgi/KGI](http://wiki.daemon.li/moin.cgi/KGI) 找到。
...
此外，还添加了对 FrameRelay AnnexD 的支持作为一个历史性的提交。许多 Cronyx 用户期待这一改动已久，并且大多数用户仍然更喜欢使用 `SPPP` 而不是 `netgraph`，因为其配置更加简单（特别是对于 `PPP`（相较于 `mpd`）和 `FR`（相较于多个 `netgraph` 模块））。此改动合并到 `MFC` 后，我将最终关闭 2000 年 10 月 5 日提交的 PR 21771。
...
和往常一样，仍然需要大量帮助。这是一个非常适合那些希望深入了解 FreeBSD 内核的开发者的项目。
...
大部分精力投入到了两个版本的发布中。5.3 版本经历了特别长的冻结期，但由于 Ports 状况良好，4.11 的冻结期保持在最低限度。新基础设施的几次迭代已在集群中进行了测试并提交。现在，集群也为 `6-CURRENT` 版本构建软件包，从而将不同构建环境的数量增加到 10。
...
此外，GNOME 和 KDE、Perl 以及 Java 框架在此期间也得到了更新。《Porter 手册》也进行了一些更新，但更多部分仍需要更新以包括近期的实践变更。在此期间，Bill Fenner 还修复了他在 [distfile 调查](http://people.freebsd.org/~fenner/portsurvey) 中的一个错误。

在 4.11 版本发布前，我们现有的 `linux_base` 因安全问题标记为禁止使用。大量工作投入到将默认版本从 7 升级到 8，以便在 4.11 版本发布时提供一个可用的 `linuxolator`。

由于 4 月至 5 月期间的稳定性问题，Alpha 架构的软件包构建暂停。在 Ken Smith 等人对 Alpha 进行了一些工作后，`4.X` 版本的软件包构建于 2004 年底重新启用。
...
`ALTQ` 是 FreeBSD 5.3 版本的一部分，可以与 `PF` 一起用于流量整形和分类。在 `CURRENT` 版本中，`IPFW` 也获得了进行 `ALTQ` 分类的功能。越来越多的网卡驱动程序已转换为支持 `ALTQ`。详情请参见 **ALTQ(4)** 手册页。
...
你可以用 `svk` 镜像整个仓库，并在离线时进行提交。它还提供历史感知的分支、合并和补丁功能。非提交者可以轻松地维护自己的分支，并在补丁审查期间跟踪上游的变更。
