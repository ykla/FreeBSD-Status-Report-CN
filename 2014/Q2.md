...
本季度授予了三个新的 src 提交权限，并暂时保管了一个提交权限。
...
在第二季度，我们增加了三位新的提交者，暂时保管了一个提交权限，并恢复了一个提交权限。
...
在 5 月初，FreeBSD 9.3-RELEASE 周期进入了代码软冻结（code slush）阶段。FreeBSD 9.3-RELEASE 周期已接近尾声，9.3-RC3 构建即将开始。9.3-RC3 计划作为此版本周期的最终发布候选版本，在本文撰写时，9.3-RELEASE 应按计划发布。
...
计划使用 pkg(8) 来实现 pkgbase。除了其他好处外，这将允许编写一个通用安装程序：只需下载一个安装映像，然后可以从中安装任何可用的 FreeBSD 版本，包括快照。

我们还打算在 Ports 中使用 pkg(8)，在软件包构建时处理构建依赖。这样做可以通过下载二进制软件包来安装构建依赖，这意味着可以安装一个带有自定义选项的软件包，而无需编译其他内容，从而节省时间。
...
可以通过完整的仿真生成 ARMV6、MIPS32 和 MIPS64 软件包。有几个软件包会阻塞构建的完整运行。可以在“Port 构建状态”链接中查看。
