...
- 原文：[FreeBSD Quarterly Status Report 2nd Quarter 2014](https://www.freebsd.org/status/report-2014-04-2014-06.html)
- 最后作者：FreeBSD 项目

## 引言

本报告涵盖 2014 年 4 月至 6 月期间与 FreeBSD 相关的项目。这是 2014 年计划中的四个报告中的第二份。

2014 年第二季度对于 FreeBSD 项目来说是一个非常忙碌且富有成效的时期。新的 FreeBSD 核心团队已选举产生，FreeBSD Ports 管理团队分支了第二个季度的 “stable” 分支，FreeBSD 发布工程团队正在完成 FreeBSD 9.3-RELEASE 的周期，许多令人兴奋的新功能加入了 FreeBSD。
...
本季度核心团队的工作包括一些深远的政策审查，以及对项目开发方法论的重大调整。

5 月，发布了新的发布政策，并在 BSDCan 开发者大会上由 John Baldwin 进行了介绍。该政策的核心思想是，每个主要的发行分支（例如 10.X）将至少支持五年，但每个分支上的单独小版本（如 10.0-RELEASE）将在规定的时间间隔发布，且仅支持最新的小版本。

另一个重大变化未获批准。当对改革核心团队选举过程的章程修改交由所有 FreeBSD 开发者投票时，该提案未能达到法定人数。

6 月，项目中长时间进行的替换缺陷跟踪系统的工作终于完成。从 6 月 3 日起，FreeBSD 项目切换到 Bugzilla 作为其缺陷跟踪系统。所有 GNATS PR 的历史记录均已保留，因此无需重新开启旧的工单。目前仍在进行相关工作，以重现以前应用于 GNATS 的一些集成调整，但所有必要的功能均已实现，项目开始受益于 Bugzilla 带来的新功能。

本期间还举行了选举，选举产生了下一个两年任期的核心团队成员。我们要感谢即将退休的核心成员多年来的服务。新的核心团队延续了以往核心团队的工作：大约一半的成员来自上届核心团队，几位前核心团队成员也在暂别后重新加入。当前核心团队包括两位 FreeBSD 基金会董事会成员和另外一位基金会工作人员，有助于项目顶层的协调。与此同时，核心秘书角色由一位新的志愿者接任。

其他活动包括为 FreeBSD 源代码树中的软件提供许可证条款咨询，以及监督 postmaster 和 clusteradm 的成员变动。

本季度授予了三个新的 src 提交权限，并暂时保管了一项提交权限。
...
在第二季度，我们增加了三位新的提交者，暂时保管了一项提交权限，并恢复了一项提交权限。
...
5 月初，FreeBSD 9.3-RELEASE 周期进入了代码软冻结（code slush）阶段。FreeBSD 9.3-RELEASE 周期接近尾声，9.3-RC3 构建即将开始。9.3-RC3 计划作为此版本周期的最终发布候选版本，在本文撰写时，9.3-RELEASE 应按计划发布。

目前正在进行将嵌入式架构支持集成到发布构建过程中的工作。现已支持多个 ARM 内核，特别是 Raspberry Pi、BeagleBone 和 WandBoard。

此外，还在进行制作虚拟机镜像的工作，作为发布周期的一部分，支持包括微软 Azure、Amazon EC2 和谷歌 Compute Engine 在内的各种云服务。
...
基于 FreeBSD 10.0 中发布的新的内核内 iSCSI 目标和发起程序堆栈，Chelsio Communications 开始开发一个卸载接口，以利用 Chelsio T4 和 T5 10G 和 40G 以太网适配器的硬件卸载功能。
...
所谓的 CUSE4BSD 已导入到 FreeBSD-11 的基本系统中。CUSE 是用户空间字符设备（character device in userspace）的缩写。CUSE 库是 **devfs(8)** 内核功能的封装，通过 **/dev/cuse** 公开。为了使其正常工作，CUSE 内核代码必须在内核配置文件中启用，或作为模块单独加载。请点击提交记录链接获取更多信息。

### FreeBSD 和 2014 年编程之夏
...
今年，FreeBSD 收到了 39 个项目提案，其中许多提案质量非常高。经过艰难的筛选过程，将这些提案缩小到我们所分配的名额，最终选择了 16 个项目参与 2014 年谷歌编程之夏与 FreeBSD 的合作。

所选项目涉及 FreeBSD 的多个领域，涵盖基本系统和 Ports 基础设施、用户空间和内核等方面。我们有学生在进行防火墙优化、Ports 打包工具、嵌入式系统、调试基础设施、改进的 Unicode 支持、加载器和安装程序的增强等多个方向的工作。我们目前刚过今年分配时间的一半，非常期待将这些项目产生的代码整合到 FreeBSD 中。

这是 FreeBSD 第十次参与谷歌编程之夏，我们感谢谷歌接纳我们作为参与组织。

### 新的自动挂载工具

**联系方式**：Edward Tomasz Napierała <[trasz@FreeBSD.org](mailto:trasz@FreeBSD.org)>

当前自动挂载工具 **amd(8)** 的缺陷是许多 FreeBSD 用户反复反映的问题。为了应对这些问题，正在开发新的自动挂载工具。

该自动挂载工具是净室设计实现，提供在大多数其他 Unix 系统中可用的功能，使用通过 autofs 文件系统实现的适当内核支持。自动挂载工具支持标准映射格式，并将与轻量级目录访问协议（LDAP）服务集成。
...
**联系方式**：Matthew Seaman <[matthew@FreeBSD.org](mailto:matthew@FreeBSD.org)>

**联系方式**：Vsevolod Stakhov <[vsevolod@FreeBSD.org](mailto:vsevolod@FreeBSD.org)>

**联系方式**：pkg 邮件列表 <[freebsd-pkg@FreeBSD.org](mailto:freebsd-pkg@FreeBSD.org)>
...
- 更加优雅地处理复杂的升级或安装场景。
- 将操作沙盒化，以处理新下载的数据，直到可以通过检查软件包签名来验证其可信度。
...
计划使用 **pkg(8)** 来实现 pkgbase。除了其他好处外，这将允许编写通用安装程序：只需下载安装映像，然后可以从中安装任何可用的 FreeBSD 版本，包括快照。

我们还打算在 Ports 中使用 **pkg(8)**，在软件包构建时处理构建依赖。这样做可以通过下载二进制软件包来安装构建依赖，这意味着可以安装带有自定义选项的软件包，只需花最少的时间编译其他内容，从而节省时间。
...
**ports-mgmt/poudriere-devel** Port 可以通过模拟器构建 Port。需要在运行 poudriere-devel 之前配置杂项二进制映像激活程序。

可以通过完整的仿真生成 ARMV6、MIPS32 和 MIPS64 软件包。有几个软件包会阻塞构建的完整运行。可以在“Ports 构建状态”链接中查看。

要通过仿真构建软件包，在 -CURRENT 或最新的 **stable/10** 上：
...
1. **PPC 在 AMD64 上的仿真**：此项工作仍在进行中，因为在大端硬件上运行 `bsd-user` 二进制文件时似乎存在一些严重问题。Justin Hibbits 正在处理此问题。
2. **SPARC64 在 AMD64 上的仿真**：此功能无法使用，且立即出现段错误。我们正在寻找能帮助解决这一问题的人。
3. **外部工具链，XDEV 支持**：目前部分支持使用 AMD64 工具链生成其他架构的二进制文件（例如，使用 AMD64 工具链构建 MIPS64 软件包）。我们目前正在跟踪 **ports-mgmt/pkg** 的链接问题。感谢 Warner Losh、Baptiste Daroussin 和 Dimitry Andric 帮助解决这一问题，以使 XDEV 目标变得可用。
4. **信号处理**：MIPS/ARMV6 目标在构建 **devel/p5-Sys-SigAction** 时仍然显示失败。
5. **大量文档更新需求**：这些修改实际上使得能够通过 chroot 进入 MIPS 或 ARMv6 环境，并使用本地工具链和库为目标平台原型化软件。

### RPC/NFS 和 CTL/iSCSI 性能优化
...
作为新的内核 iSCSI 服务器基础的 CAM Target Layer（CTL）也进行了多项锁机制优化，使其峰值请求速率从约 20 万提高到约 60 万 IOPS，预计能够达到每秒 100 万个请求的速率。这个速率足以完全饱和两个 10Gbit 以太网链路，且请求大小为 4KB。相比之下，使用相同硬件和等效配置的 **net/istgt**（用户级 iSCSI 服务器）仅显示了 10 万 IOPS。

另外，CTL 功能的改进工作仍在进行中。它已支持四个 VMware VAAI 存储加速原语中的三个（**net/istgt** 支持 2 个），而目标是在未来几个月内实现完全的 VAAI 支持。

凭借所有这些改进，以及 CAM、GEOM、ZFS 和其他内核领域即将推出的早期改进，FreeBSD 10.1 可能会成为有史以来最快的存储版本。;)
...
ZFSguru 是多功能服务器设备，强调存储功能。ZFSguru 最初是简单的 ZFS Web 界面前端，但后来发展成为具有自己基础设施的 FreeBSD 衍生版。随着项目的扩展，增加了附加软件包，提供了超越 FreeNAS 和 NAS4Free 等类似产品中传统 NAS 功能的功能。ZFSguru 旨在成为真正的多功能服务器设备，极易设置，并能将初学者和更有经验的用户结合在同一用户界面中。项目的模块化特点避免了臃肿的风险，同时仍然能轻松部署扩展功能。

今年第一季度的开发为 Samba 和 NFS 提供了拖放权限管理，而第二季度的开发重点是加强项目的基础设施。名为“Mesa”的新库和工具包解决方案正在开发中，它为项目提供了更清晰的基础。正在设置新的主服务器来提供安全的远程服务，该服务器位于高速数据中心。最重要的是，新的系统构建基础设施取得了显著进展，并很快能够为用户提供自动化的系统构建。这不仅提高了系统发布的频率，还释放了大量开发时间，可以用于项目的其他领域。

此外，正在开发新的官网和论坛，以取代功能有限的老旧网站。新网站将与服务器数据库连接，提供有关项目的实时更新。

另外，正在开发新的协作开发平台。为 GitLab 项目创建了服务附加组件，GitLab 是流行的 GitHub 网站的替代品。选择托管自己的解决方案，而不是依赖 GitHub 本身。事后看来，这个决定是正确的。最近 GitHub 在接到 DMCA 下架请求后删除了项目，这与 ZFSguru 项目提倡的自由信息流通理念不符。通过托管自己的解决方案，我们避免了对第三方项目的依赖。
...
有关该项目更详细的状态报告，可以通过 [stateoftheproject](http://zfsguru.com/news/stateoftheproject/2014) 链接找到。
...
### 在 Fiasco.OC 微内核上运行 FreeBSD 作为应用程序
...
SDIO 卡的检测和初始化现已正常工作。大多数必要的总线方法已实现并经过测试。

WiFi 驱动程序能够将固件加载到卡中并初始化它。MMC 栈的重写作为 CAM 框架的传输层正在进行中。这将允许使用经过充分测试的 CAM 锁机制和调试功能。

#### 待办事项

1. SDIO 栈：完成 CAM 迁移。MMC/SD 卡的初始化已在 XPT 层实现，但由于缺乏实现外设驱动和 CAM MMC 的 SIM 设备驱动程序，无法使用实际硬件进行测试。计划使用修改版的 BeagleBone Black SDHCI 控制器驱动作为 SIM，并使用修改版的 **mmcsd(4)** 作为外设驱动。
...
使用 `stress2` 内核测试套件对 **tmpfs(5)** 进行了广泛的测试。发现的问题均已调试并修复。
...
Ed 和 Nathan 在过去三个月中完成了多项集成任务。Nathan 添加了第一阶段加载程序 `boot1.efi`，用于支持从 UFS 文件系统链式加载其余系统。这使得 UEFI 启动过程能够与 BIOS 启动过程类似地进行。Nathan 还为 FreeBSD 安装程序和发布镜像创建脚本添加了 UEFI 支持。

EFI 帧缓冲需要 **vt(4)** 系统控制台——由于没有为传统的 **syscons(4)** 控制台实现帧缓冲驱动程序，Ed 为 UEFI 启动路径添加了自动选择 **vt(4)** 的功能。

现在快照已构建为双模式镜像，应该能够通过 BIOS 和 UEFI 启动。我们的计划是将 UEFI 和 **vt(4)** 的工作合并到 **stable/10** 分支中，以便在 FreeBSD 10.1-RELEASE 中发布。
...
自上次 [报告](http://www.freebsd.org/news/status/report-2014-01-2014-03.html#Updated-vt%284%29-System-Console) 以来，**vt(4)** 增加了早期驱动程序选择的功能。**vt(4)** 在大多数内核子系统初始化之前选择最成功探测到的驱动程序。此外，为了促进从 **syscons(4)** 向 **vt(4)** 的迁移，现在支持内核中的多个虚拟终端子系统。它由一个小型模块控制，仅需一个内核环境变量。用户可以通过设置 `kern.vty=sc` 或 `kern.vty=vt` 来选择使用的虚拟终端系统。
...
Arm64 是正在进行的将 FreeBSD 移植到 ARMv8 CPU 的项目名称，该 CPU 在 AArch64 模式下运行。直到最近，所有 ARM CPU 设计都仅支持 32 位。随着 ARMv8 架构的引入，ARM 增加了新的 64 位模式，这个新模式被命名为 AArch64。

自上次状态报告以来，在 ARM 基础模型上启动 FreeBSD 取得了很大进展。初步的 pmap 实现已完成。通过这个实现，FreeBSD 可以进入机器独立的启动代码。已添加必要的 autoconf 函数，允许 FreeBSD 开始调度任务。最后，添加了 `cpu_switch` 和 `copystr` 函数。通过这两个功能，FreeBSD 可以启动到 mountroot 提示符。

关于支持异常（包括中断）的工作也已开始。这将允许更多开发人员开始编写设备驱动程序。
...
我们很高兴宣布，基于最近引入到 Ports 框架中的 `USES=uniquefiles` 特性，Python 软件包支持在不同版本的 Python 之间无冲突地并行构建和安装。现在可将 Python 软件包标记为在同一主机上并行为不同 Python 版本构建和安装。然而，构建工具尚不支持此特性，Python 团队将与 `portmgr` 和 pkg 开发人员紧密合作，以在全局 Ports 和软件包范围内启用此支持。

5 月和 6 月期间，进行了大量清理工作，移除最后与 `easy_install` 相关的部分。7 月初，我们提交了最终的更改，完全从 Ports 框架中移除了 `easy_install` 的支持。这极大地简化了基础设施，并让我们能够以更少的精力进行现代化和维护。

我们添加了 Python 3.4，移除了 Python 3.1（在其生命周期结束后），更新了 `setuptools` Port 到 5.1 版本，并将 PyPy 的开发版本更新至 2.3.1。最新的 Python 2.7.8 和更新的 `setuptools` 即将合并到代码库中。

我们的上游合并工作继续取得积极进展，简化了维护并减少了复杂性。

展望未来，最重要的优先事项之一是在可预见的未来内遵循 `USES` 框架，并为将新的 Python 相关 Ports 集成到代码库中制定一致的维护者政策。

#### 待办事项

1. 将 `bsd.python.mk` 迁移到 `USES` 框架。
2. 制定高级别且轻量级的 Python Port 政策。
3. 为细粒度依赖（例如 `>= 1.0,< 2.0`）添加支持。
4. 查看添加 pip（Python 软件包索引）支持需要做什么。
5. 为 Python Port 添加默认的 QA 目标和函数（`TEST_DEPENDS`，回归测试等）。
...
在本季度，团队保持了大多数 KDE 和 Qt Port 的最新状态，主要更新了以下版本：

- KDE SC：4.12.5；工作空间：4.11.9

根据 PortScout 的数据，`kde@` 目前有 526 个 Port（与之前相同），其中 84.63% 是最新的（相比之前的 98.86% 略有下降）。iXsystems Inc. 继续为团队提供机器，用于构建软件包和测试更新。长期以来 iXsystems Inc. 一直为 KDE/FreeBSD 团队提供支持，我们对此表示感谢。
...
1. 更新过时的 Port，详见 [PortScout](http://portscout.freebsd.org/kde@freebsd.org.html) 中的列表
2. 移除对 HAL 的依赖
...
目前，使用 `WITH_NEW_XORG` 构建的替代 **pkg(8)** 仓库现已可用。这可以缓解用户需要使用 `WITH_NEW_XORG` 重建 Port 的问题。请参阅上面的公告，了解更多信息。

由于 Jan Kokemüller 的贡献，现在 Radeon 32 位 ioctl 在 64 位主机上工作正常。这在 FreeBSD 9.x 和 11 上通过 Wine 和《星际争霸 II》得到了成功测试。为使其与 `WITH_NEW_XORG` 一起工作，需要对 **emulators/i386-wine-devel** 进行修改，并创建了新 Port `libtxc_dxtn`，以支持《星际争霸 II》使用的纹理压缩。目前我们还没有时间进一步完善这些内容，因此仍然需要手动步骤。
...
**联系方式**：季度状态报告团队 <[monthly@FreeBSD.org](mailto:monthly@FreeBSD.org)>

这些季度状态报告帮助 FreeBSD 社区及时了解项目内外的动态。报告内容包括 FreeBSD 团队的更新、正在开发的新特性、基于 FreeBSD 开发的产品以及 FreeBSD 相关事件等，都是状态报告欢迎的内容。

自上次报告以来，月度团队一直在忙碌工作，长期的组织者 Gábor Páli 已从团队中退下——感谢 Gábor 所做的所有辛勤工作！这一变化在报告的准备工作中留下了一些空缺，因此征集条目的通知也相对较晚。为了填补这一空缺，Warren Block 和 Benjamin Kaduk 加入了 `monthly@` 团队，与 Glen Barber、Gavin Atkinson、Ed Maste 和其他团队成员一起准备此次报告。特别感谢 Glen 在同时发布 9.3-RELEASE 的过程中，承担了大部分工作！
...
OpenStack 是云操作系统，控制数据中心中大规模的计算、存储和网络资源。

OpenContrail 是网络虚拟化（SDN）解决方案，包括网络控制器、虚拟路由器和分析引擎，可以与像 OpenStack 或 CloudStack 这样的云编排系统集成。
...
- OpenContrail Agent（网络控制器节点）移植到 FreeBSD。
- 集成与性能优化。

自上次报告以来，已完成以下工作，这些工作使得在 FreeBSD 主机上使用 OpenContrail 进行网络虚拟化的 OpenStack 计算节点演示得以实现：
...
在 BSDCan2014 渥太华的 DevSummit 上展示了这一演示。此外，还在波兰克拉科夫组织了一场关于该主题的 Meetup。
...
年度董事会会议于 5 月在加拿大渥太华举行。选举了董事和管理人员，并进行了长期规划。我们讨论了我们的愿景、核心价值观、项目路线图和短期目标。我们还与核心团队会面，讨论了角色和责任、项目路线图以及我们能做些什么来更好地帮助项目。

我们是 BSDCan 的 Gold+ 赞助商，会议于 5 月 16-17 日举行，并为开发者提供了 7 个旅行赞助，用于参加会议。我们还为开发者和厂商峰会提供了赞助。
...
为支持 Sentex 的升级，购买了硬件。部署了一台新的高容量 1Gbps 交换机，以便在测试实验室中添加更多系统。主文件服务器和开发机器已升级，允许更多用户同时使用实验室。
...
一些先前完成的基金会赞助项目得到了增强或附加工作。ARM 超级页项目在去年完成，但现在在 FreeBSD-CURRENT 中默认启用。许多稳定性修复和增强提交到内核中的 iSCSI 栈。iSCSI 项目在 FreeBSD 10.0 中发布。许多稳定性修复和增强已提交，并将包含在 FreeBSD 10.1 中。

基金会赞助的工作包括继续进行 autofs 自动挂载守护进程、UEFI 启动支持、更新的 **vt(4)** 系统视频控制台、虚拟机镜像和 Intel 图形驱动更新。在 4 月到 6 月期间，基金会赞助的工作向 FreeBSD 提交了 226 次提交。
