...
- 原文：[FreeBSD Quarterly Status Report 4th Quarter 2014](https://www.freebsd.org/status/report-2014-10-2014-12/)

## 引言

本报告包括了 2014 年 10 月至 12 月之间与 FreeBSD 相关的项目。这是 2014 年计划的四个报告中的最后一份。

2014 年第四季度，FreeBSD 系统进行了许多重要的改进，特别是在与其他系统的兼容性方面取得了显著进展。这包括对 Linux 兼容层的重大改进，该层用于在 FreeBSD 上运行 Linux 二进制文件，以及对 WINE 的移植，用于运行 Windows 应用程序。虚拟化支持得到提升，FreeBSD 获得了在 Xen 的新高性能 PVH 模式下作为 Dom0 运行的能力，`bhyve` 增加了对 AMD 的支持，且新增了创建 FreeBSD 虚拟机镜像的工具。
...
在第四季度，五位新开发者获得了 Ports 提交权限（gordon@、jmg@、jmmv@、bofh@、truckman@），暂时收回了六位开发者的提交权限（sylvio@、pclin@、flz@、jsa@、anders@、motoyuki@）。

在管理方面，miwi@ 决定在 11 月辞去其 `portmgr` 职位。第四季度期间，团队无其他变动。

这一季度还发布了第四个季度分支，即 `2014Q4`。

在质量保证方面，共进行了 39 次 exp-run，以验证敏感的更新或清理工作。
...
在 2014 年第四季度，FreeBSD 核心团队完成了长期项目——重建 FreeBSD 论坛。选择的解决方案是获得 XenForo 的授权；核心团队感谢 FreeBSD 基金会支付该软件的许可费用。

在 9 月份的 EuroBSDCon 会议后，关于“新支持模型”的讨论激烈展开。大家认识到，在 10.1-RELEASE 发布之前尝试立即改变模型为时已晚，因此这一变化将瞄准 11.0-RELEASE。

为了确保 10.1-RELEASE 在发布时支持最新的 X Window 系统和 KDE4，核心团队批准了将 `new Xorg` 作为默认选项，以便及时为该版本构建软件包。

Git 正式从测试版提升为正式支持的版本控制系统。Git 作为只读资源对下游用户开放，并包含来自 SVN 的导出副本，SVN 是主库，也是唯一的读写仓库。FreeBSD 的 Git 仓库（从主 SVN 版本控制导出）将很快在 `https://git.freebsd.org/` 上提供，核心团队一直在积极确保有足够的 Git 管理员，并为他们提供适当的文档，以维护良好的 Git 服务。

核心团队调解了一些提交者之间关于系统源代码更新的争议，并处理了关于其他关键领域工作代码质量的投诉。虽然此类分歧偶尔会发生，但核心团队提倡常规使用 Phabricator 服务，以便在提交前审查工作。及早发现问题符合项目的最佳利益，在开放的审查环境中讨论变更，应该能最大程度地减少立即撤销变更的对抗性要求。

核心团队正在为拟议的 QA 团队起草章程，该团队将包括发布工程和安全团队的成员，以及对标准遵从性有兴趣的提交者。预计 QA 团队将负责将代码从 **HEAD** 合并到 **STABLE** 分支，针对这些更新进行集成测试，并处理提交给 FreeBSD 项目的第三方补丁和漏洞修复。

在这一季度，核心团队授予了两个新的提交权限，并暂时保管了两位提交者的权限。
...
`bhyve` 是在 FreeBSD/amd64 平台上运行的虚拟机监控程序。目前，它支持运行 FreeBSD（8.x 或更高版本）、Linux i386/x64、OpenBSD i386/amd64 和 NetBSD/amd64 虚拟机。目前的开发集中在启用更多的客户操作系统，并实现其他虚拟机监控程序中的功能。
...
1. 改进文档。
2. `bhyveucl` 是基于 libUCL 配置文件启动 `bhyve` 实例的脚本。更多信息请参见 <https://github.com/allanjude/bhyveucl>。
...
在年底之前，我们将基本系统中的 `clang`、`llvm` 和 `lldb` 更新到了 3.5.0 版本。这些版本都包含了许多改进。有关详细信息，请参阅链接中的发布说明。
...
1. 尽管大部分受到此更新影响的 Port 已修复，但仍有一些 Port 无法与 `clang` 3.5.0 更新兼容。
...
- 准备迁移到较新版本的工具链组件。
- 将 FreeBSD 移植到新架构。
- 从带有 `GCC` 4.2 的 FreeBSD 升级到带有 `clang` 3.5+ 的版本（`clang` 3.5+ 需要比 `GCC` 4.2 更现代的工具链来引导构建）。

最初的外部工具链工作仅支持 `clang`。该项目已扩展，支持了最近的 `GCC`（已测试 4.9.1）和最近的 `binutils`（2.24 和 2.25）。
...
构建系统已更改，以便在使用较新的 `GCC`（4.6+）构建 world 时，将 `libc++` 作为 C++ 标准库实现。
...
通过这项努力，已能够成功构建并运行使用 `GCC` 4.9.1 和 `binutils` 2.24 构建的内核和 world，支持 `sparc64`、`amd64`（稍作调整）、`powerpc` 和 `powerpc64` 架构。
...
Google Cloud 是云计算平台，允许用户在由 Google 维护的云中运行托管服务和服务器。此项目的目标是提供一种简便的方法来创建和管理在 Google Cloud 上运行的 FreeBSD 安装。
...
这些命令是 **google-cloud-sdk** Port 的一部分，包含了与 Google Cloud 交互的所有命令。此外，还有 **google-daemon** Port，用于运行实例以创建用户并为其进行设置，还有 **google-startup-scripts** Port，负责按照节点元数据中指定的内容运行启动/关机脚本。

此外，**firstboot-growfs** Port 已恢复，这样新的实例将会扩展它们的根文件系统。（感谢 Colin Percival 最初创建了这个 Port。）

还有 **firstboot-freebsd-update** Port，可以在首次启动时更新系统，但目前禁用（见下文）。类似地，**firstboot-pkgs** Port/脚本将在首次启动时安装指定的软件包。
...
7. sysctl 节点 `kern.vm_guest` 无法检测到它在虚拟机中。
8. `vtnet` 驱动程序需要在 16 CPU 实例上禁用 `wq`，但目前为了简便，已在所有地方禁用。
9. 谷歌的 `safe_format_and_mount` 命令（用于格式化和挂载新附加磁盘）还需要一些工作，但这实际上只是附加功能。
10. 我需要研究 `vtnet` 的 `irq affinity`。
11. 我们需要支持虚拟化时钟；bryanv@ 正在这方面进行工作。事实上，他在虚拟化领域的所有持续工作可能会让事情运作得更好。
12. 如果能够禁用加载器之前的 spinner，那将会更好，因为它会弄乱控制台日志。禁用该功能已在 **HEAD** 中；希望它能在 **10-STABLE** 发布之前 MFC 到 **10-STABLE**。

### 在 Acer C720 Chromebook 上运行 FreeBSD
...
Acer C720 Chromebook 是一款功能强大且价格低廉的笔记本电脑，设计用于运行 Google 的 Chrome OS。该项目旨在将 FreeBSD 引入 C720，提供简单的方式，让用户在广泛可用且廉价的硬件上体验 FreeBSD。
...
- 应 Alfred Perlstein 的要求，创建了新的邮件列表 <freebsd-git@FreeBSD.org>，用于讨论在 FreeBSD 项目中使用 `git`。
- Alfred Perlstein 提交了补丁到 `git`。这个补丁允许开发人员在 `git` 中处理源代码树，并使用 `git-svn` 将该树的更改直接推送到 Subversion 仓库并设置 Subversion 属性。在此补丁之前，`git-svn` 无法正确设置 Subversion 属性。对 FreeBSD 开发人员来说，这一点很重要，因为 FreeBSD Subversion 仓库会阻止未正确设置某些 Subversion 属性的提交。`git` 项目接受了这个变更，提交哈希为 83c9433。
- Alfred Perlstein 更新了 Git 工作流的 Wiki 文档，包含了如何使用 `git-svn` 提交到 FreeBSD Subversion 仓库的信息。
- Bartek Rutkowski 编写了脚本，将 GitHub 和 FreeBSD Bugzilla 集成。当用户在 GitHub 上提交针对 FreeBSD 源代码树的 pull 请求时，该脚本将在 FreeBSD Bugzilla 中打开新的 PR。这样，用户就可以通过 GitHub 的 pull 请求提交代码和补丁，并让 FreeBSD 开发人员在 Bugzilla 中跟踪该请求。当前，GitHub 上的 pull 请求无法直接合并到 FreeBSD 源代码树中，因为主源代码仓库目前是 Subversion。GitHub 上的 FreeBSD 源代码树是 FreeBSD Subversion 仓库的只读镜像。Craig Rodrigues 协调了 Bartek Rutkowski 和 <bugmeister@FreeBSD.org> 的工作，推动此事，并为 Bartek Rutkowski 提供了足够的 Bugzilla 访问权限，以便通过脚本打开 PR。
...
- Craig Rodrigues 在 2014 年 11 月 3 日于加利福尼亚州圣何塞举行的开发者与供应商峰会上，做了名为“Kyua 和 Jenkins 测试框架”的 BSD 专题演讲。在演讲中，Craig Rodrigues 介绍了在 FreeBSD 源代码树的每次提交时，如何在 `bhyve` 虚拟机内使用 `kyua` 运行近 3000 个测试。`kyua` 测试结果导出为 JUnit XML 格式，然后由 Jenkins 用于生成带有图表的基于 Web 的测试报告。
- 許立文设置了 Jenkins 构建，名为 `FreeBSD_Doc-igor`，来运行由 Warren Block 编写的 Igor 工具。Igor 用于校对 FreeBSD 文档，并报告各种错误。
- Craig Rodrigues 设置了 Jenkins 构建，名为 `FreeBSD_HEAD_sparc64`，来构建 `sparc64` 架构的 FreeBSD **HEAD** 分支。
...
- Craig Rodrigues 向 Homebrew 软件包管理系统提交了 `kyua` 软件包的配方，Homebrew 项目已接受该配方。现在，`kyua` 可以通过 Homebrew 软件包轻松地在 OS X 上安装。希望这将使得与 OS X 共享更多的测试基础设施和脚本变得更加容易。
- Craig Rodrigues 向 Debian 项目提交了 `kyua` 软件包，批准仍在等待中。该软件包将使得在使用 Debian 软件包的 Linux 发行版（如 Debian、Ubuntu 和 Linux Mint）上安装 `kyua` 更加容易。希望这将使得与 Linux 共享更多的测试基础设施和脚本变得更加容易。
- Brian Gardner 提交了运行 OpenJDK 回归测试工具 jtreg 的脚本。测试结果以 JUnit XML 格式提供，可以直接导入 Jenkins。
- Ahmed Kamal，经验丰富的 devops 专家和 Ubuntu 项目的过去贡献者，提供帮助，协助 Craig Rodrigues 改进使用 Saltstack 自动化框架在 FreeBSD 集群中部署 Jenkins 节点的自动化工作。Ahmed 有兴趣为 FreeBSD 项目提供帮助。
...
ELF 工具链提供了与 GNU Binutils 套件等效的工具。该项目的目标是将这些工具导入 FreeBSD 基本系统，以便我们拥有最新且得到维护的工具，且能够支持新的 CPU 架构，如 `arm64`。

以下工具已导入，且通过设置 `src.conf` 选项 `WITH_ELFTOOLCHAIN_TOOLS=yes` 可用：
...
Port 的 exp-run 揭示了这些工具中的一些 bug。这些 bug 正在 FreeBSD 源树中修复，且正在提交给上游项目。

ELF 工具链的 `readelf` 在添加缺失的 ELF 注释解析功能后，也将启用。ELF 工具链的 `elfcopy` 提供与 Binutils 的 `objcopy` 等效的功能，并接受相同的命令行参数。为了成为替代 `objcopy` 在基本系统中所有用例的可行方案，它必须支持写入可移植可执行（PE）格式的二进制文件，这些格式由 UEFI 启动代码使用。
...
1.4 版本开发方向的重要部分是稳定现有功能并改善 **pkg(8)** 在小型与嵌入式机器上的体验（减少内存使用和加速操作）。
...
关于来宾改进（在 DomU 中运行 FreeBSD），也有正在进行的工作，旨在为 Xen `blkfront` 添加未映射 IO 支持，这项工作受到阻碍，等待对通用反弹缓冲区代码进行一些修改。
...
4. 改进通用反弹缓冲区代码以支持 `blkfront` 对齐要求，从而支持未映射的 bio。
...
目前，Linux 兼容层项目已实现了支持 glibc 2.20 所需的所有系统调用以及更多，使仿真 Linux 内核版本达到 2.6.32：
...
该项目的代码位于 FreeBSD 项目的 Subversion 仓库 **base/user/dchagin/lemul**（稍微有些过时）。为了促进将改进合并到 **HEAD**，已将一些补丁放到 reviews.FreeBSD.org 上，标签为 `#lemul`。接近一半的补丁已获得 Ed Maste 和 Edward Tomasz Napierała 的批准。
...
PCI 单根 I/O 虚拟化（SR-IOV）是 PCIe 标准的可选部分，提供对 PCIe 设备虚拟化的硬件加速。当使用 SR-IOV 时，PCI 设备中的功能（称为物理功能，PF）将在 PCI 总线上呈现多个虚拟 PCI 功能（VF）。这些 VF 是完全独立的 PCI 设备，能够访问 PF 的资源。例如，在网络接口卡上，VF 可以独立于 PF 发送和接收数据包。
...
增加了 Reaper 功能，允许进程可靠地跟踪其所有子进程的运行和退出状态。该功能旨在改进像 **timeout(1)** 或 poudriere 这样的工具，防止失控的孙子进程逃脱控制进程的控制。该功能的设计参考了 DragonFlyBSD 和 Linux 中类似的设施，并借鉴了 Solaris 合同的一些做法。已在 r275800 提交到 **HEAD**。

FreeBSD 的暂停代码无法确保系统（包括软件和硬件）处于稳定一致的状态。其中一个方面是用户模式进程活动尚未停止，它们继续向硬件发出请求。在 `SUSPEND_CHILD` 后，期望驱动程序能够正确处理这些调用并不现实。

我们开发了一种机制，在安全点停止用户模式线程，在这些点上它们已知不会占用或等待内核资源，特别是不会等待设备请求完成。该机制基于现有的单线程代码，但进行了扩展，允许外部线程将某些进程置于停止状态。此外，增加了一种在暂停之前同步文件系统的功能，确保将一致的元数据和尽可能多的缓存用户数据写入稳定存储，以最大限度地减少恢复失败时可能造成的损害。

该代码对系统的一些部分施加了压力，导致在多个领域（包括进程管理、缓冲区缓存和系统调用处理程序）发现了一些 bug。已修复这些 bug，修复和功能通过一系列提交（最终在 r275745 提交）得到了完成。
...
- 扩展 shim 提供 API，供 boot1.efi 加载并验证由 shim 已知的密钥签名的二进制文件
- 编写 **uefisign(8)**，BSD 授权的工具，用于使用 UEFI 规范要求的 Authenticode 签名 EFI 二进制文件。
...
自 2006 年以来，Linux 计时器功能的初步支持已存在，但未经过测试。本次更新修正了最初的实现，并使其不仅适用于 `i386`，也可用于 `amd64` 上的 32 位 Linux 兼容层。
...
ARM 的 64 位架构引起了越来越多的关注。正式名称为 `AArch64`，也称为 `ARMv8` 和 `arm64`。Andrew Turner 于 2012 年底开始了 FreeBSD/arm64 移植的初步工作。

FreeBSD 基金会目前与 ARM、Cavium、Semihalf 团队和 Andrew Turner 合作，正在将 FreeBSD 移植到 `arm64`，且在 2014 年最后一个季度取得了显著进展。

截至年底，FreeBSD 已能够在 `arm64` 上启动至单用户模式，执行静态和动态应用程序。正在审查的补丁使 FreeBSD 能够启动到多用户模式，预计这些补丁将很快合并。这包括在用户空间和内核中实现许多存根函数。通过这些更新，FreeBSD 已能够在 ARM 基金会模型和 `QEMU` 完整系统仿真上启动到多用户模式。

Cavium 提供了他们的 Thunder X 硬件的软件仿真器。FreeBSD 在该仿真器上的启动工作已开始，包括为 ARM 通用中断控制器 v3（GICv3）编写新的驱动程序，并为 PCIe 根复合体编写初步驱动程序。通过这些更新，FreeBSD 已能够在该仿真器上启动，为硬件测试做准备。进一步的工作正在进行中，以增加完整的 PCIe 启动，并为 GICv3 中断翻译服务（ITS）和 MSI-X 增加支持。
...
许多 FreeBSD 工具提供了对正在运行的 FreeBSD 系统操作状态的洞察，因此这些工具用于定期监控系统。这些工具通常提供人类可读的输出，有时甚至优化为适应传统终端的有限宽度。其他程序通常也会使用这些工具，希望以不同的方式呈现输出或作为其他用户界面的一部分。对于这些用例，处理机器可读的输出比处理人类可读的输出要好得多。

Juniper Networks 创建了库 `libxo`，它使得工具可以轻松地生成多种格式的输出。默认情况下，输出为文本，但引入了 `--libxo` 选项后，可以将其更改为 XML、JSON 和 HTML。FreeBSD 项目已将此库导入到基本系统中，并正在重新编写工具以使用 `libxo`。

与此相关，FreeBSD 现在还拥有 `xo` 工具，允许脚本获得相同的功能。与在脚本中使用 `echo` 或 `printf` 不同，输出可以通过 `xo` 工具完成。

`df`、`w` 和 `wc` 工具已转换为使用 `libxo`。`netstat` 工具正在转换中，其他工具也计划进行转换。
...
一些 **groff(1)** 和 **mandoc(1)** 之间的格式差异也已发现，并报告给 mandoc 的上游开发者。
...
FreeBSD GNOME 团队负责维护 FreeBSD 上的 GNOME、MATE 和 CINNAMON 桌面环境及图形用户界面。GNOME 3 是 GNU 项目的一部分。MATE 是 GNOME 2 桌面的分支。CINNAMON 是使用 GNOME 3 技术，但具有 GNOME 2 外观和感觉的桌面环境。

这一季度对 GNOME 团队来说是令人兴奋的时刻。我们将 GNOME 3.14.0 和 CINNAMON 2.2.16 导入了 Ports。与此同时，我们移除了旧的 GNOME 2.32 桌面。两周后，我们将 GNOME 更新为 3.14.2，并将 CINNAMON 更新为 2.4.2，这些更新是在准备初始 GNOME 3.14.0 导入时收集的。
...
如上个季度所提到的，Alonso Schaich (alonso@) 成为了提交者，自那时以来已取得了显著进展，协助其导师 Raphael Kubo da Costa (rakuco@) 和 Max Brazhnikov (makc@) 维护所有与 Qt 和 KDE 相关的 Port。

本季度，Qt 5.3 最终提交到 Ports 中。为了实现这一目标，进行了大量的工作，包括清理或更改 Qt5 Port 基础设施，以使其更易于维护，同时也最终使得在系统上已安装较旧版本的情况下构建较新版本成为可能。
...
1. 将 Qt5 更新为 5.4.0。
2. 尝试为 FreeBSD 上去除 HAL 的工作做出贡献，近期这一工作似乎获得了更多支持。
...
在 Ports 中，Linux 模拟栈已升级以包含 CentOS 6.6，更新于 11 月 11 日。在解决了若干引入的错误后，我们终于能够将默认的 Linux 用户空间从 Fedora 10 更新到 CentOS 6.6，更新于 12 月 9 日。提供更新的 Linux 用户空间和支持库使得大量 Linux 应用能够在 FreeBSD 上运行。
...
Mesa 已升级到 10.3，然后是 10.4，用于 FreeBSD 10.1-RELEASE 和 **11-CURRENT**。我们测试发布候选版本，因此这个 Port 现在通常会在新版本发布后不久进行更新。Mesa 10.x 在 OpenGL 标准支持、性能和稳定性方面带来了巨大的改进，尤其是对于 Radeon 用户。Mesa 9.1 保持支持 FreeBSD 9.x，但我们计划修复这一点；见下文。

**graphics/gbm** 和 **devel/libclc** 是 Mesa 用于实现 OpenCL 的新 Port。下一步是完成 Mesa 的 libOpenCL.so（Clover）的 Port。这将使用户能够在 Radeon GPU 上运行 OpenCL 程序。

`xserver` 已从 1.12 升级到 1.14。这是支持 Mesa 9.1 的最后一个 `xserver` 版本。变更已在博客文章中介绍。最显著的变化是从基于 HAL 的输入设备检测后端切换到基于 **devd(8)** 的后端。许多桌面环境仍然需要 **hald(8)**，但 X.Org 服务器本身已不再依赖它。

`xserver` 是最后一个支持选项 `WITH_NEW_XORG` 的 Port。该选项现已完全移除。这是将 `WITH_NEW_XORG` 和 `WITH_KMS` 添加到废弃开关列表的契机，帮助用户清理他们的 `make.conf`。同时，新的 xorg 替代 pkg 仓库也已废弃。
...
这两个软件包现在提供了更好的开箱体验。用户如果不喜欢这些选项，可以取消并重新构建 Port。无需重新构建任何其他内容。
...
XDC 2014（X 开发者大会）是一场非常棒的会议。恢复与图形栈开发者的关系取得了成功！报告已在博客中发布。

我们的路线图中的下一项任务是：

1. 通过新 Port 将 FreeBSD 10.1-RELEASE 的 `i915` 驱动程序提供给 FreeBSD 9.x 用户。这项工作正在进行中，但它将使我们能够移除 Mesa 9.1，并使 Mesa 10.4 在所有地方可用。
2. 待移除 Mesa 9.1 后，我们可以将 `xserver` 更新为 1.16。
...
这些 Port 已为 FreeBSD 8.4、9.1+、10.0+ 和 **CURRENT** 的 `amd64` 提供了构建的软件包（通过 Port **emulators/i386-wine** 和 **i386-wine-devel** 提供）。
...
- 创建支持 64 位的 Wine Port（即 `Wine64`）。
- 创建支持 WoW64 的 Wine Port（即 Wine + `Wine64`）。
...
Xfce 是一款自由软件桌面环境，适用于 Unix 和类 Unix 平台，如 FreeBSD。它旨在提供快速和轻量级的性能，同时保持视觉上的吸引力和易用性。
...
常见问题解答正在编写中 [D1305](https://reviews.freebsd.org/D1305)。
...
可以通过他的博客获取最新状态，或在 Twitter 上关注 @mwlauthor。
...
已创建新的邮件列表，供将 FreeBSD 文档和程序从英语翻译成其他语言的人使用。讨论内容可以包括方法、工具和技巧。建议现有翻译人员加入，以便有统一的联系点。欢迎新翻译人员和希望参与翻译的人加入。

要求新成员介绍自己，并提到他们有兴趣翻译的语言。
...
[Packer](https://www.packer.io/) 是创建多个平台相同机器镜像的工具，基于单一的源配置。

[Vagrant](https://www.vagrantup.com/) 是用于创建和配置轻量、可重复、便携的开发环境的工具。
...
在 FreeBSD 基金会的赞助下，FreeBSD 论坛已迁移到 XenForo 软件。新软件更强大且易于使用。虽然整个论坛团队都有贡献，但 Daniel Geržo 出色地完成了现有用户和消息的导入工作，并恢复了用户们常常要求的“感谢”功能。此次升级已完成，正好赶上 FreeBSD 10.1 发布后新用户的大量涌入，我们已看到使用量的增加。

拥有 @FreeBSD.org 邮箱地址的开发者可以联系论坛管理员，获取备受渴望的“@”后缀的论坛用户名以及开发者标志。
...
我们是 [Grace Hopper Conference](http://gracehopper.org/2014/) 的赞助商。Dru Lavigne 进行了“FreeBSD 介绍”演讲，参加人数众多。我们还赞助了 Shteryana Shopova 与 Dru 一同代表 FreeBSD 在我们的展位上展示。
...
George 组织并主持了 MeetBSD 之后的两天硅谷供应商和开发者峰会。在这些峰会上，许多工作得以启动并完成，例如，Kirk 与各方合作推动 `ino64`（64 位 inode 编号）项目的进展。该项目于 2011 年作为编程之夏项目启动，在并入系统后进展断断续续。
...
LISA 会议上，Dru Lavigne 的 FreeBSD BoF 演讲吸引了大量观众。我们还访问了湾区的一些大型 FreeBSD 用户，讨论他们使用 FreeBSD 的情况、计划和需求，并帮助促进他们与项目之间的合作。

Cheryl Blain 加入了我们的董事会，带来了强大的商业发展和筹款经验。

我们收到了历史上最大的一笔捐款，我们的财务主管制定了捐赠基金策略，以供我们跟进。
...
Glen 还访问了 NYI 托管设施，安装和配置了基金会购买的新服务器。

我们完成了 10.1-RELEASE 发布周期。

我们的项目开发人员和承包商一直在进行各种项目，以增加功能并改进 FreeBSD。其中一些报告已包含在本报告中。这个季度的一些工作项目包括为 FreeBSD 添加对 64 位 ARM 架构的支持、更新的 **vt(4)** 控制台和 UEFI 启动支持的集成工作、Secure Boot、优化内核中的 iSCSI 目标和发起者栈、基于 autofs 的自动挂载守护进程、迁移到 ELF 工具链，以及在 FreeBSD 加密框架中实现现代 AES 模式。
