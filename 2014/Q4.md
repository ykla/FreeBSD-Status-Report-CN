...
- 原文链接：[FreeBSD Quarterly Status Report 4th Quarter 2014](https://www.freebsd.org/status/report-2014-10-2014-12/)
...
为了确保 10.1-RELEASE 在发布时支持最新的 X Window 系统和 KDE4，核心团队批准了将 `new Xorg` 作为默认选项，并及时构建了该版本的软件包。
...
还有一个 Port firstboot-freebsd-update，可以在首次启动时更新系统，但目前被禁用（见下文）。类似地，firstboot-pkgs  Port /脚本将在首次启动时安装指定的软件包。
...
- Craig Rodrigues 向 Homebrew 软件包管理系统提交了一个 kyua 软件包的配方，Homebrew 项目已接受该配方。现在，kyua 可以通过 Homebrew 软件包轻松地在 OS X 上安装。希望这将使得与 OS X 共享更多的测试基础设施和脚本变得更加容易。
...
这一季度对 GNOME 团队来说是一个令人兴奋的时刻。我们将 GNOME 3.14.0 和 CINNAMON 2.2.16 导入了 Ports。与此同时，我们移除了旧的 GNOME 2.32 桌面。两周后，我们将 GNOME 更新为 3.14.2，并将 CINNAMON 更新为 2.4.2，这些更新是在准备初始 GNOME 3.14.0 导入时收集的。
