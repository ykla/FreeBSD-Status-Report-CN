...
- 原文：[FreeBSD Quarterly Status Report 2nd Quarter 2021](https://www.freebsd.org/status/report-2021-04-2021-06/)
- 版本：2022 年 3 月 1 日
- 最后作者：Minsoo Choo
...
本报告的一些亮点包括但不限于实验性安装程序的工作、pf 的变更、Linuxulator 的额外工作、内核 sanitizers 状态的更新、ZFS 的 raidz 扩展功能的覆盖，以及有关资源记账的一些新闻。
...
你将在我们的报告以及本报告的各单独条目中，了解我们如何在第二季度使用你的捐款。
...
在二季度，基金会工作人员和赞助受益者提交了 348 个源代码树更改、19 个 ports 树更改和 11 个文档树更改。这代表了 40% 的源代码提交标明了赞助商。对于 Ports 提交，这一比例为 15%，文档提交为 18%。基金会工作人员和赞助受益者还参与了多个第三方项目的工作。两个显著的例子是 LLVM 项目的 LLDB 调试器和 Syzkaller 基于代码覆盖的系统调用模糊测试工具。
...
[FreeBSD 13.0-RELEASE 时间表](https://www.freebsd.org/releases/13.0R/schedule/)

[FreeBSD 13.0-RELEASE 公告](https://www.freebsd.org/releases/13.0R/announce/)
...
- 改进了我们主要集群站点的网络设计
  ...
  - 整理了 pkgbuild 和 pkgexp VLAN，其中托管生产和实验性软件包构建器
...
- 将软件包构建机器升级到更新版本的 FreeBSD 14-CURRENT
- 协助邮件管理员将邮件列表从 Mailman 迁移到 mlmmj
- 对一个特别棘手的 PowerPC64 软件包构建机器进行了彻底的清理（愿其安息）
- 安装了一台新的生产用 PowerPC64 软件包构建器
...
[BSDDialog 项目](https://gitlab.com/alfix/bsddialog)

[Dialog 项目](https://invisible-island.net/dialog)

[GPLinBase Wiki 页面](https://wiki.freebsd.org/GPLinBase)
...
[安装程序代码库](https://github.com/yangzhong-freebsd/lua-httpd)

[包含安装程序的 Live ISO](https://github.com/yangzhong-freebsd/ISO)

**联系方式**：Yang Zhong <[yzhong@freebsdfoundation.org](mailto:yzhong@freebsdfoundation.org)>
...
[FreeBSD 开发使用 Git Wiki](https://wiki.freebsd.org/Git)

[Git 迁移 Wiki](https://wiki.freebsd.org/GitTransition)

[文档 Git 仓库网页](https://cgit.freebsd.org/doc)

[Ports Git 仓库网页](https://cgit.freebsd.org/ports)

[源代码 (基本系统) Git 仓库网页](https://cgit.freebsd.org/src)

[提交者指南 Git 入门](https://docs.freebsd.org/en/articles/committers-guide/#git-primer)

[手册 使用 Git 附录](https://docs.freebsd.org/en/books/handbook/mirrors/#git)
...
6 月 8 日，我们与 ZFS 开发人员合作，以便更好地跟踪上游 OpenZFS 的开发。vendor/openzfs 分支被重命名为 vendor/openzfs/legacy。两个新分支直接从上游导入，vendor/openzfs/master 和 vendor/openzfs/zfs-2.1-release，并合并到 main 和 stable/13 分支。对于跟踪旧分支的人可能需要采取的纠正措施，详细信息请参考 [https://lists.freebsd.org/archives/freebsd-current/2021-June/000153.html](https://lists.freebsd.org/archives/freebsd-current/2021-June/000153.html)
...
[Moritz Systems 项目概述](https://www.moritz.systems/blog/lldb-freebsd-cpu-target-support-and-userland-debugging-improvements/)

[进展报告 1](https://www.moritz.systems/blog/freebsd-remote-process-plugin-on-non-x86-architectures/)

[进展报告 2](https://www.moritz.systems/blog/freebsd-legacy-process-plugin-removed/)

[进展报告 3](https://www.moritz.systems/blog/lldb-support-for-fork-and-vfork/)

[进展报告 4](https://www.moritz.systems/blog/lldb-core-dump-support-improvements/)
...
Simran Kathpalia <[skathpalia3@gmail.com](mailto:skathpalia3@gmail.com)>
...
**联系方式**：Jakub Klama <[jakub@conclusive.pl](mailto:jakub@conclusive.pl)>
...
[WIP GitHub 仓库](https://github.com/neelchauhan/drm-kmod/tree/5.7-wip)
...
[iwlwifi 状态 FreeBSD 维基页面](https://wiki.freebsd.org/WiFi/Iwlwifi)
...
Lukasz Hajec <[lha@semihalf.com](mailto:lha@semihalf.com)>
...
[dummynet 进行中的代码库](https://github.com/kprovost/freebsd-src/tree/netgate/dummynet)

**联系方式**：Kristof Provost <[kp@FreeBSD.org](mailto:kp@FreeBSD.org)>

pf 防火墙当前依赖于 ALTQ 进行流量整形。ALTQ 在默认的内核构建中没有启用，且并非与所有网络驱动程序兼容（仅与实现 if_start() 的驱动程序兼容）。
...
[pf-link 进行中的代码库](https://github.com/kprovost/freebsd-src/tree/netgate/pf-link)
...
[pf syncookie 进行中的代码库](https://github.com/kprovost/freebsd-src/tree/modirum/pf-syncookie-cleanup)
...
Jakub Klama <[jakub@conclusive.pl](mailto:jakub@conclusive.pl)>

Wojciech Kloska <[wojciech@conclusive.pl](mailto:wojciech@conclusive.pl)>
...
[racct PR for runj](https://github.com/samuelkarp/runj/pull/11)

**联系方式**：Cyril Zhang <[cyril@freebsdfoundation.org](mailto:cyril@freebsdfoundation.org)>

racct 是 FreeBSD 内核中的一个资源记账系统。它跟踪资源使用情况，如内存使用和 CPU 运行时间。此外，它提供了一个易于使用的接口，用于限制诸如用户和 jails 等实体的资源使用。例如，可能需要设置限制，以控制一个 jail 可以使用的 CPU 数量。
...
[2021 年 FreeBSD 开发者峰会视频](https://youtu.be/3SUKJye54aI?t=6166)

[2021 年 FreeBSD 开发者峰会幻灯片](https://docs.google.com/presentation/d/1FeQgEwChrtNQBHfWSNsPK3Y53O5BnPh3Cz5nRa5GAQY/view)

[来自 Ars Technica 的新闻文章](https://arstechnica.com/gadgets/2021/06/raidz-expansion-code-lands-in-openzfs-master/)

#### 项目

此功能可将硬盘逐个添加到 RAID-Z 组中，逐步扩展其容量。此功能对于小型池（通常只有一个 RAID-Z 组）特别有用，因为这些池没有足够的硬件来通过添加一个完整的新 RAID-Z 组（通常是将硬盘数量加倍）来扩展容量。
...
Paweł Eichler <[pawel@conclusive.pl](mailto:pawel@conclusive.pl)>

Conclusive Engineering 团队正在为 Microchip Switchtec PCI Express 存储交换机系列添加支持。

Switchtec 设备是 PCI Express Gen3/Gen4/Gen5 包交换机，提供内置的 NTB 支持，最多可支持 48 个 NT 分区，广泛的诊断功能和可编程固件，可用于实现额外的功能，如 NVMe 机箱管理。
...
[PR255706](https://bugs.freebsd.org255706/)

[提交 bb995aaf6e25e33b028fa4b32321864b48f49055](https://cgit.freebsd.org/ports/commit/?id=bb995aaf6e25e33b028fa4b32321864b48f49055)

**联系方式**：Emacs Port 团队 <[emacs@FreeBSD.org](mailto:emacs@FreeBSD.org)>
...
欢迎进行额外的测试和社区贡献，特别是如果你能帮助测试特定 Port 更新，请通过邮件列表与我们联系。
...
如果你正在开发其他 Port，USE_GCC=any 已经成为过去式，USE_GCC 不再尝试使用旧的基于 4.2 的系统编译程序（`/usr/bin/gcc`），即使该编译程序存在。
...
[Biostar 手册工具的元 Port](https://www.freshports.org/biology/biostar-tools/)

[简单、便携的集群管理器](https://acadix.biz/spcm.php)

[桌面安装程序](https://acadix.biz/desktop-installer.php)

**联系方式**：

Jason Bacon <[jwb@FreeBSD.org](mailto:jwb@FreeBSD.org)>

Yuri Victorovich <[yuri@FreeBSD.org](mailto:yuri@FreeBSD.org)>

FreeBSD 长期以来为科学计算提供了坚实的基础，其卓越的可靠性、网络和存储性能，以及 FreeBSD Ports 系统，不仅提供了大量的科学软件，还通过允许用户使用额外的编译程序标志轻松地从源代码安装来优化安装过程。
...
在 `irc.libera.chat` 上的 `#helloSystem`，镜像到 [Matrix 上的 `#helloSystem:matrix.org`](https://matrix.to/#/%23helloSystem:matrix.org?via=matrix.org)
...
[github 上的 Pot](https://pot.pizzamig.dev/)
