...
经过一个漫长、疲惫但非常富有成效的 2005 年第三季度，FreeBSD 6.0 已经发布。为了让这个版本取得成功，许多活动被推到了后台。
...
不幸的是，这次出版由于各种原因被推迟了——其中之一就是前面提到的 6.0 版本发布。因此，某些报告可能已经不如收到时那么及时，我们鼓励大家——尤其是这次——访问网页链接，获取更多的最新信息。
...
自谷歌编程之夏结束以来，我一直在努力将新的 Lua 后端集成进来。这个过程已取得成功，现在它已经超过了 BSDINSTALLER-BETA-1 发布的阶段。它现在可以安装 distfiles，但还不能安装 Ports或包。
...
链接
  ...
  [Wiki/官方项目网站](http://wiki.freebsd.org/moin.cgi/PaoloPisati) 联系人：Paolo Pisati <[p.pisati@oltrelinux.com](mailto:p.pisati@oltrelinux.com)>
...
**链接**联系人：Ivan Voras <[ivoras@yahoo.com](mailto:ivoras@yahoo.com)>
...
**链接**联系人：Daniel Braniss <[danny@cs.huji.ac.il](mailto:danny@cs.huji.ac.il)>
...
工作正在继续，目的是在 FreeBSD 6 发布之前使 pfSense 稳定。一旦 FreeBSD 6 发布，pfSense 将进入最终的 beta 和发布候选阶段，为 1.0 版本发布做准备。
...
由于一封电子邮件错误，6 月的报告未能提交，因此此报告涵盖了过去 6 个月的内容。
...
5. 我们需要一种机制来创建伪多功能卡。最初，似乎我们只需要任意驱动程序能够添加一个 sio 配件，因为这涵盖了我知道的所有情况。资源需要从创建驱动程序“捐赠”给 sio 卡。
...
您可以通过查看 [GNATS 中与声音相关的 PR](http://www.freebsd.org/cgi/query-pr-summary.cgi?category=&severity=&priority=&class=&state=&sort=none&text=sound&responsible=&multitext=&originator=&release=) 并提交反馈，告诉我们问题是否仍然存在，或是否可以关闭 PR 因为 bug 已修复。同时也欢迎提交补丁，解决下面 TODO 列表中的问题。
...
6. 可插拔的 FEEDER 基础设施。用于调试各种 feeder 内容和/或作为用户空间库和测试套件。
...
#### [OpenBSD dhclient 导入](https://www.freebsd.org/status/report-2005-07-2005-10.html#OpenBSD-dhclient-import)
...
2. 我们需要项目文档的帮助。特别是，我们需要帮助审计 [FAQ](http://www.freebsd.org/gnome/docs/faq.html)，确保内容仍然相关，并且我们没有遗漏任何重要内容。如果您感兴趣，请联系 [gnome@FreeBSD.org](mailto:gnome@FreeBSD.org)。
...
我正在为网站添加个性化新闻提要。对于每个您的 Watch List，您将拥有一个专门的新闻提要。所有对您监视列表中的 Port 的提交都会显示在您的新闻提要中。这个非常棒的新功能现在已在 [BETA 网站](http://beta.freshports.org/) 上提供，欢迎通过 RSS 订阅。我还在支持多个平台和架构方面做了一些工作。这将使 FreshPorts 能够正确报告某个 Port 在某些平台（例如 i386）上已损坏，但在其他平台上未受影响。这个功能会记录以下架构的 BROKEN、FORBIDDEN 和 IGNORE 状态：
...
1. 目前，我们非常需要文档方面的帮助。我们已经开始为各种 Tinderbox 命令创建 man 页面，但我们需要帮助以更快的速度完成这些工作。如果您擅长 mdoc 并且有兴趣提供帮助，请联系 [marcus@marcuscom.com](mailto:marcus@marcuscom.com)。
...
一个新的 netgraph-to-zaptel 模块，允许将 E1 (ISDN PRI) WAN 适配器用作开源 PBX——Asterisk 的接口卡。所需的只是一个能够在原始电话模式下工作的适配器（如 Cronyx Tau-PCI/2E1），即没有 HDLC 样的帧结构并且支持 Netgraph。

### [OpenBSD 包过滤器 - pf](https://www.freebsd.org/status/report-2005-07-2005-10.html#OpenBSD-packet-filter pf)
...
我们正在招聘新成员加入程序委员会。如果您希望在被招募之前自愿参与，请与我联系。

2006 年的日期已公布：2006 年 5 月 12-13 日。地点与之前的活动相同：渥太华大学。价格与 2005 年相同。

请开始思考您的论文。征文通知很快会发出。

### [EuroBSDCon 2005 - 巴塞尔](https://www.freebsd.org/status/report-2005-07-2005-10.html#EuroBSDCon-2005 Basel)
