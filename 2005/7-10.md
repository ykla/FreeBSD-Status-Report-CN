...
经过漫长、令人疲惫但极富有成效的 2005 年第三季度，FreeBSD 6.0 已经发布。为了让这个版本取得成功，许多活动被推到了后台。
...
到 8 月底，我成功地修改了发布构建过程，构建了一个可以加载前端和后端的 Live CD。它可以安装所有的 distfiles、安装 Ports 树，并且具备最基本的安装和卸载软件包的支持。

自谷歌编程之夏结束以来，我一直在努力将新的 Lua 后端集成进来。这个过程已取得成功，现在它已经超过了 BSDINSTALLER-BETA-1 发布的阶段。它现在可以安装 distfiles，但还不能安装 Ports 树或软件包。
...
1. 错误应得到适当处理
2. 支持获取修复
3. 支持 rcs 文件更新的难点已经完成，但没有校验和，某些选项未被遵循，性能可以进一步改进
...
为 FreeBSD（6.x 和 7.x）实现了 Fuse 项目（[Fuse 项目](http://fuse.sourceforge.net/)）提供的全面用户空间文件系统 API 的内核接口，并且采用 BSD 许可证。这为将基于 Fuse 的丰富文件系统移植到 FreeBSD 提供了可能性。
...
3. 将 基于 Fuse 的文件系统和语言绑定移植到 FreeBSD。
...
nsswitch 子系统被扩展以支持新的源（服务、协议、rpc、openssh 和 GT4）。Globus Grid Toolkit 4 补丁（为 GT4 添加 nsswitch 支持）的测试仍有待完成。为了使 nsswitch 支持缓存，缓存守护进程在缓存库的基础上实现，该缓存库也是在 SoC 期间开发的。当前版本的守护进程使用简单的 nscd 样式配置文件，并且似乎稳定。为了完成 SoC 项目，开发了启用了进程内缓存的 libc 实验版本，最近将进行基准测试。
...
由于 Alexander Leidinger 和 Craig Rodrigues 最近进行的良好提交和清理工作，基础系统 PR 的数量已经趋于稳定。新进 Ports PR 的数量继续加速增长，但除了冻结期间外，Ports 提交者一直在努力尽快提交它们。（图表非常清楚地显示了冻结发生的位置。）最终的结果是我们维持了现状，但这需要大量（大多数未被表彰的）努力。我们要感谢大量正在进行这项工作的个人。

目前正在进行工作，询问那些已经分配了 PR 较长时间的提交者，是否仍然有兴趣继续处理它们，或者是否应该将它们重新分配到公共池。这样做是为了尽可能多地“解锁” PR，以帮助改善用户对项目的感知。

作为一项实验，Mark Linimon 一直在为许多 kern 和 bin PR 添加“标签”，包括诸如 '[nfs]'、'[if_em]' 等内容。这个想法是尝试允许基于这些术语进行搜索和浏览，以便提交者更容易地使用我们当前的 PR 数据库。目前这还处于实验阶段，尽管提交者可以通过安装了数据库的系统上的 query-pr(1) 从命令行使用它们。
...
由于 FreeBSD 6.0 发布过程中的代码冻结时间比预期的要长（顺便说一句，FreeBSD 6.0 质量非常高），我决定推迟全职工作，直到冻结期结束。目前，我已在本地代码库中完成了一些 mbuf 处理和其他 netinet 清理工作。
...
此外，还计划从 Linux 的 GPL 授权的 forcedeth.c 驱动程序进行移植；不仅 Xbox 移植版将从中受益，所有 NForce 主板的用户也将受益。生成的驱动程序可以通过 kldload 加载，从而保持内核的 GPL-free。
...
我正在为网站添加个性化新闻提要。对于你的每个 Watch List，你将拥有一个专门的新闻提要。所有对你监视列表中的 Port 的提交都会显示在你的新闻提要中。这个非常棒的新功能现在已在 [BETA 网站](http://beta.freshports.org/) 上提供，欢迎通过 RSS 订阅。我还在支持多个平台和架构方面做了一些工作。这将使 FreshPorts 能够正确报告某个 Port 在某些平台（例如 i386）上已损坏，但在其他平台上未受影响。这个功能会记录以下架构的 BROKEN、FORBIDDEN 和 IGNORE 状态：
...
Edwin Groothius 实施了一个类似但略有不同的调查。他的程序会尝试访问每个 distfile 的每个列出的主站点，检查是否有更新版本可用。结果存储在数据库中。这有助于自动化此前由个别维护者查找更新的任务。该调查受到了极大的欢迎，甚至已经直接促成了数十个 Port 更新的提交。
...
Sam Lawrance 准备了一篇新的文章《维护和贡献 FreeBSD Ports Collection》，这篇文章已经过审阅，并准备提交。该文档旨在规范 Ports 维护者的权利和责任，这在过去一直只是通过邮件列表讨论的“社区传说”。
...
Ports Tinderbox 是一个用于在干净环境中构建 FreeBSD Ports 的打包系统。它可以用来测试新的 Port、更新现有 Port，或仅作为一个软件包构建引擎。Tinderbox 使用与官方 FreeBSD 软件包构建集群（pointyhat）相同的底层代码。因此，如果一个 Port 在 Tinderbox 中构建成功，它也必定能够在 pointyhat 中构建。
...
对 pfsync 进行了进一步改进，以使其在 SMP 环境中表现良好。所有 bug 修复都已被 MFC（合并到 RELENG_5），在适用的情况下。通过 OpenBSD 导入的一些 bug 修复和功能改进（最初由 FreeBSD 用户建议）也已完成。
...
我们正在招募新成员加入程序委员会。如果你希望在被招募之前自愿参与，请与我联系。
