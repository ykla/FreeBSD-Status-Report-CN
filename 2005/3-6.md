...
**链接** 联系方式：Emily Boyd <[soc-emily@freebsd.org](mailto:soc-emily@freebsd.org)>
...
**链接** 联系方式：Brian Wilson <[polytopes@gmail.com](mailto:polytopes@gmail.com)>
...
现在 TCP 代码需要进行全面的改进、精简和清理，使其易于理解、维护和扩展。此外，在这种操作过程中，还可以进行许多小的优化，使 FreeBSD 再次回到表现最好的 TCP/IP 栈的顶端，这一地位在 90 年代已经保持了很长时间。
...
**链接** 联系方式：Alexander Leidinger <[netchild@FreeBSD.org](mailto:netchild@FreeBSD.org)>
...
**链接** 联系方式：Jeff Roberson <[jeff@freebsd.org](mailto:jeff@freebsd.org)>
...
然而，CPU 缓存预取并不是银弹，必须谨慎使用，并且仅在非常特定的地方使用才能带来好处。错误的使用可能导致大量缓存污染，从而降低有效性能。而正确且非常谨慎的使用则能在常见操作中带来明显的性能提升。
...
当前，TCP 段重组实现为一个链表。考虑到今天的高带宽链接和大带宽*延迟积，这种方式无法有效扩展并且性能较差。
...
**链接** 联系方式：Sam Leffler <[sam@freebsd.org](mailto:sam@freebsd.org)>
...
Intel 发布了其 C/C++ 编译程序的第 9 版。针对 FreeBSD 移植 x86 版本的工作正在进行中，具体取决于时间的允许。移植 EM64T（amd64）版本也在待办清单上，但需要足够的空闲时间和适当的硬件支持。
...
**链接** 联系方式：Max Laier <[mlaier@freebsd.org](mailto:mlaier@freebsd.org)>
