## FreeBSD 2005 年 3-6 月状态报告原文地址：[FreeBSD 2005 年 3-6 月状态报告](https://www.freebsd.org/status/report-2005-03-2005-06.html)
...
再次感谢所有提交报告的人，并感谢 Max Laier 的组织和汇总工作。祝您阅读愉快！
...
FreeBSD 的 VFS 层已经与 FFS 文件系统一起进行了细粒度锁定，以支持 FreeBSD 6.0 版本。锁定工作已经进行了好几年，尤其是在过去的 6 个月里，这个项目得到了 Isilon Systems, Inc.（一家领先的集群存储系统供应商）的赞助，进展加快。该项目已进入稳定阶段，尽管在极端情况下报告了一些 bug，但大多数用户没有遇到任何问题。对 8 路和 16 路机器的测试结果表明并行化合理，不过，一旦系统完全稳定后，进行锁争用分析将是有益的。
...
FreeBSD 荷兰文档项目正在进行中，旨在将英文文档翻译成荷兰语。目前，我们几乎完成了 FreeBSD 手册的翻译。完成手册翻译是我们的首要任务，我们非常欢迎您的帮助。如果您愿意提供帮助，请联系 Siebrand 或我。完成手册之后，我们将继续关注其他文档的翻译，欢迎您也参与到这些工作中来。
...
我们的目标仍然是在 6.0 版本中提供“实验性”的审计；目前主要的阻碍因素是我们正在等待将最后剩余的审计文件从苹果的 APSL 许可重新授权为 BSDL，以便它们可以包含在 FreeBSD 内核中。预计这一过程将很快完成。一旦完成，所有更改可以合并到 CVS 中，然后 MFC 到 RELENG_6。如果在 6.0-RELEASE 发布之前尚未完成，这项工作将在发布后不久合并，因为所有与 ABI 相关的数据结构已根据需要更新。
...
- [显示您监视列表中来自 ports/UPDATING 的相关条目](http://www.freshports.org/phorum/read.php?f=1&i=1021&t=1021#repl%20y_1021)
...
3. 定制新闻订阅功能正在开发中。您将能够为每个监视列表创建一个新闻订阅。这项工作依赖于完成 Last-Modified 头部的工作。
